...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{\toplevelprefix/chapters/chapter8/figs/BPE1.png}\hspace{0.6in} 
...
So far, in previous sections, we have mainly shown how to learn representations of the distribution of {\em two-dimensional} (2D) images, as well as many image-based applications or tasks such representations facilitate: such as image based object recognition, segmentation, image completion etc. 

However, our world is three-dimensional (3D).\footnote{or four-dimensional (4D) if we consider time as an extra dimension as the world is dynamical.} Although our eyes perceive only a 2D projections of the world, we are able to develop a visual memory that represents a full 3D world model for our living environment and objects in it. 

In this section, we will show that,  using the methods introduced in this book, we should be able to learn the distribution of 3D shapes of natural objects and then generate new 3D object shapes from this distribution, conditioned on either an input 2D image or a text description of the object.
