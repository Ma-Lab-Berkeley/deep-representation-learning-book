...
\yima{image-conditioned CUPID and text-conditioned Michelangelo etc. (Shenghua and Yi). I will add some justification for the importance of developing a 3D visual memory, including a representation that supports ego-centric, object-centric and allo-centric reasoning of objects in a 3D scene.  }
...
However, the fundamental reason why we develop the sense of vision is to learn to perceive our three-dimensional (3D) environment.\footnote{or four-dimentional (4D) if we consider time as an extra dimension for the dynamical world.} Our brain has dedicated a significant portion of it, the visual cortex, to this task. It enables us to accurately and efficiently reconstruct shapes of 3D objects and their spatial (and temporal) relationships from the 2D images through our eyes. 

There has been a rich and long history of understanding mathematically how, in principle, we can fully reconstruct 3D geometry from (multiple) 2D images. Interested reader may refer to the book \cite{MaY2003} for a detailed account of this topic. However, as we have alluded to in the Section \ref{sec:3D-visual-model} in the previous Chapter \ref{ch:conditional-inference}. The 3D reconstruction is much better done in a Bayesian setting. 

To test this hypothesis, in this section, we turn to a significantly more challenging and ill-posed problem: inferring a complete 3D object structure from a single 2D image. This task lies at the heart of computer vision, aiming to reverse the physics of image formation. An image $\image$ is the result of a rendering process,
\[
