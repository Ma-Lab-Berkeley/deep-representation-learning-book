...
\yima{SimCLIP... and DPS implementation, classifier-free, or cross-attention etc. (Robin, Jingfeng, Daniel, and Peter)}
...
In this section, we introduce a unified probabilistic framework, which we call \textbf{Cupid}, that bridges this gap by jointly modeling the distribution over both the 3D object and the camera pose. The central idea is that an ideal system should be able to simultaneously generate a complete 3D model and reason about the precise camera viewpoint that produced the given 2D observation. In the way, the genereated 3D object can be \textit{pasted} into the original image. This approach recasts single-view 3D reconstruction as a conditional sampling problem, aiming to achieve both the geometric fidelity of reconstruction and the rich priors of a generative model.
...
\item \textbf{Low-level visual features} $\feat_i^\low$, sampled from the output of a lightweight convolutional head, which capture fine details like color and texture that may be lost in the DINOv2 representation.
\end{enumerate}
These pixel-aligned features, $\feat_i^\high$ and $\feat_i^\low$, are then concatenated with the current noisy voxel feature $\feat_i^{t}$ at timestep $t$ and fused into the flow transformer via a linear layer. This mechanism ensures that the generative process is strongly guided by the visual evidence in the input image, preventing the color drift and shape inconsistencies common in purely generative models.
