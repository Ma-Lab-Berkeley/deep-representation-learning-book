...
\begin{itemize}
    ...
    \item \Cref{sec:dictionary_learning} --- Sparse Coding and Dictionary Learning: Given noisy samples from {\em a distribution supported on combinations of a few incoherent vectors}, such as the coordinate axes, obtain an estimate of the true sample, which also has this property.
\end{itemize}

In this chapter, as described above, we make simplifying modeling assumptions that essentially assume (the support of) the data distribution have geometrically (nearly, piece-wise) linear structures and statistically independent components, as illustrated in Figure \ref{fig:subspaces}.
\begin{figure}[t]
    \centering
    ...
    = \mathop{\mathrm{col}}(\vU)$, or a mixture of low-dimensional subspaces (right), say $\mathcal{S}_j
...
\begin{remark}[Power Iteration as a Denoising Process] Notice that computing the principal component via power iteration can be interpreted as a ``denoising'' process: starting from a random noise, converges iteratively onto a one-dimensional subspace. Indeed, as we will show later in \Cref{ch:denoising}, if we assume {\em the support of the data distribution is a single subspace}, the associated denoising operator (or the score function) is precisely the same as the above power iteration. In Chapter \ref{ch:denoising}, we will show how to generalize such a denoising process to distributions of more general low-dimensional structures. This is essentially a universal and most efficient approach to learn any low-dimensional distributions.
\end{remark}
...
%\begin{remark}[Online PCA] For simplicity, all algorithms that we present in this chapter assume that a sufficient set of samples $\X$ have been given in advance. This is not always the case. In the case of continuous learning, we may need to learn the distribution (subspace) from samples that are given one at at time. This problem is often referred to as {\em online} PCA. We will leave the study to \Cref{ch:closed-loop}.
%    
%\end{remark}
...
algorithms that process sequentially-structured data with causal structure, as
we will study in \Cref{sec:tost}.
Interestingly, and more generally, the simple online PCA algorithm is suggestive
of the fact that \textit{simpler algorithms than (end-to-end) back propagation
can succeed in learning encoder-decoder pairs}, which we will briefly revisit in
\Cref{ch:future} as an important goal for the future study of intelligence.




\subsection{The Statistical View: Probabilistic PCA}\label{subsec:probabilistic PCA}
...
\subsection{Matrix Completion: Error Correction via Low-Rankness}
...
\begin{align}
    ...
    \vA_{t} - 2\nu (\vA_{t}\vZ^+ - \vX)(\vZ^+)^\top
    \right), \nonumber \\
    &\quad\mbox{where} \quad (\vA_{1})_j \simiid \dNorm(\vzero, \tfrac{1}{D}\vI), \enspace \forall
    j \in [m], 
...
\paragraph{Key Messages.} The idealistic models we have presented in this chapter---PCA, ICA, and
