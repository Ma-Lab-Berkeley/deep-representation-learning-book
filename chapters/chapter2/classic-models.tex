...
are generally more efficient than LISTA, as well, which requires large amounts
of labeled $(\vX, \vZ)$ pairs for training.
...
% only defines $\vZ_t^\ell$ for $t = \ell$ and $t=\ell+1$, and
% $\vA_t^\ell$ for $t=\ell$: it 
...
observed in current neural networks---we will have much more to say about it in
\Cref{ch:unrolling}.
