...
of many motifs, or \textit{atoms}. Mechanically, this suggests that we consider
the following data model. We say that data \(\{\vx_{i}\}_{i = 1}^{N} \subseteq
\R^{D}\) have (approximate) \(s\)-sparse coding structure if and only if there
exists a matrix \(\vA = \mat{\va_{1}, \dots, \va_{d}} \in \R^{D \times d}\),
vectors \(\{\vz_{i}\}_{i = 1}^{N} \subseteq \R^{d}\) with \(\norm{\vz_{i}}_{0}
\leq s\) for all \(i\), and small vectors \(\{\veps_{i}\}_{i = 1}^{N} \subseteq
\R^{D}\) such that
