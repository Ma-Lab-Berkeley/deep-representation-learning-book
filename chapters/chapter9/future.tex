...
As we have discussed at the beginning of this book, \Cref{ch:intro}, intelligence in nature has evolved through multiple phases and manifested in four distinct forms:
\begin{equation*}
...
\begin{enumerate}
    \item The \textit{code book} or scheme used to encode the intended information or knowledge.
    \item The \textit{information} or knowledge learned and encoded using the codebook. 
    \item The \textit{optimization mechanisms} used to create and correct the encoded information or knowledge.
\end{enumerate}
Table \ref{tab:four-stage-intelligence} summarizes their main characteristics.
\begin{table}[h]
    \centering
    \resizebox{1\columnwidth}{!}
{
    ...
    \textbf{Codebook}  & DNAs & Neurons/Brain & Natural Languages & Math  Abstractions \\ [0.5ex]
    \hline 
    
    \textbf{Information} & Genes & Memory & Empirical Knowledge & Scientific Facts\\ [0.5ex]
    \hline
    \textbf{Optimization} & Reinforcement & Error Feedback & Trial \& Error & Theorize \& Falsify \\  [0.5ex]
    \hline
    \end{tabular}
    }
...
\begin{enumerate}
    \item The first was the development of spoken and written language, which enabled humans to share and transmit learned knowledge across generations, much as DNA does in nature. 
    \item The second was the development of mathematics and formal logic roughly three thousand years ago, which became the precise language of modern science. 
\end{enumerate} 
The language of mathematics freed us from summarizing knowledge from observations only in empirical form and allowed us to formalize knowledge as theories verifiable or falsifiable through mathematical deduction or experimentation. Through hypothesis formulation, logical deduction, and experimental verification or falsification, we can now proactively discover and develop new knowledge that is far beyond what can be learned from the distributions of observed data.\footnote{For example, causal relationships cannot be learned from distributions alone \cite{Pearl-2009}.}

Despite the seemingly differences among these different stages of intelligence, the evolution of intelligence shares a common characteristic: {\em they all use certain form of feedback\footnote{loss for error and reward for success} to create and correct information learned and the feedback and correction are becoming increasingly frequent and efficient through the evolution -- from open-loop reinforcement to close-loop self-supervision, from collective trial and error to proactive hypothesizing and falsification.}
...
\end{equation*}
As we have clarified repeatedly in this book, most technological advances in machine intelligence over the past decade or so, although carried out under the name ``AI'', are actually more closely related to have machines imitate low-level forms of intelligence largely shared by both animals and humans, including phylogenetic, ontogenetic, and societal intelligence. At these levels, intelligence creates {\em empirical} memory or knowledge and conduct inference that is mainly {\em inductive} in nature. Principles and methods introduced in this book aim to reveal scientific objectives and computational mechanisms behind intelligence at these levels. They provide strong theoretical justification and computational evidence for the claim:
\begin{quote}
    \centering
    \textit{Machines can learn empirical knowledge and conduct inductive inference.}
\end{quote}
So far, however, no rigorous theoretical or scientific evidence suggests that these mechanisms alone would suffice to achieve the high-level human intelligence that the original AI program truly aimed to understand and wanted for machines to imitate.

% Start of Selection
In fact, as of today, we know little about how to rigorously verify or certify whether a system is truly capable of such high-level intelligence, even though the Turing Test, also known as the Imitation Game,\footnote{In Turing's proposal, the evaluator, or interrogator, is a human. However, most human evaluators have limited scientific training and knowledge, and their conclusions can be subjective.} was proposed in 1950 \cite{Turing-1950}, as an initial attempt to address the fundamental question:
\begin{quote}
    \centering
...
\end{enumerate} 
just to name a few? Or has it simply memorized massive amount of examples of such notions? Note that state-of-the-art large language models still struggle with simple mathematical questions like: ``Is 3.11 larger or smaller than 3.9?''\footnote{Some models have corrected their answers to such questions through targeted engineering, or have incorporated additional reasoning mechanisms that verify immediate answers and correct them during inference. However, we leave it to the reader as an exercise to rigorously test whether any state-of-the-art language model truly understands the notion of numbers (natural, rational, real, and complex) and their associated arithmetic.} 

From this book, we know modern generative AI systems primarily rely on Bayesian (hence inductive) inference to produce their answers. Can machines also truly grasp mathematical induction or general logical deduction and understand its difference from the Bayesian induction? How do we verify whether a system truly understands the rules of logical and mathematical deduction and can apply them rigorously. Or, again, has it merely memorized a large number of instances of such deductions, such as chains of thoughts data, and learned and exploited their distributional patterns? Hence we have an open question:
\begin{quote}
\centering
    \textit{Is there any  difference between \textbf{memorizing} and \textbf{understanding}?}
\end{quote}
Only by knowing the difference can we possibly attempt to provide a meaningful answer to the question: ``{\em Can machines understand?}''

Probably much more importantly, how to verify whether a self-claimed intelligent system is capable of {\em creating new knowledge and correcting its own knowledge}, such as creating new mathematical concepts and discovering new physical laws and causal relationships? The connection between understanding and self-creation might be rather fundamental, as the Physicist  Richard Feynman had famously put it:
\begin{quote}
    \centering
...
\begin{enumerate}
    \item[Level 1.] having merely learned the distribution of some knowledge-carrying data and being able to regenerate them;
    \item[Level 2.] being capable of autonomously and continuously creating new knowledge or correcting existing one from new observations and experiences;
    \item[Level 3.] being able to truly  understand abstract concepts, rules and laws and knowing how to apply them correctly in a deductive manner;
    \item[Level 4.] being able to generate new scientific hypotheses and mathematical conjectures and verify or falsify them based on experimentation or logic deduction.
\end{enumerate}
...
To evaluate and distinguish these different levels of intelligence, we suggest that there should probably be at least three different tests, as illustrated in \Cref{fig:three-tests}:
\begin{enumerate}
    \item \textit{The Norbert Wiener Test:} to determine whether a system can self-correct and create new (empirical) knowledge on its own or merely receives and updates information passively through external reinforcement or supervision;
    \item \textit{The Alan Turing Test:} to determine whether a system can create and understand abstract concepts and knowledge or merely learns and memorizes statistics of the samples and uses them for Bayesian inference;
    \item \textit{The Karl Popper Test:} to determine whether a system can proactively explore and create new abstract knowledge by forming and verifying hypotheses or theories based on experimental consistency or logic self-consistency.
\end{enumerate}
We believe that, for such tests, the evaluator, or interrogator, should not be an arbitrary single or group of human but rather a scientifically certified entity following a scientifically sound protocol and process so that any conclusions would be rigorous and trustworthy.

As we have seen throughout this book, \textit{compression} has played a fundamental role in learning or creating empirical knowledge. It is the governing principle and a universal mechanism for identifying an empirical data distribution and organizing the information encoded therein with a better representation. To a large extent, it explains most of the practice of ``artificial intelligence'' so far, discriminative or generative. An outstanding question for future study is whether \textit{compression alone} is sufficient to achieve all the higher-level capabilities of intelligence mentioned above. 
Are mathematical abstraction, causal inference, hypothesis generation, and logical reasoning and deduction extended or transcended forms of compression? Is there some fundamental difference between identifying empirical data distributions through compression and forming high-level concepts and theories through abstraction? Hence we have another open question:  
\begin{quote}
    \centering
    \textit{Is there any difference between \textbf{compression} and \textbf{abstraction}?}
\end{quote}
Only by knowing the difference can we posssibly attempt to provide a meaningful answer to the question: ``{\em Can machines abstract or create?}''

To a large extent, Science---and its associated code book, Mathematics---can be viewed as the most advanced form of intelligence, unique to educated and enlightened humans, akin to a form of high art. Philosopher sir Karl Popper once suggested:
\begin{quote}
    \centering
    \textit{``Science may be described as the art of systematic oversimplification.''}
\end{quote}
We believe that uncovering and understanding the underlying mathematical principles and computational mechanisms of such higher-level intelligence will be the final frontier for  Science, Mathematics, and Computation altogether! 
