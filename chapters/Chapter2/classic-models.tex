...
\end{align}
Thus, to compute the principal components, we find the orthogonal matrix \(\vU\), which maximizes the term: \(\tr(\vU^{\top}(\vX\vX^{\top}/N)\vU)\). In fact, the best such matrix \(\vU_{\star}\) has columns which are the \textit{top \(d\) eigenvectors} of \(\vX\vX^{\top}/N\). While this can be obtained via brute force calculation, such as an inductive proof, we do not do so here and leave it to \Cref{exercise:principal-components-derivation}. We instead provide a little intuition by considering the case \(d = 1\). Then we only have a single unit vector \(\vu\) to recover, and solve the problem \(\max_{\vu} \vu^{\top}(\vX\vX^{\top}/N)\vu\). This is the so-called \textit{Rayleigh quotient} of \(\vX\vX^{\top}/N\) and the optimal solutions to this problem are eigenvectors corresponding to the maximum eigenvalue of \(\vX\vX^{\top}/N\). Thus, the worked-out solution for the case \(d = 1\) corresponds to our proposed solution for the general \(d\) case. In summary, we have
\begin{tcolorbox}
...
\begin{equation}
    \Cov[\vx] = \vU_{o}\Cov[\vz_{o}]\vU_{o}^{\top} + \Cov[\veps] = \vU_{o}\Ex[\vz_{o}\vz_{o}^{\top}]\vU_{o}^{\top} + \Cov[\veps].
\end{equation}
