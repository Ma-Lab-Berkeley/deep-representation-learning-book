...
\end{equation}
where $\vU$ is constrained to be an orthonormal matrix, as above. We will omit
this constraint in similar statements below for the sake of concision.

\paragraph{Subspace Encoding-Decoding via Denoising.}
To simplify this problem, for a fixed \(\vU\), we have (proof as exercise):
\begin{align*}
    \min_{\{\vz_{i}\}_{i = 1}^{N}}\frac{1}{N}\sum_{i = 1}^{N}\norm{\vx_{i} - \vU\vz_{i}}_{2}^{2} 
    &= \frac{1}{N}\sum_{i = 1}^{N}\min_{\vz_{i}}\norm{\vx_{i} - \vU\vz_{i}}_{2}^{2} \\
    &= \frac{1}{N}\sum_{i = 1}^{N}\norm{\vx_{i} - \vU\vU^{\top}\vx_{i}}_{2}^{2}. 
\end{align*}
That is $\vz_i = \vU^\top\vx_i$ is the optimal solution for the above minimization problem. 

Now, we can write the original optimization problem in \(\vU\) and \(\{\vz_{i}\}_{i = 1}^{N}\) as an optimization problem just over \(\vU\), i.e.,
\begin{equation}\label{eq:pca_equals_denoising}
    \min_{\vU, \{\vz_{i}\}_{i = 1}^{N}}\frac{1}{N}\sum_{i = 1}^{N}\norm{\vx_{i} - \vU\vz_{i}}_{2}^{2} = \min_{\vU}\frac{1}{N}\sum_{i = 1}^{N}\norm{\vx_{i} - \vU\vU^{\top}\vx_{i}}_{2}^{2}.
\end{equation}
Note that the problem on the right-hand side of \eqref{eq:pca_equals_denoising} is a \textit{denoising} problem: given noisy observations \(\vx_{i}\) of low-rank data, we aim to find the \textit{noise-free} copy of \(\vx_{i}\), which is denoted as $\hat{\vx}_i$. That is, $\hat{\vx}_i = \vU\vU^\top \vx_i$. Notice that this is the point on the subspace that is closest to $\vx_i$. Here by solving the equivalent problem of finding the best subspace, parameterized by basis \(\vU\), the \textit{denoiser} is the projection matrix \(\vU\vU^{\top}\) that projects the noisy data point $\x_i$ onto the subspace. %Denoising is one of the fundamental building blocks of high-dimensional data analysis, and we will discuss it often in this book, as many algorithms we will discuss are essentially denoising with respect to a particular data model (such as in \eqref{eq:pca_dgp}, but a bit more complicated). 
...
\end{equation}
Here, $\vz \in \R^d$ can be viewed as the compact code (or a latent representation) of a data point  $\x \in \R^D$ and the learned subspace basis $\vU$ as the associated codebook whose columns are the (learned) optimal code words. The process achieves the function of denoising $\x$ by projecting it onto the subspace spanned by $\vU$.
...
\end{align}
Thus, to compute the principal components, we find the orthogonal matrix \(\vU\), which maximizes the term: \(\tr(\vU^{\top}(\vX\vX^{\top}/N)\vU)\). In fact, the best such matrix \(\vU^{\star}\) has columns which are these are the \textit{top \(d\) eigenvectors} of \(\vX\vX^{\top}/N\). While this can be obtained via brute force calculation, such as an inductive proof, we do not do so here; we instead provide a little intuition by considering the case \(d = 1\). Then we only have a single unit vector \(\vu\) to recover, and solve the problem \(\max_{\vu} \vu^{\top}(\vX\vX^{\top}/N)\vu\). This is the so-called \textit{Rayleigh quotient} of \(\vX\vX^{\top}/N\) and the optimal solutions to this problem are eigenvectors corresponding to the maximum eigenvalue of \(\vX\vX^{\top}/N\). Thus, the worked-out solution for the case \(d = 1\) corresponds to our proposed solution for the general \(d\) case. In summary, we have
\begin{tcolorbox}
    \textit{The principal components correspond to the optimal linear denoiser and are given by the top \(d\) eigenvectors of \(\vX\vX^{\top}/N\).}
\end{tcolorbox}

\begin{remark}
    In some data analysis tasks, the data matrix \(\vX\) is formatted such that each data point is a \textit{row} rather than a \textit{column} as is presented here. In this case the principal components are the top \(d\) eigenvectors of \(\vX^{\top}\vX\).
\end{remark}

\begin{remark}[Basis Selection via Denoising Eigenvalues]
    In many cases, either our data will not truly be distributed according to a subspace-plus-noise model, or we will not know the true underlying dimension \(d\) of the subspace. In this case, we have to choose\(d\); this problem is called \textit{model selection}. In the restricted case of PCA, one way to perform model selection is to compute \(\vX\vX^{\top}/N\) and look for instances where adjacent eigenvalues go from large to small; this is one indicator that the index of the larger eigenvalue is the ``true dimension \(d\)'', and the rest of the eigenvalues of \(\vX\vX^{\top}/N\) are contributed by the noise or disturbances \(\veps_{i}\). Model selection is a difficult problem and, nowadays in the era of deep learning where it's called ``hyperparameter optimization'', is usually done via brute force or Bayesian optimization. %\sdb{``thresholding noise'' to learn $\vU$}
\end{remark}
...
\begin{remark}[Neural Network Interpretation]
    If we do a PCA, we approximately recover the support of the distribution encoded by the parameter \(\vU_{\star}\). The learned denoising map then takes the form \(\vU_{\star}\vU_{\star}^{\top}\). On top of being a denoiser, this can be viewed as a \textit{simple two-layer weight-tied linear neural network}: the first layer multiplies by \(\vU_{\star}^{\top}\), and the second layer multiplies by \(\vU_{\star}\), namely
    \begin{equation}
        \operatorname{denoise}(\vx) = \underbrace{\vU_{\star} \circ \underbrace{\id \circ \underbrace{\vU_{\star}^{\top}\vx}_{\text{first ``layer''}}}_{\text{post-activation of first ``layer''}}}_{\text{output of ``NN''}}
    \end{equation}
    Contrasting this to a standard two-layer neural network, we see a structural similarity:
    \begin{equation}
        \operatorname{NN}(\vx) = \underbrace{\vW_{\star} \circ \underbrace{\mathrm{ReLU} \circ \underbrace{\vU_{\star}^{\top}\vx}_{\text{first layer}}}_{\text{post-activation of first layer}}}_{\text{output of NN}}
    \end{equation}
    In particular, PCA can be interpreted as \textit{learning a simple two-layer denoising autoencoder},\footnote{In fact, as we have mentioned in the previous chapter, PCA was one of the first problems that neural networks were used to solve \cite{Oja1982SimplifiedNM,Baldi89}.} one of the simplest examples of a non-trivial neural network. In this framework, the \textit{learned representations} would just be \(\vU_{\star}^{\top}\vx\). In this way, PCA serves as a model problem for (deep) representation learning, which we will draw upon further in the monograph. Notice that in this analogy, the representations reflect, or are projections of, the input data towards a learned low-dimensional structure. This property will be particularly relevant in the future.
\end{remark}

\begin{exercise}
    Prove that, for any symmetric matrix \(\vA\), the solution to the problem \(\max_{\vU \in \O(D, d)}\tr\left(\vU^{\top}\vA\vU\right)\) is the matrix \(\vU^{\star}\) whose columns are the top \(d\) unit eigenvectors of \(\vA\).
\end{exercise}

\subsection{Pursuing Low-rank Structure via Power Iteration}
...
\end{equation}
Then we can compute the fixed point using the following iteration:
\begin{equation}\label{eq:power_iteration}
    \vv_{0} \sim \dNorm(\vzero, \vone), \qquad \vv_{t + 1} \gets \frac{\vM\vv_{t}}{\norm{\vM\vv_{t}}_{2}}.
\end{equation}
In the limit, \(\vv_{t}\) will converge to a top unit-norm eigenvector of \(\vM\). 

Why would such an iteration work? First, note that for all \(t\), we have 
\begin{equation}
...
\begin{remark}
    ...
    \begin{equation}
        \min_{\vU}\Ex\norm{\vx - \vU\vU^{\top}\vx}_{2}^{2}.
    \end{equation}
    It is not too hard to prove that if \(\veps\) is an isotropic Gaussian random variable, i.e., with distribution \(\veps \sim \dNorm(\vzero, \sigma^{2}\vI)\),\footnote{Other distributions work so long as they support all of \(\R^{D}\), but the Gaussian is the easiest to work with here.} then for \textit{any} solution \(\vU_{\star}\) to this problem, we have 
    \begin{equation}
        \vU_{\star}\vU_{\star}^{\top} = \vU_{o}\vU_{o}^{\top}
    \end{equation}
    and so the true supporting subspace, say \(\cS_{o} \doteq \Range(\vU_{o})\), is recovered as the span of columns of \(\vU_{\star}\), since 
    \begin{equation}
        \cS_{o} = \Range(\vU_{o}) = \Range(\vU_{o}\vU_{o}^{\top}) = \Range(\vU_{\star}\vU_{\star}^{\top}) = \Range(\vU_{\star}).
    \end{equation}
    In particular, the learned \textit{denoising map} \(\vU_{\star}\vU_{\star}^{\top}\) is an orthogonal projection onto \(\cS_{o}\), pushing noisy points onto the ground truth supporting subspace. We can establish a similar technical result in the case where we only have finite samples, as in Subsection \ref{sub:pca}, but this takes more effort and technicality. %\DP{include formal error rate?}
\end{remark}
...
In this setting, compressive autoencoding amounts to \textit{correctly learning the orthogonal dictionary $\vU$}: we
can then take $\cE(\vx) = \vU^T \vx$ as the encoder and $\cD(\vz) = \vU \vz$ as the decoder, and $\cD = \cE^{-1}$.
Our task is then:
\begin{tcolorbox}
...
\end{equation}
where $\vU_o \in \R^{D \times D}$ is an unknown orthogonal dictionary, $\vz$ is a random vector with statistically independent components $z_i$, each with zero mean, and $\veps \in \R^D$ is an independent random vector of small (Gaussian) noises. The goal is to recover $\vU_o$ (and hence $\vz$) from samples of $\vx$.
...
At first sight, the assumption that the dictionary $\bm U_o$ is orthogonal might seem to be somewhat restrictive. But there is actually no loss of generality. One may consider a complete dictionary to be any square invertible matrix $\vD$. With samples generated from this dictionary: $\vX = \vD \vZ \in \mathbb{R}^{D\times N}$, it is easy to show\footnote{See for example \cite{sun2017completeI}.} that with some preconditioning of the data matrix $\vX$: 
\begin{equation}
    \bar{\vX} = \Big(\frac{1}{N\theta} \vX\vX^\top\Big)^{-\frac{1}{2}}\vX,
\end{equation}
then there exists an orthogonal matrix $\vD_o \in \mathrm{O}(D)$ such that
\begin{equation}
    \bar{\vX} = \vU_o\vZ.
\end{equation}
\end{remark}
...
\end{equation}
Let $\vX = [\x_1, \x_2, \ldots, \x_N]$ and $\vZ = [\vz_1, \vz_2, \ldots, \vz_N]$. The goal is to recover $\vU_o$ from the data $\vX$. Therefore, Given any orthogonal matrix $\vA \in \mathrm{O}(D)$, $$\vA\vx_i = \vA\vU_o \vz_i + \vA\veps_i$$ would be nearly sparse if $\vA = \vU_o^T$ (as by assumption the noise $\veps_i$ is of small magnitude). 

Also, given $\vU_o$ is orthogonal and the fact $\veps$ is small, the vector $\vx$ is (nearly) constant $\bE[\norm{\vx}_2^2] \approx \bE[\norm{\vz}_2^2]=d$. It is a known fact that for vectors on a sphere, maximizing the $\ell^4$ norm is equivalent to minimizing the $\ell^0$ norm (for promoting sparsity),
\begin{equation*}
...
For an orthogonal matrix $\vA$, it preserves the 2-norm: $\|\vA \x\|_2^2 = \|\x\|_2^2$ is nearly constant. Therefore, to find the correct orthogonal dictionary $\vU_o$ from $\vX$, we may try to solve the following optimization program:
...
\end{equation}
This is known as the $\ell^4$ maximization problem \cite{Zhai-2020}.
\begin{remark}
...
The constrained $\ell^4$ maximization problem is a nonconvex program. In general one should expect that any greedy (say gradient-descent type) algorithms would converge to the globally optimal solution. Surprisingly, one can show that, unlike general nonconvex programs, the landscape of $\ell^4$ maximization over a sphere
...
Exercise \ref{exercise:fast-ica-convergence} explores these issues in more depth. \yima{Missing reference to exercise.}
...
In contrast, Figure \ref{fig:ReconMNIST}(b) shows the result of running a classical algorithm for overcomplete dictionary learning known as K-SVD \cite{rubinstein2012analysis} on these samples.\footnote{By the end of this section, we will have developed the conceptual and computational foundations to implement this algorithm ourselves.} 
Notice that the representations become far sparser and the codebooks far more interpretable---they consist of fundamental primitives for the strokes composing the digits, i.e.\ oriented edges. \yima{Guys, Figure \ref{fig:ReconMNIST} is to illustrate the difference between PCA and MSP introduced in the previous section! It has nothing to do with the message that this example is trying to say!}
\end{example}
...
\centering
    ...
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/Ori/MNIST_Ori_100.jpg}}
    \end{minipage}
    \caption{Original Images from the MNIST dataset}
    \label{fig:OriginalMNIST_PCA}
    \end{subfigure}%
    ~
    \begin{subfigure}[t]{0.48\textwidth}
    \centering
    ...
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_01/MSP_Recon_01_100.jpg}}
    \end{minipage}
    \caption{Reconstruction with top 1 Basis by MSP}
    \label{fig:MSPReconMNIST1}
    \end{subfigure}%
    ~
    \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_01/PCA_Recon_01_100.jpg}}
    \end{minipage}
    \caption{Reconstruction with top 1 Basis by PCA}
    \label{fig:PCAReconMNIST1}
    \end{subfigure}%
    \\
    \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_03/MSP_Recon_03_100.jpg}}
    \end{minipage}
    \caption{Reconstruction of with top 3 Bases by MSP}
    \label{fig:MSPReconMNIST3}
    \end{subfigure}%
    ~
    \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_03/PCA_Recon_03_100.jpg}}
    \end{minipage}
    \caption{Reconstruction with top 3 Bases by PCA}
    \label{fig:PCAReconMNIST3}
    \end{subfigure}%
    % \\
    ...
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_81.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_82.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_83.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_84.jpg}}
    % \centering
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_85.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_86.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_87.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_88.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_89.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_90.jpg}}
    % \centering
    % \end{minipage}
    % % \vspace{0.5mm}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_91.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_92.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_93.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_94.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_95.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_96.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_97.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_98.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_99.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_04/MSP_Recon_04_100.jpg}}
    % \end{minipage}
    % \caption{Reconstruction with top 4 Bases by MSP}
    % \label{fig:MSPReconMNIST4}
    % \end{subfigure}%
    % ~
    % \begin{subfigure}[b]{0.48\textwidth}
    % \centering
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_81.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_82.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_83.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_84.jpg}}
    % \centering
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_85.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_86.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_87.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_88.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_89.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_90.jpg}}
    % \centering
    % \end{minipage}
    % % \vspace{0.5mm}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_91.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_92.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_93.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_94.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_95.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_96.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_97.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_98.jpg}}
    % \end{minipage}
    % \begin{minipage}[t]{0.08\linewidth}
    % \centering
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_99.jpg}}\vspace{0.5mm}
    % \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_04/PCA_Recon_04_100.jpg}}
    % \end{minipage}
    % \caption{Reconstruction with top 4 Bases by PCA}
    % \label{fig:PCAReconMNIST4}
    % \end{subfigure}%
    \\
    \begin{subfigure}[b]{0.48\textwidth}
    \centering
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_05/MSP_Recon_05_100.jpg}}
    \end{minipage}
    \caption{Reconstruction with top 5 Bases by MSP}
    \label{fig:MSPReconMNIST5}
    \end{subfigure}%
    ~
    \begin{subfigure}[b]{0.48\textwidth}
    \centering
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_05/PCA_Recon_05_100.jpg}}
    \end{minipage}
    \caption{Reconstruction with top 5 Bases by PCA}
    \label{fig:PCAReconMNIST5}
    \end{subfigure}%
    \\
    \begin{subfigure}[b]{0.48\textwidth}
    \centering
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/MSP_10/MSP_Recon_10_100.jpg}}
    \end{minipage}
    \caption{Reconstruction with top 10 Bases by MSP}
    \label{fig:MSPReconMNIST10}
    \end{subfigure}%
    ~
    \begin{subfigure}[b]{0.48\textwidth}
    \centering
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_81.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_82.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_83.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_84.jpg}}
    \centering
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_85.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_86.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_87.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_88.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_89.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_90.jpg}}
    \centering
    \end{minipage}
    % \vspace{0.5mm}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_91.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_92.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_93.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_94.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_95.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_96.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_97.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_98.jpg}}
    \end{minipage}
    \begin{minipage}[t]{0.08\linewidth}
    \centering
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_99.jpg}}\vspace{0.5mm}
    \frame{\includegraphics[width=\linewidth]{Figure/Reconstruction/PCA_10/PCA_Recon_10_100.jpg}}
    \end{minipage}
    \caption{Reconstruction with top 10 Bases by PCA}
    \label{fig:PCAReconMNIST10}
    \end{subfigure}%    
    \caption{Comparison of compact representation with learned dictionary from the MSP algorithm with the PCA bases for the MNIST image dataset.\yima{please use this example for MSP algorithm introduced in the previous subsection, say right after Remark \ref{rem:L4-global}.}}
...
\begin{tcolorbox}
    Suppose that $\vx$ satisfies the model \eqref{eq:model-DL-overcomplete} with overcomplete dictionary $\vA$ and sparsity level $d$.
    Given samples $\vX = [\vx_1, \dots, \vx_N]$ of $\vx$,  
    find
    \begin{enumerate}[left=0pt, label=\arabic*.]
        \item an encoder $\cE : \R^D \to \R^m$ mapping each $\vx$ to its \textit{sparse code} $\vz$,
        \item a decoder $\cD(\vz) = \vA \vz$ reconstructing each $\vx$ from its sparse code.
    \end{enumerate}
    Thus 
    $\cE(\vx)$ and $\cD(\vz) = \vA \vz$ forms a lossy autoencoding pair on $\vx$.
\begin{equation}
...
In this section, we will consider a data model which accommodates sparse linear combinations of many motifs, or \textit{atoms}. Mechanically, this suggests that we consider the following data model. We say that data \(\{\vx_{i}\}_{i = 1}^{N} \subseteq \R^{D}\) have (approximate) \(s\)-sparse coding structure if and only if there exists a matrix \(\vA_{o} = \mat{\va_{o, 1}, \dots, \va_{o, d}} \in \R^{D \times d}\), vectors \(\{\vz_{o, i}\}_{i = 1}^{N} \subseteq \R^{d}\) with \(\norm{\vz_{o, i}}_{0} \leq s\) for all \(i\), and small vectors \(\{\veps_{i}\}_{i = 1}^{N} \subseteq \R^{D}\) such that
\begin{equation}\label{eq:vectorized_sparse_dl_dgp}
    \vx_{i} = \vA_{o}\vz_{o, i} + \veps_{i}, \qquad \forall i \in [N].
\end{equation}
Note the mechanical difference between the sparse coding structure and the principal components structure: 1. the low-dimensional representations \(\vz_{o, i}\) are required to be \(s\) sparse, and 2. \(\vA_{o}\) is not required to be orthogonal. 

The \(s\)-sparsity condition comes from the desire for each data point to be representable by a  linear combination of a small number $s$ of atoms (i.e. columns of $\vA_o$). This essentially imposes that the intrinsic dimension of the data distribution is lower than $s$.

The relaxing of the orthogonality condition is because, as in our covering example above, \(d\) is much larger than \(D\) and it is impossible to build an orthogonal matrix of size \(D \times d\). Nevertheless, the dictionary \(\vA_{o}\), which is either given or learned, is often \textit{incoherent} in the sense that products \(\va_{o, i}^{\top}\va_{o, j}\) are often small, hence nearly orthogonal.\footnote{As it turns out, in a high-dimensional space, it is rather easy to pack a number of nearly orthogonal vectors that is much larger than the ambient dimension \cite{Wright-Ma-2022}. } 

Note that we can collect the \(\vx_{i}\) into \(\vX = \mat{\vx_{1}, \dots, \vx_{N}} \in \R^{D \times N}\), collect the \(\vz_{o, i}\) into \(\vZ_{o} = \mat{\vz_{o, 1}, \dots, \vz_{o, N}} \in \R^{d  \times N}\), and collect the \(\veps_{i}\) into \(\vE = \mat{\veps_{1}, \dots, \veps_{N}} \in \R^{D \times N}\), to rewrite \eqref{eq:vectorized_sparse_dl_dgp} as 
\begin{equation}\label{eq:sparse_dl_dgp}
    \vX = \vA_{o}\vZ_{o} + \vE.
\end{equation}
The first problem we consider is: what if we know the \(\vA_{o}\) matrix, and want to represent our data \(\vX\) using the \(\vA_{o}\)? This is the purview of \textit{sparse coding}. Similar to orthogonal dictionary learning, we attempt to find the sparsest signals that are consistent with our observations, and this naturally leads to the following optimization problem:
...
This problem is known as the LASSO problem. However, unlike the PCA or the complete dictionary learning case, there is no clear power iteration-type algorithm to recover \(\vZ_{\star}\). A natural alternative is to consider solving the above optimization problem with gradient descent type algorithms. Let $f(\vZ) = \norm{\vX - \vA_{o}\vZ}_{2}^{2} + \lambda \norm{\vZ}_{1}$. Conceptually we could try to find \(\vZ_{\star}\) with the following iterations:
\begin{equation}
...
\begin{equation}
    \vX = \vA_{o}\vZ_{o} + \vE,
\end{equation}
where \(\vZ_{o}\) is sparse, and our goal previously was to estimate \(\vZ_{o}\) given knowledge of the data \(\vX\) and the dictionary atoms \(\vA_{o}\). Now we turn to the more usual, more natural, and more difficult case where we do not know either \(\vA_{o}\) or \(\vZ_{o}\) and seek to learn them from a large dataset. In particular, we solve the problem
\begin{equation}
...
\paragraph{Learned ISTA.} The above deep-network  interpretation of the alternating minimization is more conceptual than practical, as the process could be rather inefficient and take many layers or iterations to converge. But this is mainly because we try to infer both $\vZ_o$ and $\vA_o$ from $\vX$. The problem can be significantly simplified and the above iterations can be made much more efficient if we only use them to learn $\vA^\ell$ from a given set of samples of input and output  $(\vX, \vZ_o)$ for the iterative sparse encoding:
\begin{align}
...
Thus, given the sequential architecture, to learn the operator \(\vA^\ell\) at each layer, it is completely natural to learn it, say via back propagation (BP),\footnote{See Appendix \ref{app:optimization} for a brief description of BP.} by minimizing the error between the final code $\vZ^L$ and the ground truth $\vZ_o$:
\begin{equation}
