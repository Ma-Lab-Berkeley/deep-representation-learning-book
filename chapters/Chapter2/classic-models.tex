...
Thus, our central task is to learn a distribution which has low intrinsic dimension in a high-dimensional space. In the remainder of this section, we discuss several \textit{classical} methods to perform this task for several somewhat {\em idealistic} models for the distribution, namely models that are geometrically linear or statistically independent. While these models and methods are important and useful in their own right, we discuss them here as they motivate, inspire, and serve as a predecessor or analogue to more modern methods for more general distributions that involve deep (representation) learning.
...
\begin{itemize}
    \item Section \ref{sec:lowrank} --- Principal Components Analysis (PCA): Given noisy samples from a distribution supported on {\em one low-dimensional subspace}, obtain an estimate of the true sample which lies on this subspace.
    \item  Section \ref{sec:ica} --- Complete Dictionary Learning and Independent Components Analysis (ICA): Given noisy samples from a distribution supported on {\em the union (\textit{not} the span) of few low-dimensional subspaces}, obtain an estimate of the true samples.
...
\begin{figure}
    ...
    \caption{Images of human faces, handwritten digits, and features on a moving object. Despite the seemingly large variety in their appearances, each set of those data spans a very low-dimensional (nearly linear) subspace.}
...
\end{equation}
Here $\vz \in \R^d$ can be viewed as the compact code (or a latent representation) of a data point  $\x \in \R^D$ and the learned subspace basis $\vU$ as the associated code book whose columns are the (learned) optimal code words. The process achieves the function of denoising $\x$ by projecting it onto the subspace spanned by $\vU$.
...
\end{align}
Thus to compute the principal components, we find the orthogonal matrix \(\vU\) which maximizes \(\tr(\vU^{\top}(\vX\vX^{\top}/N)\vU)\). In fact, the best such matrix \(\vU^{\star}\) has columns which are these are the \textit{top \(d\) eigenvectors} of \(\vX\vX^{\top}/N\). While this can be obtained via brute force calculation, such as an inductive proof, we do not do so here; we instead provide a little intuition by considering the case \(d = 1\). Then we only have a single unit vector \(\vu\) to recover, and solve the problem \(\max_{\vu} \vu^{\top}(\vX\vX^{\top}/N)\vu\). This is the so-called \textit{Rayleigh quotient} of \(\vX\vX^{\top}/N\) and the optimal solutions to this problem are eigenvectors corresponding to the maximum eigenvalue of \(\vX\vX^{\top}/N\). Thus the worked out solution for the case \(d = 1\) corresponds to our proposed solution for the general \(d\) case. In summary:
\begin{tcolorbox}
...
Notice that the above formulation makes no statistical assumptions on the data-generating process. However, it is common to include statistical elements within a given data model, as it may add further enlightening interpretations about the result of the analysis. As such, we ask the natural question: \textit{what is the statistical analogue to low-dimensional structure?} Our answer is that a low-dimensional \textit{distribution} is one whose support is concentrated around a low-dimensional geometric structure. 
...
\begin{figure}
    ...
    \label{fig:subspaces}
\end{figure}
In this section, we will begin discussing the conceptual and algorithmic foundations for compressive autoencoding when the data distribution has structure modeled by {\em a mixture of low-dimensional subspaces}, as illustrated in Figure \ref{fig:subspaces}. In this setting, the decoder mapping will be almost as simple as the case of a single subspace: we simply represent via
...
However, the task of encoding such a data $\vx$ to suitable representations $\vz$, and learning such an encoder-decoder pair from data, will prove to be more involved.
...
\begin{equation}\label{eq:mixture-of-subspaces-statistical}
    \vx \sim \sum_{k=1}^K \pi_k \cN(\mathbf{0}, \vU_k\vU_k\adj), \quad \text{for some} \enspace \pi_k \geq 0,\enspace \sum_{k=1}^K \pi_k = 1.
\end{equation}
In words, for each $k \in [K]$, $\vx$ is Gaussian on the low-dimensional subspace $\Span(\vU_k)$  with probability $\pi_k$.
The model \eqref{eq:mixture-of-subspaces-statistical}, simple as it may seem, is already sufficiently rich to act as a basis for a theory of representation learning that can be scaled to large-scale modern datasets. 
However, we will have to wait until Chapter \ref{ch:representation} before we have developed the prerequisites for this theory.

\begin{remark}[A Mixture of Gaussians versus A Superposition of Gaussians]
...
\begin{equation}
    \vx = \sum_{i=1}^n w_i \vx_i, \quad \vx_i \sim \cN(\mathbf{0}, \vU_i\vU_i\adj),
\end{equation}
...
can then take $\cE(\vx) = \vU\adj \vx$ as the encoder and $\cD(\vz) = \vU \vz$ as the decoder, and $\cD = \cE^{-1}$.
Our task is then:
\begin{tcolorbox}
    Suppose $\vx$ satisfies the sparse dictionary model \eqref{eq:mixture-of-subspaces-dictionary} with orthogonal dictionary $\vU$ and sparsity level $d$.
    ...
    $\cE(\vx) = \vU\adj \vx$ and $\cD(\vz) = \vU \vz$ forms a lossless autoencoding pair on $\vx$.
\begin{equation}
\x \xrightarrow{\hspace{2mm} \mathcal{E} = \vU^* \hspace{2mm}}  \z \xrightarrow{\hspace{2mm} \mathcal{D} = \vU \hspace{2mm}}   \hat{\x}.  
\label{eqn:autoencoding-DL}
\end{equation}    
\end{tcolorbox}
We see that the autoencoding pair $(\cE, \cD)$ for complete dictionary learning is symmetric, as in the case of a single linear subspace, making the computational task of encoding and decoding no more difficult than in the linear case. 
On the other hand, the task of learning the dictionary $\vU$ is strictly more difficult than learning a single linear subspace by PCA. 
To see why we cannot simply use PCA to learn the orthogonal dictionary $\vU$ correctly, note that the 
loss function that gave rise to PCA, namely \eqref{eq:pca_equals_denoising}, is completely invariant to rotations of the rows of the matrix $\vU$: that is, if $\vQ$ is any $d \times d$ orthogonal matrix, then $\vU$ and $\vU \vQ$ are both feasible and have an identical loss for \eqref{eq:pca_equals_denoising}.
The sparse dictionary model is decidedly not invariant to such transformations: if we replaced $\vU$ by $\vU \vQ$ and made a corresponding rotation $\vQ\adj \vz$ of the representation coefficients $\vz$, we would destroy the sparsity structure of $\vz$, violating the modeling assumption. Thus, we need to develop new algorithms for learning orthogonal dictionaries.
...
In this section, we will derive algorithms for solving the orthogonal dictionary learning problem. To be more precise,  we assume the observed vector $\vx \in \R^D$ follows a statistical model
\begin{equation}
    \vx = \vU_o \vz + \veps, 
...
\begin{remark}[The Orthogonal Assumption] At first sight, the assumption that the dictionary $\vU_o$ is orthogonal might seem to be somewhat restrictive. But there is actually no loss of generality. One may consider a complete dictionary to be any square invertible matrix $\vD$. With samples generated from this dictionary: $\vX = \vD \vZ \in \mathbb{R}^{D\times N}$, it is easy to show\footnote{See for example \cite{sun2017completeI}.} that with some precoditioning of the data matrix $\vX$: 
\begin{equation}
    \bar{\vX} = \Big(\frac{1}{N\theta} \vX\vX^*\Big)^{-\frac{1}{2}}\vX,
\end{equation}
...
Now suppose we are given a set of observations:
\begin{equation}
    \x_i = \vU_o \vz_i + \veps_i, \quad, i =1, 2, \ldots, N.
\end{equation}
Let $\vX = [\x_1, \x_2, \ldots, \x_N]$ and $\vZ = [\vz_1, \vz_2, \ldots, \vz_N]$. The goal is to recover $\vU_o$ from the data $\vX$. Therefore, Given any orthogonal matrix $\vA \in \mathrm{O}(D)$, $$\vA\vx_i = \vA\vU_o \vz_i + \vA\veps_i$$ would be nearly sparse if $\vA = \vU_o$ (as by assumption the noise $\veps_i$ is of small magnitude). 
...
\begin{equation}\label{eq:orthogonal-dictionary-learning-l4}
    ...
    -   \frac{1}{4} \norm*{
    \vA \vX
    }_4^4 \quad \mbox{subject to} \quad  \vA^*\vA = \vI.
\end{equation}
...
$$
\vA = \mathcal{P}_{\mathrm O(D)}[( {\vA \vX})^{\circ3}\vX^*],
$$
where $\mathcal{P}_{\mathrm O(D)}[\,\cdot\,]$ is a projection onto the space of orthogonal matrices $\mathrm O(D)$.\footnote{For any matrix with an SVD: $\vM = \vU \Sigma \vV^*$, $\mathcal{P}_{\mathrm O(D)}[\vM] = \vU \vV^*$. We leave this as an exercise for the reader.} 
...
\begin{equation}
    \vA_{t+1} = \mathcal{P}_{\mathrm O(D)}[( {\vA_t \vX})^{\circ3}\vX^*].
\end{equation}
This is known as the {\em mathching, streching, and projection} (MSP) algorithm proposed by \cite{Zhai-2020}. It was shown that under broad conditions that such a greedy algorithm indeed converges to the correct solution at a superlinear rate.

\begin{remark}[Global Optimality of $\ell^4$ Maximization]
The constrained $\ell^4$ maximization problem seems to be a nonconvex program. In general one should expect that any greedy (say gradient-descent type) algorithms would converge to the globally optimal solution. Surprisingly, one can show that, unlike general nonconvex programs, the landscape of $\ell^4$ maximization over a sphere
\begin{equation}\label{eq:l4-maximization-sphere}
    \min\,
    -   \frac{1}{4} \norm*{
    \vq^* \vX
    }_4^4 \quad \mbox{subject to} \quad  \vq^*\vq = 1.
\end{equation}
...
$\delta \vA_{t+1} = \vA_{t+1}\vA_{t}^*$ and $\vZ_t = \vA_t \vX$, then it is easy to show that
$$\delta \vA_{t+1} = \mathcal{P}_{\mathrm O(D)}[(\vZ_t)^{\circ3} \vZ_t^*].$$ 
...
Note that the computation of the increment transform $\delta \vA_{t+1}$ at each layer depends only on the feature output from the previous layer $\vZ_t$. The network is naturally stable as each layer is a norm-preserving orthogonal transform. Despite its resemblance to a linear deep network, there is no need of any back propagation to learn each layer. All layers are learned in one forward pass!
\end{remark}
...
Thus a natural procedure for seeking non-Gaussian independent components is to search for a set of mutually-orthogonal directions $\vV \in \R^{d \times k}$ such that $\vV\adj \vX$ has maximal kurtosis, where $\vX = \vU \vZ \in \R^{D \times N}$ is the Bernoulli-Gaussian ICA data matrix.
...
\begin{equation}\label{eq:kurtosis-maximization-sphere-finitesample}
    \max_{\norm{\vv}_2^2 = 1}\, \kurt(\vX\adj \vv).
\end{equation}
First, we will perform some very basic analysis of this objective to verify its correctness. Notice by the change of variables $\vw = \vU\adj \vv$ that this problem is equivalent to
\begin{equation*}
    \max_{\norm{\vw}_2^2 = 1}\, 
    \mathrm{kurt}(\vZ\adj \vw).
\end{equation*}
...
\begin{split}\label{eq:fast-ica}
   ...
   \end{split}
\end{equation}
It turns out that the FastICA algorithm converges extremely rapidly (actually at a super-linear \textit{cubic} rate) to columns of the dictionary $\vU$. 
...
As we have seen, complete dictionary learning enjoys an elegant computational theory of compressive autoencoding in which we maintain a symmetric autoencoding structure $\cE(\vx) = \vU\adj \vx$, $\cD(\vz) = \vU \vz$, with a scalable power-method-like algorithm (the MSP algorithm) for learning the orthogonal dictionary/codebook $\vU$ from data.
Nevertheless, for learning representations of general high-dimensional data distributions, one must expand the size of the codebook beyond the orthogonality requirement---meaning that we must have $\vA \in \R^{D \times m}$, with $m \gg D$, corresponding to the case of an \textit{overcomplete} dictionary/codebook,\footnote{We change the notation here from $\vU$ to $\vA$ in order to emphasize the non-orthogonality and non-square-shape of the overcomplete dictionary $\vA$.} and the signal model
...
Geometrically, recall that in our original reduction from the mixture of subspaces data model to the sparse dictionary model, a mixture of $K$ subspaces in $\R^D$, each of dimension $d$, led to a dictionary of shape $\vA \in \R^{D \times Kd}$.
...
Given data samples of hand-written digits, Figure \ref{fig:}(a) shows the result of fitting an orthogonal dictionary to the dataset. 
%\sdb{describe the codes... describe the representations}
In contrast, \ref{fig:}(b) shows the result of running a classical algorithm for overcomplete dictionary learning known as K-SVD on these samples.\footnote{By the end of this section, we will have developed the conceptual and computational foundations to implement this algorithm ourselves.} 
...
\begin{tcolorbox}
    Suppose $\vx$ satisfies the model \eqref{eq:model-DL-overcomplete} with overcomplete dictionary $\vA$ and sparsity level $d$.
...
Although this problem is NP-hard in general, it can be solved efficiently and scalably for dictionaries $\vA$ which are \textit{incoherent}, i.e.\ having columns that are not too correlated.
...
\begin{exercise}\label{exercise:gaussian-rot-invar}
    ...
    Prove that for any orthogonal matrix $\vQ$ (i.e., $\vQ\adj \vQ = \vI$), the random variable $\vQ \vz$ is distributed identically to $\vz$. 
...
\begin{exercise}\label{exercise:whitening}
    ...
    \begin{enumerate}
        ...
        \item Show that the \textit{whitened matrix} $(\vY \vY\adj)^{-1/2}\vY$ exists in expectation whenever $\Cov(\vz)$ is nonsingular, and that it has identity empirical covariance.\footnote{In particular, it can be proved mathematically that this is enough to guarantee that the whitened matrix exists with high probability whenever $\vz$ satisfies a suitable concentration inequality and $N$ is sufficiently large.}
        \item Show, by using the singular value decomposition of $\vU$, that the matrix $\vV$ can be chosen so that the whitened matrix satisfies $(\vY \vY\adj)^{-1/2} \vY = \vW [\vz_1, \dots, \vz_N]$, where $\vW$ is an orthonormal matrix.
    \end{enumerate}
\end{exercise}
...
\begin{exercise}\label{exercise:sphere-calculus}
    ...
    \begin{enumerate}
        ...
        \end{equation*}
        and that the orthogonal projection onto this subspace is $\vP_{\vu}^\perp = \vI - \vu\vu\adj$.
...
\begin{exercise}\label{exercise:orthogonal-group-calculus}
    This exercise follows the structure and formalism introduced in Exercise \ref{exercise:sphere-calculus}, but applies it instead to the orthogonal group $\O(d) = \set{\vU \in \R^{d \times d} \given \vU\adj \vU = \vI}$. 
    Consult the description of Exercise \ref{exercise:sphere-calculus} for the necessary conceptual background; the formalism applies identically to the case where the ambient space is the set of $d \times d$ matrices as long as one recalls that the relevant inner product on matrices is $\ip{\vX}{\vY} = \tr(\vX\adj \vY)$.
    ...
    \begin{equation}\label{eq:exercise-orthogonal-group-constrained-max}
        \max_{\vQ\adj \vQ = \vI}\, f(\vQ). 
    \end{equation}
    \begin{enumerate}
        \item  It is easily seen that the orthogonal group has the defining equation $F(\vQ) = \vQ\adj \vQ = \vI$.
        Show, using this fact, that the tangent space to the orthogonal group at $\vQ$ is given by
        \begin{equation*}
            T_{\vQ} \O(d) = \set{\vQ \vOmega \in \R^{d\times d} \given \vOmega\adj = - \vOmega},
        \end{equation*}
        and that the orthogonal projection onto this subspace is 
        \begin{equation*}
        \cP_{T_{\vQ}\O(d)}(\vDelta) =  \vQ \Skew(\vQ \adj \vDelta),
        \end{equation*}
        where $\Skew(\vDelta) = \tfrac{1}{2} ( \vDelta - \vDelta\adj)$ is the orthogonal projection onto the set of skew-symmetric matrices.
        The vector field 
        ...
        \begin{equation}\label{eq:exercise-riemann-hess-orthogonal-group}
            ...
            \right) \cP_{T_{\vQ}\O(d)},
        \end{equation}
        where $\Symm(\vDelta) = \tfrac{1}{2}(\vDelta + \vDelta\adj)$ denotes the orthogonal projection onto the set of symmetric matrices, and $\kron$ denotes the Kronecker product of matrices.
        ...
        \begin{equation*}
            ...
            \vU \vV\adj,
        \end{equation*}
        where $\vX = \vU \vS \vV\adj$ is a singular value decomposition of $\vX$. 
        ...
        \begin{enumerate}
            ...
            \begin{align*}
                \left( \vQ\adj \vX \right)\adj &= \vQ\adj \vX, \\
                \vQ\adj \vX &\succeq \mathbf{0}.
            \end{align*}
            (\textit{Hint: use linearity of the Kronecker product in either of its two arguments when the other is fixed.})
            \item Using these conditions, argue that at every local minimizer $\vQ$ of \eqref{eq:projection-onto-ogrp-defn}, one has $\vQ\adj \vX = (\vX\adj \vX)^{1/2}$.
            %the loss value achieved is $\norm{(\vX\adj \vX)^{1/2} - \vI}_F^2$.
            (\textit{Hint: Use %unitary invariance of the $\ell^2$ norm and 
            the fact from linear algebra that if $\vS \succeq \mathbf{0}$ is a symmetric positive semidefinite matrix, then $(\vS\adj \vS)^{1/2} = \vS$.})
            \item Using the singular value decomposition $\vX = \vU \vS \vV\adj$, conclude that
            \begin{equation*}
                \vU \vV\adj
                =
                \mathrm{proj}_{\O(d)}(\vX).
