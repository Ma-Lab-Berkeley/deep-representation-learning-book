...
\end{equation}
A clear characterization and distinction are necessary and important because we want to study intelligence as a scientific and mathematical subject. It is highly likely that, even if they all may share the common objective of learning useful knowledge of the world, the specific computational mechanisms and physical implementations behind each level/form of intelligence could be different. We believe that the reader would better understand and appreciate these differences after having finished study this book. Therefore, we will leave more discussions about general intelligence to the last \Cref{ch:future}.
...
Nevertheless, we could not help but marvel at Wiener's foresight about the importance of nonlinearity. As we will see in this book, the answer was found only recently: nonlinearity can be effectively dealt with through progressive linearization and transformation realized by deep neural networks (see \Cref{ch:representation}). In addition, we will attempt to show in this book how all these mechanisms listed above can be naturally integrated into a complete system which would exhibit characteristics of an autonomous intelligent system (see \Cref{ch:autoencoding}). 
...
\begin{quote}
    \textbf{The Main Assumption:} {\em Any intelligent systems or learning methods should and could rely on is that the world is predictable, and hence that the distribution of the observed high-dimensional data samples have low-dimensional supports.}     
\end{quote}
...
\end{equation}
where $*$ is the convolution operator, $z$ is the input, and $h$ is the impulse response function.\footnote{Normally $h$ is assumed to have certain nice structures, say finite support or banded spectrum.}  Here $\epsilon[n]$ is some additive noise in the observations. The problem is: given the input process $\{z[n]\}$ and observations of the output process $\{x[n]\}$, how can we find the optimal $h[n]$ such that $\hat x[n] = h[n]*z[n]$ predicts $x[n]$ in an optimal way? In general, we measure the goodness of the prediction by the minimum mean squared error (MMSE):
\begin{equation}
...
\begin{figure}
    ...
    \caption{A distribution with two principal components. Recall that the principal components are the orthogonal vectors whose span approximately supports the structured data, ordered by the variance of the dataset along the given direction.}
    \label{fig:PCA}
\end{figure}
...
\begin{figure}
    ...
    \caption{PCA (left) versus ICA (right). While PCA learns the principal components, i.e., \textit{orthogonal} vectors which span the dataset, ICA learns vectors, not necessarily orthogonal, which span the dataset using linear combinations with \textit{independent} coefficients.}
    \label{fig:ICA-PCA}
\end{figure}
...
\end{equation}
where $\|\cdot \|_1$ is the sparsity-promoting $\ell^1$ norm of a vector and $\epsilon$ is some small constant. Any solution to this problem essentially gives a sparse (auto)encoding:
\begin{equation}
...
Generalizations to other measurement distributions (beyond Gaussian noise) have been made by Eero Simoncelli's group \cite{Raphan10}. The modern application of Tweedie's formula and the score function is to perform generation, say of images or videos, via iterative denoising \cite{Kadkhodaie21a,ho2020denoising}.
...
We will discuss later in this chapter and in \Cref{ch:compression} how such a seemingly simple concept of denoising and compression leads to a very unifying and powerful method for learning general low-dimensional distributions in a high-dimensional space, including the distribution of natural images.\footnote{So far we have discussed the problem of denoising a high-entropy noise source to a lower-entropy data distribution. Another common line of work is to identify better encoding schemes for data in order to compress it better \cite{yang2023lossy}. As we will see in \Cref{ch:compression}, these two problems are highly related and even dual to each other.}
...
\begin{figure}
    ...
    \caption{AlphaGo: Using deep neural networks to model the optimal policy (e.g., the probability of each move given the current state which maximizes the expected reward) or the optimal value function (e.g., the expected reward given the current state) for the game Go. }
    \label{fig:Alpha-Go}
\end{figure}

\paragraph{Generation and prediction.}
One may view early practices of deep networks in the 2010s as focused more on extracting relevant information from the data $\X$ and encoding it to a representation $\Z$, which is converted to a task-specific output $\vY$ (say $\vY$ represents class labels in classification tasks):
\begin{equation}
    \X   \xrightarrow{\hspace{2mm} f \hspace{2mm}} \Z \xrightarrow{\hspace{2mm} h \hspace{2mm}} \vY.
       \label{eqn:encoding-deep-networks}
\end{equation}
In this setting, the mapping $f$ (and $h$) to be learned does not need to preserve most distributional information about $\X$ but only the sufficient statistics needed for a specific task. For example, a sample $\x$ in $\X$ could be an image of an apple, which is mapped by $f \circ h$ to its class label $\vy =$ ``apple.'' The {\em information bottleneck framework} \cite{Tishby-ITW2015} explicitly utilized this compositional framework to train task-specific deep representations.
 
However, in many modern situations such as those so-called large foundation models, people often need to decode $\vY$ to recover the corresponding $\X$ to a certain degree of precision:
\begin{equation}
    \vY   \xrightarrow{\hspace{2mm} g  \hspace{2mm}} \hat \X.
       \label{eqn:decoding-deep-networks}
\end{equation}
As $\X$ typically represents data observed from the external world, a good decoder would allow us to simulate or predict what happens in the world. For example, in a ``text to image'' or ``text to video'' task, $\vy$ normally represents the texts that describe the content of a desired image $\x$. The decoder should be able to generate an $\hat \x$ that has the same content as $\x$. For example, given an object class $\vy = $ ``apple'', the decoder $g$ should generate an image $\hat \x$ that looks like an apple, although not necessarily exactly the same as the original $\x$. 
...
\begin{tikzpicture}
    ...
    \node (output) [right=of Xhat] {$\{0, 1\}$};
    \node (X) [below=of Xhat] {$\X$};
    
    % Define arrows
    \draw[->, thick] (Z) -- node[above] {$g$} (Xhat);
    \draw[->, thick] (X) -- node[below right] {$d$} (output);
    \draw[->, thick] (Xhat) -- node[above] {$d$} (output);
\end{tikzpicture}
\label{eqn:gan-networks}
\end{figure}
where $0$ and $1$ indicate an image is generated or not.
Intuitively, the harder it is to separate $\hat \X$ and $\X$ using a discriminator, the closer they are. 
...
\end{equation}
generated by a predicting function $f$. Note that without loss of generality, here we have assumed that all sequences are of the same length $D$. Therefore, each sequence can be viewed as a vector in $\mathbb{R}^D$. Secondly, we may introduce a coding scheme (with a code book), denoted as $\mathcal E$, that converts every segment $S_i$ to a unique stream of binary bits $\mathcal{E}(S_i)$ with length $L(\mathcal{E}(S_i))$, and a corresponding decoding scheme $\mathcal D$ which approximately recovers the segment $S_i$ from the encoding $\mathcal{E}(S_i)$. The simplest coding scheme is to fill the space spanned by all segments with $\epsilon$-balls, as shown in \Cref{fig:coding-schemes}. We then number all the balls. Each sequence is encoded as the (binary) number of its closest ball. In contrast, the decoding operation maps the input bit stream to the center of the corresponding ball. Hence, each segment can be recovered up to precision $\epsilon$ from its corresponding bit stream. 
\begin{figure}
    \centering
...
Then the complexity of the predicting function $f$ can be evaluated as the average coding length of all sequences $S$ generated by $f$, known as the coding rate:\footnote{One may make this more precise by taking $R(f\mid \mathcal{E})$ to be the expected coding length for all segments of length $D$. }
\begin{equation}
...
\end{equation}
we may say the coding scheme $\mathcal{E}_2$ is better. This difference can be viewed as a measure of how much more information $\mathcal{E}_2$ has over $\mathcal{E}_1$ about the distribution of the data. To a large extent, the goal of learning in this situation is equivalent to finding the most efficient encoding and decoding scheme that minimizes the coding rate subject to a given precision, for a fixed $f$:
\begin{equation}
   \min_{\mathcal{E}, \mathcal{D}} R(f \mid \mathcal E), \quad \text{subject to} \quad \operatorname{dist}(S, \mathcal{D}(\mathcal{E}(S))) \leq \epsilon. 
\end{equation}
As we will see in \Cref{ch:compression}, the achievable minimal rate is closely related to the notion of entropy $H(S)$ \eqref{eqn:entropy-definition}. This conceptual framework will provide a valuable insight to the learning performed within deep networks, as discussed in \Cref{ch:representation}.
...
\begin{remark}\label{rem:computable-complexity}
    ...
    \begin{equation}
        \frac{1}{N}\Bigg( L(\mathcal{E}) + \sum_{i=1}^N L(\mathcal{E}(S_i))\Bigg) \approx \frac{1}{N}\sum_{i=1}^N L(\mathcal{E}(S_i))
    \end{equation}
    as $N$ becomes large.}
\end{remark}
...
\begin{figure}
    ...
    \label{fig:expansion}
\end{figure}
As we will show in \Cref{ch:compression}, these desired structures in the final representation can be precisely promoted by choosing a natural measure of {\em information gain} based on the coding rates of the chosen coding schemes.\footnote{This notion of information gain is analogously defined in several more modern information-theoretic frameworks, including the one we introduce in this book, but also others such as the \(\mathcal{V}\)-entropy discussed in \cite{xu2020theory}. However, the latter notion of information breaks down when the data has low-dimensional structure, and is unsuitable for our purposes.} As we see throughout this book, such an explicit and constructive coding approach provides a powerful computational framework for learning good representations of low-dimensional structures for real-world data, as in many cases of practical importance, the coding length function can be efficiently computed or approximated accurately. In some benign cases, we can even obtain closed-form formulae, e.g., subspace and Gaussian (see \Cref{ch:compression}). 

In addition, such a computational framework leads to a principled approach that naturally reveals the role that deep networks play in this learning process. As we will derive systematically in \Cref{ch:representation}, the layers of a deep network are trying to perform operations that optimize the objective function of interest in an incremental manner. From this perspective, the role of deep networks can be precisely interpreted as to emulate a certain iterative optimization algorithm, say gradient descent, to optimize the objective of information gain. Layers of the resulting deep architectures can be endowed with precise statistical and geometric interpretation, namely performing incremental compressive encoding and decoding operations. As a result, the derived deep networks become transparent ``white boxes'' that are mathematically fully explainable.  
