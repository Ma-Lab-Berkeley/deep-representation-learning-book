...
\Cref{eq:ideal-cfg-denoiser} demonstrates an interesting alternate
...
\sdb{A graf about the fact that CFG is practically-motivated (as the derivation
shows), and rigorous theory that tries to study them may be too high-resolution
given this. We propose an understanding based on the rate reduction from ch4...}
...
\subsection{Visual World Model from Image Pairs and Sequences}
A typical problem of this kind is actually behind how we can learn a working model of the external world from the  images perceived, say through our eyes,  telescopes or microscopes. In particular, humans and animals are able to build a model of the 3D world ((or 4D for a dynamical world) through a sequence of its 2D projections -- a sequence of 2D images (or stereo image pairs). The mathematical or geometric model of the projection is generally known:
\begin{equation}
    \y_i = h(\x, \theta_i) +\vw, 
\end{equation}
...
\end{equation}
where $\theta_0$ and $\theta_1$ can be assumed to be known. $\y^0$ and $\y^1$ are two 2D-projections of the 3D scene $\x$. We may also assume that they have the same marginal distribution $p(\y)$ and we have learned a diffusion and denoising model for it. That, we know the denoiser:
\begin{equation}
  \bE[\vy \mid \vy_t=\vnu] =
...
Consider the denoising processing for $\y_t$ in \eqref{eq:y-posterior-sampling-denoiser}. We define a corresponding process $\x_t$ that satisfies: 
\begin{equation}
    \y_t = h(\x_t) + \vw_t,
\end{equation}
where $\vw_t$ is some small error. In general, we know if $t \rightarrow 0$ and $\vw_t \rightarrow \mathbf{0}$, we would expect $\y_t \rightarrow \y$ and $\x_t \rightarrow \x$ in distribution. 
...
\end{equation}
Suppose $\z = \x_t + \vv$ for some small increment $\vv$. We have
\begin{equation}
    h(\z) \approx h(\x_t) + \frac{\partial h}{\partial \x}\mid_{\x_t} \vv \doteq h(\x_t) + \vA(\x_t) \vv. 
\end{equation}
Hence, we can solve for $\hat{\x}_{t-s}$ approximately as:
\begin{equation}
    \hat{\x}_{t-s} = \x_t + st\vA(\x_t)^\dagger \nabla \log p_t(h(\x_t)).  
\end{equation}
In general, we may expect that the so defined process would converge to the distribution of $\x$. 

\paragraph{Uncalibrated image sequences.}
In the above derivation, we have assumed that the poses (and calibration) of the camera views are known, hence the map $h(\cdot)$ is fully known. In practice, at least some of the poses or calibration of the given images are unknown or only partially known. How can we generalize the problem of learning the distribution of 3D scenes to this more general setting? In fact, without specifying the view of an image pair, it might lead to a more compact representation of the 3D scene that is invariant to the viewpoint (if the camera views are specified with respect to some canonical reference frame of the scene). 
