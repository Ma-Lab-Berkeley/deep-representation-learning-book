...
Following \textcite{Ho2022-ry} and the common practice of training deep network
...
F(\x) = \va,\quad     G(\y) = \vb.
\end{equation}
In particular, $\{\x \mid G(h(\x)) = \vb\}$ is a super set of $\{\x \mid F((\x)) = \va\}$. 
Now given $\y = \vnu$, to solve for $\x$, we can solve the following contrained optimization problem:
\begin{equation}
    \min_{\x} \|h(\x) - \vnu\|_2^2 \quad \mbox{s.t.} \quad G(h(\x)) = \vb. 
\end{equation}
Using augmented Lagrange multiplier method, we can solve the following unconstrained program:
\begin{equation}
   \min_{\x} \|h(\x) - \vnu\|_2^2  + \vlambda^T (G(h(\x)) - \vb) + \frac{\mu}{2} \|G(h(\x)) - \vb\|_2^2.
\end{equation}
This is equivalent to the following program:
\begin{equation}
\min_{\x} = \|h(\x) - \vnu\|_2^2  - \log \exp\big( - \frac{\mu}{2} \|G(h(\x)) - \vc\|_2^2\big),   
\end{equation}
for some constant $c$ and vector $\vmu$. That is, the augmented Lagrange multiplier method that enforces the constraint $G(\y) = \vb$ is equivalent to use the score function of the probability density:
\begin{equation}
...
\begin{remark}[Federated denoising.]
\yima{Something very interesting about the above equation \eqref{eqn:pullback-denoise}. It seems to suggest we could try to learn the distribution of $\x$ through a process that coupled with (many of) its (partial) observations $\y^i = h^i(\x), i =1, \ldots, K$. In this case we obtain a set of equations:
\begin{equation}
    \vA^i(\x_t) \vv = t \nabla_{\y^i} \log p_t(h^i(\x_t)),
\label{eqn:federated-pushforward}
\end{equation}
where $\vA^i(\x_t) = \frac{\partial h^i}{\partial \x}\rvert_{\x_t}$. The final $\vv$ can be chosen as a ``centralized'' solution that satisfies all the above equations, or it could be chosen as a ``federated'' solution that is some aggregated version of all the distributed computed $\vv^i$:
\begin{equation}
    \vv^i = t\vA^i(\x_t)^\dagger \nabla_{\y^i} \log p_t(h^i(\x_t)), \quad i = 1, \ldots, K.
\end{equation}
An open question here is, exactly what the so-defined ``denoising'' process for $\x_t$ converges to, even in the linear measurement model case? A distribution that has the same low-dimensional support as the original $\x_0$, as $\y_t$ converges to $\y = h(\x_0)$? Maybe we can work on the case for a subspace or a mixture of subspaces?}
\end{remark}
