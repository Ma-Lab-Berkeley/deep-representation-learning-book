...
\end{equation} As $\mu$ becomes large when enforcing the constraint via continuation, $\|\vc\|_2$ becomes small.
...
\begin{example}\label{example:denoising-gaussian-mixture-cfg}
  ...
  We now perform a concrete analysis of the form of this guided denoiser in
  ...
  \Cref{eq:cfg-denoiser-mog-low-rank}.
  ...
  distributions, as the ideal denoiser \Cref{eq:cfg-denoiser-mog-low-rank} has
  ...
  % loosely parallels the structure of the rate reduction gradient ascent
  % iteration we studied in the classification setting in \Cref{ch:unrolling}, in
  % the special case where a single sample is used.
\end{example}
