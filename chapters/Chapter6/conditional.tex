...
\paragraph{Statistical Interpretation via the Bayes' Rule.} Generally speaking, to accomplish such tasks well, we essentially need to get a hold of the conditional distribution $p(\x\mid \y)$. Either the maximal likelihood estimate (prediction): $$\hat{\x} = \arg\max_{\x} p(\x\mid \y),$$ the conditional expected estimate: $$\hat{\x} = \mathbb{E}(\x \mid \y) = \int \x p(\x\mid \y)~d \x,$$ or the conditional generation (sampling):  $$\hat{\x} \sim p(\x \mid \y)$$ can be inferred from the conditional distribution. 

Notice that from the {\em Bayes' rule}, we have
\begin{equation}
...
Notice that solving the above program via gradient ascent is equivalent to the above maximal likelihood estimate \eqref{eqn:loglikelihoo-gradient-ascent}, 
in which the gradient  takes the form:
\begin{equation}
   \nabla_{\x} \log p(\y \mid \x) + \nabla_{\x} \log p(\x)   =  J_h(\x)\big(\y - h(\x)\big) + \mu J_F(\x) \big(\vc - F(\x)\big),
\end{equation}
where $J_h(\x), J_F(\x)$ are the Jacobian of $h(\x), F(\x)$, respectively. Notice that, the augmented Lagrange multiplier method that enforces the constraint $F(\x) = \boldsymbol{0}$ via gradient ascent is equivalent to use the score function of the probability density $p(\x)$ and conditional probability density $p(\y\mid \x)$ (in the space of $\x$).
