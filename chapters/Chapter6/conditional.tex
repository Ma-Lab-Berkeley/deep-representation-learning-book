...
\end{equation} 
where $\vc \doteq {\vlambda}/{\mu}$ can be viewed as a ``mean''  for the constraint function.
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{figs_chap6/ambient_diffusion.png}
...
\bR^{D \times N}$ in this context, since it corresponds to a sequence of
...
\begin{figure}[tbp]
  ...
  \begin{subfigure}{0.47\textwidth}
    \includegraphics[width=\linewidth]{figs_chap6/tti-train.pdf}
    \caption{}
    % \label{}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.47\textwidth}
    \includegraphics[width=\linewidth]{figs_chap6/tti-inf.pdf}
    \caption{}
    % \label{}
  \end{subfigure}
  % \includegraphics[width=0.8\linewidth]{figs_chap6/tti.pdf}
  ...
  text to a sequence of vectors, see \Cref{sec:clm_text}. \sdb{TODO: pngs}}
  \label{fig:text-to-image}
\end{figure}

Stable Diffusion follows the conditional generation methodology we outline in
\Cref{sub:cfg}, with two key modifications: (i) The conditioning signal is an
encoded text prompt $\vY$, rather than a class label; (ii) Image denoising is
...
\begin{enumerate}
  \item \textbf{At training time},
    ...
    \bR^{d_{\mathrm{img}}}$;
  \item \textbf{At generation time}, sampling is performed on the representations $\vz_t$, and
    the final image is generated by applying the decoder $g(\vz_0)$.
\end{enumerate}
...
\textit{outputs the value-projected text embeddings, weighted by correlations
between the image features and the text embeddings.}
