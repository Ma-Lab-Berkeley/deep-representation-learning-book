...
DPS algorithm \eqref{alg:iterative_denoising_conditional}. This is a powerful
...
\begin{equation}
  f_{\theta_{\mathrm{c}}} : (t, \vx_t) \mapsto \softmax(\vW_{\mathrm{head}} \vz_t).
\end{equation}
This expression combines the final representations $\vz_t$ (which also depend on
...
\end{equation}
where $\theta_0$ and $\theta_1$ can be assumed to be known. $\y_0$ and $\y_1$ are two 2D-projections of the 3D scene $\x$. We may also assume that they have the same marginal distribution $p(\y)$ and we have learned a diffusion and denoising model for it. That, we know the denoiser:
\begin{equation}
  \bE[\vy \mid \vy_t=\vnu] =
...
The main question here is: How to learn (a representation for) the distribution of the 3D scene $\x$ from its two projections with known relationships? Or equivalently, how to learn a denoiser for $\x$
\begin{equation}
  \bE[\vx \mid \vx_t=\vxi] =
