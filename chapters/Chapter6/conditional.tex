...
\begin{equation}
    \y = h(\x) + \boldsymbol{w},
\end{equation}
where $h(\cdot)$ represents measurements of a part of $\x$ or its certain observed attributes and $\vw$ represents some measurement noise and even (sparse) corruptions, solve the ``inverse problem'' of obtaining a most likely estimate $\hat \x(\y)$ of $\x$ or generating a sample $\hat{\x}$ that is at least consistent with the observation $\y \approx h(\hat{\x})$. 

\paragraph{Statistical Interpretation.} Generally speaking, to accomplish such tasks well, we essentially need to get a hold of the conditional distribution $p(\x\mid \y)$. Either the maximal likelihood estimate (prediction): $$\hat{\x} = \arg\max_{\x} p(\x\mid \y),$$ the conditional estimate: $$\hat{\x} = \mathbb{E}(\x \mid \y),$$ or the conditional generation (sampling):  $$\hat{\x} \sim p(\x \mid \y)$$ can be inferred from the conditional distribution. 

Notice that from the Bayes' rule, we have
\begin{equation}
...
\end{equation}
Computing the conditional distribution naturally depends on the low-dimensional distribution $p(\x)$ of the data $\x$ and the observation model $h(\cdot)$ that determines the conditional distribution $p(\y \mid \x)$. 

\paragraph{Geometric Interpretation as Constrained Optimization}

As the support of the distributions of $\x$ is low-dimensional, we may assume that there exist functions:
\begin{equation}
F(\x) = \boldsymbol{0}
\end{equation}
such that $\mathcal{S}_{\x} \doteq \{\x \mid F((\x)) = \va\}$ is the low-dimensional support of the distribution $p(\x)$. Geometrically, one choice of $F(\x)$ is the ``sign distance function'' to the support $\mathcal{S}_{\x}$.
...
\begin{equation}
   \max_{\x} -\frac{1}{2}\|h(\x) - \y\|_2^2  + \vlambda^T F(\x) - \frac{\mu}{2} \|F(\x)\|_2^2
\end{equation}
for some constant Lagrange multipliers $\vlambda$.
This is equivalent to the following program:
\begin{equation}
\max_{\x} \log \exp\Big(- \frac{1}{2}\|h(\x) - \y\|_2^2\Big) + \log \exp\Big( - \frac{\mu}{2} \big\|F(\x) - \vlambda/\mu\big\|_2^2\Big),   
\end{equation} where $\vc \doteq {\vlambda}/{\mu}$ can be viewed as a ``mean.'' 

We may view the first term as the conditional probability of $\y$ given $\x$, and the second term as a probability density for $\x$:
\begin{equation}
p(\y\mid \x) \propto \exp\Big(- \frac{1}{2}\|h(\x) - \y\|_2^2\Big), \quad 
    p(\x) \propto \exp\Big( - \frac{\mu}{2}\|F(\x) - \vc\|_2^2\Big).
\end{equation} As $\mu$ becomes large when enforcing the constraint via continuation, $\|\vc\|_2$ becomes increasingly small.

The gradient of the above objective function takes the form:
\begin{equation}
   \nabla_{\x} \log p(\x) +  \nabla_{\x} \log p(\y \mid \x) =  \mu J_F(\x) \big(\vc - F(\x)\big) +    J_h(\x)\big(\y - h(\x)\big),
\end{equation}
where $J_F(\x), J_h(\x)$ are the Jacobian of $F(\x), h(\x)$, respectively. Notice that, the augmented Lagrange multiplier method that enforces the constraint $F(\x) = \boldsymbol{0}$ via gradient ascent is equivalent to use the score function of the probability density $p(\x)$ and measurement matching function of $p(\y\mid \x)$ (in the space of $\x$).
%One may view this as the geometric interpretation of the conditional generation score developed earlier in \eqref{eqn:conditional-denoising-score}. 
...
\section{Conditional Generation with a Learned Data Representation}
In the previous subsection, the reason we can infer $\X$ from the partial observation $\Y$ is because (support of) the distribution of $\X$ is given, say as the set of all low-rank matrices. For many practical dataset, we do not have its distribution in an analytical form as the low-rank matrices, say the set to of all natural images. 
...
One empirical approach is to find an encoding and decoding scheme by
...
\end{equation}
This usually works very well in practice, say for many image restoration tasks, as shown in \cite{daras2023ambient}.   
...
\section{Conditional Generation with Paired Data and Measurements}
...
$\vx_t$. To see how this structure arises, we begin with an `idealized' version
of 
the classifier guidance denoiser $\bar{\vx}_{\theta}^{\mathrm{CG}}$ defined in
...
\yima{Following Remark \ref{rem:geometry}, how effective the above approaches are to utilize the conditional probability $p(y\mid \x)$ depends on whether the conditional probability is given in a form of an effective constraint on $\x$.}
...
\begin{algorithm}
  \caption{Conditional sampling with classification data, using classifier-free
  guidance.}
    ...
    $p_{\vx}$).}
		\Require{Scale and noise level functions \(\alpha, \sigma \colon \{t_{\ell}\}_{\ell = 0}^{L} \to \R_{\geq 0}\).}
    \Require{Guidance strength $\gamma > 0$ ($\gamma > 1$ preferred for
    performance).}
...
Below, we will give a rather complete interpretation in the simplifying setting
of a Gaussian mixture model data distribution and denoiser, which will
demonstrate an interesting connection to the rate reduction objective studied in
chapters \Cref{ch:compression,ch:unrolling}, and thereby an insight into the
\textit{parameterization} of the denoiser in the presence of such
...
\begin{example}\label{example:denoising-gaussian-mixture-cfg}
  ...
  \sdb{The example may focus on comparing CFG with $\Delta R$ to give some
  understanding of this methodology. Challenging to work in the parameterization
  issue here...}
\end{example}

\sdb{We can focus on giving some understanding of the parameterization issue in
the next session? Not clear how to do it in the mixture of Gaussians case...}
...
In this last section, we consider the more extreme, but actually ubiquitous, case for distribution learning in which we only have a set of observed samples $\Y = \{\y_1,\ldots, \y_N\}$ of the data $\x$. To make the problem well-defined, we do assume that the observation model between $\y$ and $\x$ is known to belong to a certain family of analytical models, denoted as $\y = h(\x, \theta)$, with $\theta$ either known or not known.
...
\paragraph{The Under-determined Case.} 
First, we consider the case when $\vA$ is of full row rank, i.e., under-determined. Let us define the corresponding process $\x_t$ as one that satisfies:
\begin{equation}
\y_t = \vA \x_t.   
\end{equation}
...
\begin{equation}
    \vA(\x_{t-s} - \x_t) = st \nabla \log p_t(\vA\x_t). 
\end{equation}
Among all $\x_{t_s}$ that satisfy the above constraint, we choose the one that minimizes the distance $\|\x_{t-s} - \x_t\|_2^2$. Therefore, we obtain a ``denoising'' process for $\x_t$:
\begin{equation}
    \x_{t-s}  = \x_t + st\vA^\dagger \nabla \log p_t(\vA\x_t). 
...
\subsection{Visual World Model from Calibrated Image Pairs}
...
\begin{equation}
    \y^i = h(\x, \theta^i) +\vw, 
\end{equation}
where $h(\cdot)$ represents a  (perspective) projection of the 3D (or 4D) scene from a certain camera view at time $t_i$ to a 2D image (or a stereo pair) and $\vw$ is some possibly additive small measurement noise. Figure \ref{fig:projection-2D} illustrates this relationship. A full exposition of geometry related to multiple 2D views of a 3D scene is beyond the scope of this book. Interested readers may refer to the book \cite{MaY2003}. For now, all we need to proceed is that such projections are well understood and multiple images of a scene contains sufficient information about the scene.
\begin{figure}[t]
    \centering
...
\begin{equation}
    h(\x_{t-s}) \approx h(\x_t) + \frac{\partial h}{\partial \x}\bigg\rvert_{\x_t} \vv s \doteq h(\x_t) + \vA(\x_t) \vv s. 
\end{equation}
Hence, we have
\begin{equation}
    \vA(\x_t) \vv = t \nabla_{\y} \log p_t(h(\x_t)).
...
\end{equation}
Geometrically the vector $\vv$ in the domain of $\x$ can be viewed as the pullback of the vector field $t \nabla \log p_t(\y)$ under the map $\y = h(\x)$. In general, we may choose $\vv$ to be the minimum 2-norm vector that satisfies the pullback relationship. Hence, we can express  $\hat{\x}_{t-s}$ approximately as:
\begin{equation}
...
In general, we may expect that the so defined process would converge to the distribution of $\x$ or a distribution that has the same support as $\x$? 

\begin{remark}[Geometric Interpretation via Constrained Optimization]
{
As the support of the distributions of $\x_0$ and $\y_0 = h(\x)$ is low-dimensional, we may assume that there exist functions:
\begin{equation}
F(\x) = \boldsymbol{0},\quad     G(\y) = \boldsymbol{0}.
\end{equation}
Let $\mathcal{S}_{\y} \doteq \{\y \mid G(\y) = \boldsymbol{0}\}$ be the support of $p(\y)$. We further assume that $G(\mathcal{S}_{\x}) \subset \mathcal{S}_{\y}$. 
In general, $G^{-1}(\mathcal{S}_{\y}) = \{\x \mid G(h(\x)) = \boldsymbol{0}\}$ is a super set of $\mathcal{S}_{\x} \doteq \{\x \mid F(\x) = \boldsymbol{0}\}$. 
Now given $\y = \vnu$, to solve for $\x$, we can solve the following constrained optimization problem:
\begin{equation}
...
\begin{remark}[Distributed Compressive and Federated Denoising.]
...
\end{equation}
where $\vA^i(\x_t) = \frac{\partial h^i}{\partial \x}\rvert_{\x_t}$. The final $\vv$ can be chosen as a ``centralized'' solution that satisfies all the above equations, or it could be chosen as a ``federated'' solution that is some (stochastically) aggregated version of all $\vv^i$:
\begin{equation}
    \vv^i = t\vA^i(\x_t)^\dagger \nabla_{\y^i} \log p_t(h^i(\x_t)), \quad i = 1, \ldots, K,
\end{equation}
that are computed in a distributed fashion?
An open question here is, exactly what the so-defined ``denoising'' process for $\x_t$ converges to, even in the linear measurement model case? When would it converges to a  distribution that has the same low-dimensional support as the original $\x_0$, as $\y_t$ converges to $\y = h(\x_0)$? }
\end{remark}



\subsection{Visual World Model from Uncalibrated Image Sequences}
In the above derivation, we have assumed that the measurement model $h(\cdot)$ is fully known. In the case of stereo vision, this is rather reasonable as the poses (and calibration) of the camera views are usually known in advance.\footnote{The relative position of our two eyes is well known to our brain.} In practice, this is not always the case though. For the 3D vision problem, in general at least some of the poses or calibration of the given images are unknown or only partially known. For example, when we move around and survey our surroundings, the relative motion of our eyes between different times is usually not known. Or there are objects moving in the scene and we normally do not know their motion. How can we generalize the problem of learning the distribution of 3D scenes to such more general settings? In fact, without specifying the ego-centric frame as the reference for each stereo pair, it might lead to a more compact representation of the 3D scene that is invariant to the ego-centric viewpoint (say, if the camera views are specified with respect to some canonical world frame of the scene). In fact, our brain seems to be able to learn such a representation that decouples objects in a 3D scene and the motion of the observer or the objects.

Let us model a sequence of stereo pairs as:
\begin{equation}
    \y^k = h(\x^k, \theta^k), \quad k=1, \ldots, K,
\end{equation}
where $h(\cdot)$ represents the projection map from 3D to 2D. $\theta^k$ denotes the motion parameters of the $k$th view. $\x^k$ represents the 3D scene at time $k$. If the scene is static, $\x^k$ should all be the same. To simplify the notation, we may denote the set of $k$ equations as one:
\begin{equation}
    \Y = H(\X,\Theta). 
\end{equation}

Now suppose we have learned the distribution of $\Y$ via a denoisng process $\Y_t$. Similar as before, we may assume $\X_t$:
\begin{equation}
    \Y_t \approx H(\X_t, \Theta_t). 
\end{equation}

Heuristically, we then can iteratively solve for $\X_{t-s}, \Theta_{t-s}$ from
\begin{equation}
    (\hat{\X}_{t-s}, \Theta_{t-s}) = \argmin_{\Z, \Xi} \|H(\Z, \Xi) - H(\X_t, \Theta_t) - st \nabla \log p_t(H(\X_t,\Theta_t))\|_2^2.
\end{equation}
Suppose $\Z = \X_t + \Delta \X, \Xi = \Theta_t + \Delta \Theta$ for some small increments. We have
\begin{equation}
    H(\Z, \Xi) \approx H(\X_t, \Theta_t) + \frac{\partial H}{\partial \Z}\bigg\rvert_{\X_t, \Theta_t} \Delta \X +  \frac{\partial H}{\partial \Xi}\bigg\rvert_{\X_t, \Theta_t} \Delta \Theta \doteq H(\X_t,\Theta_t) + \vA \Delta \X + \vB \Delta \Theta. 
\end{equation}
Hence, we can solve for $\hat{\X}_{t-s}, \hat{\Theta}_{t-s}$ approximately in an alternating fashion:
\begin{eqnarray}
    \hat{\X}_{t-s} &\approx& \X_t + \vA^\dagger(st  \nabla \log p_t(H(\X_t, \Theta_t))),\\
    \hat{\Theta}_{t-s} &\approx& \Theta_t +  \vB^\dagger(st \nabla \log p_t(H(\X_t, \Theta_t)) - \vA (\hat{\X}_{t-s} - \X_t)).
\end{eqnarray}
\yima{The order of alternation might be important for better convergence... }
