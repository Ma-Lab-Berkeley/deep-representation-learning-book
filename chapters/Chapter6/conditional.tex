...
diffusion models such as StableDiffusion 3.5
...
\begin{example}\label{example:denoising-gaussian-mixture-cfg}
  ...
  \end{enumerate}
  \sdb{The above could easily be repackaged into an exercise, which would be
  a more rigorous version of this calculation.}
  ...
  $\sqrt{P}$, by the concentration of measure phenomenon\footnote{One of the few
  \textit{blessings of dimensionality}---see \textcite{Wright-Ma-2022}.}. Then
...
of \textit{embeddings} of the class label $y$ into a common space with $\vx$ in
order to provide a concise and unified scheme for parameterizing the optimal
denoisers (conditional and unconditional).
Actually, this basic scheme can be scaled to massive-scale datasets, and
provides the basis for the majority of the most popular text-to-X models (where
``X'' denotes image, video, ...), such as StableDiffusion
\cite{DBLP:conf/icml/EsserKBEMSLLSBP24}.
Below, we will describe an early such instantiation, called unCLIP, which
underlies OpenAI's DALL-E 2 text-to-image system \cite{Ramesh2022-nu}.
