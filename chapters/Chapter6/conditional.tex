...
A typical problem of this kind is actually behind how our brain generate a model of the external world through our eyes. That is, we are able to build a 3D model (or 4D if the scene is dynamic) of the world through a sequence of its 2D projections -- a sequence of 2D images (or stereo image pairs):
\begin{equation}
    \y_i = h(\x, t_i) +\vw 
\end{equation}
where $h(\cdot)$ represents the projection of the scene at time $t_i$ from a certain view and $\vw$ is some small measurement noise.

In general, we would like to learn the distribution of the 3D (or 4D) world $p(\x)$ from the perceived images. The primary function of such a world model is to allow us to predict what the world would look alike from a new view or at a future time. \yima{}
