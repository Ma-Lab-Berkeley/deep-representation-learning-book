...
\begin{equation}
    \cL(\phi) = \cS(\phi) + \cR(\phi) \leq u_{\theta, \beta}(\phi) \doteq \cS(\theta) + \ip{\nabla \cS(\theta)}{\phi - \theta} + \frac{\beta}{2}\norm{\phi - \theta}_{2}^{2} + \cR(\phi).
\end{equation}
Note that (proof as exercise)
\begin{equation}
    \argmin_{\phi}u_{\theta, \beta}(\phi) = \argmin_{\phi}\bs{\frac{\beta}{2}\norm*{\phi - \bp{\theta - \frac{1}{\beta}\nabla \cS(\theta)}}_{2}^{2} + \cR(\phi)}.
\end{equation}
Now if we try to minimize the upper bound \(u_{\theta, \beta}\), we are picking a \(\phi\) that:
\begin{itemize}
    \item is close to the gradient update \(\theta - \frac{1}{\beta}\nabla \cS(\theta)\);
    \item has a small value of the regularizer \(\cR(\phi)\)
\end{itemize}
...
\begin{equation}
    \prox_{h, \cR}(\theta) \doteq \argmin_{\phi}\bs{\frac{1}{2h}\norm{\phi - \theta}_{2}^{2} + \cR(\phi)}.
\end{equation}
...
\begin{example}\label{example:prox-of-characteristic-function}
    ...
    \begin{equation}
        \prox_{h, \chi_{\Gamma}}(\theta) = \argmin_{\phi \in \Gamma}\frac{1}{2}\norm{\phi - \theta}_{2}^{2} = \argmin_{\phi \in \Gamma}\norm{\phi - \theta}_{2}.
    \end{equation}
\end{example}
...
\begin{example}\label{example:prox-of-l1}
    ...
    \begin{equation}
        S_{h}(\theta) \doteq \prox_{h, \lambda \norm{\cdot}_{1}}(\theta) = \argmin_{\phi}\bs{\frac{1}{2h}\norm{\phi - \theta}_{2}^{2} + \lambda\norm{\phi}_{1}}
    \end{equation}
...
\begin{example}\label{example:prox-of-nonnegative-l1}
    ...
    \begin{equation}
        T_{h}(\theta) \doteq \prox_{h, \lambda\norm{\cdot}_{1} + \chi_{\R_{+}^{n}}}(\theta) = \argmin_{\phi \in \R_{+}^{n}}\bs{\frac{1}{2h}\norm{\phi - \theta}_{2}^{2} + \lambda\norm{\phi}_{1}},
    \end{equation}
