...
Finally, let us consider the case where we are to \textit{train} a causal CRATE model, and want to find the mostt efficient way to do this given a full input sequence \(\vX = [\vx_{1}, \dots, \vx_{N}]\) simultaneously. The ISTA step is parallelizable to become the regular ISTA step from the non-causal CRATE, i.e.,
\begin{equation}
