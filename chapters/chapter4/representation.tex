...
\label{ch:lossy-compression}\label{ch:representation-learning}
...
the \textit{rate distortion}, denoted as $\cR_{\epsilon}(\x)$.

To understand this rate distortion better, we turn to a deep theorem of information theory, originally proved in \textcite{shannon1959coding}, which relates this rate distortion to other information-theoretic quantities. To understand this theorem, we first need to define a measure of \textit{difference} between two distributions known as the \textit{Kullback-Leibler} (KL) divergence, or \textit{relative entropy}, which is denoted by \(\KL(\cdot \mmid \cdot)\) and defined in relation to the entropy and cross-entropy (see \Cref{sub:min_entropy}) as
\begin{align}
	\KL(p \mmid q) &= \CE(p \mmid q) - h(\vx) = \Ex_{\vx \sim p}[\log 1/q(\vx)] - \Ex_{\vx \sim p}[\log 1/p(\vx)] \\
	&= \Ex_{\vx \sim p}[\log (p(\vx)/q(\vx))].
\end{align}
The KL divergence is always non-negative, and equals zero if and only if \(p = q\);\footnote{Technically, this equality should be taken to mean ``almost everywhere'', i.e., except possibly on a set of zero measure (volume), since this set would not impact the value of any integral.} in the case where \(p\) and \(q\) have the same support, this can be proved by the following calculation based on Jensen's inequality and the fact that the function \(\log(\cdot)\) is strictly concave:
\begin{align}
	- \KL(p\mmid q)
	&= -\Ex_{\vx \sim p}[\log(p(\vx)/q(\vx))] = \Ex_{\vx \sim p}[\log(q(\vx)/p(\vx))] \\
	&\leq \log \Ex_{\vx \sim p}[q(\vx)/p(\vx)] =  \log \int_{\R^{D}} \frac{q(\vxi)}{p(\vxi)}p(\vxi)\odif{\vxi} \\
	&= \log \int_{\R^{D}}q(\vxi)\odif{\vxi} = \log \Ex_{\vx \sim q}[1] = \log 1 = 0.
\end{align}
Then, we define the \textit{mutual information} \(I(\cdot; \cdot)\), which is a measure of \textit{dependence} between two random variables, as
\begin{equation}\label{eq:mutual-information}
	I(\vx; \hat{\vx})
	= \KL(p(\vx, \hat{\vx}) \mmid p(\vx) p(\hat{\vx})),
\end{equation}
With the previous definitions in place, the theorem states that the rate distortion \(\cR_{\epsilon}\) can be expressed in purely probabilistic terms as
\begin{equation}
	\cR_{\epsilon}(\x) 
...
{\color{blue}
\begin{example}[Rate distortion of Gaussian source (see Theorem 13.3.3 in \cite{Cover-Thomas})]\label{exam:rate Gaussian}
    Consider a Gaussian random vector $\vx \sim \cN(\bm 0, \bm \Sigma)$, where $\bm \Sigma = \mathrm{diag}(\sigma_1^2,\dots,\sigma_d^2)$. For this case, it is well-known that the rate-distortion function can be computed in closed form using the \emph{reverse water-filling} principle over the eigenvalues of the covariance matrix. Specifically, the rate distortion function is 
    \begin{align}\label{eq:rate 1}
        \mathcal{R}_{\epsilon}(\bm x) = \frac{1}{2}\sum_{i=1}^d\log\left( \frac{\sigma_i^2}{\lambda_i} \right),
    \end{align}
    where the ``water level''$\lambda$  satisfies 
    \begin{align}
    \begin{cases}
        & \lambda_i = \min\{\lambda, \sigma_i^2\},\ i = 1,\dots,d, \\
        & \sum_{i=1}^d \lambda_i = \epsilon^2.  
    \end{cases}
    \end{align}
     Intuitively, directions with variance $\sigma_i^2$ larger than $\lambda$ are 
    effectively encoded, while directions with smaller variance are discarded, 
    ensuring that the distortion constraint is satisfied with minimal mutual information. 
    Please refer to \Cref{exer:KKT} for a detailed derivation of this closed form. 

    Importantly, this result naturally extends to a Gaussian random vector with an arbitrary positive-definite covariance matrix, i.e., $\vx \sim \mathcal{N}(\mathbf{0}, \hat{\bm \Sigma})$ with $\hat{\bm \Sigma} \succ \mathbf{0}$. Let $\lambda_1 \ge \dots \ge \lambda_d > 0$ denote the eigenvalues of $\hat{\bm \Sigma}$. Then \eqref{eq:rate 1} remains valid, with $\sigma_i^2$ replaced by $\lambda_i$, reflecting the effective variance along each principal direction of $\hat{\bm \Sigma}$.
\end{example}}
...
\begin{remark}
	...
	\begin{equation}
		I(\x; \hat\x) = H(\x) - H(\x \mid \hat \x),
	\end{equation}
    where $H(\x)$ is defined in \eqref{eqn:entropy-discrete} of \Cref{sub:min_entropy}. 
...
\begin{itemize}
	\item First, one may notice that $R_{\epsilon}(\X)$ closely resembles the rate distortion function of a Gaussian source \cite{Cover-Thomas} (see \Cref{exam:rate Gaussian}). Indeed, when $\epsilon$ is small, the above expression is a close approximation to the rate distortion of a Gaussian source, as pointed out by   \cite{MaY2007-PAMI}.
...
From the above example of classification, we see that the so-learned representation $\z$ of $\x$ gives a very simple encoder that essentially maps each class of data to only one code word: the one-hot vector representing each class. From the lossy compression perspective, such an encoder is too lossy to preserve information in the data distribution. Other information, such as that useful for tasks such as image segmentation, completion, or generation, is severely lost in such a supervised learning process. 

To remedy this situation, we want to learn a different encoding scheme such that the resulting feature representation $\z$ can capture a much information about the data distribution as possible, not limited to that useful for classification alone. More precisely, we want the representation $\z = f(\x, \theta)$ to be such that there exists a decoder $g(\z,\eta)$ so that: 
\begin{equation}
\|\x - \hat \x\|_2 \le \epsilon \quad \mbox{where} \quad \hat{\x} = g(f(\x)).
\end{equation}
We will formally introduce such an encoding-decoding scheme as an ``autoencodor'' and such a representation $\z$ as a ``consistent'' representation  in Chapter \ref{ch:autoencoding}. For now, the reader may simply view this as a generalization to the quantization code $\hat \x$ introduced in the preceding section for the rate distortion (in the native data $\x$ domain).  

In the formulation of information bottleneck \eqref{chap4-eqn:information-bottleneck}, it does not require the learned feature representation $\z$ to be consistent. Hence, at least it should be fixed with an objective for a (consistent) representation learning (RL):
\begin{eqnarray}
	\max_{\theta,\eta}\; \mbox{RL}(\x, \y, \z) \doteq I(\z; \y) - \beta I(\x; \z) \nonumber \\ \mathrm{s.t.}\quad  \z = f(\x,\theta),\quad  \|\x - g(\z, \eta) \|_2 \le \epsilon. 
	\label{chap4-eqn:representation-learning}
\end{eqnarray}
Solving the above problem in the most general setting is a very daunting task. In general, we do not even how what classes of encoders or decoders could ensure the consistency\footnote{In practice, people tend to learn them, separately from the above objective, through trial and error, as we will discuss more in Chapter \ref{ch:autoencoding}.} Also, since we do not know the distribution of the representation $\z$, computing the mutual information $I(\z;\y$ can be very challenging, if not impossible.
...
The second property requires that for samples that belong to different classes or are believed to be dissimilar, their features should be made (geometrically) incoherent or (statistically) independent, as illustrated before in Figure \ref{fig:representation-learning}. \Cref{chap4-fig:mcr-diagram} illustrates  when the data distribution is actually a mixture of low-dimensional submanifolds.  Through compression (denoising or clustering), we first identify that the true data distribution (left). We then would like to transform the distribution so that the submanifolds eventually become mutually incoherent/independent linear subspaces (right). 
...
\end{equation}
We refer to this as the principle of {\em maximal coding rate reduction} (MCR$^2$), as an embodiment of Aristotle's famous quote:
\begin{quote}
	\centering
	``{\em The whole is greater than the sum of its parts.}''
\end{quote}
To learn the best representation, we require that {\em the whole is maximally greater than the sum of its parts}. To a large extent, the quantity $\Delta R$ can be viewed as a computable realization to the mutual information term $I(\z; \y)$ introduced the objective \eqref{chap4-eqn:representation-learning}. 

\begin{example}
To further elucidate the ideas, let us examine the example shown in \Cref{fig:informative-representation}. 
\begin{figure}[t]
	\centering
...
We here present how the MCR$^2$ objective \eqref{eqn:maximal-rate-reduction} helps learn better representations than the cross entropy (CE) \eqref{chap4-eqn:cross-entropy} for image classification. Here, for simplicity,\footnote{We will rigorously study and justify how design or derive a proper deep architecture from and for the MCR$^2$ objective in the next \Cref{ch:unrolling}.} we adopt the popular neural network architecture, the ResNet-18~\cite{he2016deep}, to model the feature mapping $\z = f(\x,\theta)$. We optimize the neural network parameters $\theta$ via the backpropagation algorithm (detailed in Appendix \Cref{app:BP-section}) to maximize the coding rate reduction. More implementation details of this experimental can be found in \cite{chan2021redunet}. 

\Cref{fig:train-test-loss} illustrates how the two rates and their difference (for both training and test data) evolve over epochs of training: After an initial phase, $R_\epsilon$ gradually increases while $R^c_\epsilon$ decreases, indicating that features $\bm Z$ are expanding as a whole while each class $\bm Z_k$ is being compressed.


We first evaluate the performance with the CIFAR10 image classification dataset~\cite{krizhevsky2009learning} and the results are shown in the table of Figure \ref{fig:CIFAR10-performance}. As we can see, the MCR$^2$ objective achieves a comparable performance as the cross entropy (CE) loss. In addition, the MCR$^2$ objective is more robust to corruptions in the class labels since it learns a joint (subspace) representation for each class instead of trying to fit the labels of individual samples. 
...
\begin{figure}
    ...
    \caption{Comparison of classification accuracy between training with the cross entropy (CE) loss or the MCR$^2$ objective, with different ratio of the class labels being randomly corrupted.}
...
\begin{figure}[t]
		\begin{subfigure}[t]{0.45\textwidth}
			...
			\caption{PCAs of features for individual classes learned by MCR$^2$.}
			\label{fig:MCR-pca}
		\end{subfigure}
        \hfill
		\begin{subfigure}[t]{0.45\textwidth}
	...
	\includegraphics[width=\textwidth]{\toplevelprefix/chapters/chapter4/figs/ce_pca_individual.pdf}
			\caption{PCAs of features for individual classes learned by Cross Entropy.}
			\label{fig:CE-pca}
		\end{subfigure}

		\caption{\small Comparison of the principal components of learned features from MCR$^2$ versus those from cross entropy.}
	...
	\begin{figure*}[t]
		\begin{center}
			...
			\hspace{0.25cm}
		\hfill 	\includegraphics[width=0.45\textwidth]{\toplevelprefix/chapters/chapter4/figs/heatmap_ce.png}
...
\Cref{fig:feature-pca} shows the distribution of singular values of the learned features $\Z_k$ per class. From the distribution of the singular values shown in \Cref{fig:MCR-pca}, we see that the  features $\z$ learned from MCR$^2$ for each class are distributed on a subspace of dimension around 10-12, whereas the features from cross entropy \Cref{fig:CE-pca} are of much lower dimension.\footnote{If one continue to train with more iterations, the CE features will may even collapse to a single dimension, known as the neural collapse phenomenon discussed earlier in \Cref{remark:neural-collapse}. } 

\Cref{fig:heatmap-plot} shows the cosine similarities between the learned features sorted by class. We also compare the similarities of the learned features by using the cross-entropy (CE) \eqref{chap4-eqn:cross-entropy} and the MCR$^2$ objective \eqref{eqn:maximal-rate-reduction}. From the plots, one can clearly see that the features learned by the  MCR$^2$ objective for different classes are much more incoherent (or independent) than features learned by using cross-entropy loss. Within each class, the features also are less coherent hence more diverse, as indicated by the distribution of their singular values shown in \Cref{fig:MCR-pca}.

We visualize the images that are mapped along different singular vectors in the feature subspace of each class, which we call the ``principal'' images. Figure \ref{fig:pca-images} show these principal images along each of the top-10 singular vectors. As we can see, the images along each singular vector are very similar in their appearances and structures and images along different singular vectores are very distinct from each other.  
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{chapters/chapter4/figs/visual_2class.pdf}
    \caption{Top-10 ``principal'' images for  class - ``Bird'' and ``Ship'' in the CIFAR10 image dataset.}
    \label{fig:pca-images}
\end{figure}


Despite the fact that the MCR$^2$ objective seems to allow us to learn better representations, there has been an apparent lack of justification of the network architectures used in the above experiments. It is yet unclear why the network adopted here (the ResNet-18) is suitable for representing the map $f(\x, \theta)$, let alone for interpreting the layer operators and parameters $\theta$ learned inside. In the next Chapter \ref{ch:unrolling}, {\em we will show how to derive network architectures and components entirely as a ``white box'' from the desired objective (say the rate reduction)}.
...
Now we present how to extend the MCR$^2$ principle to the unsupervised setting. Contrary to supervised learning where the class labels are known, in unsupervised learning, the group memberships of the data samples are unknown. In this case, we can apply certain class of augmentations or transformations, say $\tau$ with a distribution $P_{\tau}$, to each sample. For example, for images,  augmentations typically include translations, rotations, or cropping of each image sample. We may view the augmented samples of each data point as belonging to the same group or cluster. 

More specifically, given a set of $N$ data samples $\X = [\x_1,\dots,\x_N]$, we generate $m$ augmented views $\{\tau_i\}_{i=1}^m$ for each data point $\x_k$ , denoted as $\tilde{\X}_{k} = [\tau_1(\x_k), \tau_2(\x_k), \dots, \tau_m(\x_k)]$. We treat the augmented samples $\tilde{\X}_{i}$ as the $i$-th class with $K=N$ groups/clusters in total. Now, we can apply the MCR$^2$ principle as we did before in the supervised learning case. Specifically, we learn the feature mapping $f(\x,\theta)$ by maximizing the coding rate reduction as follows:
\begin{equation}
	\begin{aligned}
		\max_{\theta } & \;  \Delta R_{\epsilon}\big(\Z \big) \doteq R_{\epsilon}(\Z) - R_{\epsilon}^c(\Z ), \\
		\mbox{s.t.} & \ \ \, \Z = f(\tilde{\X}, \theta),\  \|\Z_k\|_F^2                                          = m,\ k=1,\dots,N. 
	\end{aligned}
	\label{eqn:maximal-rate-reduction-unsupervised}
\end{equation}
Here, $\tilde{\X} = [\tilde{\X}_1,\dots,\tilde{\X}_N]$ and $\Z_k = f(\tilde{\X}_k,\theta)$ denotes the features of the augmented samples of $\x_k$ for each $k=1,\dots,N$.

Let us again adopt ResNet-18 as the feature mapping $f(\x,\theta)$ and optimize the network parameters $\theta$ to maximize the unsupervised objective \eqref{eqn:maximal-rate-reduction-unsupervised}. 
After training on the CIFAR10 dataset, we visualize the evoluation of the two rates and their difference (for both training and test data) over epochs of training in \Cref{fig:train-test-loss-selfsup-mcr}. Similar to the supervised learning case, after an initial phase, $R_\epsilon$ gradually increases while $R^c_\epsilon$ decreases, indicating that features $\bm Z$ are expanding as a whole while each cluster $\bm Z_k$ is being compressed. Compared to the supervised case shown in \Cref{fig:train-test-loss}, we can observe that, in the unsupervised case, the total coding rate $R_\epsilon$ expands rapidly and the overall MCR$^2$ objective saturates at a local optimum. To alleviate this, we can alter the optimization strategy by controlling the dynamics of $R_\epsilon$ and $R^c_\epsilon$. Specifically, we can rescale the rates by replacing the coding rate $R_\epsilon(\Z)$ in \eqref{eqn:maximal-rate-reduction-unsupervised} with $$\tilde{R}_\epsilon(\Z) \doteq \frac{1}{2\gamma_1} \log \det \left(\I + \frac{\gamma_2 d}{N m \epsilon^{2}}\Z \Z ^{\top}\right).$$ By setting $\gamma_1 = \gamma_2 = N$, we arrive at a modified learning objective, referred to as MCR$^2$-{\scriptsize CTRL}, whose training dynamics are shown in \Cref{fig:train-test-loss-selfsup-mcr-ctrl}. As we can see, with this controlled MCR$^2$ objective, features are first compressed and then gradually expanded, leading to a higher value of the overall objective $\Delta R_{\epsilon}(\Z)$.

To assess the quality of the learned representations and verify whether they have good subspace structures, we adopt a subspace clustering algorithm EnSC \cite{you2016oracle}. For evaluation metrics, we consider normalized mutual information (NMI), clustering accuracy (ACC), and adjusted rand index (ARI) \footnote{Refer to the Appendix in \cite{yu2020learning} for their specific definitions.}. For comparison, we consider methods including JULE \cite{yang2016joint}, RTM \cite{nina2019decoder}, DEC \cite{xie2016unsupervised}, DAC \cite{chang2017deep}, and DCCM \cite{wu2019deep}. The clustering results are summarized in Table \ref{fig:unsupervised-clustering-results}. As we can see, the features learned by maximizing the MCR$^2$-{\scriptsize CTRL} objective achieve better clustering accuracy than those learned by other algorithms.
\begin{figure}[t]
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
	...
	\end{subfigure}
	\caption{\small Evolution of the rates of MCR$^2$ in the training process for unsupervised learning on CIFAR-10.}
...
\begin{figure}
    ...
    \caption{Comparison of clustering performance accuracy between training with the MCR$^2$-{\scriptsize CTRL} objective and other unsupervised learning algorithms on CIFAR10, CIFAR100, and STL10 datasets.}
    \label{fig:unsupervised-clustering-results}
\end{figure}

There is one main drawback though: the MCR$^2$-based unsupervised learning method requires a costly $\log \det$ operation of $\O(d^3)$ complexity \textit{per sample} when computing $R^c_\epsilon$. This quickly becomes computationally prohibitive as we try to scale to larger datasets. Instead of minimizing the exact coding rate for each group of augemented samples $\tilde{\X}_{i}$, we can approximate the effect of compressing features in each group by maximizing the {\em cosine similarity} between the features of the augmented samples:\footnote{We leave as an exercise for the readers to verify under what conditions that minimizing the per class coding rate $R^c(\Z)$ is (approximately) equivalent to maximizing the cosine similarity among the features $\Z$.}

\begin{equation}
	\begin{aligned}
		\max_{\theta } & \;  R_{\epsilon}(\Z) + \frac{\gamma}{N}\sum_{k=1}^N \sum_{i \neq j} \z_{k,j}^\top \z_{k,i}, \\
		\mbox{s.t.} &  \ \ \, \z_{i,j} = f(\tau_j(\x_i),\theta),\  \|\z_{i,j}\|_2^2                                          = 1, 
	\end{aligned}
	\label{eqn:maximal-rate-reduction-unsupervised-simdino}
\end{equation}
where $\gamma > 0$ is a hyperparameter that controls the strength of the cosine similarity term, as it differs from the coding rate in magnitude. This objective avoids the costly $\log \det$ operation and is much more scalable. In fact, this objective is closely related to popular (unsupervised) {\em contrastive learning} methods such as SimCLR \cite{chen2020simple} and DINO \cite{caron2021emerging}, which have been engineered to learn high-quality representations from massive amount of images. 

As we will show later in Section \ref{sub:contrastive_learning_objective} of Chapter \ref{ch:applications}, this objective \eqref{eqn:maximal-rate-reduction-unsupervised-simdino} can be implemented very efficiently to learn a highly structured  representation $\Z$ that is highly linear and discriminative. 
% \yima{Robin, maybe some experimental results that illustrate qualitatively the learned features are discriminative and linear? Say some results on linear probing or visual results on segmentation? (it is ok to say leaving the implementation details to Chapter 8.) This will help justify RAE later in Chapter 6.} 
The learned feature representation leads to state-of-the-art performance on large real-world datasets comparable or even better than features learned by DINO, but without the heavy engineering efforts needed for DINO. Hence, we refer to this objective as {\em Simplified DINO} (SimDINO). As a preview, \Cref{tab:dino_imagenet_linear_probing_teaser} show that SimDINO outperforms DINO on ImageNet-1k classification using both \(k\)-nearest neighbor accuracy and linear probing (i.e. training a linear classifier on pretrained models). This demonstrates that the objective leads to highly structured and linearized representations. We can also utilize these representations for downstream tasks such as unsupervised segmentation\footnote{Specifically, we adopt MaskCut \cite{wang2023cut}, an effective unsupervised approach of extracting features from a frozen vision backbone for object detection and instance segmentation.}, as illustrated in \Cref{fig:maskcut_segmentation}. More implementation details and experimental results of SimDINO will be presented in Chapter \ref{ch:applications}.
...
\begin{table}
    ...
    \caption{\small\textbf{Classification performance} on ImageNet-1k test set for DINO and SimDINO, using both \(k\)-nearest neighbor accuracy (\(k = 20\)) and linear probing. SimDINO outperforms DINO in both accuracy and training stability. DINO training on ViT-L is unstable and quickly diverges, while SimDINO trains stably.}
...
\begin{figure}
    ...
    \caption{\small \textbf{Visualization of unsupervised segmentation results} from DINO ViT-B/16 (row 1), SimDINO ViT-B/16 (row 2) and SimDINO ViT-L/16 (row 3).}
...
In this Chapter, we have studied basic concepts and ideal behind how to learn a distribution from finite samples and obtain a {\em computable} encoding and decoding scheme for the distribution. For distributions that are continuous and possibly low-dimensional, we have argued that such a scheme is necessarily {\em lossy} -- hence the importance of the concept of rate distortion. The noise introduced through distortion, due to quantization, plays a related but different role from the noise we have studied in the preceding chapter. One may think of the noise considered in Chapter \ref{ch:denoising} is ``outside'' of the submanifold of the support of the distribution whereas the noise considered in this chapter is ``inside'' the support. Informally, we may refer to the diffusion/denoising process outside of the support as ``continuation'' and the diffusion/denoising process inside the support as ``percolation.'' A more unified analysis and understanding of their compounded effect is due for future study. 

In this chapter, we introduced for the first time the concept of learning a {\em representation} for a distribution. We have considered arguably the simplest case, the case of classification. If one views the classification label $\y$ is a fixed simple (grossly lossy) code for the data $\x$, we may seek a consistent\footnote{That is, we can faithfully recover $\x$ from $\z$ as we will make the notion of ``consistent'' representations clear later in Chapter \ref{ch:autoencoding}.} representation $\z = f(\x)$ of the data $\x$ such that it tends to maximize the mutual information between the representation and this simple code:
\begin{equation}
    \max I(\z, \y) = H(\z) - H(\z \mid \y).
\end{equation}
In addition, we require such a representation $\z$ is not only consistent to $\x$ but also much more {\em structured} than the native data distribution of $\x$ -- classes are linearly discriminative. This would make the learned representation much more efficient to use than the native data distribution for subsequent tasks such as recognition, segmentation and completion. The maximal coding rate reduction (MCR$^2$) objective introduced is a natural and computable realization of these requirements. In later Chapter \ref{ch:conditional-inference} we will see how to generalize this objective to a much broader context when $\y$ could be a more general lossy code (say image caption) or a random variable related to $\x$.
...
\begin{exercise}
    Let $\z_1$ and $\z_2$ be two vectors of unit length. Let $\Z = [\z_1, \z_2]$ Show that the following three objectives are equivalent:
\begin{itemize}
    \item $\max \z_1^\top \z_2$.
    \item $\min \|\z_1 - \z_2\|_2^2$.
    \item $\max \log \det (\I + \Z\Z^\top)$.
\end{itemize}
...
{\color{blue}
\begin{exercise}\label{ex:rate Gaussian}
    Please show that the rate distortion function for a $X \sim \mathcal{N}(0,\sigma^2)$ source is 
    \begin{align*}
        \mathcal{R}_{\epsilon}(X) = \begin{cases}
            \frac{1}{2}\log\left( \frac{\sigma^2}{\epsilon^2} \right),\ & 0 \le \epsilon \le \sigma,\\
            0,\ & \epsilon > \sigma. 
        \end{cases}
    \end{align*}
{\bf Hint:}  This proof consists of two steps. First, we find a lower bound for the rate distortion function, i.e., for all $\mathbb{E}[(X-\hat{X})^2] \le \epsilon^2$, 
\begin{align*}
    I(X;\hat{X}) \ge  \frac{1}{2}\log\left( \frac{\sigma^2}{\epsilon^2} \right). 
\end{align*}
Second, we construct $p(\hat{X} \mid X)$ to prove this lower bound is achievable. If $\epsilon \le \sigma$, we choose
\begin{align*}
    X = \hat{X} + Z,\ \hat{X} \sim \mathcal{N}(0,\sigma^2 - \epsilon^2),\ Z \sim \mathcal{N}(0,\epsilon^2), 
\end{align*}
where $\hat{X}$ and $Z$ are independent. If $\epsilon > \sigma$, we choose $\hat{X} = 0$ with probability 1. 
\end{exercise}


\begin{exercise}\label{exer:KKT}
    Please derive the rate-distortion function \eqref{eq:rate 1} for the Gaussian random vector introduced in \Cref{exam:rate Gaussian}. \\
\textbf{Hints:} The proofs consists of the following steps:
\begin{enumerate}
    \item We first find a lower bound for the rate distortion function, i.e., for all $\mathbb{E}[\|\bm{x}-\hat{\bm x}\|_2^2] \le \epsilon^2$,
    \begin{align}
        I(\bm x; \hat{\bm x}) \ge \sum_{i=1}^d \max\left\{\frac{1}{2}\log\left( \frac{\sigma_i}{\lambda_i} \right), 0\right\},
    \end{align}
    where $\lambda_i = \mathbb{E}[(x_i - \hat{x}_i)^2]$.

    \item Second, we reduce the problem of finding the rate distortion function to the followign problem:
    \begin{align}
        \min_{\lambda_1,\dots,\lambda_d}  \sum_{i=1}^d \max\left\{\frac{1}{2}\log\left( \frac{\sigma_i}{\lambda_i} \right), 0 \right\}\quad \mathrm{s.t.}\ \sum_{i=1}^d \lambda_i = \epsilon^2. 
    \end{align}
    
    % \item Consider setting up the Lagrangian
    % \[
    %     \mathcal{L}(\lambda_1,\dots,\lambda_d,\nu) = \frac12 \sum_{i=1}^d \log \frac{\sigma_i^2}{\lambda_i} 
    %     + \nu \left(\sum_{i=1}^d \lambda_i - \epsilon^2\right),
    % \]
    % where \(\lambda_i\) are the variances of the reconstruction errors along each principal direction.

    % \item Compute the partial derivative \(\partial \mathcal{L}/\partial \lambda_i = 0\) to find a relation between \(\lambda_i\) and the Lagrange multiplier \(\nu\).

    % \item Apply the constraints \(0 \le \lambda_i \le \sigma_i^2\) to identify which directions are saturated and which follow the water-filling allocation.

    % \item Solve for the ``water level'' \(\lambda\) using \(\sum_{i=1}^d \lambda_i = \epsilon^2\), and verify that \(\lambda_i = \min(\sigma_i^2, \lambda)\).
\end{enumerate}
\end{exercise}}
