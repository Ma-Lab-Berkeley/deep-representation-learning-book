...
There are two types of two-player games we cover here, which correspond to different information structures:
\begin{itemize}
    \item Simultaneous games, where both players pick their parameters without advance knowledge of the other player's choice.
    \item Sequential games, where one player (without loss of generality Player 1) picks their parameters before the other (i.e., Player 2), and the second player (i.e., Player 2) can use the full knowledge of the first player's (i.e., Player 1) play to make their choice.
\end{itemize}
...
\end{align}
In words, both sides will independently try to optimize their objectives \(\cL^{i}\) at each step using the learning rates \(\alpha_{k}\) and \(\beta_{k}\), crucially not necessarily equal to each other. This is totally sensible for our model problem where \(\cL^{1}(\theta, \eta) = u(\theta)\) and \(\cL^{2}(\theta, \eta) = v(\eta)\), but may not obviously lead to critical points or saddle points otherwise. In the sequel, we will how to make GDA learn a Nash equilibrium and a minimax equilibrium.
...
\end{equation}
in order to establish convergence to (local) minimax optima.
