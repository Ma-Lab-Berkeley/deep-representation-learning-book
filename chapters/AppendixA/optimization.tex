...
Optimization is concerned with the question of how to find where a function, say $f(\x)$, reaches its minimal (or maxmimal) values. Mathematically, this is stated as a problem:
\begin{equation}
    \arg\min_{\x} f(\x), \quad \x \in \mathcal{C},
\end{equation}
where $\mathcal{C}$ represents a domain to which the argument $\x$ is confined. Often $\mathcal{C}$ is simply $\R^n$. Without loss of generality, we may assume that here the function $f(\x)$ is smooth\footnote{In case the function is not smooth, we may replace its gradient with subgradient.}. 

Obviously, how efficiently one can find the (global) minima depends on what information we have about the function, also known as the oracle. For most optimization problems considered in this book, the dimension of $\x$ is very large. That makes computing or accessing (local) information about the function, say gradients or Hessians, very expensive. Hence, it is typical to assume that we only has the first-order information, namely the gradient, of the function as the oracle. Therefore, all optimization algorithms that we introduce below only use the gradient of the function.\footnote{We refer the readers to the book by \cite{Wright-Ma-2022} for a more systematic introduction to optimization algorithms in a high-space, including algorithms assuming higher-order oracles.}  

\subsection{Gradient Descent and SGD}
Probably the simplest, also the most widely used, method for optimization is {\em gradient descent} (GD). It was first introduced by Cauchy in 1847. The idea is very simple: starting from an initial state, say $\x_0$, one iteratively takes small steps such that each step reduces the value of the function $f(\x)$. 

Suppose that the current state is $\x$. We want to take a small step, say of distance $t$, in a direction, indicated by a vector $\vv$, to reach a new state $\x'=\x + t \vv$ such that the value of the function decreases:
\begin{equation}
    f(\x + t \vv) \le f(\x).
\end{equation}
To find such a direction $\vv$, we can approximate $f(\x + t \vv)$ through the Taylor expansion:
\begin{equation}
    f(\x + t \vv) = f(\x) + t \nabla f(\x)^\top \vv + \mbox{h.o.t.}(t). 
\end{equation}
As long as $\nabla f(\x) \not= \boldsymbol{0}$, the value of $f(\x)$ decreases the fastest along the direction $\vv = - \nabla f(\x)$. This leads to the gradient descent method: From the current state $\x_k$ ($k=0, 1, \ldots$), we take a step of size $t$ in the direction of the negative gradient to reach the next iterate,
\begin{equation}
    \x_{k+1} = \x_k - t \nabla f(\x_k). 
\end{equation}

The remaining question is what the step size $t$ should be? If we choose $t$ to be too small, the value of the function may decrease very slowly, as shown by the plot in the middle in Figure \ref{fig:step-size}. If $t$ is too large, the value might not even decrease at all, as shown by the plot on the right in Figure \ref{fig:step-size}.
...
\begin{figure}[h]
    ...
    \caption{The effect of step size $t$ on the convergence of the gradient descent method.}
    \label{fig:step-size}
\end{figure}

So the step size $t$ should be chosen based on the local landscape of the function $f(\x)$. Ideally, to choose the best step size $t_k$, one would prefer solve the following optimization problem with a single variable $t$:
\begin{equation}
    t_k = \arg\min_{t>0} f(\x_k - t \nabla f(\x_k)).
\end{equation}
However, when the function $f(\x)$ is complicated, which is usually the case for training a deep neural network, this one-dimensional optimization is not so easily solvable. 

Then how should we choose a proper step size $t$ then? One common approach is to try to obtain a good approximation of the local landscape around the current state $\x$ based on some knowledge about the overall landscape of the function $f(\x)$. A common assumption is that the derivative (or gradient) of $f(\x)$ changes smoothly or, more precisely, is Lipschitz continuous:
\begin{equation}
    \|\nabla f(\x) - \nabla f(\y)\| \le L \|\x - \y\|, \quad \forall \x, \y \in \mathcal{C},
\end{equation}
for some constant $L > 0$. Then it is easy to show\footnote{We leave this as an exercise to the reader.} that, around every point $\x$, the function $f$ has a natural upper bound $g$:
\begin{equation}
    f(\y) \le g(\y) \doteq  f(\x) + \langle \nabla f(\x), \y - \x \rangle + \frac{L}{2}\|\y-\x\|^2 
    = \frac{L}{2}\Big\|\y-(\x - \frac{1}{L} \nabla f(\x))\Big\|^2 + c(\x)
\end{equation}
for some function $c(\x)$ that does not depend on $\y$. Note that at the point $\x$, we have $g(\x) = f(\x)$. 

Hence, at any iterate $\x_k$, to ensure that the value of the function $f(\x_{k+1})$ at the next iterate $\x_{k+1}$ is strictly smaller than $f(\x_k)$, we could find the minimal point of this upper bound to find the next iterate $\x_{k+1}$:
\begin{eqnarray}
 \x_{k+1} &:=& \argmin_{\x} \Big\{g(\x) = f(\x_{k})+\langle \nabla f(\x_{k}), \x-\x_{k}\rangle + \frac{L}{2}\|\x-\x_{k}\|_2^2 \Big\}\\
&:=& \argmin_{\x} \Big\{\langle \nabla f(\x_{k}), \x-\x_{k}\rangle + \frac{L}{2}\|\x-\x_{k}\|_2^2 \Big\}
\label{eqn:steepest-descent-2-norm}
\end{eqnarray}
Since $g(\x_k) = f(\x_k)$ and $g(\x)$ is an upper bound of $f(\x)$ around $\x_k$, we have the following relationship:
\begin{equation}
    f(\x_{k+1}) \le g(\x_{k+1}) \le g(\x_k) = f(\x_k) 
\end{equation}
This is easily to see from the geometric relationship between a function and its (local) upper bound, illustrated in Figure \ref{fig:optimization-upper-bound}. The above process leads to the following 
\textbf{gradient descent iteration}: 
\begin{equation}
\x_{k+1}= \x_{k} - \frac{1}{L} \nabla f(\x_{k}).
\end{equation}
The step size $t = 1/L$ is also known as the {\em learning rate}. In practice, however, when optimizing a deep neural network, we normally do not know an accurate value of the (local) Lipschitz constant $L$. Hence the learning rate $t$ is usually chosen based on some empirical rules. 
...
\subsection{Sign Descent and ADAM}
We see that in the above gradient descent scheme, a key property of the next iterate $\x_{k+1}$ that it minimizes the objective \eqref{eqn:steepest-descent-2-norm} in which the distance between $\x_k$ and $\x_{k+1}$ is measured by the $\ell^2$-norm (or known as the Euclidean norm). 

In general, this distance can be measured by any norm to promote different properties of the descent. For instance, if we adopt the $\ell^1$-norm, it would promote the number of coordinates updated from $\x_k$ to $\x_{k+1}$ is sparse. Another typical choice is the $\ell^\infty$-norm:
\begin{eqnarray}
 \x_{k+1} &:=& \argmin_{\x} \Big\{\langle \nabla f(\x_{k}), \x-\x_{k}\rangle + \frac{L}{2}\|\x-\x_{k}\|^2_\infty \Big\}
\label{eqn:steepest-descent-infinity-norm}
\end{eqnarray}
It has been shown by \cite{bernstein2024oldoptimizernewnorm} that this leads to the following {\em sign descent} scheme for the iterates:
\begin{equation}
    \x_{k+1} = \x_k - \frac{\|\nabla f(\x_k)\|_1}{L} \mbox{sign}(\nabla f(\x_k)) = \x_k - \frac{\|\nabla f(\x_k)\|_1}{L} \cdot \frac{\nabla f(\x_k)}{\sqrt{\nabla f(\x_k)^2}}.
\end{equation}

\yima{Now introduce the Adam algorithm as the stochastic version of the sign descent scheme, including how to estimate $\nabla f(\x_k)$ and $\nabla f(\x_k)^2$ ... also probably justify the general form of Adam with the connection to the heavy ball method that tries to approximate some second-order information with momentum...  Druv? }
