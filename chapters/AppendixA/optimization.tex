...
\begin{enumerate}
    \item Cover gradient descent from the Taylor expansion perspective. If we
        want, we can work in nonsmoothness here (Taylor expand the smooth part).
        Second order info can lead us to write Adam.
    \item Present some material on diffusion processes for optimizing functions:
        classical asymptotic convergence results, connections to some modern
        optimization workflows (how to set batch size, eg).
\end{enumerate}}
...
\begin{figure}[h]
    \centering
    \includegraphics[height=3.3cm]{figs_appA/GD-1.png}
    \hspace{3mm}
    \includegraphics[height=3.3cm]{figs_appA/GD-2.png}
    \hspace{3mm}
    \includegraphics[height=3.3cm]{figs_appA/GD-3.png}
...
Then how should we choose a proper step size $t$ then? One common approach is to try to obtain a good approximation of the local landscape around the current state $\x$ based on some knowledge about the overall landscape of the function $f(\x)$. A common assumption is that the derivative (or gradient) of $f(\x)$ is Lipschitz continuous:
\begin{equation}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\linewidth]{figs_appA/convex-1.png}
    \caption{Minimizing a (local) quadratic upper bound of a function.}
