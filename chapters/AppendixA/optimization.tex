...
\begin{exercise}
    Compute the differential of softmax.
\end{exercise}
...
\begin{example}
    ...
    \begin{equation}
        f^{\ell}(\vZ, \vW^{\ell}, \vb^{\ell}) := \vW^{\ell}\vZ + \vb^{\ell}\vone^{\top}.
    \end{equation}
    We can compute the derivative w.r.t.~both parameters as
    \begin{align}
        \odif{f}^{\ell}
    ...
    \begin{equation}
        T[\vA, \vu] = \vA\vZ + \vu\vone^{\top} = \mat{\vA & \vu}\mat{\vZ \\ \vone^{\top}}.
    \end{equation}
    We calculate the adjoint \(T^{*}\) w.r.t.~the sum-over-coordinates (Frobenius) inner product by the following procedure:
    \begin{align}
        \ip{T[\vA, \vu]}{\vB}_{F} 
        &= \tr\rp{\mat{\vA & \vu}\mat{\vZ \\ \vone^{\top}}\vB^{\top}} \\ 
        &= \tr\rp{\vB\mat{\vZ^{\top} & \vone}\mat{\vA^{\top} \\ \vu^{\top}}} \\
        &= \tr\rp{\vB\mat{\vZ^{\top} & \vone}\mat{\vA & \vu}^{\top}} \\
        &= \ip{\vB\mat{\vZ^{\top} & \vone}}{[\vA, \vu]}. 
    \end{align}
    So \(T^{*}\vB = \vB[\vZ^{\top}, \vone]\).
\end{example}

\begin{example}
    Compute the adjoint derivative for the softmax function, i.e., 
    \begin{equation}
