...
There are two types of two-player games that people normally consider. They correspond to different information structures:
\begin{itemize}
    \item Simultaneous games, where both players pick their parameters without advance knowledge of the other player's choice.
    \item Sequential games, where one player (without loss of generality Player 1) picks their parameters before the other (i.e., Player 2), and the second player (i.e., Player 2) can use the full knowledge of the first player's (i.e., Player 1) play to make their choice. \yima{Is this the Stackelberg model?}
\end{itemize}
To each game we associate a different notion of \textit{equilibrium}, as follows.
\begin{itemize}
    \item For simultaneous games the correct notion of equilibrium is the \textit{Nash equilibrium}, or \textit{saddle point}. A pair \((\theta^{\star}, \eta^{\star})\) is a \textit{Nash equilibrium} when
    \begin{equation}
        \theta^{\star} \in \argmin_{\theta \in \Theta}\cL^{1}(\theta, \eta^{\star}), \qquad \eta^{\star} \in \argmin_{\eta \in \Eta}\cL^{2}(\theta^{\star}, \eta).
    \end{equation}
    That is, either player changing their option will increase their loss.
    \item For sequential games the correct notion of equilibrium is the \textit{minimax equilibrium}. To explain this equilibrium, note that since Player 2 can choose \(\eta\) reactively to Player 1's choice \(\theta_{1}\), Player 2 would of course choose the \(\eta\) which minimizes \(\cL^{2}(\theta_{1}, \cdot)\). But of course a rational Player 1 would realize this, and so pick a \(\theta_{1}\) such that the worst-case \(\eta\) picked by Player 2 according to this rule is not too bad. More formally, let \(\cS^{2}(\theta) := \argmin_{\eta \in \Eta}\cL^{2}(\theta, \eta)\) be the set of \(\eta\) minimizing \(\cL^{2}(\theta, \eta)\), i.e., the set of all \(\eta\) which Player 2 is liable to play given that Player 1 has played \(\theta\). Then \((\theta^{\star}, \eta^{\star})\) is a minimax equilibrium if 
    \begin{equation}
        \theta^{\star} \in \argmin_{\theta \in \Theta}\max_{\eta \in \cS^{2}(\theta)}\cL^{1}(\theta, \eta), \qquad \eta^{\star} \in \argmin_{\eta  \in \Eta}\cL^{2}(\theta^{\star}, \eta).
    \end{equation}
\end{itemize}
In the rest of the section, we will briefly discuss some algorithmic approaches to learn equilibria. The intuition you should have is that learning an equilibrium is like letting the different parts of the system automatically figure out tradeoffs between the different objectives they want to optimize. 

The algorithmic template we can consider is the following \textit{gradient descent-ascent} (GDA) method:
\begin{align}
    \theta_{k + 1} &= \theta_{k} - \alpha_{k}\nabla_{\theta} \cL^{1}(\theta_{k}, \eta_{k}), \\ 
    \eta_{k + 1} &= \eta_{k} - \beta_{k}\nabla_{\eta} \cL^{2}(\theta_{k}, \eta_{k})
\end{align}
In words, both sides will independently try to optimize their objectives \(\cL^{i}\) at each step using the learning rates \(\alpha_{k}\) and \(\beta_{k}\), crucially not necessarily equal to each other. This is totally sensible for our model problem where \(\cL^{1}(\theta, \eta) = u(\theta)\) and \(\cL^{2}(\theta, \eta) = v(\eta)\), but may not obviously lead to critical points or saddle points otherwise. In the sequel, we will how to make GDA learn a Nash equilibrium and a minimax equilibrium. \yima{I am not sure where this is done for Nash equilibrium? In Section A.3.1? It only discusses a special case and the message is mostly pessimistic and vague about any guarantee at all... }

Throughout the book and in the rest of the appendices, we consider \textit{zero-sum games} where the objectives \(\cL^{1}\) and \(\cL^{2}\) are negatives of each other:
\begin{equation}
    \cL^{1}(\theta, \eta) = -\cL(\theta, \eta) = -\cL^{2}(\theta, \eta).
\end{equation}
Using this notation a Nash equilibrium becomes 
\begin{equation}
    \theta^{\star} \in \argmax_{\theta \in \Theta}\cL(\theta, \eta^{\star}), \qquad \eta^{\star} \in \argmin_{\eta \in \Eta}\cL(\theta^{\star}, \eta),
\end{equation}
and a minimax equilibrium becomes 
\begin{equation}
...
\begin{proof} \DP{Just to convince myself this is true; feel free to comment.}
    Note that 
    \begin{equation}
        \cS^{2}(\theta) = \argmin_{\eta \in \Eta}\cL^{2}(\theta, \eta) = \argmin_{\eta \in \Eta}\cL(\theta, \eta),
    \end{equation}
    so it holds 
    \begin{equation}
        \min_{\eta \in \cS^{2}(\theta)}\cL(\theta, \eta) = \min_{\eta \in \argmin_{\eta^{\prime} \in \Eta}\cL(\theta, \eta^{\prime})}\cL(\theta, \eta) = \min_{\eta \in \Eta}\cL(\theta, \eta).
    \end{equation}
\end{proof}
...
\end{equation}
then every Nash equilibria is a minimax equilibria \textit{and vice versa}, and furthermore each Nash/minimax equilibrium has the (same) objective value \(\max_{\theta \in \Theta}\min_{\eta \in \Eta}\cL(\theta, \eta)\).
\begin{proof} \DP{Just to convince myself this is true; feel free to comment.}
    Suppose that indeed 
    \begin{equation}
        \max_{\theta \in \Theta}\min_{\eta \in \Eta}\cL(\theta, \eta) = \min_{\eta \in \Eta}\max_{\theta  \in \Theta}\cL(\theta, \eta).
    ...
    First suppose that \((\theta^{\star}, \eta^{\star})\) is a Nash equilibrium. We will show it is a minimax equilibrium. By definition we have 
    \begin{equation}
    ...
    \begin{equation}
        \theta^{\star} \in \argmax_{\theta \in \Theta}\min(\eta \in \Eta)\cL(\theta, \eta).
    \end{equation}
    From the condition of Nash equilibrium we have \(\eta^{\star} \in \argmin_{\eta \in \Eta}\cL(\theta^{\star}, \eta)\). So \((\theta^{\star}, \eta^{\star})\) is a minimax equilibrium. Furthermore we have also proved that all Nash equilibria obey \eqref{eq:nash_equilibrium_minmax_maxmin}.

    Now let \((\theta^{\star}, \eta^{\star})\) be a minimax equilibrium. We claim that it is a Nash equilibrium, which completes the proof. By the definition of minimax equilibrium,
    \begin{equation}
...
\end{proof}
Conditions under which this equality holds are given by so-called \textit{minimax theorems}; the most famous of these is a theorem of von Neumann. However, in the cases we think about, this property usually does not hold.
