...
\paragraph{A single low-dimensional Gaussian model.} Suppose that we consider $K=1$ in \eqref{eq:MoG1}, i.e., 
$$\vx \sim   \dNorm(\vzero, \vU\vU^{\top}),$$ 
where \(\vU \in \O(D, P) \subseteq \R^{D \times P}\) is an orthogonal matrix. According to \eqref{eq:gmm_lowrank_denoiser}, the  Bayes optimal denoiser is 
\begin{align}
