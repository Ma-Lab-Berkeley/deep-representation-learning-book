...
\subsection{Rate Distortion and Geometry}
...
support of $p(\x)$ is a compact set, and $p(\x)$ is uniformly distributed on its
support.
...
\begin{proposition}\label{prop:covering-number-rate-distortion}
	...
	\end{equation}
	If, in addition, $\vx$ is uniformly distributed on $K$ and has
	a density,\footnote{In fact, the requirement that $\vx$ has a density is not
	necessary, but the statement of the result becomes far more technical: c.f.\
	Riegler et al.\ \cite{Riegler2018-jh,Riegler2023-rr}. We give a simple proof
	in \Cref{app:} which shows a sufficient condition on the density for the
	result to hold.}
