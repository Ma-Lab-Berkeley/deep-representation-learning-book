...
Building on the discussion of denoisers that simply memorize the training data, we now turn to a deeper investigation of when diffusion models transition from memorization to generalization. %While an overly expressive network can reproduce the empirical distribution exactly, practical diffusion models often capture the underlying structure of the data, enabling the generation of novel samples. 
In theory, the training of diffusion models is designed to learn the empirical distribution 
\begin{align*}
    p_{\text{emp}}(\vx) = \sum_{i=1}^N \delta(\vx -  \vx^{(i)})
\end{align*}
over the training samples $(\vx^{(i)})_{i=1}^N$ via neural networks (i.e., a summation of delta functions $\delta(\cdot)$ of the training data points), implying that a perfectly learned neural network would simply memorize the training data. But in practice, diffusion models can creatively generate new, sensible examples that differ significantly from the training data. In this section, we analyze the mechanisms and principles that govern when a diffusion model memorizes versus generalizes, providing insights into network design and training strategies. 
...
\end{align}
where $\tau>0$ is some threshold, $\mathcal M_{\text{SSCD}}(\vx_1,\vx_2)$ measures the cosine similarity between $\vx_1$ and $\vx_2$ in the latent space of a self-supervised copy-detection (SSCD) neural descriptor, and $\vx_1$ and $\vx_2$ are generated by two different diffusion models starting from the same initial noise. Strong model reproducibility implies that different diffusion models learn the same score functions. However, we have not yet answered the critical question: Why are diffusion models able to generalize? To answer this question, we introduce a generalization (GL) score that quantifies the distinction between a generated sample $\vx$ and the training samples $(\vx^{(i)})_{i=1}^N$:
\begin{align}
    \text{GL Score} := 1- \mathbb P( \max_{i \in [N]} \mathcal M_{\text{SSCD}}(\vx,\vx^{(i)})>\tau),
\end{align}

\Cref{fig:reproducibility-transition} depicts the RP and GL scores for diffusion models that are trained with varying data sizes and model capacities, implying that these models learn two distinct distributions that correspond to two separate training regimes:
\begin{itemize}
    \item \textbf{Memorization regime:} When model capacity far exceeds training data size, all diffusion models overfit by memorizing the same empirical distribution of training data, ultimately leading to strong model reproducibility without generalizability.
    \item \textbf{Generalization regime:} When model capacity is insufficient for memorization, diffusion models generate new samples. Here, reproducibility and generalizability co-emerge as all models learn the score function of the true distribution instead of memorizing the training data.
\end{itemize}
...
\paragraph{Towards a mathematical theory of generalization}
...
However, the complexity of real-world image distributions makes it difficult to rigorously verify our claims on image datasets. To mathematically understand generalization behavior, we therefore consider a simplified setting: a low-rank Gaussian distribution, i.e., 
\begin{equation} 
	\vx \sim \dNorm(\vzero, \vU^*\vU^{*\top}),
\end{equation}
...
\end{align}
where $\bm a_i \overset{i.i.d.}{\sim} \mathcal{N}(\bm 0, \bm I)$ is coefficients for each $i \in [N]$, and $\bm e_i \in \R^D$ is noise for each $i \in [N]$. Note that the noise $\bm e_i$ captures variability arising from real-world factors---such as measurement error, modeling mismatch, or deviations from an ideal low-rank structure. According to \eqref{eq:training loss}, the empirical training loss of diffusion models is 
\begin{equation}\label{eq:loss}
	\min_{\theta} \frac{1}{N}\Ex_{t, \bm g \sim \mathcal{N}(\bm 0, \bm I)}\norm{\bar{\vx}_{\theta}(t, \vx^{(i)}_t) - \vx^{(i)}}_{2}^{2},
\end{equation}
where $ \vx^{(i)}_t = \alpha_t \bm x^{(i)} + \sigma_t\bm g$ and $\bm g \sim \mathcal{N}(\bm 0, \bm I)$ as defined in \eqref{eq:gen_additive_gaussian_noise_model}.  Using \eqref{eq:gen_additive_gaussian_noise_model} and the same calculation in \eqref{eq:optimal denoiser}--\eqref{eq:gaussian_lowrank_denoiser_simplified}, the optimal denoiser is  
\begin{align}
    \bar{\vx}^{\ast}(t, \vx_{t}) = \frac{\alpha_t}{\alpha_t^2 + \sigma_t^2}\bm U^*\bm U^{*T}\bm x_t.
\end{align}
Therefore, the optimal network parameterization is
\begin{align}
    \bar{\vx}_{\theta}(t, \vx^{(i)}_t) = \frac{\alpha_t}{\alpha_t^2 + \sigma_t^2}\bm U\bm U^T\bm x_t,
\end{align}
where $\bm U \in \O(D, P)$. Using similar arguments in \eqref{eq:optimal nn}--\eqref{eq:PCA}, we can show that Problem \eqref{eq:loss} is equivalent to the following PCA problem: 
...
\begin{theorem}\label{thm:2}
    Suppose that the training samples $\{\bm x^{(i)}\}_{i=1}^N$ are generated according to \eqref{eq:Ua+e}. Let $\hat{\bm U}$ denote an optimal solution of Problem (\ref{eq:em loss}). The following statements hold: 
    \begin{itemize} 
