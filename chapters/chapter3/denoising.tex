...
\begin{equation}\label{eq:PCA}
    \min_{\vV \in \O(D, P)}\Ex_{\vx}\norm{\vV^{\top}\vx}_{2}^{2}
\end{equation}
which is exactly the probabilistic PCA formulation \eqref{eq:PPCA}.
...
\begin{remark}
	...
	\begin{equation}\label{eq:memorizing_denoiser}
		\bar{\vx}^{\star}(t, \vx_{t}) = \sum_{i = 1}^{N}\frac{\exp({-\|\vx_{t} - \alpha_{t}\vx_{i}\|_{2}^{2}/(2\sigma_{t}^{2})})}{\sum_{j = 1}^{N}\exp({-\|\vx_{t} - \alpha_{t}\vx_{j}\|_{2}^{2}/(2\sigma_{t}^{2})})}\vx_{i}.
	\end{equation}
...
\begin{equation} 
	\vx \sim \dNorm(\vzero, \vU^*\vU^{*\top}),
\end{equation}
where $\bm U^* \in \O(D, P)$ denotes an orthonormal basis. Accordingly, the training samples $\{ \vx^{(i)}\}_{i=1}^N$ are generated according to  
\begin{align}\label{eq:Ua+e} 
      \vx^{(i)} = \bm U^\star \bm a_i + \bm e_i, 
\end{align}
where $\bm a_i \overset{i.i.d.}{\sim} \mathcal{N}(\bm 0, \bm I)$ is coefficients for each $i \in [N]$, and $\bm e_i \in \R^D$ is noise for each $i \in [N]$. According to \eqref{eq:training loss}, the empirical training loss of diffusion models is 
\begin{equation}\label{eq:loss}
	\min_{\theta} \frac{1}{N}\Ex_{t, \bm g \sim \mathcal{N}(\bm 0, \bm I)}\norm{\bar{\vx}_{\theta}(t, \vx^{(i)}_t) - \vx^{(i)}}_{2}^{2},
\end{equation}
where $ \vx^{(i)}_t = \alpha_t \bm x^{(i)} + \sigma_t\bm g$ and $\bm g \sim \mathcal{N}(\bm 0, \bm I)$ as defined in \eqref{eq:gen_additive_gaussian_noise_model}.  According to \eqref{eq:gen_denoising_iteration}, one can show that the optimal network parameterization is
\begin{align}
    \bar{\vx}_{\theta}(t, \vx^{(i)}_t) = \frac{\alpha_t}{\alpha_t^2 + \sigma_t^2}\bm U\bm U^T\bm x_t,
\end{align}
where $\bm U \in \O(D, P)$. Using similar arguments in \eqref{eq:optimal nn}--\eqref{eq:PCA}, Problem \eqref{eq:loss} is equivalent to the following PCA problem: 
...
\begin{theorem}\label{thm:2}
   ...
   \begin{align}\label{rst2:thm 2}
       \left\|\hat{\bm U}\hat{\bm U}^T - \bm U^\star\bm U^{\star T}\right\|_F \ge \alpha - 
