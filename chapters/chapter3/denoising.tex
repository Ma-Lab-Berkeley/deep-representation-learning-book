...
{\color{brown}\paragraph{Diffusion process.}}
...
\begin{figure}
	...
	\caption{\small\textbf{Diffusing a mixture of Gaussians.} From left to right, we observe the evolution of the density as \(t\) grows from \(0\) to \(10\), along with some representative samples. Each region is colored by its density (\(0.0\) is completely white, \(> 0.01\) is very dark blue, every other value maps to some shade of blue in between.) We observe that the probability mass gets less concentrated as \(t\) increases, signaling that entropy increases. {\color{red} What is specific setup? How to take the mean and variance?}}
	\label{fig:ve_forward_density}
\end{figure}
{\color{brown}
\begin{example}[Adding Noise to a Mixture of Gaussians]\label{exam:MoG}
    Suppose that $\bm x$ is sampled from a mixture of $K$ Gaussians. To start, let us fix parameters for the distribution: mixture weights \(\vpi \in \R^{K}\), component means \(\{\vmu_{k}\}_{k = 1}^{K} \subseteq \R^{D}\), and component covariances \(\{\vSigma_{k}\}_{k = 1}^{K} \subseteq \PSD(D)\). Now, suppose \(\vx\) is generated by the following two-step procedure:
    	\begin{itemize}
    	...
    	\end{equation}
        Suppose that $K=1$, i.e., $\vx \sim \dNorm(\vmu, \vSigma)$. It follows from \eqref{eqn:entropy-Gaussian-multi} that the entropy of $\bm x_t$ is 
        \begin{align}
            h(\x_t) = \frac{D}{2}(1 + \log(2\pi)) + \frac{1}{2}\log\det(\bm{\Sigma}+t^2\bm I). 
        \end{align} 
        Then, one can verify that the entropy $h(\x_t)$ increases as $t$ increases. 
    \end{example}
}


{\color{brown}\paragraph{Denoising process.}}
...
\begin{example}[Denoising Gaussian Noise from a Mixture of Gaussians]\label{example:denoising_gaussian_mixture}
	In this example we compute the Bayes optimal denoiser for an incredibly important class of distributions, the Gaussian mixture model {\color{brown} in \Cref{exam:MoG}}. 
 %    To start, let us fix parameters for the distribution: mixture weights \(\vpi \in \R^{K}\), component means \(\{\vmu_{k}\}_{k = 1}^{K} \subseteq \R^{D}\), and component covariances \(\{\vSigma_{k}\}_{k = 1}^{K} \subseteq \PSD(D)\), where  \(\PSD(D)\) is the set of \(D \times D\) symmetric positive semidefinite matrices. Now, suppose \(\vx\) is generated by the following two-step procedure:
	% \begin{itemize}
	% 	\item First, an index (or \textit{label}) \(y \in [K]\) is sampled such that \(y = k\) with probability \(\pi_{k}\).
	% 	\item Second, \(\vx\) is sampled from the normal distribution \(\dNorm(\vmu_{y}, \vSigma_{y})\).
	% \end{itemize}
	...
	\begin{align}
        ...
        & = \vmu + \vV\mat{\lambda_{1}/(\lambda_{1} + t^{2}) & & \\ & \ddots & \\ & & \lambda_{D}/(\lambda_{D} + t^{2})}\vV^{\top}(\vx_{t} - \vmu),\label{eq:singel Gaus}
	\end{align}
	...
	\end{itemize}
	% Now, we show that the Bayes optimal denoiser contracts the current \(\vx_{t}\) towards the mean \(\vmu\):
	% \begin{align}
	% 	\norm{\bar{\vx}^{\ast}(t, \vx_{t}) - \vmu}_{2} & {\color{brown} =  \left\| \mat{\frac{\lambda_{1}}{\lambda_{1} + t^{2}} & & \\ & \ddots & \\ & & \frac{\lambda_{D}}{\lambda_{D} + t^{2}}}\vV^{\top}(\vx_{t} - \vmu) \right\|_2} \notag \\
 %        & \le \max\left\{ \frac{\lambda_i}{\lambda_i+t^2}: i=1,\dots,D \right\}\norm{\vx_{t} - \vmu}_{2}. \label{eq:contraction} \\        & \leq \norm{\vx_{t} - \vmu}_{2}.
	% \end{align}
...
% \begin{remark}
%   According to \eqref{eq:contraction}, the Bayes optimal denoiser $\bar{\vx}^{\ast}(t, \vx_{t})$ shrinks inputs toward the mean  \(\vmu\) in the single Gaussian model, which is a contraction mapping. At first glance, it resembles power iterations (see \Cref{subsec:power iterations}).  However, power iterations implement a contraction mapping towards the first principal component of a matrix. In contrast, the iterates in \eqref{eq:contraction} converge to the mean \(\vmu\) of a data distribution.   
% \end{remark}
...
{\color{brown}\paragraph{Incremental denoising.}}
...
\begin{example}[Iterative denoising for a single Gaussian] 
    To understand the above iterative denoising iteration intuitively, we apply it to denoise $\bm x_T $ defined in \eqref{eq:additive_gaussian_noise_model}, where $\bm x \sim \mathcal{N}(\bm \mu, \bm \Sigma)$. Let \(\vSigma = \vV\vLambda \vV^{\top}\) be an eigenvalue decomposition of $\vSigma$, where $\bm V \in  \O(D)$ and \(\lambda_{1}, \dots, \lambda_{D}\) are the eigenvalues of \(\vSigma\). Using \eqref{eq:singel Gaus} and \eqref{eq:iter denoise}, we obtain 
    \begin{align}
     \hat{\vx}_{t_{\ell - 1}} - \bm \mu % & = \hat{\vx}_{t_{\ell}} - \frac{1}{\ell} \left( \hat{\vx}_{t_{\ell}} - \bar{\vx}^{\ast}(t_{\ell}, \hat{\vx}_{t_{\ell}}) \right) \\
     & =  \bp{1 - \frac{1}{\ell}} \hat{\vx}_{t_{\ell}} - \bm \mu + \frac{1}{\ell} \left(  \vmu + \vV\mat{\frac{\lambda_{1}}{\lambda_{1} + t_{\ell}^{2}} & & \\ & \ddots & \\ & & \frac{\lambda_{D}}{\lambda_{D} + t_{\ell}^{2}}}\vV^{\top}(\hat{\vx}_{t_{\ell}} - \vmu) \right) \\
     % & =  \bp{1 - \frac{1}{\ell}}\left( \hat{\vx}_{t_{\ell}} - \bm \mu \right) + \frac{1}{\ell}  \vV\mat{\frac{\lambda_{1}}{\lambda_{1} + t_{\ell}^{2}} & & \\ & \ddots & \\ & & \frac{\lambda_{D}}{\lambda_{D} + t_{\ell}^{2}}}\vV^{\top}(\hat{\vx}_{t_{\ell}} - \vmu) \\
     & = \left(  \bp{1 - \frac{1}{\ell}}  \bm I +   \frac{1}{\ell}  \vV\mat{\frac{\lambda_{1}}{\lambda_{1} + t_{\ell}^{2}} & & \\ & \ddots & \\ & & \frac{\lambda_{D}}{\lambda_{D} + t_{\ell}^{2}}}\vV^{\top} \right)(\hat{\vx}_{t_{\ell}} - \vmu) \\
     & = \bm V \mat{1 - \frac{t_{\ell}^2}{\ell(\lambda_{1} + t_{\ell}^{2})} & & \\ & \ddots & \\ & & 1 - \frac{t_{\ell}^2}{\ell(\lambda_{D} + t_{\ell}^{2})}} \vV^{\top} (\hat{\vx}_{t_{\ell}} - \vmu). 
    \end{align}
    Iteratively applying the above argument yields
    \begin{align}
     \hat{\vx}_{0} - \bm \mu & = \bm V   \mat{ \prod_{\ell=1}^T\left(1 - \frac{t_{\ell}^2}{\ell(\lambda_{1} + t_{\ell}^{2})}\right)  & & \\ & \ddots & \\ & & \prod_{\ell=1}^T\left(1 - \frac{t_{\ell}^2}{\ell(\lambda_{D} + t_{\ell}^{2})}\right)}  \vV^{\top} (\hat{\vx}_{T} - \vmu) \\
     & = \bm V   \mat{ \prod_{\ell=1}^T\left(1 -  \frac{\ell T^2}{\lambda_1L^2 +\ell^2T^2}\right)  & & \\ & \ddots & \\ & & \prod_{\ell=1}^T\left(1 - \frac{\ell T^2}{\lambda_D L^2 +\ell^2T^2}\right)}  \vV^{\top} (\hat{\vx}_{T} - \vmu). \label{eq:contraction}
    \end{align}
    Then, we have 
    \begin{align}\label{eq:contraction1}
    \|\hat{\vx}_{0} - \bm \mu\|_2 \le \max\left\{ \prod_{\ell=1}^T\left(1 -  \frac{\ell T^2}{\lambda_i L^2 +\ell^2T^2}\right): i = 1,\dots,D\right\}  \left\| \hat{\vx}_{T} - \vmu \right\|_2. 
    \end{align}
According to \eqref{eq:contraction}, the Bayes optimal denoiser $\bar{\vx}^{\ast}(t, \cdot)$ iteratively shrinks noisy samples $\bm x_T$ toward the mean  \(\vmu\) in the single Gaussian model, which is a contraction mapping. At first glance, it resembles power iterations (see \Cref{subsec:power iterations}).  However, power iterations implement a contraction mapping towards the first principal component of a matrix. In contrast, the iterates in \eqref{eq:contraction1} converge to the mean \(\vmu\) of a data distribution.
\end{example}
}
{\color{brown}For general distribution,} the effect of this iteration is as follows. At the beginning of the iteration, where \(\ell\) is large, we barely trust the output of the denoiser and mostly keep the current iterate. This makes sense, as the denoiser can have huge variance (cf. \Cref{example:denoising_twopoints}). When \(\ell\) is small, the denoiser will ``lock on'' to the modes of the data distribution, as a denoising step basically takes a gradient step on the true distribution's log-density, and we can trust it not to produce unreasonable samples, so the denoising step mostly involves the output of the denoiser. At \(\ell = 1\) we even throw away the current iterate and just keep the output of the denoiser.
...
{\color{brown}\paragraph{How to compute the denoiser.}}
...
%We now extend the above analysis from the single Gaussian case to a Gaussian mixture model with $K\ge 2$ components. 


% \paragraph{A Gaussian mixture model.} 
...
\begin{align}
    \min_{\vV \in \O(D, P)} &\ \Ex_{\vx} \left\|\vx - \frac{1}{1 + t^{2}} \vV\vV^\top \vx\right\|_{2}^{2} \\
    = &\ \Ex_{\vx} \|\vx\|_2^2 + \left( \frac{1}{(1+t^2)^2} - \frac{2}{1+t^2} \right) \Ex_{\vx}\|\vV^\top \vx\|_2^2.
\end{align}
This is further equivalent to 
\begin{align}
    \max_{\vV \in \O(D, P)} \Ex_{\vx}\|\vV^\top \vx\|_2^2,
...
\begin{example}[A Mixture of Gaussians] For a Gaussian mixture model (see \Cref{exam:MoG}), its denoiser \eqref{eq:gmm_bayes_optimal_denoiser} becomes 
...
{\color{blue}\paragraph{Sampling error rate of the denoising process.}}
