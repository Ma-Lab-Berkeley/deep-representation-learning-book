...
\end{align}
which the latter formulation shows is equivalent to \eqref{eq:PPCA}.

Thus, if we were to assume our data were drawn from a low-rank Gaussian, our denoising algorithm essentially would consist of many PCA iterations in a row. While this method sounds very simple, it can actually produce complex and relatively powerful denoising behaviors when applied to practical data, as demonstrated by PCANet \parencite{chan2015pcanet}. Nevertheless, we will consider a more broad class of data distributions: the ubiquitous class of low-rank \textit{Gaussian mixture models},  whose denoiser we computed in \Cref{ex:gaussian_mixture} and \Cref{example:denoising_gaussian_mixture}. In the context of denoising, since Gaussian mixture models can approximate a very broad class of distributions arbitrarily well, in practice optimizing among the class of denoisers for Gaussian mixture models can potentially give us something close to the optimal denoiser for the real data distribution. 
...
\end{align}
This is a softmax operation weighted by the projection of \(\vx_{t}\) onto each subspace measured by \(\norm{\vU_{i}^{\top}\vx_{t}}_{2}\) (tempered by a temperature \(2t^{2}(1 + t^{2})\)). Meanwhile, the component denoisers can be written in the same way as \eqref{eq:gaussian_lowrank_denoiser_simplified}, to obtain 
\begin{equation}
...
\yima{Qing and Druv, a new section about memorization and generalization probably should go here. It should elucidate the role of quantization error (small noise) introduced by the network capacity... denoising to death means memorizing individual samples, hence leads to the topic of next chapter. }{\color{blue} Add a transition here to connect the previous chapters!}
