...
which maps noisy data samples $\vx_t$ to a probability distribution over labels
$\vy$. This is most straightforward to implement in the setting where $\vy$
corresponds to a class label in a classification problem (recall \Cref{a}), and
indeed, historically, methods for conditioning the generation of diffusion
models were first developed in the classification setting \cite{}. Below, we
will first discuss this important special case, then discuss how to implement
the likelihood term in
\Cref{eq:conditional-denoising-mmse-denoiser-paired-data} in more general
settings of interest.

\sdb{Working here. Classifier guidance and CFG. Need a simulation of this.}

\sdb{Cross attention connection: slight generalization of the previous
``measurement'' example, relevant to CFG parameterization of networks.}
