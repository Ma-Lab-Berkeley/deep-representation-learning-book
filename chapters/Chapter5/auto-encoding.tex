...
satisfies
\begin{equation}
  p_{\vx_t \mid \vy}(\vxi \mid \vnu) = \frac{p_{\vy \mid \vx_t} p_{\vx_t}}{p_{\vy}}.
\end{equation}
...
\begin{example}\label{example:denoising-conditional-gaussian}
  ...
  \Cref{eq:posterior-denoiser-gaussian-case}, and the measurement
...
approximation for the measurement matching term \Cref{eq:}---we can use ...
Although the numerical simulation in
\Cref{fig:conditional_sampling_computational_gaussian} suggests that this
approximation is not without its caveats in practice, it has proved to be
a reliable baseline in practice, after being proposed by Chung et al.\ \cite{},
and there are even principled and generalizable approaches to improve it by
...
which we discuss further in the end-of-chapter summary. We will therefore
proceed in developing it further. \sdb{working here}
...
\sdb{Short discussion of the moment matching improvement to DPS for fixing the
collapse issue. Short discussion of other approaches to solving inverse problems
with diffusion models, such as variable splitting and exact methods, and
surveys.}
