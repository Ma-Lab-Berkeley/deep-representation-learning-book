...
representation. Such a representation reveals intrinsic low-dimensional structures of the data distribution and facilitate subsequent tasks such as classification and generation.
...
\sdb{Perhaps better: just keep it as short as a paragraph, but present as
a conceptual proposal. Deeper into the geometric perspective here: the approach
is less scalable but more ``principled'' wrt the goal / the structure. We should
frame around the manifold hypothesis, and reference key deep technical works on
this, as well as its (past) empirical interest. Then we could discuss Michael's
paper as one of the few computational approaches (yet hard to scale).}
=======
\sdb{Deeper into the geometric perspective here: the approach is less scalable
  but more ``principled'' wrt the goal / the structure. We should
  frame around the
  manifold hypothesis, and reference key deep technical works on
  this, as well as
  its (past) empirical interest. Then we could discuss Michael's paper as one of
the few computational approaches (yet hard to scale).}
>>>>>>> 6169914394b7c508bc37bc6e50c44cbff3b8023f

Notice that in the proposed NLPCA autoencoder, the purpose and
function of each layer is a ``black box'' ---- their function in the
overall task of flattening the data manifold is not clear. The
question is, then, whether it is possible to construct such a NLPCA
autoencoder in a more interpretable or ``white box'' way, such that
the purpose of each layer towards flattening the data manifold is clear.
<<<<<<< HEAD
Indeed, the recent work of \cite{Psenka-JMLR24} has shown that under
rather general conditions, a nonlinear manifold can indeed be
=======
Indeed, the recent work of \cite{Psenka-JMLR24} has shown that under rather general conditions, a nonlinear manifold can indeed be
>>>>>>> 6169914394b7c508bc37bc6e50c44cbff3b8023f
flattened (i.e., \textit{encoded}) and reconstructed (i.e.,
\textit{decoded}) via a series of simple, interpretable, and local
operations, which either flatten a neighborhood on the manifold or
invert the flattening operation using learned information about the
<<<<<<< HEAD
curvature of the manifold.

Both the forward encoding and backward decoding transforms can be
constructed explicitly based on local geometric information (such as
curvature) that can be efficiently computed
(\href{https://arxiv.org/abs/2305.01777}{Manifold Flattening and
Reconstruction}).
=======
curvature of the manifold. Both the forward encoding and backward decoding transforms $f$ and $g$, respectively, can be constructed explicitly based on local geometric information (such as
curvature) that can be efficiently computed. More details can be found in 
the work \href{https://arxiv.org/abs/2305.01777}{Manifold Flattening and
Reconstruction}.
...
a Gaussian distribution with parameters given by an encoder MLP $f = (f_1,
f_2)$ with parameters $\vphi$:
\begin{equation*}
...
\label{sec:MAE}
\yima{A good autoencoding should enable us to utilize the learned distribution and representation for various estimation and generation tasks under different conditions.}
...
\subsection{Diffusion Models for Text-Conditioned  Generation}
