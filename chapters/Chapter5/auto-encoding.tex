...
\begin{align*}
  ...
  \left\{
    ...
    + \frac{1}{2}\sum_{i=1}^d \left(
      f_{2, i}(\vx) - f_{1, i}(\vx) - e^{f_{2, i}(\vx)}
    \right)
...
the decoder when latents $\vz$ are distribited according to the approximate
...
posterior concentrates on its mean $f_1(\vx)$, for any $\vx$: this is the limit
where its log-variance $f_{2,i}(\vx) \to -\infty$ for each coordinate $i = 1, \dots, d$.
...
\\
  ...
  %   (\vx - g_1\circ f_1(\vx))^\top \diag(e^{g_2\circ f_1(\vx)})^{-1} (\vx - g_1
  %   \circ f_1(\vx))
