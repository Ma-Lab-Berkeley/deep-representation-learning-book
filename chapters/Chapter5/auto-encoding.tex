...
In the past chapters, we have established a basic fact the
fundamental goal of learning is to learn a (low-dimensional) data
distribution and transform it to a compact and structured
representation. A fundamental approach to learn such a distribution
and a good  representation is through {\em compression}. To make the
...
optimization, there is no guarantee that the solution $\Z$ found by
incremental optimization is the correct solution. In fact, even if
...
a complete distribution or representation of the data.\footnote{This
...
might be incorrect.} Hence, an outstanding question is how we can
ensure that the learned data distribution or its representation is
...
learned distribution or representation to reproduce the original data:
\begin{equation}
  \X
...
processes of encoding and decoding, which we have alluded to briefly
in Chapter \ref{ch:intro} (see Figure \ref{fig:autoencoder}) and
studied in Chapter \ref{ch:classic} for some special cases. In this
chapter, we study how to extend autoencoding to represent more
general distributions.

We here give a formal definition of consistent representations, which
...
to be the identity map! Hence, the purpose of seeking an autoencoding
is precisely to try to ensure that the so-obtained $\Z$ is both more
compact and more structured that $\X$. More precisely,  $\Z$ should
better reveal the intrinsic low-dimensionality of $\X$\footnote{For
  example, $\Z$ is typically in an ambient space of lower dimension
than that of $\X$, although not have to be. } and the distribution of
$\Z$ should be better structured.\footnote{For example, the
  distribution of $\Z$ is (piecewise) linear/Gaussian or its components
are largely independent etc.} But most importantly, we should always
remember that the main purpose of learning about the distribution of
the data $\X$ is to help us to make better predictions by exploiting
the low-dimensionality of its distribution.
...
Notice that in the above construction, the linear transform $\vU$
...
question was answered by the work of Oja \cite{Oja1982SimplifiedNM} in 1982.
\begin{example}[Normalized Hebbian learning scheme] Consider a
...
Based on the modern practice of deep networks,  such classical
