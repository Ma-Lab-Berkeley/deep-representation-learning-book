...
distribution, say a Gaussian or (in
...
Unfortunately, unlike the above PCA case, there is in general no
...
\eqref{eqn:auto-encoding-objective-ch5}, we obtain a new objective for
...
\end{equation*}
%\yima{We might need to elucidate what are the true implications of the above two assumptions on the forms of the two conditional distributions. How do these assumptions enforce certain desired structures of the representation and how are the structures related to what rate reduction and sparsity are promoting? }
This makes probabilistic encoding and decoding simple: we simply map our data
$\vx$ to mean and variance parameters of a Gaussian distribution for encoding, or vice versa.
...
completion tasks in the data domain. More precisely,
we study how to exploit the (compressive) auoencoding for completing a data
conditioned on partial observation of the data. We leave the study of
data sampling and generation conditioned on features or general observations to the next Section
...
nuclear norm -- the sum of all singular values of the matrix $\X$. As
 shown in the compressive sensing literature, under fair
broad conditions,\footnote{Typically, such conditions specify the necessary and sufficient amount of entries needed for the completion to be computationally feasible. These conditions have been systematically characterized in \cite{Wright-Ma-2022}.} the matrix completion problem \eqref{eqn:rank-min}
...
$\X$. In practice, we often convert the above constrained convex optimization with an unconstrained one:
\begin{equation}
  \min \|\X\|_*  + \lambda \|
  \Y - \mathcal{P}_{\Omega}(\X)\|_2,
  \label{eqn:nuclear-min}
\end{equation}
...
Figure \ref{fig:matrix-completion} shows a real example in which the matrix $\hat \X$ is actually recovered by solving the above program. 

It has been shown that fo4 images (or more accurately textures)
...
For a general image such as the one shown on the left of Figure
\ref{fig:crate_mae_pipeline}, we can no longer view it as a low-rank
matrix. However, humans still demonstrate remarkable ability to complete a scene and recognize familiar objects despite severe
occlusion. This suggests that our brain has learned the low-dimensional distribution of natural images and can use it for
...
But let us examine the image completion task studied in the previous section: Given the
...
low-dimensional distribution of natural images! This partially explains why some of the recovered patches are a little blurry. 

For many practical purposes, we would like to learn (a representation of) the conditional distribution $p(\X_m \mid \X_v)$, or equivalently $p(\X \mid \X_v)$, 
and then sample this distribution directly. Notice that, when the distribution of $\X$ is low-dimensional, it is possible that if a sufficient part of $\X$, $\X_v$, is observed, it fully determines $\X$ hence the missing part $\X_m$. In other words, the distribution $p(\X \mid \X_v)$ is a generalized function -- the delta function. 

To generalize the above (image) completion problems, we may consider that a random vector $\vx$ is partially observed through a more general observation function:
\begin{equation}
    \vy = h(\vx) + \vw, 
\end{equation}
where $\vw$ usually stands for some random measurement noise, say of a Gaussian distribution $\vw \sim \mathcal{N}(\mathbf{0}, \sigma^2 \boldsymbol{I})$.
Please note that, in practice, there are typically two situations: one is that the observation function $h(\cdot)$ is explicitly known, and the other is when $h(\cdot)$ is not known but only samples of the joint distribution $p(\vx, \vy)$ are given.

A common goal is that we want the learned representation of the data to facilitate us to sample the condition distribution $p(\vx\mid \vy)$ effectively and efficiently. More precisely, we want to generate samples $\x$ such that:
\begin{equation}
\x \sim   p(\x\mid \y).  
\end{equation}

In Chapter
\ref{ch:compression}, we have seen a natural and effective way to establish a
...
between a normal (Guassian) distribution $p(\x_T) =  \mathcal{N}(\mathbf{0}, \boldsymbol{I})$ and the distribution of the data $p(\x_0)$. Hence,  one can sample the data distribution by sampling the
Gaussian distribution and solving the reserve (denoising) SDE:
\begin{equation}
    d\vx_t = - \nabla_{\vx_t} \log p_t(\vx_t) dt + d\vw_t.
\end{equation}
However, because such an autoencoding is only
between two distributions, it is difficult to control the resulting sample $\vx_0$ by sampling $\vx_T$. As a result, most follow-up works for
diffusion-/denoising-based generation tried to control the sampling process when certain conditions were given for the desired samples. \yima{Below we follow the survey paper on conditional sampling:  \href{https://arxiv.org/pdf/2410.00083}{A Survey on Diffusion Models for Inverse Problems}.}
...
\begin{equation}
      \nabla_{\vx} \log p(\vx \mid \vy) = \underbrace{\nabla_{\vx} \log p(\vy \mid \vx)}_{\text{measurement matching}} + \underbrace{\nabla_{\vx} \log p(\vx)}_{\text{score matching}}.
    \label{eqn:score-x}
\end{equation}
Notice that in the above expression, the second term is the {\em score matching} term introduced in Chapter \ref{ch:compression}. The first term here, known as the {\em measurement matching} term, produces a direction for $\vx$ in which the likelihood is higher for the observed $\vy$.

\yima{Maybe give some representative examples for the measurement matching term where it has explicit formulae.}

Now if we want to learn the condition distribution $p(\vx\mid \vy)$ via the diffusion and denoising process $\vx_t$ and then sample from it, we have we have: 
\begin{equation}
   \nabla_{\vx_t} \log p(\vx_t \mid \vy) = {\nabla_{\vx_t} \log p(\vy \mid \vx_t)} + {\nabla_{\vx_t} \log p(\vx_t)}. 
    \label{eqn:score-xt}
\end{equation}
Therefore, the associated SDE for the conditional denoising is given by:
\begin{eqnarray}
    d\vx_t &=& - \nabla_{\vx_t} \log p_t(\vx_t \mid \vy) dt + d\vw_t\\
    &=& - \big({\nabla_{\vx_t} \log p(\vy \mid \vx_t)} -  {\nabla_{\vx_t} \log p(\vx_t)}\big)dt  + d\vw_t.
\end{eqnarray}


Below we give some vignettes about solving several typical conditional sampling problems with the diffusion/denoising method, including the previous image-completion problem. For different applications, in practice, the measurement-matching term could be introduced into the (iterative denoising) deep architecture with different means.

\subsection{Image Completion}
The work of
\cite{wei2023diffusion} has shown how to use diffusion/denoising
models to learn such a conditional autoencoder for image completion  
(\href{https://weichen582.github.io/diffmae.html}{Diffusion Models as
Masked Autoencoders}).

\subsection{Conditional Pose Prediction}
...
\sdb{Align Visual Data with Texts, ControlNet, ... DALLE type
