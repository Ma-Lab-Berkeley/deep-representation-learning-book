...
distribution on $\cM$ is mapped to a sufficiently regular distribution, say a Gaussian or (in
...
In the most straightforward instantiation of sparse autoencoder training,
...
to texts containing samples of computer code in different programming languages,
with our task being to try to identify interpretable features in the feature map
$h$ corresponding to different salient aspects of the input, such as the
specific programming language (distinct across input ``classes'') or the need to
insert a matching parenthesis (common across input ``classes''). We discuss the
use of deep networks for text representation learning in greater detail in
\Cref{ch:applications}.} For
...
variable model $\vz$. Indeed, we may write by conditioning $p(\vx, \vz ;\, \vtheta)
...
to the underlying probabilistic framework for the sake of computational tractability.
...
\begin{enumerate}
  ...
  \item We jointly learn $\vtheta$ and $\veta$ via maximizing a tractable lower
    ...
    bound (ELBO).
\end{enumerate}
We will focus on the most useful instantation of the VAE framework for practice, namely
