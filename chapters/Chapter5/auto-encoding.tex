...
Indeed, the fundamental purpose of autoencoding is to grant
\textit{control} over the distribution of the data. In the previous Sections, we
have developed the methodology of autoencoding in an abstract,
...
physical poses, that we would like to learn an autoencoding for.
In such cases, the modern practice is to establish such an autoencoding
indirectly: rather than a direct encoder-decoder mapping between $\vx$ and
$\vz$, we seek to establish a \textit{binding} via the joint distribution $(\vx,
\vz)$, which grants us the ability to generate samples of $\vx$ conditioned on
...
The above (masked) autoencoding problems can be viewed to sample the
learned low-dimensional distribution such that the generated samples
are consistent with certain observations or conditions.
...
explains why some of the recovered patches are a little blurry.

For many practical purposes, we would like to learn (a representation
of) the conditional distribution $p(\X_m \mid \X_v)$, or equivalently
$p(\X \mid \X_v)$,
...
$p(\X \mid \X_v)$ is a generalized function -- the delta function.
...
that a random vector $\vx$ is partially observed through a more
...
\boldsymbol{I})$.
Please note that, in practice, there are typically two situations:
...
A common goal is that we want the learned representation of the data
to facilitate us to sample the condition distribution $p(\vx\mid
\vy)$ effectively and efficiently. More precisely, we want to
generate samples $\x$ such that:
\begin{equation}
\x \sim   p(\x\mid \y).
\end{equation}
...
between a normal (Gaussian) distribution $p(\x_T) =
\mathcal{N}(\mathbf{0}, \boldsymbol{I})$ and the distribution of the
data $p(\x_0)$. Hence,  one can sample the data distribution by sampling the
Gaussian distribution and solving the reverse (denoising) SDE:\yima{We did not quite use the SDE or DE notation in Chapter \ref{ch:compression}when discussing the denoising process. Reconcile please?} 
\begin{equation}
\odif\vx_t = - \nabla_{\vx_t} \log p_t(\vx_t) \odif t + \odif\vw_t,
\end{equation}
or via the deterministic differential equation:
\begin{eqnarray}
\odif\vx_t  &=& - \frac{1}{2} {\nabla_{\vx_t}
\log p(\vx_t)}\odif t.
\end{eqnarray}
However, because such an autoencoding is only
between two distributions, it is difficult to control the resulting
sample $\vx_0$ by sampling $\vx_T$. As a result, most follow-up works for
diffusion-/denoising-based generation tried to control the sampling
process when certain conditions were given for the desired samples.
...
(\Cref{exercise:conditional_gaussian}) shows that
\begin{equation}
  \vx \mid \vy \sim
\end{equation}
\end{example}
...
Now if we want to learn the condition distribution $p(\vx\mid \vy)$
...
\begin{remark}
Consider the condition-guided denoising as a continuous process, the associated stochastic differential equation (SDE) for the conditional denoising is given by:
\begin{eqnarray}
\odif\vx_t &=& - \nabla_{\vx_t} \log p_t(\vx_t \mid \vy) \odif t + \odif\vw_t\\
&=& - \big({\nabla_{\vx_t} \log p(\vy \mid \vx_t)} -  {\nabla_{\vx_t}
\log p(\vx_t)}\big)\odif t  + \odif \vw_t.
\end{eqnarray}
The associated (equivalent) deterministic differential equation (DE) is
\begin{eqnarray}
\odif\vx_t 
&=& - \frac{1}{2}\big({\nabla_{\vx_t} \log p(\vy \mid \vx_t)} -  {\nabla_{\vx_t}
\log p(\vx_t)}\big)\odif t.
\end{eqnarray}
\end{remark}
