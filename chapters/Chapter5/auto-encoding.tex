...
developed in \Cref{sub:compression_denoising}, namely around
\Cref{eq:denoising-iteration-basic}, we obtain a practical algorithm for
conditional sampling of the posterior $p_{\vx \mid \vy}$ given measurements
following \eqref{eq:measurement-matching-observation}, written up for
convenience in \Cref{alg:iterative_denoising_conditional} for the case of
Gaussian noise with known standard deviation $\sigma$. In our discussion above,
we have made the simplifying choices $\alpha_t = 1$, $\sigma_t = t$, and
$t_{\ell} = T\ell / L$, as for \Cref{eq:denoising-iteration-basic} in
\Cref{sub:compression_denoising} (and so
\Cref{alg:iterative_denoising_conditional} should be initialized with
$\cN(\Zero, t_L^2 \vI)$ given these choices).

\sdb{TODO: Need a small fix in the previous, because we used Tweedie for simple
$\vx_t = \vx + t \vg$, but the algorithm for general DDIM needs to use it for
$\vx_t = \alpha_t \vx + \sigma_t \vg$...}

\sdb{TODO: More realistic numerical example, eg MRI?}
...
realizations of $\vx$, and we assume for simplicity that $\vx_t = \vx + t \vg$
with $\vg \sim \cN(\Zero, \vI)$ independent of $(\vx, \vy)$.
as in \Cref{fig:posterior-sampling-cds}.
...
and conditional independence of $\vy$ and $\vx_t$ given $\vx$, and thus we can
...
\sdb{Working here. Classifier guidance and CFG.}
