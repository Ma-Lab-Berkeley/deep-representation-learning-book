...
$p(\X \mid \X_v)$ is a generalized function---if $\vX$ is fully determined by
...
from the posterior $p(\x \mid \y)$ without access to any clean/ground-truth
...
\begin{enumerate}
  \item \textbf{Known $h$: Inverse Problems.} Like image/matrix completion, we
    are often faced with a setting where $\vy$ denotes a degraded or corrupted
    observation of the input $\vx$. For example, in various scientific or
    medical imaging problems, the measured data $\vy$ may be a compressed or
    otherwise corrupted observation of the underlying data $\vx$; however, by
    virtue of mathematical modeling and our design of the measurement system, we
    know $h$, and can use this knowledge to help reconstruct or sample $\vx$.
  \item \textbf{Paired Samples $(\vx, \vy)$: Controllable Generation.} In more
    ...
    it is common for data $\vx$ to have known \textit{attributes} that we want
    to use for conditional sampling of the data distribution. Below, we will
    study detailed vignettes from vision, where $\vy$ represents a text caption
    for an image or the pose of a 3D rigid body.
    \sdb{Revisit... more like model-based vs.\ model-free... (eg the pose case
    is more like the 1st)}
\end{enumerate}
The attentive reader may ask why we need to study the first class of scenarios,
given that it is a special case of the latter. We will see later, both in theory
and in practice, that exploiting knowledge of $h$ can lead to superior
performance.

A common goal between these two situations is that we want the learned representation of the data
...
$p(\x)$. The ingredients are the denoisers $\bar{\x}^\ast(t, \vx_t) = \bE[ \x \mid
\vx_t ]$, or their learned approximations $\bar{\x}_{\theta}(t, \vx_t)$, for different levels of noisy observations $\x_t = \x + t \vg$, for
Gaussian noise $\vg \sim \sN(\mathbf{0}, \vI)$ and $t \in
[0, T]$, and a choice of times $0 = t_1 < \hdots < t_{L} = T$ at which to
perform the iterative denoising, starting from $\hat{\vx}_{t_L} \sim
\sN(\mathbf{0}, T^2 \vI)$ (recall \Cref{eq:denoising-iteration-basic}).\footnote{Recall from our discussion in \Cref{sub:sampling_denoising}
