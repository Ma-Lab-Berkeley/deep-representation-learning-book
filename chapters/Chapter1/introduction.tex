...
Quite miraculously, about a few thousand years ago, another quantum leap in human intelligence occurred, which allowed philosophers and mathematicians to develop knowledge that transcends empirical observations and experiences with mathematical abstraction\footnote{such as the concept of numbers, space, and time.} and logic deduction. This new type of language serves as the basis for modern science. The ability to develop these high-level forms of knowledge is believed to be unique to humans and is referred to as ``artificial intelligence'' (AI), coined by John McCarthy at the Dartmouth summer workshop in 1956. 
...
As the readers may have known, in the past decade or so,  intelligent technologies have undergone explosive development, powered mainly by the practice of deep artificial neural networks, triggered by the work of Geoffrey Hinton and students in 2012 \cite{Krizhevsky2012imagenet}. This era is also hailed as the ``Renaissance'' of Artificial Intelligence (AI). However, in terms of tasks that people have actually tried to tackle (recognition, generation, and prediction) and techniques that people have adopted and implemented so far (reinforcing, imitating, encoding, decoding, denoising, and compression), we are very much just emulating the mechanisms that are common to the intelligence of life and animals. Even in that regard, strictly speaking, current ``AI'' models and systems have yet to completely implement all necessary mechanisms known to the Cybernetics movement in the 1940s. 
...
\begin{figure}
    ...
    \label{fig:expansion}
\end{figure}
As we will show in Chapter \ref{ch:compression}, such desired structures in the final representation can be precisely introduced by choosing a natural family of coding schemes and a natural measure of {\em information gain} based on such a coding scheme.  As we see in this book, such an explicit and constructive coding approach provides a powerful computational framework for learning good representations of low-dimensional structures for real-world data. In many cases of practical importance, the coding length function can be efficiently computed or approximated accurately. In some benign cases, we can even obtain closed-form formulae, e.g. subspace and Gaussian (see Chapter \ref{ch:compression}). 

In addition, such a computable and quantifiable formulation leads to a principled approach that naturally reveals the role that deep networks play in the learning process. As we will derive systematically in Chapter \ref{ch:representation}, their layers are precisely trying to realize operations that optimize the objective function of interest in an incremental manner. From this perspective, the role of deep networks can be precisely interpreted as to emulate a certain iterative optimization algorithm, say gradient descent, to optimize the objective of information gain. Layers of the resulting deep architectures can be endowed with precise statistical and geometric interpretation, namely performing incremental compressive encoding and decoding operations. As a result, the derived deep networks become transparent ``white boxes'' that are mathematically fully explanable.  
