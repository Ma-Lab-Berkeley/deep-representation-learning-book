...
\end{itemize}
This book aims to provide some answers to these fundamental  questions. These answers will help us understand intelligence better, at least the computational principles and mechanisms that enable it. There is reason to believe, intelligence and science follow the same set of principles and mechanisms to evolve.

\paragraph{Evolution of intelligence in nature.}
...
A necessary condition for the emergence of life on earth about 3.7 billion years ago is that the earth's environment is largely predictable. In such an  environment, life has developed  mechanisms that enable it to learn  what is predictable and use the information for survival. We call this ability {\em intelligence}. To a large extent, the evolution of life is also that of intelligence. In nature, intelligence is mainly realized through two types of learning mechanisms: {\em phylogenetic} and  {\em ontogenetic} \cite{Wiener-Cybernetics-1961}.%\footnote{Actually there is a third type of intelligence that is unique to the human, which is believed to be rather different from these two. The human intelligence is formally referred to as ``artificial intelligence'' (AI). We will discuss it later. Here we restrict our discussions to intelligence common in nature, including plants and animals.} 

{\em The phylogenetic intelligence} refers to learning through the evolution of species. Species inherit and survive mainly based on knowledge inherited via genes from their parents\footnote{Nature's pre-trained models.}. Species evolve based on (random) gene mutation and then selected by nature -- survival of the fittest The main characteristic of phylogentic learning is that individuals do not have much learning capacity. Learning is done by essentially a trial-and-error mechanism by species\footnote{Nature's reinforcement learning.} The process of phylogenetic learning is extremely slow, unpredictable, and expensive.\footnote{From the beginning of life about 3.5 billion years ago, life has relied on this form of evoluation for about 3 billion years!} 

{\em The ontogenetic intelligence} refers to the learning mechanisms that enable individual life to learn from  its interactions with a specific living environment and to improve and adapt its behaviors. Ontogentic learning took place mainly after the first emergence the nerve system and the brain about 550 million years ago. That is, with a brain,\footnote{Senses, including vision, also first  appeared around the same time.} the individual is  capable of forming its own unique memory, besides what is inherited from its genes. This capability has significantly enhanced the survival of the individual and had led to the explosion of life forms in the Cambrian period about 530 million years ago. Compared to phylogentic learning, ontogenetic learning must be fast, predictable, and efficient.\footnote{That can be implemented within the time and resource of an individual.}

Notice that both types of learning mechanisms rely on the feedback, in terms of reward or penalty, of a species' or an individual's  actions\footnote{Gene mutation of the species or decisions made by the individual.} to learn. As a result, all intelligent lives, either as a species or an individual, rely on  a closed-loop mechanism to learn and to improve. Also notice that from plants, to birds, and to mammals, more advanced species appear to rely more on their ontogenetic learning capabilities. They stay with  and learn from their parents longer and longer after birth, because individuals of the same species need to survive in very diverse environments. 

Since the emergence of homo sapiens about 315 thousand years ago, a new and higher form of intelligence emerged and started to evolve at an even faster pace. First, {\em languages} were developed a few thousand years ago,\footnote{It was believed that Sanskrit was the first spoken language, dated as back as 5000 BC.} allowing individuals to share useful information they have learned with others. Hence a community can behave like a single intelligent being that can learn much faster than an individual and can pass on its knowledge through generations, besides genes. Science is part of that knowledge. We may refer to this type of intelligence as the {\em communal intelligence}, to distinguish the phylogentic intelligence for species and the ontogenetic intelligence for individuals.  

Somewhat miraculously, about a couple of thousand years ago, another quantum leap in human intelligence took place, which allow philosophers and mathematicians to develop knowledge that does not seem to be directly learnable from empirical observations and experiences, such as  mathematical abstractions\footnote{concepts of numbers, space, and time.} and logic. These high-level intelligence capabilities that are believed to be unique to humans are referred to as ``artificial intelligence'' (AI), coined by John McCarthy at the   Dartmouth summer workshop in 1956. 

Hence, from what we can learn from the nature, from now on, whenever we use the word ``intelligence'', we need to be very specific about which level of intelligence we mean:
\begin{equation}
\mbox{\textbf{phylogentic}} \;
   \Longrightarrow \; \mbox{\textbf{ontogenetic}} \; \Longrightarrow \; 
   \mbox{\textbf{communal}}
   \; \Longrightarrow \; 
   \mbox{\textbf{artificial}}.
\end{equation}
The distinction is necessary and important because if we want to study intelligence as a scientific or mathematical subject, it is highly likely that, for each level of intelligence, the associated learning and  computing mechanisms could be related but  different. 
...
In 1940s, partly due to the war effort, intelligence in nature had inspired scientists in the 1940s to emulate animal intelligence by machines, which led to the ``Cybernetics''  movement advocated by Norbert Wiener. Wiener studied zoology at Harvard as an undergraduate but later became a mathematician and control theorist. Wiener had a life-long passion for  understanding and developing autonomous systems that could emulate intelligent behaviors of animals. Today, the Cybernetics program is often narrowly interpreted by people as mainly about feedback control systems where indeed Wiener made his most significant technical contributions. But the Cybernetics program was much broader and deeper than that. It was actually more about understanding intelligence as a whole\footnote{At least at the level of animals.} and had actually influenced the work of a whole generation of renowned scientists, including Warren McCulloch, Walter Pitts, Claude Shannon, John von Neumann, and Alan Turing.  
...
\begin{itemize}
    ...
    \item How to {\em correct errors} with feedback in prediction and estimation using existing information. Norbert Wiener himself helped formalize the theory for control systems based on closed-loop feedback in the 1940s. 
    \item How to learn to {\em make better decisions} via interacting with a potentially non-cooperative opponent or adversarial environment. This was formalized by John von Neumann as the game theory in 1944. 
\end{itemize}
In 1943, very much motivated by Wiener's Cybernetics program, psychiatrist  Warren McCulloch and logician Walter Pitts together formalized the first computational model for a neuron \cite{McCulloch-Pitts}, called {\em an artificial neuron}, as illustrated later in Figure \ref{fig:neuron}. Based on this model, in 1950s, Frank Rosenblatt built a physical machine, called {\em Mark I Perceptron}, with a network of hundreds of such artificial neurons. Perceptron was the first physically implemented artificial neural networks, see Figure \ref{fig:perceptron}. Notably, John von Neumann's universal computer architecture, proposed in 1945, was also designed to facilitate the goal of building {\em computing machines} that can physically realize the  mechanisms of Cybernetics. 

Acute readers probably have noticed that the 1940s was truly a magical era: so many fundamental ideas were invented and influential theories formalized in that era, including the mathematical model of neurons, artificial neural networks, information theory, control theory, game theory, and computing machines. As we now know, each work above had grown to become the foundation of a scientific or engineering field for the following many decades. All these fundamental discoveries or theories were inspired and motivated by the goal of trying to develop machines that can emulate intelligence in nature. Based on historical notes, Wiener's Cybernetics movement had influenced almost all of these people and work.  

Although Wiener in his work had identified many key characteristics and mechanisms of intelligence, there was no indication that he knew how to properly integrate all these mechanisms together to build a complete autonomous intelligent system. Judging from today's knowledge, some of his views on these mechanisms were not entirely accurate or complete. In particular, in the last chapter of the second edition of his Cybernetics book \cite{Wiener-Cybernetics-1961}, he pointed out it is crucial to {\em deal with nonlinearality} if a machine learning system is designed to emulate typical learning mechanisms in nature. But he did not provide any concrete and effective solutions to this difficult issue. To his defense, at the time, few people knew how since even the theory dealing with linear models and systems was still in its infancy. 

Nevertheless, we could not help but marvel at Wiener's foresight about the importance of nonlinearity. As we will see in this book, the answer was found only recently: nonlinearity can be effectively dealt with through progressive linearization and transformation realized by deep neural networks (see Chapter \ref{ch:representation}). In addition, we will attempt to show in this book how all these mechanisms listed above can be naturally integrated into a complete learning system that exhibits  characteristics of an autonomous intelligent system (see Chapter \ref{ch:autoencoding} and \ref{ch:closed-loop}). 
...
As the readers may have known, in the past decade or so,  intelligent technologies have undergone explosive development, powered mainly by the practice of deep artificial neural networks, triggered by a work of Geoffrey Hinton and students in 2012  \cite{Krizhevsky2012imagenet}. This era is also hailed as the ``Renaissance'' of Artificial Intelligence (AI). Nevertheless, in terms of tasks  people actually have tried to solve (recognition, generation, and prediction) and techniques people actually have adopted and implemented so far (reinforcing, imitating, encoding, decoding, denoising and compression), we are very much just emulating the mechanisms that are common to the intelligence of life and animals. Even on that regard, strictly speaking, current ``AI'' models and systems have not yet completely implemented all necessary mechanisms known to the Cybernetics movement about 80 years ago. 

Hence, technically speaking, advancements made in the name of ``Artificial Intelligence'' in the past decade largely do not quite belong to the ``Artificial Intelligence'' program laid out in the 1956 Dartmouth workshop. Instead, what has been predominantly accomplished so far is more closely related to the objectives of the classic Cybernetics program laid out by Norbert Wiener in the 1940s. It is probably more appropriate to call the current era the ``Renaissance of Cybernetics''.\footnote{The recent rise of the so-called ``Embodied AI'' for autonomous intelligent robots share even more similarity with the goals of the Cybernetics program.} Only after we have fully understood what we have truly done from the scientific and mathematical perspective, can we know what remains to be done and which direction to go to pursue the true nature of intelligence. This is the main reason why we have decided to write this book and the main purpose for which we want it to serve. 
...
Data that carry predictable information manifest in many different forms. In the most natural form, they can be modeled as sequences that are predictable and computable. The notion and properties of predictable and computable sequences, were at the heart of the theory of computing, and very much led to the invention of computers \cite{Turing-1936}. The role of predictable sequences in (inductive) inference was studied by Ray Solomonoff, Andrey Kolmogorov, and many others in 1960s \cite{Kolmogorov1998OnTO} as a generalization to Claude Shannon's classic Information Theory \cite{Shannon-1948}. To understand the concept of predictable sequences, let us first start with some concrete examples. 
...
Conceptually, an even harder problem than the sparse coding problem \eqref{eqn:sparse-model} is when the observation matrix $\boldsymbol{A}$ is not even known in advance and we need to learn it from a set of observations:
\begin{equation}
    \X = \boldsymbol{A} \Z, \quad \boldsymbol{A} \in \mathbb{R}^{m\times n}.
...
The distributions of real-world data such as images, videos, and audio are too complex to be modeled by above, somewhat idealistic, linear models or Gaussian processes. We normally do not know {\em a priori} they are generated from what family of parametric models.\footnote{Although in history there had been many attempts to develop analytical models for these data, such as random fields or stochastic processes for imagery data \cite{Mumford-1999}, as we have discussed in the previous section.} In practice, we typically only have many samples from their distributions -- the empirical distributions. Obviously, in such cases, we normally cannot expect to have any closed-form solutions for their low-dimensional structures, nor for the resulting denoising operators.\footnote{Unlike the cases of PCA, Wiener filter, and Kalman filter.} So we need to develop a more general solution to these empirical distributions, not necessarily in closed form but at least efficiently computable. If we did this correctly, solutions to the aforementioned linear models should become their special cases. 

In the 1950s, statisticians became interested in the problem of denoising for data drawn from an arbitrary distribution. Let $\x_o$ be a random variable with probability density function $p_o(\cdot)$. So if we observe a noisy version of $\x_o$:
\begin{equation}
...
\begin{figure}
    ...
    \caption{Geometric interpretation of a score function $\nabla \log p(\x)$ for a distribution with density $p(\x)$. The operation generated by the score function pushes the distribution towards areas of higher density. The goal is that, by a certain measure of compactness, the resulting distribution is more ``compressed''. For example, the entropy $H(\x) = - \int p(\x) \log p(\x)~d \x$, or the coding rate of the distribution $R(\x)$ by some lossy coding scheme, would become smaller.}
    \label{fig:score-function}
\end{figure}

In fact this function has a very intuitive information-theoretic and geometric interpretation. Note that in information theory $-\log p(\x)$ generally corresponds to the number of bits needed to encode $\x$\footnote{at least in the case of a discrete variable, as we will explain in more details in Chapter \ref{ch:compression}.}. The gradient $\nabla \log p(\x)$ hence points to a direction in which the density gets higher and the number of bits needed to encode $\x$ decreases if it was moved towards that direction. Hence, the overall effect of the operator $\nabla \log p(\x)$ is to push the distribution to ``shrink''  towards areas of higher-density so as to reduce the overall coding length/rate of the resulting distribution -- hence more ``compressed.'' Actually one can formally show that the entropy of the resulting distribution from such an operation will be reduced. Figure \ref{fig:score-function} illustrates the geometric meaning of this operation associated with the score function. 



Because this is such an important and useful result, it has been rediscovered and used in many different contexts and areas. For instance, it was rediscovered a few years later by \cite{Miyasawa61}. In the early 2000s, the function $\nabla \log p(\x)$ was rediscovered again  in the context of learning general distributions and named as the ``score function'' by Aapo Hyv\"{a}rinen \cite{hyvarinen05a}, actually with an intention to help solve the ICA problem \eqref{eqn:ICA-model}. But its connection to (empirical Bayesian) denoising was soon recognized by \cite{Vincent2011}.
...
CNN-type networks continued to evolve in the 1980s and many different variants had been introduced and studied. However, despite the remarkable capacities of deep networks and the improved architectures inspired by neuroscience, it remained extremely difficult to train such deep networks to learn any real tasks from data, such as image classification. How to get a network to work depended on many unexplainable heuristics and tricks that really limited the appeal and applicability of neural networks. One major breakthrough came around 1989, Yann LeCun successfully used {\em back-propagation} (BP) to learn a deep  convolutional neural network for recognizing images of hand-written digits \cite{LeCun-1989}, later known as the LeNet (see Figure \ref{fig:LeNet-5}). After several years' persistent development, his perseverance paid off: performance of the LeNet eventually became good enough for practical usage in late 1990s \cite{LeCun-1998}: it was used by the US Post Office for recognizing handwritten digits (for zip codes).  The LeNet was considered as the ``prototype'' network for all modern deep neural networks, such as the AlexNet and ResNet, which we will discuss later. Because of this work, Yann LeCun was awarded the 2018 Turing Award.\footnote{Together with two other pioneers of deep networks, Yoshua Bengio and Geoffrey Hinton.}

\begin{figure}
    \centering
\includegraphics[width=0.95\linewidth]{figures/LeNet-5.png}
    \caption{The LeNet-5 network designed by Yann LeCun in 1989. }
    \label{fig:LeNet-5}
\end{figure}
...
\end{equation}
In order to train the network weights $\{\bm W_l\}_{l=1}^L$, say to reduce the prediction/classification error based on a gradient descent algorithm, we need to compute the gradient ${\partial h}/{\partial \bm W_l}$. It has been well known, from the {\em chain rule} in calculus, one can show that the gradients can be computed efficiently for this type of functions, later known as back propagation (BP). See Appendix \ref{app:optimization} for a detailed description. The technique of back propagation was already known and practiced by people in fields such as optimal control and dynamic programming in the 1960s and 1970s. For example, it appeared in the 1974 PhD thesis of Dr. Paul Werbos \cite{Werbos-1974, Werbos1994TheRO}. In 1986, David Rumelhart et al. were the first to apply the back propagation technique to train multiple layer perceptron (MLP) networks \cite{Rumelhart1986}. Since then,  BP has become increasingly popular since it gives a {\em scalable} algorithm to learn large deep neural networks.\footnote{as it can be efficiently implemented on computing platforms that facilitate parallel and distributed computing.} It is now an almost dominant technique for training deep neural networks today. Nevertheless, it is believed that nature does not learn with back propagation since such a mechanism is still way too expensive for physical implementation by  nature.\footnote{As we have discussed earlier, nature almost ubiquitously learns to correct error via closed-loop feedback.} This obviously leaves tremendous room for improvement in the future, as we will discuss more later.
...
\begin{equation}
    \min d(\X, \hat \X).
\end{equation}
As it turns out, most theoretically motivated  distances are extremely difficult, if not impossible, to compute and optimize for distributions in high-dimensional space but with a low intrinsic dimension.\footnote{This is the case even if a parametric family of distribution for $\X$ is given. The distance often becomes ill-conditioned or ill-defined for distributions with low-dimensional supports. What is even worse is the chosen  family might not be able to approximate well the true distribution of interest.} 
...
So far, we have given a brief account of the main objective and history of machine intelligence and many important ideas and approaches associated with it. In recent years, after the empirical success of deep neural networks, tremendous  effort has been made to develop theoretical frameworks that can help us understand all the empirically designed deep neural networks, either certain seemingly necessary components (e.g., dropout,  normalization, attention, etc.) or their overall behaviors (e.g., double descent, neural collapse,  etc.). 
...
\begin{figure}
    ...
    \caption{Comparison between two coding schemes. Image the true distribution of the data is around the green balls. One can code the events with the a code book consisting of all blue balls; one can also code the events with a code book consisting of only the green balls. Obviously, the second coding schemes will result in much lower coding length/rate.}
...
In addition, such a computable and quantifiable formulation leads to a principled approach that naturally reveals the role that deep networks play in the learning process. As we will derive systematically in Chapter \ref{ch:representation}, their layers are precisely trying realizing operations that optimize the objective function of interest in an incremental manner. From this perspective, the role of a deep network can be precisely interpreted as to emulate a certain iterative algorithm, say gradient descent, to optimize the objective of information gain. Layers of the resulting deep architectures can be endowed with precise statistical and geometric interpretation, namely performing incremental compressive encoding and decoding operations. As a result, the so derived deep networks become transparent ``white boxes.'' 
...
\end{equation}
where $\z(i)$ indicates the $k$-th entry of the vector $\z$. As the early practice of deep networks indicated \cite{Krizhevsky2012imagenet}, if enough data are given, such an encoding scheme can often be represented by a deep network and learned in an end-to-end fashion by optimizing the cross-entropy. 
\end{example}

The cross-entropy $L(\hat \z, \z)$ can be viewed as a special measure of parsimony $\rho(\z)$ associated with a particular family of encoding schemes suitable for classification. However, such an encoding is obviously {\em very lossy}. The learned $\z$ does not contain much other information about $\x$ except for its class. For instance, by assigning an image with (a code representing) the class label ``apple'', we no longer know what specific type of apple  is in the original image from such a label alone. 

Of course, the other extreme is to require the coding scheme to be {\em lossless}. That is, there is a one-to-one mapping between $\x$ and its code $\z$. However, as we will see in Chapter \ref{ch:compression},  lossless coding (or compression) is impractical unless the distribution and value of $\x$ is discrete hence admits a finite coding length. For a continuous random variable, we may only consider lossy coding schemes so that the coding length for the data can be finite. That is, we can only encode the data up to certain prescribed precision. As we will elaborate more in Chapter \ref{ch:compression}, lossy coding is not merely a practical choice, it plays a fundamental role in making learning the underlying continuous distribution possible from finite samples of the distribution. 

For most purposes of learning, we want the feature $\z$, although {\em lossy}, to preserve most information about  $\x$, not just its class information. In this book, we will introduce a more general measure of parsimony based on coding length/rate associated with a more general family of coding schemes -- coding with a mixture of subspaces. This family has the capability to closely approximate arbitrary real-world distributions, up to certain precision. As we will see in Chapter \ref{ch:representation}, such a measure will not only preserve most information about the distribution of $\X$ but will also promote certain nice desired geometric and statistical structures for the learned representation $\Z$. 
...
Generally, we would prefer that the decoding is nearly an ``inverse'' to the encoding such that the data $\hat \X$ decoded from $\Z$ would be  similar to the original data $\X$ to some extent.\footnote{We will make it more precise what we mean by being similar later.} If so, we would be able to recover or predict from $\Z$ what is going on in the original data space. In this case, we say the pair $(\mathcal{E}, \mathcal{D})$ gives a {\em consistent}  autoencoding. For most practical purposes, not only we need such a decoding to exist, but also it can be efficiently realized and physically implemented. For example, if $\x$ is a real-valued variable, quantification is needed in order for any decoding scheme to be realizable on a finite-state machine, as we will explain more in Chapter \ref{ch:compression}. Hence, in general, one should expect realizable encoding and decoding schemes are necessarily lossy. A central question is how to learn a compact (lossy) representation $\Z$ such that it can be used to predict $\X$ well.
...
In the past few years, many theoretical frameworks have been proposed and developed to help understand deep networks. But many fell short in providing scalable solutions that match the performance of empirical methods on real-world data and tasks at scale. Many theories fail to provide useful guidance on how to further improve the practice. Chapter \ref{ch:applications} will show how the framework presented in this book may help bridge the gap between theory and practice. We will provide convincing experimental evidence that networks and systems designed from first principles can achieve competitive and potentially better performance on a variety of tasks such as image classification, completion, segmentation, and text generation.


\paragraph{Back to intelligence.}
As we have mentioned in the very beginning, a fundamental task of any intelligent being is to learn predictable information from its  sensed data. Now we have understood a little bit about the computational nature of this task, one should realize this is a never-ending process, for the following reasons:
\begin{itemize}
...
Hence, intelligence is not about simply collecting in advance all data and training a model to memorize all  predictable information in the data. On the contrary, it is about being equipped with computational  mechanisms that can constantly improve its current information and acquire new one when needed. That is, a fundamental characteristic of any intelligent system\footnote{An animal, a human being, an intelligent robot, the scientific community, and even the entire civilization.} is to {\em be able to continuously gain information} (or knowledge). We may illustrate the relationship between intelligence and information (or knowledge) as following:
\begin{equation}
\mbox{Intelligence} = \frac{d\,}{dt} \mbox{Information}, \qquad 
\mbox{Information}  = \int_0^t \mbox{Intelligence}.
\end{equation}
We believe the closed-loop framework introduced above is a universal mechanism that enables self-improving and self-learning via closed-loop feedback and gaming. All intelligent beings in nature utilize closed-loop learning mechanisms at all levels. Its ubiquity had inspired early studies that tried to model and emulate animal intelligence by machines and computers,  particularly the Cybernetics movement led by Norbert Wiener in the 1940s.
