...
Hence, intelligence is not about simply collecting in advance all data and training a model to memorize all predictable information in the data. On the contrary, it is about being equipped with computational  mechanisms that can constantly improve its current information and acquire a new one when needed. That is, a fundamental characteristic of any intelligent system\footnote{An animal, a human being, an intelligent robot, the scientific community, and even the entire civilization.} is to {\em be able to continuously gain information} (or knowledge). We may illustrate the relationship between intelligence and information (or knowledge) as follows:
\begin{equation}
\mbox{Intelligence} = \frac{d\,}{dt} \mbox{Information}, \qquad 
\mbox{Information}  = \int_0^t \mbox{Intelligence}\ dt.
\end{equation}
