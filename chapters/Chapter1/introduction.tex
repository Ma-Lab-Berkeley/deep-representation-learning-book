...
In the 1950s, statisticians became interested in the problem of denoising data drawn from an arbitrary distribution. Let $\x_o$ be a random variable with probability density function $p_o(\cdot)$. So if we observe a noisy version of $\x_o$:
\begin{equation}
...
\end{equation}
As one can see from the formula, the function $\nabla \log p(\x)$ plays a very special role of denoising the observation $\x$ here. The noise $\boldsymbol{\epsilon}$ can be explicitly estimated as
\begin{equation}
    \hat{\boldsymbol{\epsilon}} = \x - \hat \x_o = \sigma^2 \nabla \log p(\x),
\end{equation}
for which we only need to know the distribution $p(\cdot)$ of $\x$ not the ground truth $p_o(\cdot)$ for $\x_o$. An important implication of this result is that if we add Gaussian noise to any distribution, the denoising process can be easily done if we can somehow get hold of the function $\nabla \log p(\x)$.
...
In the past few years, many theoretical frameworks have been proposed and developed to help understand deep networks. However, many were unable to provide scalable solutions that match the performance of empirical methods on real-world data and tasks. Many theories do not provide useful guidance on how to further improve practice. Chapter \ref{ch:applications} will show how the framework presented in this book may help bridge the gap between theory and practice. We will provide convincing experimental evidence that networks and systems designed from the first principles can achieve competitive and potentially better performance on a variety of tasks such as image classification, completion, segmentation, and text generation.
...
\begin{itemize}
    \item The current learned knowledge from the data, say the encoding and decoding schemes, can never really be optimal. There is always room to improve if there remain errors in predicting new observations. 
    \item The data observed so far do not yet cover all possible predictable information. There should be mechanisms to recognize that current knowledge is inadequate, and one must learn and add new information.
\end{itemize}

Hence, intelligence is not about simply collecting in advance all data and training a model to memorize all the predictable information in the data. In contrast, it is about being equipped with computational mechanisms that can constantly improve your current information and acquire a new one when needed. That is, a fundamental characteristic of any intelligent system\footnote{An animal, a human being, an intelligent robot, the scientific community, and even the entire civilization.} is {\em being able to continuously gain information} (or knowledge). We may illustrate the relationship between intelligence and information (or knowledge) as follows:
\begin{equation}
\mbox{Intelligence} = \frac{d\,}{dt} \mbox{Information}, \qquad 
\mbox{Information}  = \int_0^t \mbox{Intelligence}\ dt.
\end{equation}
We believe that the closed-loop framework introduced above is a universal mechanism that enables self-improving and self-learning via closed-loop feedback and gaming. All intelligent beings in nature utilize closed-loop learning mechanisms at all levels. Its ubiquity had inspired early studies that tried to model and emulate animal intelligence by machines and computers,  particularly the Cybernetics movement initiated by Norbert Wiener in the 1940s.
