...
\begin{equation}
\begin{aligned}
   \max_{f \in \mathcal{F}}\ \mathbb{E}_{\Z}\big[ \Delta R_{\epsilon}(\Z \mid \vU_{[K]}) - \lambda \|\bm{Z}\|_0 \big] \qquad \text{s.t.}\ \Z = f(\X),
   \label{eq:sparse-rr}
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
   \max_{f \in \mathcal{F}}\ \mathbb{E}_{\Z}\big[ \Delta R_{\epsilon}(\Z \mid \vU_{[K]}) - \lambda \|\bm{Z}\|_1 \big] \qquad \text{s.t.}\ \Z = f(\X),
...
Although easy to state, each term in the above objective is computationally challenging to optimize \cite{Wright-Ma-2022,chan2021redunet}. Hence it is natural to adopt an approximation approach that realizes the global transformation $f$ to optimize 
...
\end{align}
In this approximation, we can compute the similarity between projected token representations $\{\bm U_k^\top\bm z_i\}$ through an auto-correlation among the projected features as $(\vU_{k}^\top\vZ)^\top(\vU_{k}^\top\vZ)$ and convert it to a distribution of membership with a softmax, namely $\softmax{(\vU_{k}^\top\vZ)^\top(\vU_{k}^\top\vZ)}$. 
...
\end{equation}
% Next, we \textit{sparsify} the compressed tokens, with a suitably-relaxed proximal gradient step on the difference of the sparsity penalty and the expansion term to minimize the remaining term $\lambda \norm{\vZ}_{1} - R_\epsilon(\vZ)$. 
% \begin{align}
%     \bm Z^{l+1} = \mathrm{argmin}\ \frac{1}{2} \left\| \bm Z^{\ell+1/2} - \bm D^\ell\bm Z \right\|_F^2 + \lambda \|\bm Z_1\|.
% \end{align}
% These two operators are applied incrementally and repeatedly, as each $f^\ell$ in \eqref{eq:incremental} is instantiated with these two steps.  
...
\paragraph{MLP as Proximal Gradient Descent for Sparse Coding of Tokens.} For the second step of alternating minimization, we need to minimize the problem $\lambda \norm{\vZ}_{1} - R_{\epsilon}(\vZ)$. However, this problem is highly nonsmooth and nonconvex, which is difficult to optimize. To get around this problem, we posit a 


\paragraph{One Layer of the \textsc{CRATE} Model} We now design a white-box transformer architecture, named the Coding RATE Transformer (\textsc{crate}), by applying unrolled optimization to the sparse rate reduction objective \eqref{eq:sparse-rr}. Each layer of \textsc{crate} contains two blocks: the compression block and the sparsification block, which correspond to a two-step alternating optimization procedure for optimizing the sparse rate reduction objective \eqref{eq:sparse-rr}. 



% Specifically, the $\ell$-th layer of \textsc{crate} is defined as
% \begin{equation}
%     \vZ^{\ell+1} = f^{\ell}(\vZ^{\ell}) = {\texttt{ISTA}(\vZ^{\ell+1/2}\,|\,\vD^{\ell})},\, \mathrm{where}\ \vZ^{\ell + 1/2}
%     = \vZ^{\ell} + \MSSA(\vZ^{\ell}).
% \end{equation}

% \textbf{{Compression block MSSA.}} 
% The compression block in \textsc{crate}, called \textbf{M}ulti-head \textbf{S}ubspace \textbf{S}elf-\textbf{A}ttention block (\(\MSSA\)), is derived for compressing the token set $\vZ=[\vz_1, \dots, \vz_{N}]$ by optimizing the compression term $R_\epsilon^c$ (defined in~\eqref{eq:def-mcr-Rc}), i.e.,
