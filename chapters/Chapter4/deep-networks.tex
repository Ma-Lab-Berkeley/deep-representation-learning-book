...
&\doteq \underbrace{\frac{1}{2}\log\det \Big(\I + {\alpha} \Z \Z^{\top} \Big)}_{R_{\epsilon}(\Z)} \;-\; \underbrace{\sum_{j=1}^{k}\frac{\gamma_{j}}{2} \log\det\Big(\I + {\alpha_{j}} \Z \bm{\Pi}_{j} \Z^{\top} \Big)}_{R_{\epsilon}^c(\Z \mid\bm \Pi)},
\end{split}
\end{equation}
...
\frac{1}{2}\frac{\partial \left( \gamma_{j}  \log \det (\I + \alpha_{j} \Z \bm \Pi_j \Z^{\top} )  \right)}{\partial \bm Z}\bigg|_{\Z_\ell} \\= \gamma_{j}  \underbrace{ \alpha_{j}  (\I +  \alpha_{j} \Z^\ell \bm \Pi_j (\Z^\ell)^{\top})^{-1}}_{\bm C^{\ell}_j \; \in \Re^{d\times d}} \Z^{\ell} \bm \Pi_j.
\end{equation}
Notice that in the above, the matrix $\bm E^{\ell}$  only depends on $\Z^{\ell}$ and it aims to {\em expand} all the features to increase the overall coding rate; the matrix $\bm C^{\ell}_{j}$ depends on features from the $j$-class and aims to {\em compress} them to reduce the coding rate of each class. 
Then the complete gradient $\frac{\partial \Delta R}{\partial \bm Z}\big|_{\Z^\ell} \in \Re^{d\times N}$ is of the  form:
\begin{equation}
\frac{\partial \Delta R}{\partial \bm Z}\bigg|_{\Z^\ell}  = \underbrace{\bm E^{\ell}}_{\text{Expansion}} \Z^{\ell} \;-\; \sum_{j=1}^k \gamma_{j} \underbrace{\bm C_{j}^{\ell}}_{\text{Compression}}  \Z^{\ell} \bm{\Pi}_j.
