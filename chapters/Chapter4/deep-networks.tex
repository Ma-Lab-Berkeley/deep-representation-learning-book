...
\end{split}
\end{equation}
where $\epsilon > 0$ is a prescribed quantization error and for simplicity we denote $\alpha=\frac{d}{N\epsilon^2}$, $\alpha^{j}=\frac{d}{\textsf{tr}(\bm{\Pi}^{j})\epsilon^2}$, $\gamma^{j}=\frac{\textsf{tr}(\bm{\Pi}^{j})}{N}$ for $j = 1,\ldots, k$. 


The question really boils down to whether there is a {\em constructive} way of finding such a continuous mapping $f(\cdot)$ from $\bm x$ to $\bm z$? To this end, let us consider incrementally maximizing the objective $\Delta R(\Z )$ as a function of $\Z \subseteq \mathbb{S}^{d-1}$. Although there might be many optimization schemes to choose from, for simplicity we first consider the arguably simplest projected {\em gradient ascent} (PGA)  scheme:\footnote{Notice that we use superscript $j$ on $\Z^j$ to indicate features in the $j$th class and subscript $\ell$ on $\Z_\ell$ to indicate all features at $\ell$-th iteration or layer.} 
\begin{equation}
...
&= \; \z_\ell +  \eta \cdot g(\z_\ell, \bm \theta_\ell) \quad \mbox{s.t.} \quad \z_{\ell +1} \in \mathbb{S}^{d-1},
\end{aligned}
\end{equation}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Require{$\vX = [\vx_1, \ldots, \vx_N]\in \Re^{D \times N}$, $\bm{\Pi}$, $\epsilon > 0$, $\lambda$, and a learning rate $\eta$.}
        ...
        \Procedure{ReduNetTraining}{$\vX, \vPi, \epsilon, \lambda, \eta$}
            ...
            \State{\(\vZ_{1} = \mat{\vz_{1,1}, \dots, \vz_{1,N}} \gets \vX\)} \Comment{Initialize the ReduNet per-layer iteration}
            \For{\(\ell \in \{1, \dots, L\}\)}
                \Statex{\quad\ \texttt{\# Step 1: Compute network parameters} \(\vE_{\ell}, \{\vC_{\ell}^{j}\}_{j = 1}^{k}\)}
                \State{\(\vE_{\ell} \gets \alpha(\vI + \alpha \vZ_{\ell}\vZ_{\ell}^{\top})^{-1} \in \R^{d \times d}\)}
                \For{\(j \in \{1, \dots, k\}\)}
                    \State{\(\vC_{\ell}^{j} \gets \alpha^{j}(\vI + \alpha^{j}\vZ_{\ell}\vPi^{j}\vZ_{\ell}^{\top})^{-1} \in \R^{d \times d}\)}
                \EndFor
                \Statex{}
...
Consider a mixture of three Gaussian distributions in $\R^{3}$ that is projected onto $\mathbb{S}^2$. We first generate data points for 3 classes: for $k=1,2,3$, $\bm{X}^{k}=[\bm{x}_{k,1}, \ldots, \bm{x}_{k,N}] \in \R^{3\times N}$, $\bm{x}_{k,i} \sim \mathcal{N}(\bm{\mu}^{k}, \sigma_{k}^{2} \I)$, and $\bm{\pi}(\x_{k,i}) = k$.
...
\end{equation}
We refer the reader to \cite{Kra2012OnCM} for properties of circulant matrices. For simplicity, let $\bm Z_1 \doteq [ \z_{1,1}, \dots, \z_{1,N} ] = \X \in \Re^{d \times N}$\footnote{Again, to simplify discussion, we assume for now that the initial features $\Z_1$ are $\X$ themselves hence have the same dimension $d$, i.e., $D=d$. But that does not need to be the case as we will soon see that we need to lift $\X$ to a higher dimension.}. Then what happens if we construct the ReduNet from their circulant families $\circm(\bm Z_1) = \left[ \circm(\z_{1,1}), \dots, \circm(\z_{1,N}) \right] \in \Re^{d \times (dN)}$? That is, we want to compress and map all these into the same subspace by the ReduNet. 

Notice that now the data covariance matrix: 
\begin{eqnarray}
\circm(\bm Z_1) \circm(\bm Z_1)^* 
&=& \left[ \circm(\z_{1,1}), \dots, \circm(\z_{1,N}) \right] \left[ \circm(\z_{1,1}), \dots, \circm (\z_{1,N}) \right]^* \nonumber \\
&=& \sum_{i =1}^N \circm(\z_{1,i}) \circm(\z_{1,i})^\top \;\in \Re^{d\times d} 
\end{eqnarray}
...
where $\bm e_1 \in \Re^d$ is the first column vector of $\bm E_1$ and ``$\circledast$'' is circular convolution defined as
\begin{equation*}
...
That is, the incremental feature transform in \eqref{eqn:layer-approximate} applied to all shifted versions of a $\z_1 \in \Re^d$, given by
\begin{equation}
...
$$\circm(\bm Z_2) = \big[ \circm( \bm z_{1,1} + \eta g( \bm z_{1,1}, \bm \theta_1)), \dots, \circm( \bm z_{1,N} + \eta g(\bm z_{1,N}, \bm \theta_1)) \big].$$
...
\begin{equation}
\bar \z \doteq \bm \tau \left( \big[\circm(\bm k_1) \x, \ldots, \circm(\bm k_C) \x \big]^* \right) \quad \in \Re^{C \times d}.
...
The ReduNet constructed from circulant version of these multi-channel features $\bar\Z \doteq [\bar \z^1, \ldots, \bar \z^N] \in \Re^{C \times d \times N}$, i.e., $\circm(\bar\Z) \doteq [ \circm(\bar\z^1), \dots, \circm(\bar\z^N)] \in \Re^{dC \times dN}$, retains the good invariance properties described above: the linear operators, now denoted as $\bar{\bm E}$ and $\bar{\bm C}^j$, remain block circulant, and represent {\em multi-channel 1D circular convolutions. }
...
\begin{equation*}
    ...
    \end{smallmatrix}\right] \in \Re^{dC \times dC},
\end{equation*}
...
\begin{figure}
     \centering
     \includegraphics[width=0.65\textwidth]{figs_chap4/crate_encoder_architecture.pdf}
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{\small {\bf The roles of forward pass and backward propagation in deep networks}. (a) Given fixed subspaces and dictionaries $\{(\bm U_{[K]}^{\ell}, \bm D^{\ell})\}_{l=1}^L$, each layer performs compression and sparsification on representations in the forward pass; (b) Backpropagation learn subspaces and dictionaries $\{(\bm U_{[K]}^{\ell}, \bm D^{\ell})\}_{l=1}^L$ from training data. }
...
Recall that  \(\X = [\x_{1}, \dots, \x_{N}] \in \R^{D \times N}\) is typically a matrix-valued variable, and the transformer seeks to find a \textit{representation mapping}, or a mapping of the tokens $\phi\colon \R^{D \times N} \to \R^{d \times N}$ such that the features $\Z = \phi(\X) = [\z_1, \ldots, \z_N] \in \R^{d \times N}$ are appropriate for the task at hand. In MCR$^2$, the tokens are assumed to belong to an underlying set of \(K\) groups, which may (for example) represent different modes within the population of tokens, and the MCR$^2$ objective seeks to find token features of each group which are \textit{compressed} within a group while the set of all token features is as expansive as possible.  In particular, let $\bm \Pi = [\bm \pi_1, \ldots, \bm \pi_K] \in \R^{N \times K}$ be a stochastic ``group assignment'' matrix (i.e., $\bm \Pi \bm 1 = \bm 1$ and $\Pi_{ik} \geq 0, \  \forall (i,k) \in [N] \times [K]$ ) that assigns a group membership probability to each token (i.e., the $i^\text{th}$ row of $\bm\Pi$ is a probability vector for group membership for the $i^\text{th}$ token).  Then, letting $\epsilon > 0$ and $N_k \doteq \langle \bm\pi_k, \bm 1 \rangle$ for each \(k \in [K]\), the MCR$^2$ objective, denoted as $\Delta R(\bm Z,\bm \Pi)$, is:
\begin{equation}
    \label{eq:mcr2} 
...
\begin{equation}
    \label{eq:tssa}
   \texttt{TSSA}(\Z \mid \{\bm U_{k}\}_{k = 1}^{K}) \doteq -\frac{\tau}{n}\sum_{k = 1}^{K}\bm U_{k}\bm D(\Z, \bm\pi_{k} \mid \bm U_{k})\bm U_{k}^{\top}\Z\diag(\bm \pi_{k}),
\end{equation}
where \(\bm\pi_{k}\) are the columns of \(\bm\Pi = \bm\Pi(\Z \mid \{\bm U_{k}\}_{k = 1}^{K})\), defined in \eqref{eq:pi}, and \(\bm D\) is defined in \eqref{eq:D_def}. 
