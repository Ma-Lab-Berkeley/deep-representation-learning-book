...
\begin{align*}
    \mathrm{max}\ \Delta R_{\epsilon}(\Z \mid \vU_{[K]}) = R_{\epsilon}(\Z) - R^c_{\epsilon}(\Z \mid \vU_{[K]}) \quad \mathrm{s.t.}\ \Z = f(\bm X). 
\end{align*}
...
\begin{remark}
    The form \eqref{eq:coding_rate} of the coding rate can be viewed as a generalization to the coding rate \(R_\epsilon^{c}\) in the original rate reduction objective \eqref{eq:MCRc}. In particular, the original objective is defined with respect to a set of known membership labels \(\{\vPi_{k}\}\) specific to the particular data realization \(\vX\). In contrast, the objective here is defined with respect to subspaces \(\vU_{[K]}\) which are defined externally to any specific data realization, though they support the token feature distribution. Suppose that a token representation $\bm z_i$ belongs to a subspace $\bm U_k$ and these subspaces are orthogonal to each other, i.e., $\bm U_k^\top \bm U_l = \bm 0$ for all $k \neq l$. Then, one can verify that the projections $\bm U_k\bm U_k^\top \bm z_i = \bm z_i$ and  $\bm U_l\bm U_l^\top \bm z_i = \bm 0$ for all $l \neq k$. These orthogonal projections effectively serve as implicit membership labels, identifying the subspace to which each token representation belongs. 
\end{remark}
