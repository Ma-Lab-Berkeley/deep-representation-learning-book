...
\begin{split}
\Delta R_{\epsilon}(\Z \mid \bm{\Pi}) &= R_\epsilon(\Z) - R_{\epsilon}^c(\Z \mid  \bm{\Pi})\\
&\doteq \underbrace{\frac{1}{2}\log\det \Big(\I + {\alpha} \Z \Z^{\top} \Big)}_{R_{\epsilon}(\Z)} \;-\; \underbrace{\sum_{j=1}^{k}\frac{\gamma^{j}}{2} \log\det\Big(\I + {\alpha^{j}} \Z \bm{\Pi}^{j} \Z^{\top} \Big)}_{R_{\epsilon}^c(\Z \mid\bm \Pi)},
\end{split}
\end{equation}
