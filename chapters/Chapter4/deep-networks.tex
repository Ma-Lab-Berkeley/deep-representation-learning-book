...
\begin{align*}
    \alpha=\frac{d}{N\epsilon^2},\ \alpha_{j}=\frac{d}{\mathrm{tr}(\bm{\Pi}_{j})\epsilon^2},\ \gamma_{j}=\frac{\mathrm{tr}(\bm{\Pi}_{j})}{N},\ \text{for}\ j = 1,\ldots, k.
\end{align*}
...
\frac{1}{2}\frac{\partial \left( \gamma_{j}  \log \det (\I + \alpha_{j} \Z \bm \Pi_j \Z^{\top} )  \right)}{\partial \bm Z}\bigg|_{\Z^\ell} \\= \gamma_{j}  \underbrace{ \alpha_{j}  (\I +  \alpha_{j} \Z^\ell \bm \Pi_j (\Z^\ell)^{\top})^{-1}}_{\bm C^{\ell}_j \; \in \Re^{d\times d}} \Z^{\ell} \bm \Pi_j.
\end{equation}
...
\begin{equation}
    g(\z^\ell, \bm \theta^\ell) \; \doteq \; \bm E^\ell \z^\ell - \sum_{j=1}^k \gamma_{j} \pi_j(\z^\ell)\bm C_{j}^{\ell}  \z^{\ell}  \in \Re^d,
    \label{eqn:DR-gradient-transform}
\end{equation}
where  $\pi_j(\z^\ell) \in [0,1]$ indicates the probability of $\z^{\ell}$ belonging to the $j$-th class. The increment map parameters $\bm \theta^\ell$ depend on: First, a set of linear maps represented by $\bm E^{\ell}$ and $\{ \bm C^{\ell}_{j}\}_{j=1}^{k}$ that depend only on statistics of features of the training $\Z^\ell$; Second, the membership $\{ \pi_j(\z^\ell)\}_{j=1}^k$ of any feature $\z^\ell$. 
Notice that on the training samples $\Z^\ell$, for which the memberships $\bm \Pi_j$ are known,  the so defined $g(\z^\ell, \bm \theta)$ gives exactly the values for the gradient $\frac{\partial \Delta R}{\partial \bm Z}\big|_{\Z^\ell}$.


Since we only have the membership $\bm \pi_j$ for the training samples, the function $g(\cdot)$ defined in \eqref{eqn:DR-gradient-transform} can only be evaluated on the training. To extrapolate $g(\cdot)$ to the entire feature space, we need to estimate $\pi_j(\z^\ell)$ in its second term. In conventional deep learning, this map is typically modeled as a deep network and learned from the training data, say via {\em back propagation}. Nevertheless, our goal here is not to learn a precise classifier $\pi_{j}(\z^\ell)$ already. Instead, we only need a good enough estimate of the class information in order for $g(\cdot)$ to approximate the gradient $\frac{\partial \Delta R}{\partial \bm Z}$ well. 



From the geometric interpretation of the linear maps $\bm E^\ell$ and $\bm C_j^\ell$ given by Remark \ref{rem:regression-interpretation}, the term $\bm p_{j}^{\ell} \doteq \bm C^{\ell}_j \z^{\ell}$ can be viewed as (approximately) the projection of $\z^{\ell}$ onto the orthogonal complement of each class $j$. Therefore, $\|\bm p_{j}^{\ell}\|_2$ is small if $\z^\ell$ is in class $j$ and large otherwise. This motivates us to estimate its membership based on the following softmax function:
\begin{equation}
    \widehat{\vpi}(\z^\ell) \doteq \softmax\rp{-\lambda\mat{\norm{\vC^{\ell}_{1}\vz^{\ell}}_{2} \\ \vdots \\ \norm{\vC^{\ell}_{k}\vz^{\ell}}_{2}}} = \frac{1}{\sum_{j = 1}^{k}\exp(-\lambda \norm{\vC^{\ell}_{j}\vz^{\ell}}_{2})}\mat{\exp(-\lambda\norm{\vC^{\ell}_{1}\vz^{\ell}}_{2}) \\ \vdots \\ \exp(-\lambda\norm{\vC^{\ell}_{k}\vz^{\ell}}_{2})} \in [0,1]^{k}.
\end{equation}
Hence, the second term of \eqref{eqn:DR-gradient-transform} can be approximated by this estimated membership:
\begin{align}
\sum_{j=1}^k \gamma_{j} \pi_j(\z^\ell)\bm C_{j}^{\ell}  \z^{\ell} 
\; \approx \;  \sum_{j=1}^k \gamma_{j} \widehat{\pi}_j(\z^\ell)  \bm{C}^{\ell}_j  \z^{\ell} 
...
\end{aligned}
\end{equation}
with the nonlinear function $\bm \sigma(\cdot)$ defined above and $\bm \theta^\ell$ collecting all the layer-wise parameters. That is  $\bm \theta^\ell =\left\{\bm E^\ell, \bm{C}^{\ell}_{1}, \dots, \bm{C}^{\ell}_{k}, \gamma_{j}, \lambda\right\}$. Note features at each layer are always ``normalized'' by projecting onto the unit sphere $\mathbb S^{d-1}$, denoted as $\mathcal P_{\mathbb S^{d-1}}$. The form of increment in \eqref{eqn:layer-approximate} can be illustrated by a diagram in  Figure~\ref{fig:arch-ReduNet}(a).
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        \Require{$\vX = [\vx_1, \ldots, \vx_N]\in \Re^{D \times N}$, $\bm{\Pi} = \{\bm \Pi_j\}_{j=1}^k$, $\epsilon > 0$, $\lambda$, and a learning rate $\eta$.}
        \Ensure{The learned parameters \(\{\vE^{\ell}\}_{\ell = 1}^{L}, \{\{\vC^{\ell}_{j}\}_{j = 1}^{k}\}_{\ell = 1}^{L}, \{\gamma_{j}\}_{j = 1}^{k}\)}
        ...
        \Procedure{ReduNetTraining}{$\vX, \vPi, \epsilon, \lambda, \eta$}
            ...
            \For{\(j \in \{1, \dots, k\}\)}
            \State{\(\alpha_{j} \gets D/(\tr(\vPi_{j})\epsilon^{2})\)}
            \State{\(\gamma_{j} \gets \tr(\vPi_{j})/D\)}
            \EndFor
            \Statex{}
            ...
            \For{\(\ell \in \{1, \dots, L\}\)}
                \Statex{\quad\ \texttt{\# Step 1: Compute network parameters} \(\vE^{\ell}, \{\vC^{\ell}_{j}\}_{j = 1}^{k}\)}
                \State{\(\vE^{\ell} \gets \alpha\left(\vI + \alpha \vZ^{\ell}(\vZ^{\ell})^{\top}\right)^{-1} \in \R^{d \times d}\)}
                \For{\(j \in \{1, \dots, k\}\)}
                    \State{\(\vC^{\ell}_{j} \gets \alpha_{j}\left(\vI + \alpha_{j}\vZ^{\ell}\vPi_{j}(\vZ^{\ell})^{\top}\right)^{-1} \in \R^{d \times d}\)}
                \EndFor
                \Statex{}
                ...
                \For{\(i \in \{1, \dots, N\}\)}
                    ...
                    \State{\(\displaystyle\vz^{\ell + 1}_i \gets \cP_{\bS^{d - 1}}\rp{\vz^{\ell}_i + \eta \bp{\vE^{\ell}\vz^{\ell}_i - \sum_{j = 1}^{k}\gamma_{j}\hat{\pi}_{j}(\vz^{\ell}_i) \vC^{\ell}_{j}\vz^{\ell}_i}} \in \R^{d}\)} \Comment{Update features \(\vz^{\ell + 1}_i\) from \(\vz^{\ell}_i\)}
                \EndFor
            \EndFor
            \State{\Return{\(\{\vE^{\ell}\}_{\ell = 1}^{L}, \{\{\vC^{\ell}_{j}\}_{j = 1}^{k}\}_{\ell = 1}^{L}, \{\gamma_{j}\}_{j = 1}^{k}\)}} \Comment{Return all network parameters.}
        \EndProcedure
    \end{algorithmic}
\end{algorithm}


\paragraph{Deep Network for Optimizing Rate Reduction.} Notice that the increment is constructed to emulate the gradient ascent for the rate reduction $\Delta R$. Hence by transforming the features iteratively via the above process, we expect the rate reduction to increase, as we will see in the experimental section. This iterative process, once converged say after $L$ iterations, gives the desired feature map $f(\x, \bm \theta)$ on the input $\x = \z_1 $, precisely in the form of a {\em deep network}, in which each layer has the structure shown in Figure~\ref{fig:arch-ReduNet} left:
\begin{equation}\label{eqn:ReduNet}
\begin{aligned}
f(\x, \bm \theta)\; =&  \;\;f^L \circ f^{L-1} \circ  \cdots \circ f^2 \circ
    f^1(\z^1),  \\ 
...
\begin{algorithm}[!htbp]
    ...
    \begin{algorithmic}[1]
        \Require{Input \(\vx \in \R^{D}\), network parameters \(\{\vE^{\ell}\}_{\ell = 1}^{L}, \{\{\vC^{\ell}_{j}\}_{j = 1}^{k}\}_{\ell = 1}^{L}, \{\gamma_{j}\}_{j = 1}^{k}\), learning rate \(\lambda\)}
        ...
        \State{\(\vz^{\ell + 1} \gets \cP_{\bS^{d - 1}}\rp{\vz^{\ell} + \eta \bp{\vE^{\ell}\vz^{\ell} - \sum_{j = 1}^{k}\gamma_{j}\hat{\pi}_{j}(\vz^{\ell}) \vC^{\ell}_{j}\vz^{\ell}}} \in \R^{d}\)} \Comment{Update feature \(\vz^{\ell + 1}\) using \(\vz^{\ell}\)}
        \EndFor
...
To construct the network (computing $\bm{E}^{\ell}, \bm{C}^{\ell}_{j}$ for the $\ell$-th layer), we set the number of iterations/layers $L=2,000$, step size $\eta=0.5$, and precision $\epsilon=0.1$. 
...
\end{equation*}
Similarly, the matrices $\bm C^1_j$ associated with any subsets of $\bm Z^1$ are also circular convolutions. 
\label{prop:circular-conv-1}
\end{proposition}

Not only do the first-layer parameters $\bm E^1$ and $\bm{C}^1_j$ of the ReduNet become circulant convolutions but also the next-layer features remain circulant matrices. 
...
Continuing inductively, we see that all matrices $\bm E^\ell$ and $\bm C^\ell_j$ based on such $\circm(\bm Z^\ell)$ are circulant, and so are all features. 
