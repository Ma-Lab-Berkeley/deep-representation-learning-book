...
&\doteq \underbrace{\frac{1}{2}\log\det \Big(\I + {\alpha} \Z \Z^{\top} \Big)}_{R_{\epsilon}(\Z)} \;-\; \underbrace{\sum_{k=1}^{K}\frac{\gamma_{k}}{2} \log\det\Big(\I + {\alpha_{k}} \Z \bm{\Pi}_{k} \Z^{\top} \Big)}_{R_{\epsilon}^c(\Z \mid \bm \Pi)},
\end{split}
\end{equation}
...
Simple calculation shows that the gradient ${\partial \Delta R_{\epsilon}}/{\partial \bm Z}$ entails evaluating the following derivatives of the two terms in $\Delta R_{\epsilon}(\Z )$:
...
\end{equation}
where \(\bm\pi_{k}\) are the columns of \(\bm\Pi = \bm\Pi(\Z \mid \{\bm U_{k}\}_{k = 1}^{K})\) defined in \eqref{eq:pi} and \(\bm D\) is defined in \eqref{eq2:grad Rcf}. 
...
% With the advent of deep learning, empirically designed deep networks have demonstrated remarkable success in capturing complex nonlinear structures. Architectures such as CNNs, RNNs, LSTMs, ResNets, and Transformers have been widely adopted across domains. Yet, the design of these models has been largely empirical, driven by trial-and-error, ablation studies, and large-scale hyperparameter searches, with little theoretical understanding of their inner workings. 
