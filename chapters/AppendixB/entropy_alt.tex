...
\begin{equation}\label{eq:pt_representation}
...
\begin{proof}
    ...
    Namely, from \Cref{lem:diffusion_rv_heat} we have that \(p_{t}\) satisfies the heat-like PDE
    \begin{equation}
...
\subsubsection{Auxiliary Results for \Cref{thm:diffusion_entropy_increases}}
...
\begin{proof}
    ...
    To prove \Cref{lem:diffusion_entropy_exists}.2, suppose that \Cref{assumption:entropy_x_compact_support} holds. We want to show that \(h(\vx) < \infty\) and \(h(\vx_{t}) < \infty\). The mechanism for doing this is the same, and involves the maximum entropy result \Cref{thmx:max_entropy}. Namely, since \(\vx\) is absolutely bounded, it has a finite covariance which we will denote \(\vSigma\). Then the covariance of \(\vx_{t}\) is \(\vSigma + t^{2}\vI\). Thus the entropy of \(\vx\) and \(\vx_{t}\) can be upper bounded by the entropy of normal distributions with the respective covariances, which is indeed \(< \infty\).
\end{proof}

Next, we rigorously show that the density \(p_{t}\) follows a heat-like PDE.

\begin{lemma}\label{lem:diffusion_rv_heat}
    Let \(\vx\) be any random variable such that \Cref{assumption:entropy_x_compact_support,assumption:entropy_x_density} hold, and let \((\vx_{t})_{t \in [0, T]}\) be the stochastic process \eqref{eq:app_additive_gaussian_noise_model}. For \(t \in (0, T)\), the density \(p_{t}\) of \(\vx_{t}\) obeys the following heat-like PDE:
    \begin{equation}\label{eq:fokker_planck_ve}
        \pdv{p_{t}}{t}(\vxi) = t\Delta p_{t}(\vxi).
    \end{equation}
\end{lemma}
\begin{proof}
    Let us start with the characterization of \(p_{t}\) yielded by \eqref{eq:pt_representation}, i.e.,
    \begin{equation}
        p_{t}(\vxi) = \Ex[\phi_{t}(\vxi - \vx)] = \int_{\R^{D}}\phi_{t}(\vxi - \vu)p(\vu)\odif{\vu}.
    \end{equation}
    First taking the time derivative, a computation yields that \Cref{prop:dutis} applies,\footnote{We use \(f_{t}(\vxi) = p(\vxi) \phi_{t}(\vxi - \vx)\), noting that it is twice continuously differentiable in \(\vxi\) and (more than) twice continuously differentiable in \(t\). Then to check the local integrability of \(f_{t}\) we compute \(\pdv{f_{t}}{t}(\vxi) = f_{t}(\vxi)\cdot \frac{1}{t^{3}}(\norm{\vxi - \vx}_{2}^{2} - Dt^{2})\), which is is easy to check integrable over \(\vxi\) and \(t \in [t_{\min}, t_{\max}]\) where \(t_{\min} > 0\). (Indeed one can connect this integral to the trace of the second moment of \(\vx_{t}\), which is finite since \(\vx_{t}\) has covariance \(\Cov(\vx) + t^{1}\vI\), and \(\Cov(\vx)\) is finite since \(\vx\) is bounded.)} so we can bring the derivative inside the integral/expectation as:
    \begin{equation}
...
Finally, we fill in the integration-by-parts argument alluded to in the proof of \Cref{thm:diffusion_entropy_increases}. The argument is conceptually pretty simple but requires some technical estimates to show that the boundary term in the integration-by-parts vanishes.

\begin{lemma}\label{lem:diffusion_ibp}
    Let \(\vx\) be any random variable such that \Cref{assumption:entropy_x_compact_support,assumption:entropy_x_density} hold, and let \((\vx_{t})_{t \in [0, T]}\) be the stochastic process \eqref{eq:app_additive_gaussian_noise_model}. For \(t \geq 0\), let \(p_{t}\) be the density of \(\vx_{t}\). Then it holds 
    \begin{equation}
        \int_{\R^{D}}\Delta p_{t}(\vxi)[1 + \log p_{t}(\vxi)]\odif{\vxi} = -\int_{\R^{D}}\ip{\nabla \log p_{t}(\vxi)}{\nabla p_{t}(\vxi)}\odif{\vxi}.
    \end{equation}
\end{lemma}
\begin{proof}
    ...
    \end{itemize}
    Green's theorem says that for any compact set \(\cK \subseteq \R^{D}\), twice continuously differentiable \(\varphi \colon \R^{D} \to \R\), and continuously differentiable \(\psi \colon \R^{D} \to \R\),
    \begin{equation}
        \int_{\cK}\bc{\psi(\vxi) \Delta \varphi(\vxi) + \ip{\nabla \psi(\vxi)}{\nabla \varphi(\vxi)}}\odif{\vxi} = \int_{\partial \cK}\psi(\vxi)\ip{\nabla \varphi(\vxi)}{\vn(\vxi)}\odif{\sigma(\vxi)}
    \end{equation}
    where \(\odif{\sigma(\vxi)}\) denotes an integral over the ``surface measure'', i.e., the inherited measure on \(\partial \cK\), namely the boundary of \(\cK\), and accordingly \(\vxi\) takes values on this surface and \(\vn(\vxi)\) is the unit normal vector to \(\cK\) at the surface point \(\vxi\). Now, taking \(\varphi(\vxi) := p_{t}(\vxi)\) and \(\psi(\vxi) := 1 + \log p_{t}(\vxi)\), over a ball \(B_{r}(\vzero)\) of radius \(r > 0\) centered at \(\vzero\) (so that \(\partial B_{r}(\vzero)\) is the sphere of radius \(r\) centered at \(\vzero\) and \(\vn(\vxi) = \vxi/\norm{\vxi}_{2} = \vxi/r\)):
    \begin{align}
        &\int_{B_{r}(\vzero)}\bc{\Delta p_{t}(\vxi)[1 + \log p_{t}(\vxi)] + \ip{\nabla \log p_{t}(\vxi)}{\nabla p_{t}(\vxi)}}\odif{\vxi} \\
        &= \int_{\partial B_{r}(\vzero)}[1 + \log p_{t}(\vxi)]\ip*{\nabla p_{t}(\vxi)}{\frac{\vxi}{r}}\odif{\sigma(\vxi)} \\
        &= \frac{1}{r}\int_{\partial B_{r}(\vzero)}[1 + \log p_{t}(\vxi)]\ip*{\nabla p_{t}(\vxi)}{\vxi}\odif{\sigma(\vxi)}.
    \end{align}
    Sending \(r \to \infty\), it holds that 
    \begin{align}
        &\int_{\R^{D}}\bc{\Delta p_{t}(\vxi)[1 + \log p_{t}(\vxi)] + \ip{\nabla \log p_{t}(\vxi)}{\nabla p_{t}(\vxi)}}\odif{\vxi} \label{eq:app_diffusion_r_infinity_limit} \\
        &= \lim_{r \to \infty}\int_{\partial B_{r}(\vzero)}\bc{\Delta p_{t}(\vxi)[1 + \log p_{t}(\vxi)] + \ip{\nabla \log p_{t}(\vxi)}{\nabla p_{t}(\vxi)}}\odif{\vxi} \\ 
        &= \lim_{r \to \infty}\frac{1}{r}\int_{\partial B_{r}(\vzero)}[1 + \log p_{t}(\vxi)]\ip*{\nabla p_{t}(\vxi)}{\vxi}\odif{\sigma(\vxi)},
    \end{align}
    ...
    \end{equation}
    Now to compute the gradient, we argued before using \Cref{prop:dutis} that we can take the derivative inside the expectation, obtaining
    \begin{align}
        \nabla p_{t}(\vxi)
        &= \nabla \Ex[\phi_{t}(\vxi - \vx)] \\ 
        &= \Ex[\nabla \phi_{t}(\vxi - \vx)] \\
        &= -\frac{1}{t^{2}}\Ex\rs{\bp{\vxi - \vx}\phi_{t}(\vxi - \vx)}.
    \end{align}
    Now to evaluate the inner product \(\ip{\nabla p_{t}(\vxi)}{\vxi}\), we use linearity of expectation to find
    \begin{align}
        \ip{\nabla p_{t}(\vxi)}{\vxi}
    ...
    \begin{align}
        [1 + \log p_{t}(\vxi)]\ip{\nabla p_{t}(\vxi)}{\vxi}
        &\leq -\bs{1 + \log C_{t} - \frac{r^{2} + 2R(r + R)}{2t^{2}}} \cdot \frac{r(r - R)}{t^{2}}C_{t} e^{-[r^{2} + 2R(r + R)]/(2t^{2})} \\ 
        [1 + \log p_{t}(\vxi)]\ip{\nabla p_{t}(\vxi)}{\vxi}
        &\geq -\bs{1 + \log C_{t} + \frac{[-r^{2} + 2Rr]}{2t^{2}}}\cdot\frac{r(R + r)}{t^{2}} C_{t}e^{[-r^{2} + 2Rr]/(2t^{2})}.
    \end{align}
    So one can see that, letting the surface area of \(\partial B_{r}(\vzero)\) be \(\omega r^{D - 1}\) where \(\omega\) is a constant, it holds
    \begin{align}
        &\frac{1}{r}\int_{\partial B_{r}(\vzero)}[1 + \log p_{t}(\vxi)]\ip{\nabla p_{t}(\vxi)}{\vxi}\odif{\vxi} \\
        &\leq -\omega r^{D - 2}\bs{1 + \log C_{t} - \frac{r^{2} + 2R(r + R)}{2t^{2}}} \cdot \frac{r(r - R)}{t^{2}}C_{t} e^{-[r^{2} + 2R(r + R)]/(2t^{2})} \\
        &\frac{1}{r}\int_{\partial B_{r}(\vzero)}[1 + \log p_{t}(\vxi)]\ip{\nabla p_{t}(\vxi)}{\vxi}\odif{\vxi} \\
        &\geq -\omega r^{D - 2}\bs{1 + \log C_{t} + \frac{[-r^{2} + 2Rr]}{2t^{2}}}\cdot\frac{r(R + r)}{t^{2}} C_{t}e^{[-r^{2} + 2Rr]/(2t^{2})},
    \end{align}
    and, because both are overall exponentially decaying in \(r\), we can write 
    \begin{equation}
...
\subsection{Auxiliary Results for \Cref{thm:conditioning_reduces_entropy}}

Here we provide some results which are appropriate generalizations of corresponding results in \cite{Gribonval2011-pf}.
...
\begin{proof}
    ...
    \begin{equation}
        p_{t}(\vxi) = \int_{\R^{D}}\phi_{t}(\vxi - \vu)p(\vu)\odif{\vu},
    \end{equation}
    (and similarly for \(\nabla p_{t}(\vxi)\) and \(\nabla^{2}p_{t}(\vxi)\)) and do a calculation for \(\vv \neq \vzero\):
    \begin{align}
        &\vv^{\top}\bs{\bar{\vx}^{\prime}(\vxi)}\vv \\ 
        &= p_{t}(\vxi)^{-2}\vv^{\top}\Bigg[p_{t}(\vxi)^{2}\vI + \bp{1 - \frac{s}{t}}t^{2}\int_{\R^{D}}\int_{\R^{D}}\phi_{t}(\vxi - \vu)\phi_{t}(\vxi - \vw)\bs{\frac{\vw\vw^{\top}}{t^{4}} - \frac{\vI}{t^{2}}}p(\vu)p(\vw)\odif{\vu}\odif{\vw} \\ 
        &\qquad \qquad \qquad - \bp{1 - \frac{s}{t}}t^{2}\int_{\R^{D}}\int_{\R^{D}}\phi_{t}(\vxi - \vu)\phi_{t}(\vxi - \vw)\bs{-\frac{\vu}{t^{2}}}\bs{-\frac{\vw}{t^{2}}}^{\top}p(\vu)p(\vw)\odif{\vu}\odif{\vw}\Bigg]\vv \\ 
        &= p_{t}(\vxi)^{-2}\vv^{\top}\Bigg[\bp{1 - \frac{s}{t}}t^{2}\int_{\R^{D}}\int_{\R^{D}}\phi_{t}(\vxi - \vu)\phi_{t}(\vxi - \vw)\bs{\frac{\bp{1 - \frac{s}{t}}^{-1}\vI}{t^{2}}}p(\vu)p(\vw)\odif{\vu}\odif{\vw} \\
        &\qquad \qquad \qquad + \bp{1 - \frac{s}{t}}t^{2}\int_{\R^{D}}\int_{\R^{D}}\phi_{t}(\vxi - \vu)\phi_{t}(\vxi - \vw)\bs{\frac{\vu\vu^{\top}}{2t^{4}} + \frac{\vw\vw^{\top}}{2t^{4}} - \frac{\vI}{t^{2}}}p(\vu)p(\vw)\odif{\vu}\odif{\vw} \\ 
        &\qquad \qquad \qquad - \bp{1 - \frac{s}{t}}t^{2}\int_{\R^{D}}\int_{\R^{D}}\phi_{t}(\vxi - \vu)\phi_{t}(\vxi - \vw)\bs{\frac{\vu\vw^{\top}}{t^{4}}}p(\vu)p(\vw)\odif{\vu}\odif{\vw}\Bigg]\vv \\ 
        &= p_{t}(\vxi)^{-2}\bp{1 - \frac{s}{t}}t^{2}\int_{\R^{D}}\int_{\R^{D}}\phi_{t}(\vxi - \vu)\phi_{t}(\vxi - \vw)\bs{\vv^{\top}\vA(\vu, \vw)\vv}p(\vu)p(\vw)\odif{\vu}\odif{\vw}
    \end{align}
    where 
    \begin{align}
        &\vA(\vu, \vw) = \frac{1}{t^{2}}\bp{\bp{1 - \frac{s}{t}}^{-1} - 1}\vI + \frac{1}{2t^{4}}(\vu - \zeta)(\vu - \vw)^{\top} \\ 
        \implies 
        &\vv^{\top}\vA(\vu, \vw)\vv = \frac{1}{t^{2}}\bp{\bp{1 - \frac{s}{t}}^{-1} - 1}\norm{\vv}_{2}^{2} + \frac{1}{2t^{4}}[(\vu - \vw)^{\top}\vv]^{2}.
    \end{align}
    This term is zero precisely when \(s = 0\) \textit{and} \(\vu - \vw\) is orthogonal to \(\vv\). Then, the whole integral (i.e., the original quadratic form) is \(0\) only when \(s = 0\) and \(\vx\) is supported on a subspace which is orthogonal to \(\vv\). But this is ruled out by assumption (i.e., that \(\vx\) has a density), so the Jacobian \(\bar{\vx}^{\prime}(\vxi)\) is symmetric positive definite.
\end{proof}
...
\begin{proof}
    ...
    \end{equation}
    Now we can differentiate under the integral sign to obtain
    \begin{align}
        \nabla p_{t}(\vxi)
        &= \nabla \Ex[\phi_{t}(\vxi - \vx)] \\
        &= \Ex[\nabla \phi_{t}(\vxi - \vx)] \\
        &= \Ex\rs{-\frac{\vxi - \vx}{t^{2}}\cdot\phi_{t}(\vxi - \vx)} \\
        \Delta p_{t}(\vxi)
        &= \Delta \Ex[\phi_{t}(\vxi - \vx)] \\
        &= \Ex[\Delta \phi_{t}(\vxi - \vx)] \\
        &= \Ex\rs{\frac{\norm{\vxi - \vx}_{2}^{2} - Dt^{2}}{t^{4}} \cdot \phi_{t}(\vxi - \vx)}.
    \end{align}
    Therefore writing the terms in \(\Delta p_{t}(\vxi)\), we obtain
    \begin{align}
