...
\begin{proof}
    ...
    \end{align}
    This term is zero precisely when \(s = 0\) \textit{and} \(\vx - \vy\) is orthogonal to \(\vv\). Then, since \(\vx\) and \(\vy\) are i.i.d., the whole integral (i.e., the original quadratic form) is \(0\) only when \(s = 0\) and \(\vx\) has support entirely contained in a subspace which is orthogonal to \(\vv\). But this is ruled out by assumption (i.e., that \(\vx\) has a density), so the Jacobian \(\bar{\vx}^{\prime}(\vxi)\) is symmetric positive definite.
\end{proof}
