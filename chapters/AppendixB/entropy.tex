...
\begin{equation*}
  p_t(\vx) = \int \varphi_t(\vx - \vx') \odif \mu(\vx').
\end{equation*}
...
functions of their instantaneous \textit{spatial derivatives} (in $\vx$), and it
...
\textit{entropy increases as $t$ increases}: calculate
...
First consider the case where the differential entropy $h(\vz_o)$ is finite.
Then we can write with the fundamental theorem of calculus
\begin{align*}
  h(\vz_{\sigma^2}) &= h(\vz_o) + \int_0^{\sigma^2} \partial_t h(\vz_t) \odif t,
  \\
  &\geq h(\vz_o),
\end{align*}
by \Cref{eq:de-bruijn}, as desired. \sdb{Make it strict}

As an extension, we consider also the case of certain degenerate distributions
$\vz_o$, where $h(\vz_o)$ may not be well-defined. \sdb{Expand on this}
