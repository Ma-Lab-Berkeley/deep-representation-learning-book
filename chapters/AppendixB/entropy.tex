...
\begin{remark}
  ...
  Indeed, this works because for any random variable $\vz_o$, Gaussian smoothing 
  produces a random variable with finite entropy.
  To see this, first note that by Tonelli's theorem,
  \begin{equation*}
    \bE[\norm{\vz_t}_2^2] 
  ...
  where $h(\vz_o) = -\infty$.
  This offers a scheme to rigorously establish that low-dimensional
  distributions have entropy $-\infty$ in a precise sense: their entropy can
  moreover be identified with the sequence of entropies of their noisy
  approximations as the level of noise goes to zero.
...
\begin{proof}[Proof of {\Cref{thm:conditioning_reduces_entropy}}.]
  ...
  \begin{align*}
    ...
    \\
    &=
    -\frac{Mt}{d} \int p_t(\vz) \norm*{\nabla \log p_t(\vz)}_2^2\odif{\vz} \leq 0,
  \end{align*}
