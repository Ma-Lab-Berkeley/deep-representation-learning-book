...
$\sigma$.} This entails the modified definitions
...
auxiliary time variable $t \in [0, \sigma^2]$, and a random process $t\mapsto \vz(t)$
such that for any $t$, one has $\vz(t) | \vz_o \sim \cN(\vz_o, t\vI)$.
In words, the distribution of $\vz(t)$ conditional on $\vz_o$ is Gaussian with
mean $\vz_o$ and covariance $t\vI$. This means in particular that
$\vz(\sigma^2)$ has the same distribution as $\vz_{\sigma}$.
By properties of Gaussian smoothing, for every $t > 0$,
the random variable $\vz(t)$ has a density $p_t$, which can be expressed
...
$t \vI$.\footnote{Note that although we use subscripts for the density $p_t$ for
convenience, it does not follow the same notational conventions as we use for
$\vz_{\sigma}$: in particular, the subscript $t$ in $p_t$ specifies the
\textit{variance}, whereas it specifies the standard deviation of the noise
added for $\vz_{\sigma}$.} A very fundamental fact about the density $p_t$ is
...
$h(\vz(t)) = \int p_t \log p_t$; and the second line uses an integration by parts.
The identity in the final line of \Cref{eq:de-bruijn}, which equivalently
relates the \textit{Fisher information} of $\vz(t)$ to the time derivative of its
entropy, is called de Bruijn's identity. Because $\vz(0)$ has the same
distribution as $\vz_o$ and $\vz(\sigma^2)$ has the same distribution as
$\vz_{\sigma}$, \Cref{thm:diffusion-and-entropy} immediately follows from
\Cref{eq:de-bruijn}.
...
\begin{equation*}
  ...
  + \sigma^2 \nabla \log p_{\sigma^2}(\vz_{\sigma}),
\end{equation*}
where we applied Tweedie's formula (\Cref{thm:tweedie}).
This can be seen as a perturbation of the distribution of the random variable
$\vz_{\sigma}$ by the \textit{score function vector field}, suggesting
...
the observation $\vz_{\sigma}$.\footnote{For those familiar with diffusion
...
update map $\vmu_{\sigma}$. As in the previous section, we define a family of
maps $\vmu(t)$ analogously to our definition of the intermediate noisy random
variables $\vz(t)$ for $t \in [0, \sigma^2]$.
...
positive codimension---the maps $\vmu(t)$ are actually invertible for every
$t>0$ \cite[Lemma II.1]{Gribonval2011-pf}.
This implies that the density of the denoised signal $\vmu(t)(\vz(t))$
\sdb{notational train wreck, fix todo...} is given precisely by
\begin{equation*}
...
\begin{proof}[Proof of {\Cref{thm:conditioning_reduces_entropy}}.]
  ...
  \begin{equation*}
    ...
    \frac{1}{d},
  \end{equation*}
  which, together with convexity of the function $x \mapsto (1 + x/d)^d$ (for $x
  \geq -1$) and the
  fact that $(1 + 1/d)^d \leq e$, implies for all such $t\leq 1/L$
  \begin{equation*}
    \int p_t \left(
    1 + \frac{t}{d} \tr\left(\nabla^2 \log p_t \right)
    \right)^d
    \leq
    \frac{et}{d}\int p_t \Delta \log p_t.
    % \int p_t \left(
    % 1 + \frac{(e-1)t}{d} \Delta \log p_t
    % \right).
  \end{equation*}
  ...
  \begin{equation*}
    ...
    \leq
    -1 + 
    % \frac{(e-1)t}{d} \int p_t \Delta \log p_t.
    \frac{et}{d}\int p_t \Delta \log p_t.
  \end{equation*}
...
\begin{lemma}\label{lem:entropy-reduction-regularity}
  Suppose $p$ is a density on $\R^d$ which is strictly positive and bounded in
  the sense that $\sup_{\vx} | p(\vx) | < +\infty$.
...
\begin{proof}
  ...
  \begin{equation*}
    p_t(\vx) = \int_{\R^d} \varphi_{t}(\vx - \vx') p(\vx') \mathrm{d} \vx',
  \end{equation*}
  and {differentiating under the integral} gives
  \begin{align*}
  ...
  \begin{equation*}
    ...
    \varphi_{t}(\vx - \vx') p(\vx') \mathrm{d} \vx'.
  \end{equation*}
  Using H\"{o}lder's inequality and the fact that the Gaussian distribution has
  finite moments of all orders, it follows from these expressions and the
  assumption that $p$ is bounded that $\nabla p_t$ and $\Delta p_t$ are bounded
