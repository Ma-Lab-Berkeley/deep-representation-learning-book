...
\begin{proof}[Proof of {\Cref{thm:conditioning_reduces_entropy}}.]
  ...
  \begin{equation*}
    ...
    \right),
  \end{equation*}
  so changing coordinates once again gives \sdb{being more judicious with change
  of coords makes this simpler}
  \begin{equation*}
    h(\vmu_t(\vz_t))
    -
    h(\vz_t)
  ...
  Laplacian.
  By our assumptions on $C^2$ smoothness of $p$, it follows by properties of
  Gaussian smoothing that $(t, \vx) \mapsto \Delta \log p_t(\vx)$ is
  a continuous function, so that \sdb{hand-wavy, needs more details}
  \begin{equation*}
    \sup_{0 \leq t \leq 1,\, \vx} \Delta \log p_t(\vx) < +\infty.
  \end{equation*}
  ...
  fact that $(1 + 1/d)^d \leq e$, implies
  \begin{equation*}
    \int p_t \left(
    1 + \frac{t}{d} \tr\left(\nabla^2 \log p_t \right)
    \right)^d
