...
\begin{proof}[Proof of {\Cref{thm:conditioning_reduces_entropy}}.]
  ...
  p(\vx)\odif \vx = 0$ would necessarily follow, contradicting the assumption that $p$ is
  ...
  \begin{equation*}
    ...
    \right)^d\odif{\vx}
    \leq
    1 + \frac{Mt}{d}\int p_t(\vx) \Delta \log p_t(\vx)\odif{\vx}.
  \end{equation*}
  ...
  \begin{equation*}
    ...
    -
    h(\vz_t)
    \leq \frac{Mt}{d}\int p_t(\vx) \Delta \log p_t(\vx)\odif{\vx}.
  \end{equation*}
...
\begin{proof}
  ...
  Meanwhile, because $\int p(\vx)\odif \vx = 1$ and the Gaussian density is strictly positive
