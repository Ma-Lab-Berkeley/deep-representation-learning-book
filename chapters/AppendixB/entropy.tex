...
\end{equation*}
where $\partial_t$ denotes differentiation in time, and $\Delta = \sum_{i=1}^d \partial^2_{\vx_i}$ denotes the Laplacian operator.
...
\begin{equation}\label{eq:de-bruijn}
  \begin{split}
    \partial_t h(\vz_t)
  &=
  -\frac{1}{2} \int (1 + \log p_t(\vx)) \Delta p_t(\vx) \odif{\vx}
  \\
  &=
  \frac{1}{2} \int \ip*{\nabla \log p_t(\vx)}{\nabla p_t(\vx)} \odif{\vx}
  \\
  &= \frac{1}{2} \int p_t(\vx) \norm{\nabla \log p_t(\vx)}_2^2
  \geq 0,
  \end{split}
\end{equation}
where the first line uses the chain rule, the heat equation, and the expression
$h(\vz_t) = -\int p_t(\vx) \log p_t(\vx) \odif{\vx}$; and the second line uses an integration by parts.
...
\begin{proof}[Proof of {\Cref{thm:conditioning_reduces_entropy}}.]
  ...
  p = 0$ would necessarily follow, contradicting the assumption that $p$ is
  ...
  \begin{align*}
    h(\vmu_t(\vz_t))
    &=
    - \int \frac{(p_t \circ \vmu_t^{-1})(\vx)}{\det[D \vmu_t(\vx)]}
    \log \frac{(p_t \circ \vmu_t^{-1})(\vx)}{\det[D \vmu_t(\vx)]}\odif{\vx}
    \\
    &=
    - \int \frac{(p_t \circ \vmu_t^{-1})(\vx)}{\det[D\vmu_t(\vx)]}
    \log[(p_t \circ \vmu_t^{-1})(\vx)]\odif{\vx} \\
    &\qquad +
    \int \frac{(p_t \circ \vmu_t^{-1})(\vx)}{\det[D\vmu_t(\vx)]}
    \logdet[D\vmu_t(\vx)]\odif{\vx}
    \\
    &=
    h(\vz_t)
    +
  ...
  \begin{equation*}
    ...
    =
    -\int
    \frac{(p_t \circ {\vmu_t}^{-1})(\vx)}{\det( \vI + t \nabla^2 \log p_t(\vx))}
    \log \left(
    \frac{1}{\det( \vI + t \nabla^2 \log p_t(\vx))}
    \right)\odif{\vx}.
  \end{equation*}
  ...
  \begin{align*}
    ...
    &\leq
    \int
    (p_t \circ {\vmu_t}^{-1})(\vx)\left(
    1 - 1/\det\left( \vI + t \nabla^2 \log p_t(\vx) \right)
    \right)\odif{\vx}
    \\
    &=
    \int p_t(\vx) \det\left( \vI + t \nabla^2 \log p_t(\vx) \right)\odif{\vx}
    - 1,
  \end{align*}
  ...
  u \leq 1/d$ we have
  \begin{align*}
    (1+u)^d &\leq \left( 1 - \frac{1}{d} \right)^d + \left[ \left(
    1 + \frac{1}{d} \right)^d - \left( 1 - \frac{1}{d} \right)^d \right] u
    \\
    &\leq 1 + eu.
  \end{align*}
  ...
  \begin{equation*}
    ...
    \right)^d\odif{\vx}
    \leq
    1 + \frac{et}{d}\int p_t(\vx) \Delta \log p_t(\vx)\odif{\vx}.
  \end{equation*}
  ...
  \begin{equation*}
    ...
    -
    h(\vz_t)
    \leq \frac{et}{d}\int p_t(\vx) \Delta \log p_t(\vx)\odif{\vx}.
  \end{equation*}
  ...
  \begin{align*}
    ...
    h(\vz_t)
    &\leq
    -\frac{et}{d} \int \ip{\nabla p_t(\vx)}{ \nabla \log p_t(\vx) }\odif{\vx}.
    \\
    &=
    -\frac{et}{d} \int p_t(\vx) \norm*{\nabla \log p_t(\vx)}_2^2\odif{\vx} \leq 0,
  \end{align*}
  as claimed.
\end{proof}
