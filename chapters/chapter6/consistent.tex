...
\paragraph{Online PCA.} 
%\sdb{I think we can cut this section.}\yima{agree. We can probably move this to the later incremental learning section, as a motivating example.}
...
have developed in \Cref{ch:unrolling}.
...
% \subsection{Open-Loop Representation Learning with SimDINO}
...
\sdb{Either new subsections here about SimDINO and RAE, or we want to refactor
this into a new section? I think subsection is appropriate, because with
a well-structured refactor of the previous, this would end up like a natural
culmination of the previous development. It could also be possible to introduce
these in the closed-loop parts, because the SimDINO objective is very similar to
...
\begin{equation}
    \X \rightarrow \Z,
\end{equation}
based on some end-to-end supervised or self-supervised methods, including the MCR$^2$ and SimDINO methods introduced in Sections \ref{sec:MCR2-classification} and  \ref{sec:SimDINO}. As we have also argued that so learned representations have better structures than the original data distribution. The representations tend to be a mixture of low-dimensional linear subspaces (or low-rank Gaussians). One remaining question is, however, how good are these learned representations? Can we make sure the so-learned representations are to large extent {\em consistent} with the original data $\X$. That is, can we establish from them a reverse decoder:
\begin{equation}
    \Z \rightarrow \hat{\X}
\end{equation}
such that $\hat \X$ and $\X$ are close.

Furthermore, those low-dimensional (linear) structures are still embedded in a high-dimensional representation space. Hence in order to use the so learned representations, we need to be able to access their distributions too. In Chapter \ref{ch:denoising}, we have learned that denoising is a general approach to identify hence access such low-dimensional representations. We have also learned that if the target distributions can be modeled (approximately) as a mixture of subspaces or low-rank Gaussians, their optimal denoiser operators share a similar structures as the Transformers, or the CRATE introduced in \Cref{ch:unrolling}. 

Hence, altogether, we want to learn an autoencoder whose representation can be easily accessed. 
...
\begin{enumerate}
  ...
  \item The representation $\Z$ can be modeled as a low-dimensional distribution so that we can learn a denoiser $D$ to access the distribution of $\Z$.
\end{enumerate}

\Cref{fig:RAE} illustrates this process. From \Cref{ch:compression}, we know that the the learned representation $\Z$ by optimizing the (unsupervised) MCR$^2$ objectives already satisfies the second requirement and can be modeled as a mixture of subspaces or low-rank Gaussians. By incorporating a sample-consistency objective, we can extend the rate reduction objective in \ref{sec:SimDINO} to learn a consistent representation:
\begin{equation}
  \min_{f, g}
  [ - \Delta R_{\epsilon}(\Z) + d(\X, \hat \X)],
  \label{eqn:autoencode-consistent}
\end{equation}
where $d(\X, \hat \X)$ is a measure of similarity between the original data $\X$ and the reconstructed data $\hat \X = g(\Z)$. In practice, because the objective $\Delta R_{\epsilon}(\Z)$ alone already promotes low-dimensional structure of $\Z$, we can pretrain the encoder $f$ to maximize $\Delta R_{\epsilon}(\Z)$ first, and then learn the decoder $g$ to minimize the reconstruction loss $d(\X, \hat \X)$ while fixing the encoder $f$. This two-step approach has been demonstrated to be effective, as we will see in later chapters. 

Once we have learned such a consistent representation $\Z$, we can then learn a denoiser $D$ to access its distribution, as we have studied in \Cref{ch:denoising}. Concretely, suppose we have a probabilistic model $p(\z)$ that captures the distribution of the learned representations $\cZ$ and an initial standard Gaussian distribution $\cN(0, I)$. Our goal is to learn a denoiser $\bar{\z}$ such that it iteratively denoises a sample from the initial distribution to the target distribution of the learned representations. This denoiser can be trained using objectives introduced in \Cref{ch:conditional-inference}. For instance, let's take the simple case of flow matching. We can define the denoiser as a time-dependent vector field $\bar{\z}_{\theta}(t, \z_t)$ parameterized by $\theta$ that transforms a sample from the initial distribution to the target distribution over time $t \in [0, 1]$. Here, $\z_t$ is an interpolation between $\z_0$ and $\z_1$ at time $t$:
\begin{equation}
    \z_t = (1 - t)\z_0 + t\z_1,
\end{equation}
...
\end{equation}
Once we have trained the denoiser, we can use it to generate new samples from the learned representation distribution via a diffusion process. This completes the learning of a representation autoencoder (RAE) with an accessible representation distribution. In \Cref{ch:applications}, we will see a specific implementation of RAE and its applications in image generation.

\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{chapters/chapter6/figs/mental-picture.pdf}
    \caption{Illustration of learning an autoencoder and a denoiser for a good representation.}
    \label{fig:RAE}
\end{figure}
...
\sdb{In this and the next section, we need to make some adjustments for
consistency after we write the new SimDINO/RAE sections above. We can also/in
particular, in
this section, introduce a more precise discussion (?) of Peter/Yi/Yann's paper
about closed loop and the connection with SimDINO.}
