...
and  \ref{sec:SimDINO}. As we have also argued, the so-learned representations
...
to identify hence access such low-dimensional representations. We have also
learned that if the target distributions can be modeled (approximately) as
...
Thus, altogether, we want to learn an autoencoder whose representation can be easily accessed. 
...
\begin{enumerate}
  ...
  \item The representation $\Z$ can be modeled as a low-dimensional distribution so that we can learn a denoiser to access the distribution of $\Z$.
\end{enumerate}



\Cref{fig:RAE} illustrates this process. From \Cref{ch:compression}, we know
that the the learned representation $\Z$ by optimizing the (unsupervised)
MCR$^2$ objective already satisfies the second requirement and can be modeled as
...
demonstrated to be effective, as we will see in \Cref{ch:applications}.
