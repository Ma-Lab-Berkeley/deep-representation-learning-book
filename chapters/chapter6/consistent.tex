...
\cite{rombach2022high}, where a VAE is first trained to learn a compact latent representation of high-dimensional data such as images, and then a diffusion model is trained in the latent space to generate new samples, as illustrated in Figure \ref{fig:VAE-SD}.
\begin{figure}[t]
    \centering\includegraphics[width=0.8\linewidth]{chapters/chapter6/figs/VAE-SD.png}
    \caption{Diagram of the Latent Diffusion Model from the work of \cite{rombach2022high} which utilizes a variational autoencoder (red box on the left) to learn a latent representation of the data $\x$. Then a diffusion/denoising model (green box in the middle) is learned to access the representation $\z$. The block on the right is about how to control the denoising sampling process via a control signal which we will study in Chapter \ref{ch:conditional-inference}.}
    \label{fig:VAE-SD}
\end{figure}
