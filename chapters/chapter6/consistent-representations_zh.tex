...
\begin{quote}
    ...
    $~$\hfill —— 阿尔伯特·爱因斯坦 (Albert Einstein)
\end{quote}

\vspace{5mm}

在前面的章节中，我们已经论证了学习的根本目标是\textit{识别}具有低维支撑的数据分布，并将其\textit{转换}为紧凑且结构化的表征。这涉及两个相关的问题：
\begin{enumerate}
    \item \textbf{分布学习：} 我们如何掌握数据 $\vX$ 的分布，以实现可泛化的采样/生成？
...
而在 \Cref{ch:deep-networks} 中，我们看到流行的深度网络架构的层可以被解释为最大化信息增益的增量优化步骤。这使我们准备好为表征学习问题提出一个具有数学基础的解决方案。
...
然而，当我们试图通过优化来实现某个目标时，并不能保证最终通过贪婪优化找到的解 $\Z$ 就是正确的解。事实上，即使优化过程成功找到了全局最优解 $\Z^*$，也不能保证该解对应于数据分布的完整表征。\footnote{这可能是由于许多原因造成的：例如，可用于学习分布的数据可能不足，或者优化程序的公式未能考虑一些额外的约束或条件。}因此，一个悬而未决的问题是：我们如何确保学到的数据分布表征是正确的或足够好的？

当然，验证这一点的唯一方法是看是否存在一个解码映射（例如 $g$），它可以对学到的表征进行解码，从而足够好地重现原始数据（分布）：
\begin{equation}
    \X
...
\begin{equation}
    d(\X, \hat \X).
\end{equation}
这引出了{\em 一致表征}（consistent representation）的概念。正如我们在 \Cref{ch:intro} 中简要提到的（见 \Cref{fig:autoencoder}），集成了编码和解码过程的{\em 自编码}（autoencoding）是学习这种表征的自然框架。我们在 \Cref{ch:classic-models} 中研究了一些重要的特例，其中分布的支撑是分段线性的。在本章的 \Cref{sec:consistent-representation} 中，我们将研究如何将自编码扩展到支撑是非线性的更一般类别的分布（如图 \ref{fig:manifold-autoencoding} 所示），同时强制 $\X$ 和 $\hat \X$ 之间的一致性。
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{chapters/chapter6/figs/manifolds-autoencoding.png}
    \caption{为支撑在低维子流形混合体上的一般分布学习一个良好的自编码表征。请注意，除了像我们在 \Cref{ch:deep-networks} 中那样以端到端的方式通过映射 $f(\x, \theta)$ 学习一个良好的表征之外，我们还希望确保这样学到的表征可以通过“逆”映射 $g(\x, \eta)$ 解码，以近似恢复原始数据分布。}
    \label{fig:manifold-autoencoding}
\end{figure}

给定这种经典的自编码形式，自然会产生一个问题：\textit{分布学习在哪里结束，表征学习又从哪里开始？}
...
归结为\textit{效率}。正如现代实践所证明的那样，通常更有效的方法是为数据分布学习一个紧凑的表征，作为对分布的一种“准备”，然后再对这样学到的表征进行分布学习。这是实际问题中的主流方法。
...
其中“表征自编码器”（representation autoencoder）框架展示了如何将开环学习的表征（我们在 \Cref{sec:SimDINO} 中讨论过，并将进行回顾）
%learned from DINO/SimDINO 
与简单的解码器和 $\vZ$ 空间扩散过程相结合，以实现高效、同步的表征学习和采样。

更根本的是，在许多实际和自然的学习场景中，比较数据 $\X$ 和 $\hat \X$ 的分布可能是困难的，甚至是不可能的。我们唯一的选择是比较学到的特征 $\Z$ 及其在编码器 $f$ 下的像 $\hat \Z$：
\begin{equation}
    \X
    \xrightarrow{\hspace{1mm} \mathcal{E} = f \hspace{1mm}} \Z
    \xrightarrow{\hspace{1mm} \mathcal{D} = g \hspace{1mm}} \hat{\X}
...
\end{equation}
这引出了{\em 自一致表征}（self-consistent representation）的概念。在 \Cref{sec:self-consistency} 中，我们将研究何时以及如何仅通过闭环转录框架强制 $\Z$ 和 $\hat \Z$ 之间的自一致性来学习一致表征。

此外，在许多实际和自然的学习场景中，我们通常无法一次性获得数据分布的充足样本。例如，动物和人类通过在他们的一生中不断吸收增量观察来发展他们的视觉记忆。在 \Cref{sec:continuous} 中，我们将研究如何扩展闭环转录框架，以在{\em 连续学习}（continuous learning）设置中学习自一致表征。

当然，我们之所以想要识别数据分布中的低维结构并找到一个良好表征，其根本动机是为了便于将数据用于各种智能任务，如分类、补全和预测。因此，得到的联合表征 $(\x, \z)$ 必须以最适合这些任务的方式进行结构化。在下一章中，我们将看到如何对学到的表征进行结构化，以促进条件补全或生成任务。
...
\begin{definition}[一致表征]\label{def:bidirectional_rep}
    ...
    \begin{enumerate}
        ...
        \item 如果 \(\Law(\vX) \approx \Law(\hat{\vX})\)，我们称该表征为\textit{分布级一致}（distributionally consistent）。
    \end{enumerate}
\end{definition}

敏锐的读者可能已经注意到，如果我们不对所求的表征 $\Z$ 施加某些要求，上述问题将有一个平凡解：人们可以简单地选择函数 $f$ 和 $g$ 为恒等映射！因此，寻求自编码的真正目的是确保由此获得的 $\Z$ 比 $\X$ 更紧凑且更具结构化。
%First, for compactness, $\Z$
%should better reveal the intrinsic low-dimensionality of $\X$.
正如我们在 \Cref{ch:deep-networks} 中所见，只需设计表征以最大化编码率衰减（rate reduction）即可：
\begin{equation}
    \Delta R_{\epsilon}(\Z).
\end{equation}%
...
从一致表征的定义来看，要求表征 $\Z$ 足以在一定精度上恢复原始数据分布 $\X$。对于样本级一致性，典型的选择是最小化预期重建误差：
\begin{equation}
    d(\X, \hat \X) = \mathbb{E}[\|\X - \hat\X\|_2^2].
\end{equation}
对于分布一致性，典型的选择是最小化某种分布距离，例如 KL 散度\footnote{请注意，对于没有共同支撑的分布（这对于退化分布很典型），KL 散度甚至可能没有良好定义。事实上，许多分布学习文献试图通过用定义良好且可高效计算的量来替换或近似它，以解决这一技术困难。}：
\begin{equation}
    d(\X, \hat \X) = \mathcal{D}_{KL}(\X\|\hat\X).
\end{equation}

因此，撇开计算不谈，当我们为数据分布 $\X$ 寻求一个良好的自编码时，在概念上我们试图找到一个编码器 $f$ 和解码器 $g$，使得
\begin{equation}
...
在本节的其余部分，我们将研究如何在不同条件下解决此类自编码问题，从简单理想的情况到越来越具有挑战性和现实的条件。我们最终将给出 \eqref{eqn:autoencode-objective-ch5} 的一个高效实例（在 \Cref{sec:RAE} 中），它确保表征空间 $\vZ$ 紧凑且结构化，同时允许准确的重建和高效的采样。

\subsection{基于 PCA 的线性自编码}% or Online PCA}

根据 \cite{Baldi2011} 的说法，“自编码器”（autoencoder）一词最早由 Hinton 和 Rumelhart \cite{Rumelhart1986} 引入，以便可以通过反向传播（BP）以自监督的方式学习深度表征——重建原始数据即为自监督任务。然而，寻求紧凑且一致表征的相同概念植根于许多经典研究中。正如我们在 \Cref{ch:classic-models} 中已经看到的，经典的 PCA、ICA 和稀疏字典学习都有着相似的目标。唯一的区别在于，当底层数据分布很简单（线性和独立）时，编码或解码映射变得易于表示和学习：它们不需要很深，通常可以以闭式解或通过显式算法计算。
...
正如我们在 \Cref{ch:classic-models} 中看到的，当 $\vx$ 的分布确实支撑在低维子空间 $\vU_o$ 上时，由 $\mathcal{E}$ 产生的表征 $\vz$ 的紧凑性是正确估计（并强制）该子空间维度的直接结果。回想一下，对重建准则进行梯度下降恰好会产生这些样本级一致的映射：事实上，该问题的最优解
\begin{equation}\label{eq:pca-reconstruction-ch5}
    \min_{\vU^\top \vU = \vI}\, \bE_{\vx}\left[\norm*{\vx
    - \vU\vU^\top \vx}_2^2\right]
\end{equation}
在表征的维度足够大时，恰好与 $\vU_o$ 一致。在这种情况下，我们免费获得了样本级一致性，因为这保证了 $\vU_o\vU_o^\top \vx = \vx$。请注意，在 PCA 的情况下，\eqref{eqn:autoencode-objective-ch5} 中的编码率衰减项变得无效，因为对所求表征 $\Z$ 的正则化是显式的：它跨越了整个低维子空间\footnote{在这种情况下，可以认为 $\Delta R = 0$。}。

\subsection{非线性 PCA 与自编码}\label{sub:nonlinear-pca}\label{sec:NLPCA}
当然，可以预见，当我们处理底层低维结构是非线性的更复杂分布时，事情将不再那么简单。

\paragraph{非线性子流形上的数据。} 因此，为了超越 PCA 所处理的线性结构，我们可以假设数据分布位于一个（平滑的）子流形 $\mathcal{M}$ 上。子流形的内在维度（假设为 $d$）通常远低于环境空间 $\mathbb{R}^D$ 的维度。从这个几何视角来看，我们通常希望找到一个非线性映射 $f$，使得得到的流形 $f(\mathcal{M})$ 被展平，如 \Cref{fig:idealized_interpolation} 所示的例子。得到的特征 $\vz$ 空间通常比 $\vx$ 空间更紧凑（维度更低），并且流形是平坦的。从统计视角来看（它与几何视角互补，但通常是不同的），我们可能还希望确保 $\mathcal{M}$ 上的数据分布被映射到 $\vz$ 空间中足够规则的分布，例如高斯分布或均匀分布（具有非常低维的支撑）。
...
一般来说，为这类数据分布学习这种自编码映射的问题被称为{\em 非线性主成分分析}（NLPCA）。
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node at (9, -0.5) {\(\hat{\x}\)};
    \end{tikzpicture}
    \caption{在 \(\R^{3}\) 中维度为 \(d = 2\) 的流形上通过流形展平进行插值的示意图。要在数据流形上插值两个点，通过展平映射 \(\fl\) 将它们映射到展平空间，取它们的凸插值，然后通过重建映射 \(\re\) 将它们映射回数据流形。}
    \label{fig:idealized_interpolation}
\end{figure}

\paragraph{通过双层网络的经典尝试。} 正如我们在上面看到的，在 PCA 的情况下，单层线性神经网络就足够了。对于 NLPCA 来说，情况不再如此。1991 年，Kramer \cite{Kramer1991NonlinearPC} 提出通过使用双层神经网络来表示编码器映射 $f$（或其逆映射 $g$）来解决 NLPCA，这是基于具有 sigmoid 激活函数的双层网络的通用表示性质：
\begin{equation}
...
\end{equation}
Cybenko \cite{Cybenko1989ApproximationBS} 证明了上述形式的函数（具有足够的隐藏节点）可以以任意精度近似任何平滑的非线性函数，例如编码器 $f(\spcdot)$。特别是，它们可以表示支撑在低维流形并集上的数据分布的展平和重建映射，如 \Cref{fig:idealized_interpolation} 所示。Kramer 提出的原始网络的整体架构如 \Cref{fig:NLPCA} 所示。
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.6\linewidth]{\toplevelprefix/chapters/chapter6/figs/kramer1991nonlinearPCA.png}
    \caption{由 Kramer \cite{Kramer1991NonlinearPC} 提出的，通过深度为 2 的自联想神经网络进行编码和解码映射的非线性 PCA。}
    \label{fig:NLPCA}
\end{figure}

不幸的是，与上面的 PCA 情况不同，这些网络的参数 $\boldsymbol{\theta} = (\vW, \vb)$ 通常没有闭式学习方案。因此，有人提出在重建误差的监督下通过反向传播来训练网络：
\begin{equation}\label{eq:nonlinear-pca-ch5}
    \min_{\boldsymbol{\theta}} \mathbb{E}[ \|\x -
    \hat{\x}(\boldsymbol{\theta})\|_2^2].
\end{equation}
与 PCA 的简单情况相比，我们利用相同的重建目标进行学习，但使用复杂得多的非线性模型类来参数化和学习编码器和解码器。尽管像 Cybenko 那样的通用近似性质表明，\textit{原则上}通过该框架学习一致的自编码器是可能的——因为对于任何随机数据样本，只要给定足够的参数，这样的自编码对就存在——但人们通常发现使用梯度下降找到它们是非常困难的。此外，为了获得足够信息量的重建目标和模型以适应图像等高维真实世界数据的分布，所需的样本和隐藏节点数量可能是巨大的。另外，作为学到表征紧凑性的度量，瓶颈层 $\z$ 的（较低）维度通常是启发式选择的。\footnote{在后来的工作 \cite{Hinton-1993} 中，Hinton 等人建议使用最小描述长度（MDL）原则来促进学到的编码方案的紧凑性，其精神与本书中引入的率失真度量非常相似。}

\paragraph{通过更深的网络进行流形展平。}
基于深度网络的现代实践，已知这种经典的浅而宽的网络架构很难通过反向传播（BP）有效且高效地训练，部分原因是 sigmoid 函数的梯度消失问题。因此，现代实践通常建议将非线性变换 $f$（或 $g$）进一步分解为更多层简单变换的组合，从而产生更深的网络架构 \cite{Hinton504}，如 \Cref{fig:ccnet_layers} 所示。
...
\begin{figure}[htb]
    ...
    \end{tikzpicture}
    \caption{展平与重建对 \((\fl, \re)\) 构造过程的示意图，其中编码器 \(\fl = \fl_{L} \circ \fl_{L - 1} \circ \cdots \circ \fl_{1}\) 由展平层组合而成，解码器 \(\re = \re_{1} \circ \re_{2} \circ \cdots \circ \re_{L}\) 由每个 \(\fl_{\ell}\) 的逆组合而成。}
    \label{fig:ccnet_layers}
\end{figure}

鉴于像 Cybenko 那样的通用近似定理，人们最初可能会想，在概念上为什么更深的自编码器应该优于浅层自编码器。纯粹从表达能力的角度来看，我们可以通过与展平假设数据分布所支撑的非线性流形任务相关的几何角度来理解这一现象。一种纯粹构造性的流形展平方法是增量进行的，这与我们在 \Cref{ch:denoising,ch:lossy-compression,ch:deep-networks} 中看到的扩散、去噪和压缩之间的相互作用相平行。在几何设置中，对应于 $f_{\ell}$ 的增量\textit{展平过程}采取的形式是：将流形上一点的邻域变换为更接近平坦流形（即子空间），并强制与其余数据样本保持局部一致性；解码器中相应的增量操作 $g_{\ell}$ 则撤销这种变换。该过程精确地结合了关于底层流形的曲率信息，这些信息是从数据样本中估计出来的。给定来自流形的足够样本并仔细实例化这一概念过程，就有可能将该草图实现为一种计算程序，以白盒方式可验证地展平非线性流形 \cite{Psenka-JMLR24}。然而，由于可扩展性不佳，该方法在图像等高维数据分布中的适用性受到限制，这促使人们开发更灵活的增量自编码方法。

\subsection{稀疏自编码}
在上述自编码方案中，特征空间维度 $d$ 通常被选择为远低于原始数据空间维度 $D$，以便显式地强制或促进学到的表征是低维的。然而，在实践中，我们通常不知道数据分布的内在维度。因此，自编码特征空间维度的选择通常是凭经验进行的。在更一般的情况下，数据分布可以是几个低维子空间或子流形的混合。在这些情况下，对所有特征一起强制使用单一的低维空间不再可行。

稀疏自编码器旨在解决其中一些局限性。特别是，特征空间的维度可以等于甚至高于数据空间的维度，如 \Cref{fig:SAE} 所示。然而，要求特征在特征空间中高度稀疏。因此，如果我们在目标 \eqref{eqn:autoencode-objective-ch5} 中的编码率衰减之外，将稀疏性作为简约性的度量施加进去，我们将获得稀疏自编码的新目标：
\begin{equation}
    \min_{f, g}
    [\|\Z\|_0 - \Delta R_{\epsilon}(\Z) + d(\X, \hat \X)],
    \label{eqn:autoencode-sparse}
\end{equation}
其中已知 $\ell^0$-“范数” $\|\spcdot\|_0$ 可以促进稀疏性。这与我们在前面的 \Cref{ch:deep-networks} 中用于推导白盒 CRATE 架构的稀疏编码率衰减目标 \eqref{eq:sparse-rr} 非常相似。
...
一种改进的、更现代的自编码方法是\textit{变分自编码}（variational autoencoding）\cite{Kingma2013-sb,Kingma2019-zh}，至今仍有重要应用。
...
% parameterized
%by $\vtheta$.
它用概率术语来表述表征学习问题，即寻找一个模型 $p(\vx; \vtheta)$，以最大化数据样本（或更一般地，总体数据分布）的似然：
...
%
这意味着可以通过\textit{先验} $p(\vz)$ 和条件分布 $p(\vx \mid \vz)$（更一般地对应于解码器）来学习 $\vx$ 的概率模型。在技术上，贝叶斯法则意味着编码所需的\textit{后验} $p(\vz \mid \vx)$ 可以用先验和解码器（以及 $\vx$ 的样本）来表示，但这在计算上是难以处理的。因此，在 VAE 框架中，我们引入了一个“近似后验”
...
\end{align*}
我们可以将其理解为一个正则化的自编码目标，其中在 ELBO 的第一项中有一个有趣的额外一致性强制。

同时，读者应注意 VAE 形式化所做的大量任意决定（例如，高斯分布的选择和参数化）。这导致了一个复杂的训练目标，尽管经过了十多年的研究，高效优化该目标仍然是一个挑战。最近的表征学习方法对这一基线进行了显著改进，它们甚至可以在我们在 \Cref{ch:deep-networks} 中开发的白盒框架中进行表述，正如我们很快就会看到的。
...
VAE 的一个重要应用是在潜在扩散模型（latent diffusion models）\cite{rombach2022high} 的背景下，其中首先训练 VAE 以学习图像等高维数据的更紧凑和受规则化的潜在表征。然后可以在潜在空间中训练扩散模型以访问表征的分布，从而从中生成新样本，如图 \ref{fig:VAE-SD} 所示。
\begin{figure}[t]
    \centering\includegraphics[width=0.8\linewidth]{chapters/chapter6/figs/VAE-SD.png}
    \caption{\cite{rombach2022high} 工作中的潜在扩散模型图示，该模型利用变分自编码器（左侧红框）来学习数据 $\x$ 的潜在表征。然后学习一个扩散/去噪模型（中间绿框）以访问表征 $\z$。右侧的模块是关于如何通过（基于文本的）控制信号来控制去噪采样过程，我们将在第 \ref{ch:conditional-inference} 章中对此进行研究。}
    \label{fig:VAE-SD}
\end{figure}
这种方法通过利用学到的潜在空间的高度压缩结构，实现了高质量图像的高效生成。\footnote{许多流行的早期图像或视频生成模型都是基于这种潜在扩散框架的，正如我们在 \Cref{sub:text-cond} 中进一步描述的那样，并在 \Cref{ch:applications} 中从实现角度进行了说明。}然而，由于后验坍塌以及平衡 ELBO 中的重建项和正则化项等问题，训练 VAE 可能具有挑战性。此外，VAE 中的高斯后验是应用于每个数据点的，没有跨样本学习联合结构，这可能会严重限制学到表征的质量。这些挑战促使了替代方法的发展，例如接下来要介绍的表征自编码器（RAEs），旨在解决 VAE 的一些局限性，同时仍为生成建模任务提供有效的潜在表征。
...
以前用于表征学习的训练自编码对的方法都有一个共同的局限性：它们没有\textit{显式地组织}表征空间 $\vZ$，而只是间接地鼓励它（例如，通过在稀疏自编码器中强制稀疏性或在 VAE 中强制高斯性）。正如我们在本章开头所暗示的，我们之前（在 \Cref{ch:lossy-compression} 中）已经看到了如何做得更好，即通过最大化编码率衰减 $\Delta
R_{\epsilon}(\vZ)$ 来学习表征 $\vZ$，同时确保重建一致性。我们将在本节中更详细地描述这一过程，特别是还将展示如何在学到的表征空间 $\vZ$ 中进行\textit{高效采样}。

正如我们在 \Cref{ch:lossy-compression} 末尾（第 \ref{sec:representations-image} 节）所看到的，我们通常可以学习到数据 $\X$ 的良好表征 $\Z$：
\begin{equation}
    f: \X \rightarrow \Z,
\end{equation}
基于一些端到端的监督或自监督方法，包括分别在第 \ref{sec:MCR2-classification} 节和第 \ref{sec:SimDINO} 节中介绍的 MCR$^2$ 和 SimDINO 方法。正如我们还论证过的，这样学到的表征比原始数据分布具有更好的结构。这些表征倾向于是低维线性子空间（或低秩高斯分布）的混合。然而，我们还没有回答如何评估这样学到的表征的好坏的问题。也就是说，我们能否从中建立一个反向解码器：
\begin{equation}
    g: \Z \rightarrow \hat{\X}
\end{equation}
\end{document}
\end{equation}
使得 $\hat \X$ 和 $\X$ 足够接近？鉴于前面经典自编码器设置的例子，
我们应该期望形如
\eqref{eqn:autoencode-objective-ch5} 的目标函数能够起作用——但是应该使用什么样的样本级
一致性损失呢？

此外，这些低维（线性）结构仍然嵌入在
高维表征空间中。因此，为了使用这样学到的
表征，我们还需要能够访问它们的分布。在
...
与 Transformer 或在 \Cref{ch:deep-networks} 中引入的 CRATE 中发现的结构具有相似性。

\begin{figure}[t]
    \centering
    \includegraphics[width=0.6\linewidth]{chapters/chapter6/figs/mental-picture.pdf}
    \caption{为数据 $\X$ 学习自编码表征 $\Z$（上）以及
        学习扩散/去噪模型
    （下）以访问表征 $\Z$ 的示意图。}
    \label{fig:RAE}
\end{figure}
...
\begin{enumerate}
    \item 表征 $\Z = f(\X)$ 是原始数据 $\X$ 的一致性
        表征，如
        \Cref{def:bidirectional_rep} 中所定义。
    \item 结构化表征 $\Z$ 可以近似建模为
        低维高斯混合，从而
...
样本一致性目标，我们可以扩展 \Cref{sec:SimDINO} 中的率衰减（rate reduction）
目标，以学习一致性表征：
\begin{equation}
    \min_{f, g}
...
\end{equation}
其中 $d(\X, \hat \X)$ 是衡量原始数据 $\X$
与重建数据 $\hat \X = g(\Z)$ 之间相似度的度量。在实践中，因为仅靠目标
$\Delta R_{\epsilon}(\Z)$ 就已经能够促进 $\Z$ 的低维结构，
我们可以先预训练编码器 $f$ 以最大化 $\Delta R_{\epsilon}(\Z)$，
然后再固定编码器 $f$，学习解码器 $g$ 以最小化重建损失 $d(\X,
\hat \X)$。这种两步法已被
证明是有效的，正如我们将在 \Cref{ch:applications} 的
第 \ref{sec:RAE-implement} 节中看到的那样。
...
一个依赖于时间的向量场 $\bar{\z}_{\theta}(t, \z_t)$，
由 $\theta$ 参数化，它随着时间 $t \in [0, 1]$ 将样本从初始分布
变换到目标分布。
这里，$\z_t$ 是在时间 $t$ 时 $\z_0$ 和 $\z_1$ 之间的插值：
\begin{equation}
    \z_t = (1 - t)\z_0 + t\z_1,
\end{equation}
其中 $\z_0 \sim p(\z)$ 是来自学习到的表征分布的样本，
...
值得注意的是，为通过 VAE 学习到的表征与通过 MCR$^2$ 或 SimDINO 等方法
学习到的表征学习去噪器之间存在差异。
正如在 \Cref{sec:vae} 中所讨论的，VAE 表征被
强制要求\textit{逐样本}近似高斯后验，并且
没有对潜在空间的联合分布进行建模，这使得
表征空间的结构性较弱，去噪过程
可能更具挑战性。相比之下，通过 MCR$^2$ 或 SimDINO 学习到的表征
往往具有更明显的线性和
低维结构，例如子空间混合或低秩
...
学习到的表征分布中生成新样本，正如我们在
\Cref{ch:denoising} 中所展示的那样。这就完成了具有可访问表征分布的
\textit{表征自编码器}（Representation Autoencoder, RAE）的
学习。
在 \Cref{ch:applications}（特别是 \Cref{sec:RAE-implement}）中，我们将看到 RAE 的
具体实现及其在图像生成中的应用。
...
网络。拥有数十亿
参数的大型模型，在数以万计的强大 GPU 上使用数万亿数据点进行训练，
在记忆现有知识方面展示了超人类的能力。
这使得许多人相信，
随着规模的不断扩大，这些模型的“智能”将继续提高。

虽然我们庆祝这种大型人造
机器学习系统的工程奇迹，但我们也必须承认，与
自然界的智能相比，这种提高机器智能的方法
对资源的需求是不必要地巨大的。自然界的智能
生物，包括动物和人类，根本无法承受这种
暴力的学习解决方案，因为它们必须在
能量、空间和时间预算非常有限的情况下运作，并受到许多
严格的物理约束。
...
$\hat \X$ 在某种相似度度量 $d(\X, \hat \X)$ 下足够接近。
在自然界中，动物或人类很少能直接获取真实数据（ground truth）
$\X$。例如，我们永远无法直接获取场景中物体的真实 3D
形状、距离或动力学信息。然而，我们都
学会了非常准确和高效地估计和预测它们。
因此，一个突出的问题是：即使我们不能直接将我们的
估计 $\hat \x$ 与真实值 $\x$ 进行比较，我们如何才能学习到 $\X$ 的真实
分布？正如我们将在本章中看到的，
答案同样在于闭环反馈，以及
数据分布的低维性。

正如我们从上一章所知，为了确保
表征是一致的，我们需要比较生成的 $\hat
\X \sim p(\hat\x)$ 和原始的 $\X \sim p(\x)$，至少在
分布上进行比较。即使我们确实可以访问 $\X$ 和 $\hat \X$，
从技术上讲，计算并最小化两个
分布之间的距离也可能存在问题，特别是当分布的支撑集（support）
是低维的时候。在 \Cref{ch:denoising} 中引入的 KL 散度，
在两个支撑集不重叠的分布之间甚至
没有良好的定义。
...
$d$（通常由深度网络建模），以辨别生成的样本 $\hat \X$ 和真实样本 $\X$
之间的差异：
\begin{equation}
...
\emph{斯塔克尔伯格博弈（Stackelberg game）}来对齐 $\hat \x$ 和 $\x$
之间的分布：
\begin{equation}
...
\end{equation}
也就是说，判别器 $d$ 试图最小化真实样本 $\X$ 和生成样本 $\hat
\X$ 之间的交叉熵，
而生成器 $g$ 则试图做相反的事情。

可以证明，寻找上述斯塔克尔伯格博弈的均衡
等价于最小化 \emph{Jensen-Shannon 散度（JS 散度）}：
\begin{equation}
...
不重叠，JS 散度也是
良定义的。然而，即使在两个高斯分布之间，JS 散度也没有闭式
表达式，而 KL 散度则有。此
外，由于数据分布是低维的，
优化 JS 散度可能是高度病态的（ill-conditioned）。\footnote{如
\cite{arjovsky2017wasserstein} 所示。} 这或许解释了为什么在 GAN 的许多后续变体中
通常会使用许多额外的启发式方法。
...
函数上的期望的最大差值。} 此外，无论是 JS 散度还是
Wasserstein 距离都没有闭式公式，即使对于高斯
分布也是如此。\footnote{（$\ell^1$-范数）Wasserstein 距离可以
    被具有闭式解的（$\ell^2$-范数）Wasserstein 距离所界定 \cite{salmona2021gromovwasserstein}。然而，正如
    高维几何中众所周知的那样，随着维度的升高，$\ell^1$-范数和 $\ell^2$-范数
    在几何和统计特性方面会产生显著偏差
...
\end{equation}
这就引出了 \emph{自一致表征（self-consistent representation）} 的概念。
\begin{definition}[自一致表征]\label{def:closed_loop}
    ...
    \begin{enumerate}
        ...
        \item 如果 \(\Law(\vZ)
            \approx \Law(\hat{\vZ})\)，我们称该表征为\textit{分布级自一致}。
    \end{enumerate}
\end{definition}
...
我们如何努力确保学习到的表征是自一致的？
像往常一样，假设 $\X = \cup_{k=1}^K \X_k$，其中每个样本子集
$\X_k$ 属于一个低维子流形：$\X_k
\subset \mathcal{M}_k, k = 1,\ldots, K$。遵循 \Cref{ch:denoising} 中的符号，
我们使用矩阵 $\bm \Pi_k(i,i) = 1$ 来表示
样本 $i$ 属于类别 $k$，否则 $\bm
\Pi_k(i,j) = 0$。我们寻求一个连续映射
$f(\cdot,\bm \theta): \x \mapsto \z$，将 $\X$ 映射到最优
表征 $\Z = [\z_1, \ldots, \z_N] \subset \mathbb{R}^{d \times N}$：
\begin{equation}
...
通过最大化上述目标 \eqref{eq:mcr2-formulation} 来学习这种单向映射 \eqref{eqn:LDR} 的一个问题是，
它倾向于扩展每个类别特征的学习子空间的维度\footnote{如果特征空间 $d$ 的维度过高，
    最大化率衰减可能会高估每个类别的维度。因此，为了学习一个好的表征，
    需要预先为特征空间选择一个合适的维度，正如 \cite{yu2020learning} 的实验中所实现的那样。
    事实上，即使在最简单的单子空间情况（即经典的 PCA \cite{Jolliffe1986}）中，同样的“模型选择”问题也依然存在。
    据我们所知，在异构噪声情况下选择正确的主成分数量仍然是一个活跃的研究课题
\cite{hong2020selecting}。}，正如我们在
\Cref{ch:denoising} 的 \Cref{sec:chap4-representation-learning-problem} 中所见。
为了验证学习到的特征是否
一致（即既不高估也不低估
内在的数据结构），我们可以考虑学习一个解码器
$g(\cdot,\eta): \z \mapsto  \x$，将表征 $\Z =
...
“判别器”，通过简单地最大化距离 $d(\X, \hat \X)$，
来\emph{放大}所有 $\X_k$ 和 $\hat \X_k$ 对之间的任何差异：
\begin{equation}
...
在某种程度上，这衡量了解码后的 $\hat \X$ 与原始数据 $\X$
对齐的好坏——从而
衡量了编码器 $f$ 的“单射性（injectivity）”的好坏。请注意，
这种判别性度量与 GAN
\eqref{eqn:GAN} 的思想是一致的，后者试图将 $\X$ 和 $\hat \X$ 分成两类，
并通过交叉熵来衡量。
...
将编码器 $f$ 视为一个广义的判别器，它取代了
\eqref{eqn:GAN} 中的二元分类器 $d$：
\begin{equation}
...
\begin{figure}[t]
    ...
    \caption{{\bf %
        闭环转录。} 编码器 $f$ 具有双重作用：它
        通过最大化 $\z$ 的率衰减来为数据 $\x$ 学习表征 $\z$，
        同时它也是一个“反馈传感器”，用于检测
        数据 $\x$ 和解码后的 $\hat \x$ 之间的任何差异。
        解码器 $g$ 也具有双重作用：它是一个“控制器”，用于
        纠正 $\x$ 和 $\hat \x$ 之间的差异，同时它也
        旨在最小化学习到的表征的整体编码率。} \label{fig:auto-encoding-closed}
\end{figure}

\paragraph{作为双人博弈的编码器和解码器。}
显然，为了确保学习到的自编码是自一致的，
解码器 $g(\cdot, \bm \eta)$ 的主要目标是\emph{最小化}
...
\begin{example}
    人们可能会想，为什么我们需要映射 $f(\cdot, \bm \theta)$
    通过最大化 $\max_{\bm \theta} \Delta R_\epsilon\big(f(\X,\bm \theta), f(\hat \X, \bm \theta)\big)$
    来充当 $\X$ 和 $\hat \X$ 之间的判别器。
    \Cref{fig:decoder} 给出了一个
    简单的说明：可能存在许多解码器 $g$，使得
    $f\circ g$ 是一个恒等（Id）映射。对于特征空间中子空间 $S_{\z}$ 内的
    所有 $\z$，$f\circ g(\z) = \z $。然而，
    $g\circ f$ 并不一定是原始分布 $S_{\x}$（为简单起见，此处画为子空间）中 $\x$ 的
    自编码映射。
    也就是说，$g\circ f(S_{\x}) \not\subset S_{\x}$，更不用说
    $g\circ f(S_{\x}) = S_{\x}$ 或 $g\circ f(\x) = \x$ 了。我们
    应该预料到，如果不仔细控制 $g$ 的像（image），
    这种情况很可能会发生，特别是当
    $\x$ 分布的支撑集在原始高维数据空间中
    是极低维的时候。
\end{example}
...
比较 \eqref{eqn:min-distance} 和 \eqref{eqn:max-distance} 在相同
距离度量上的收缩（contractive）和对比（contrastive）性质，
我们自然地将编码器 $f(\cdot, \bm \theta)$ 和解码器 $g(\cdot, \bm \eta)$ 的作用
视为“{\bf 一场双人博弈}”：\emph{编码器 $f$ 试图放大
    原始数据与其转录数据之间的差异，
...
请注意，这仅仅衡量了原始数据（的特征）
与其转录版本之间的差异。它并没有衡量
表征 $\Z$（或 $\hat \Z$）对于 $\X$（或 $\hat \X$）中的多个类别
有多好。为此，我们可以将
上述距离与原始的 MCR$^2$ 类型目标
\eqref{eq:mcr2-formulation} 结合起来：即为 $\X$ 学习到的 LDR $\Z$ 的率衰减 $\Delta R_\epsilon(\Z)$，
以及为解码后的 $\hat \X$ 学习到的 $\hat \Z$ 的率衰减 $\Delta R_\epsilon(\hat \Z)$。请注意，
尽管编码器 $f$ 试图\emph{最大化}数据 $\X$ 的特征 $\Z$ 的多类率衰减，
但解码器 $g$
应该\emph{最小化}解码后的 $\hat \X$ 的多类特征 $\hat \Z$ 的率衰减。
也就是说，解码器 $g$ 试图
使用实现良好解码质量所需的最小编码率。
...
或者将所有这些项固定为常数，上述目标
正是 \Cref{ch:denoising} 中引入的原始 MCR$^2$ 目标。
在无监督设置中，如果我们将每个
样本（及其数据增强）视为其自己的类别，上述
公式保持完全相同。类别数 $k$ 仅仅是
独立样本的数量。此外，请注意，
上述博弈的目标函数仅依赖于数据 $\X$（的特征），
因此可以在不需要采样或匹配任何额外分布（正如 GAN 或 VAE 中通常需要的那样）的情况下，
学习编码器和解码器（参数）。
...
目标就是将所有类别的联合分布与其
解码后的 $\hat \X$ 对齐。这通常是一个比多类任务 \eqref{eq:MCR2-GAN-objective} 更容易实现的
任务，因为它不
需要为数据学习更精细的多类 CTRL，
正如我们稍后将在实验中看到的那样。请注意，
上述公式的一个良好特征是\emph{目标中的所有量
都是根据学习到的特征的率衰减来衡量的}
（假设特征最终变成子空间高斯分布）。
...
而将解码网络 $g$ 视为用于误差
校正的“控制器”。然而，请注意，这里的控制“目标”
不是标量也不是有限维向量，而是一个连续的
分布——为了使 $\hat \x$ 的分布与
数据 $\x$ 的分布相匹配。这通常是一个在
无限维空间中的控制问题。$f$ 试图建模的子流形的可能微分同胚（diffeomorphisms）空间
是无限维的
...
误差反馈传感器和减少误差的控制器之间的博弈。

剩下的问题是，上述框架是否确实能够
学习到给定数据集的良好（自编码）表征？在
我们给出一些正式的理论证明（在下一
小节中）之前，我们先展示一些经验结果。

\paragraph{可视化特征 $\Z$ 和解码特征 $\hat \Z$ 的相关性。}
我们可视化了在 MNIST、CIFAR-10 和 ImageNet（10 个类别）上从多类目标
\eqref{eq:MCR2-GAN-objective} 学习到的 $\Z$
和 $\hat{\Z}$ 之间的余弦相似度，
这表明了 $\hat{\z} = f\circ g(\z)$ 与 $\z$ 有多接近。
...
我们在 MNIST、CIFAR-10 和 ImageNet（10 个类别）上比较了一些具有代表性的 $\X$ 和 $\hat{\X}$，
以验证 $\hat \x = g\circ f(\x)$ 与 $\x$ 有多接近。
结果显示在
\Cref{fig:justify_xhat_equals_x} 中，可视化是从
训练样本中创建的。从视觉上看，自编码的 $\hat \x$ 忠实地
捕捉了其各自训练样本 $\x$ 的主要视觉特征，
特别是姿态、形状和布局。对于更简单的数据集
如 MNIST，自编码图像几乎与原始图像完全相同。
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{在 MNIST、CIFAR-10 和 ImageNet 上可视化学习到的
        闭环转录的自编码属性 \mbox{($\x \approx \hat{\x} = g\circ
        f(\x)$)}（放大以获得更好的
    可视化效果）。}
...
分布，例如通过学习将 \(\vX\) 编码为
支撑在\textit{正交}子空间混合上的表征 \(\vZ\)，
以及将 \(\vZ\) 解码回
\(\vX\)。那么，我们想要实现的表征可以通过
前面讨论过的信息增益版本来最大化，即 \(
    \Delta R_{\epsilon}(\vZ) = R_{\epsilon}(\vZ) - \sum_{k=1}^K
R_{\epsilon}(\bm Z_k) \)，这强制要求每个类别 \(k\) 的表征
\(\vZ_{k}\) 张成一个与
其他类别的支撑子空间正交的子空间。衡量
解码一致性的方法，如前所述，由 \(\sum_{k =
1}^{K}\Delta R_\epsilon(\vZ_{k}, \hat{\vZ}_{k})\) 给出，这强制要求
每个类别 \(k\) 的表征 \(\vZ_{k}\) 及其自编码
...
\end{equation}
请注意，这比实践中使用的设置更简单——
例如，没有 \(\Delta R_\epsilon(\hat{\vZ})\) 项，
并且我们在带有类别标签的监督设置中工作
（尽管用于证明以下结果的技术很容易
扩展到无监督公式）。此外，表征的
一致性仅通过 \(\Delta R_\epsilon\) 在分布意义上进行衡量
（尽管如果需要，这可以替换为
样本级距离度量，如 \(\ell_{2}\) 范数，
并且可以得出等效的结论，\textit{作必要修改即可}）。

在温和的条件下，为了实现所需的编码器和
解码器，从已经服从相关低维高斯混合分布的数据源 \(\vX\) 中
实现 \(\vZ\)，
我们只需要一个线性编码器和
...
解码器的最优解完全相对于编码器来定义
（特别是，不相对于解码器的某些其他内在
属性），即解码器跟随编码器的\textit{斯塔克尔伯格均衡}。 %
也就是说，在这样的均衡下，解码器应该优化其
...
学习率。序贯均衡的详细阐述
超出了本书的范围，我们在
\Cref{sec:minimax} 中提供了更多技术细节。在这种设置下，我们有以下结果：
\begin{theorem}[\cite{pai2022pursuit}，节选]\label{thm:ctrl_theory}
    假设 \(\vX\) 分布在子空间混合上。
    在某些现实但技术性的条件下，
    \eqref{eq:ctrl_msp_game} 的所有序贯均衡都服从：
    \begin{itemize}
        \item \(\vZ_{k}\) 位于正交子空间上，并且
            在这些子空间上是各向同性的，即最大化了信息增益。
        \item 自编码是自一致的，即
            对于所有 \(k\)，\(\vZ_{k}\) 和 \(\hat{\vZ}_{k}\) 张成的
            子空间是相同的。
    \end{itemize}
\end{theorem}
如果对数据只有几何假设，即没有统计假设，
这种自一致性的概念是人们所能期望的最好的结果。
如果我们假设 \(\vX\) 的列
提取自低秩高斯混合模型，那么该定理的类似版本
证明了 \(\vZ_{k}\) 也是低秩高斯分布，
例如，其协方差是各向同性的。 %
本质上，这一结果通过子空间上高斯混合的简单情况验证了，
优化信息增益和自一致性的极小极大博弈
可以达到最优解。
...
都展示了为数百甚至数千个对象类别学习表征的强大
能力。
然而，网络通常必须离线训练，同时使用来自所有类别的
均匀采样数据。众所周知，
当一个（开环）网络在没有旧类别数据的情况下被更新以学习新类别时，
以前学到的知识将成为
\emph{灾难性遗忘（catastrophic forgetting）}问题的牺牲品
\cite{McCloskey1989catastrophic}。这在神经科学中被称为
稳定性-可塑性困境（stability-plasticity dilemma）：确保
神经系统能够从新环境中学习，同时保留
以前环境中的基本知识的挑战 \cite{Grossberg1987CompetitiveLF}。

相比之下，自然神经系统（例如动物大脑）似乎根本不
受这种灾难性遗忘的困扰。它们能够
在保留以前学过的对象记忆的同时，发展对新对象的新记忆。
这种能力，无论是对于自然神经系统还是
人工神经系统，通常被称为\emph{增量学习（incremental learning）}、
\emph{持续学习（continual learning）}、\emph{序贯学习（sequential learning）}或\emph{终身
...
\begin{figure*}[t]
    ...
    \caption{我们基于闭环转录的结构化 LDR 记忆增量学习的\textbf{整体框架}。
        ...
        其约束条件是过去类别的旧记忆
        $\Z_{old}$ 通过闭环转录
        （或回放）保持完整：$\Z_{old} \approx \hat{\Z}_{ old} = f(g(\Z_{ old}))$。
    \vspace{-0.2in}}
    \label{fig:framework}
\end{figure*}
...
请注意，利用学习到的自编码
\eqref{eqn:autoencoding-ch6}，人们可以回放并使用与记忆特征相关的图像（例如
$\hat\X_{old} = g(\Z_{old}, \bm \eta)$），
以避免在学习新类别时发生遗忘。这
...
考虑增量学习一个新对象类别的任务。\footnote{当然，人们也可以考虑任务包含一小批新类别的更一般设置，而无需进行重大修改。}我们将相应的新样本集表示为
$\X_{new}$。$\X_{new}$ 的特征表示为 $\Z_{new}(\bm
\theta) = f(\X_{new}, \bm \theta)$。我们将它们与
旧类别的原型特征 $\Z_{old}$ 拼接在一起，形成 $\Z =
[\Z_{new}(\bm \theta), \Z_{old}]$。我们将所有特征回放的图像表示为
$\hat{\X} = [{\hat{\X}_{new}(\bm \theta,\bm \eta)},
{\hat{\X}_{old}(\bm \eta)}]$，尽管我们实际上不需要
显式地计算或使用它们。我们只需要回放图像的特征，
记为 $\hat{\Z} = f(\hat{\X}, \bm \theta) =
[{\hat{\Z}_{new}(\bm \theta,\bm \eta)}, {\hat{\Z}_{old}(\bm \theta,\bm \eta)}]$。

反映多类 CTRL 目标
\eqref{eq:MCR2-GAN-objective} 的动机，我们希望新类别的特征
$\Z_{new}$ 与所有旧特征 $\Z_{old}$ 不相干。
由于 $\Z_{new}$ 是唯一需要学习其特征的新类别，
...
修改为一个\emph{受约束的}极小极大博弈，我们称之为
\emph{增量闭环转录（incremental closed-loop transcription, i-CTRL）}。
这个博弈的目标与标准的多类
CTRL 目标 \eqref{eq:MCR2-GAN-objective} 相同，但仅仅包含一个
额外的约束：
\begin{eqnarray}
...
在实践中，受约束的极小极大规划可以通过在编码器 $f(\cdot, \bm \theta)$ 和解码器 $g(\cdot, \bm \eta)$ 之间进行\emph{交替}
最小化和最大化来求解，如下所示：
\begin{eqnarray}
    &\max_{\bm \theta}
...
我们还引入了另一个系数 $\lambda$，用于
加权与新数据相关的率衰减项。

\paragraph{通过增量复习实现联合最优记忆。}
正如我们将看到的，上述受约束的极小极大规划已经可以
在增量学习中实现最先进的性能。
然而，为\emph{所有类别}开发最优记忆
不能仅仅依赖于优雅的遗忘。即使对于人类来说，如果一个
对象类别只学习一次，我们应该预料到，随着我们继续学习新的类别，
学习到的记忆会逐渐消退，除非记忆
可以通过复习旧的对象类别来巩固。 %

为了模拟记忆形成的这个阶段，在增量学习了
整个数据集之后，我们可以回过头来再次复习所有类别，一次
复习一个类别。我们将遍历所有类别一次称为一个
复习“周期（cycle）”。\footnote{以区别于传统联合学习设置中使用的术语“轮次（epoch）”。}如果需要，可以进行多个
复习周期。完全可以预料到，
复习可以改善学习到的（LDR）记忆。但有些
令人惊讶的是，闭环框架甚至允许我们在
“类无监督（class-unsupervised）”的方式下进行复习：当复习旧类别（例如 $\X_j$）的数据时，
系统不需要类别标签，可以简单地
将 $\X_j$ 视为新类别 $\X_{new}$。也就是说，系统优化
相同的受约束极小极大规划 \eqref{eqn:constrained-minimax}，
