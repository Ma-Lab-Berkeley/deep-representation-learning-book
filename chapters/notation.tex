...
\begin{itemize}
    ...
    \item \(\ell^{p}\) norms on vectors are denoted by \(\norm{\vx}_{p}\), \(p \in \{0\} \cup [1, \infty]\).
    ...
    \item Transpose is \(\top\), e.g., \(\vA^{\top}\). Pseudo-inverse is \(\dagger\), e.g., \(\vA^{\dagger}\).
    ...
    \item Element-wise multiplication is \(\vX \odot \vY\). Element-wise squaring is \(\vX^{\odot 2}\).
...
\begin{itemize}
    ...
    \item When \(f\) and \(g\) are implemented as neural networks, they will have the same number of layers without loss of generality; we write them as \(f = f^{L} \circ f^{L - 1} \circ \cdots \circ f^{2} \circ f^{1} \circ f^{\pre}\) and \(g = g^{\post} \circ g^{1} \circ g^{2} \circ \cdots \circ g^{L - 1} \circ g^{L}\). \DP{\(g^{\ell} \circ f^{\ell} \approx \id\)} 
\end{itemize}
