...
\end{enumerate}
In \Cref{ch:compression}, we have developed a complete computational solution to
...
and we have defined a compression-based objective
function, the information gain, whose maximization leads to compact and
structured representations.
In \Cref{ch:unrolling}, we have seen that the layers of popular deep
network architectures can be interpreted as incremental optimization steps
towards maximizing the information gain.
Thus all the conceptual and computational ingredients are in place to present
a solution to the representation learning problem, which we will take up in this
chapter.

First, however, there is a natural question to resolve that will guide our
development of representation learning methodology in this chapter:
...
reconstruct the data $\vX$ from the representation $\vZ$, then we have technically
learned the data distribution. In fact, many of the classical representation
...
comes down to \textit{efficiency}: it is often far more
effective (in terms of sample and compute efficiency, for example) to learn
a compact representation for the data distribution as a type of `preparation'
of the distribution,
then to perform distribution learning on the so-learned representation. This is
in fact the dominant and most successful method in modern practice, as we will
see in this chapter. And as we have seen in both \Cref{ch:compression} and
\Cref{ch:unrolling}, the denoising operators at the heart of the
diffusion-denoising process take on a highly tractable form once the data
distribution has been transformed to a linear discriminative representation
$\vZ$, as maximization of the information gain promotes. 
%We are thus led to a unifying view of the diffusion-denoising and
%compression methodologies we have developed, as a practical basis for learning
%representations of high-dimensional data distributions.
...
encoding and decoding processes, is a natural framework for learning such a representation. We have studied some important special cases in \Cref{ch:classic}. In
...
In many practical and natural learning scenarios, it can be difficult or even impossible to compare distributions of the data $\X$ and $\hat \X$. We are left with the only option of comparing the learned feature $\Z$ with its image $\hat \Z$ under the encoder $f$: 
\begin{equation}
 \X
...
\sdb{Rewrite the exposition in this section. We should be more explicit about
measuring compactness with the rate reduction, given our exposition in the
new beginning.}
...
compact and more structured than $\X$. First, for compactness, $\Z$
...
\begin{equation}
  \Delta R_{\epsilon}(\Z)
\end{equation}
introduced in \Cref{subsec:MCR2}. Second, the main purpose of
...
\sdb{We probably want to condense this section and the next one? Making it
similar in length to the manifold flattening or VAE parts? Also adjust the
previous transition when doing so.}
...
\sdb{We should rewrite this section to be a much abbreviated version, just
presenting VAE as a vignette about one way to enforce structure on the
representation in a learning-driven way.}
...
variable model $\vz$. Indeed, by conditioning we may write $p(\vx,
\vz ;\, \vtheta)
...
In the variational autoencoding framework, we navigate this trade-off through
three key insights:
\begin{enumerate}
