...
On the other hand, the linear model can be limited when dealing with real-world applications, especially when the input data $\bm{x}$ is complex, such as speech and natural languages, images and videos, and robotic motions. The low-dim distributions of such data are typically {\em nonlinear}. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.45\linewidth]{\toplevelprefix/chapters/chapter5/figs/MoE.png} \hspace{5mm}
...
\bar{\bm \Sigma}(\bar\z) &\doteq 
    \left[\begin{matrix}
    \circm(\z[1]) \\ \vdots \\ \circm(\z[C]) \end{matrix} \right]\left[\begin{matrix}\circm(\z[1])^\top,\ldots, \circm(\z[C])^\top\end{matrix} \right] \in \Re^{dC\times dC},
...
\colon \X \in \bR^{D \times N} \to \Z\in \bR^{d \times N}\) which transforms input tokens \(\{\bm x_i\}_{i=1}^N \subset \R^D\) with a potentially nonlinear and multi-modal distribution to a (piecewise) \textit{linearized and compact} token representations \(\{\bm z_i\}_{i=1}^N \subset \bR^{d}\). While the {\em joint} distribution of tokens representations \(\{\z_{i}\}_{i = 1}^{N}\) may be sophisticated (and task-specific), we further contend that it is reasonable and practical to
require that the target {\em marginal} distribution of individual token representations should be highly compressed and structured, amenable for compact coding. Particularly, we require the distribution to be \textit{a mixture of low-dimensional (say \(K\)) Gaussian
...
\begin{figure}[t]
    \centering \includegraphics[width=1\textwidth,trim={0 5.0cm 0 0},clip]{\toplevelprefix/chapters/chapter5/figs/V-CRATE.pdf}
    \vspace{-1mm}
...
\begin{table}[t!]
        \small
        \caption{\small Zero-shot validation loss of the Causal-CRATE-Base model and GPT2-Small, GPT2-Base model evaluated on the test split of the datasets (lower is better). 
        }
        \centering
...
\yima{Causal version of ToST: relations to nonlinear filtering, RNNs, and other linear time architectures...}
...
\begin{exercise}
    Derive a causal version of the TSSA operator. Implement it and compare its performance to CRATE on a causal language modeling task.
\end{exercise}
