...
\begin{equation}
	\x \mapsto \hat \x
\end{equation}
such that the expected error caused by the quantization is bounded by $\epsilon$: 
\begin{equation}
	\Ex[d(\vx, \hat \vx)] \le \epsilon.
\end{equation}
Typically, the distance is chosen to be the Euclidean distance, or the 2-norm.
...
\begin{equation}
	\cR_{\epsilon}(\x) = \min_{p(\hat \x \mid \x): \Ex[d(\x, \hat \x)] \le \epsilon} H(\x) - H(\x \mid \hat{\x}),
    \label{eqn:rate-distortion-general}
\end{equation}
where minimization is over all conditional distributions $p(\hat \x \mid \x)$ that satisfy the distortion constraint $\Ex_{\x, \hat \x}[d(\x, \hat \x)] \le \epsilon$. From the definition, we know that $\cR_{\epsilon}(\x)$ is a {\em non-increasing} function in $\epsilon$.
...
\begin{remark}
    ...
    \begin{equation}
		\cR_{\epsilon}(\x) = \min_{p(\hat \vx \mid \vx): \mathbb{E} [d(\x, \hat \x)] \le \epsilon} H(\hat \x) - H(\hat \x \mid \x).  \label{eqn:rate-distortion-general-2}
	\end{equation}
\end{remark}
...
\begin{remark}
	For small but finite \(\epsilon\), working with the distribution of the lossy encoding \(\hat \vx\) instead of the raw data \(\vx\) is much easier. While it approximates the distribution of \(\vx\), it is discrete, hence having a well-defined entropy; it also has a well-defined volume. Thus, this finite rate distortion approach via sphere packing re-enables or generalizes all previous measures of complexity of the distribution, allowing us to differentiate between and rank different distributions in a unified way.
...
Now suppose we are given a set of data samples $\X = \{\x_1, \ldots, \x_N\}$ from any distribution.\footnote{Or these data points could be viewed as an (empirical) distribution themselves.} We would like to come up with a constructive scheme that can encode the data up to certain precision, say
\begin{equation}
