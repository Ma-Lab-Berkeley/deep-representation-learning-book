...
Nevertheless, in this methodology the encoding of the distribution is implicit in the denoiser's functional form and parameters, if any. In fact, acute readers might have noticed that, for a general distribution, we have never explicitly specified what the functional form for the denoiser is. In practice, people typically model it by some deep neural network with an empirically designed architecture. In addition, although we know the above denoising process reduces the entropy, we do not know by how much,  nor do we know the entropy of the intermediate and resulting distributions. In fact, acute readers might have noticed that our general goal is to identify and model a (continuous) distribution with low-dimensional supports. The differential entropy of such distributions is always negative infinity or the volume of their supports are always zero. Then among all distributions that can generate the same data samples, how can we compare which one is better? This section aims to address these important matters.

In the remainder of this chapter, we discuss a framework that allows us to associate the model of the learned distributions with an explicit computable encoding and decoding scheme, following the second approach suggested at the end of \Cref{sub:min_entropy}. As we will see, such an approach essentially allows us to accurately approximate the entropy of the learned distributions in terms of a (lossy) coding length or coding rate associated with the coding scheme. With such a measure, not only can we accurately measure how much the entropy is reduced, hence information gained, by any processing (including denoising) of the distribution, but also we can derive explicit form of the optimal operator that can conduct such operations in the most efficient way. As we will see in the next chapter \ref{ch:representation}, this will lead to a principled explanation for the architecture of deep networks, as well as to more efficient deep-architecture designs.

\subsection{Necessity for Lossy Coding}
...
Without loss of generality, assume that the distribution $\mathcal D$ of the random vector $\x$ is supported on a mixture of distributions, i.e., $\mathcal D = \cup_{j=1}^k \mathcal{D}_j$,  where each $\mathcal{D}_j \subset \Re^D$ has a low intrinsic dimension in the high-dimensional ambient space $\Re^D$. Let $\bm X_j \in \Re^{D\times N_j}$ denote the data matrix whose columns are samples drawn from the distribution $\mathcal{D}_j$, where $N_j$ denotes the number of samples for each $j=1,\dots,k$. Then, we use $\bm X=[\bm X_1,\dots,\bm X_k] \in \Re^{D\times N}$ to denote all the samples, where $N=\sum_{j=1}^kN_j$. 
...
\end{equation}
the input samples are mapped to $\vz_i = f(\vx_i)$ for each $i=1,\dots,N$. With an abuse of notation, we also write $\Z_j = f(\X_j)$ and $\Z = f(\X)$. Therefore, we have $\Z = [\bm Z_1,\dots,\bm Z_k]$ and $\Z = [\vz_1,\dots\vz_N]$. 
...
Recall that the columns of $\Z_j$ denotes the features of the samples in $\X_j$ for each $j=1,\dots,k$. The coding rate for the features in $\bm Z_j$ can be computed as follows: 
\begin{align}
    R_{\epsilon}(\Z_j) = \frac{N_j}{2N}\log\det\left(\I + \frac{d}{N_j\epsilon^{2}}\Z_j\Z_j^{\top}\right)
\end{align}
Then, the sum of the average coding rates of features in each class is 
\begin{equation}
	 R_{\epsilon}^c(\Z) \doteq \sum_{j=1}^k R_{\epsilon}(\Z_j),
...
\begin{equation}
	\begin{aligned}
		...
		\mbox{s.t.} \ \ \, \Z & = f(\bm X, \bm \theta),\  \|\Z_j\|_F^2                                          = N_j,\ j=1,\dots,k. 
	\end{aligned}
...
\begin{remark}
    Note that the above MCR$^2$  principle is designed for supervised learning problems, where the group memberships (or class labels) are known. However, this principle can be naturally extended to unsupervised learning problems by introducing a membership matrix, which encodes the (potentially soft) assignment of each data point to latent groups or clusters. Specifically, let $\bm \Pi = \{\bm \Pi^j\}_{j=1}^k \subset \R^{N\times N}$ be a set of diagonal matrices whose diagonal entries encode the membership of the $N$ samples into $k$ classes. That is, $\bm \Pi$ lies in a simplex $\Omega := \{\bm \Pi: \bm \Pi^j \ge \bm 0: \sum_{j=1}^k \bm \Pi^j = \bm I_N\}$. Then, we can define the average coding rate with respect to the partition $\bm \Pi$ as
    \begin{align*}
        R_{\epsilon}^c(\Z \vert \bm \Pi) := \sum_{j=1}^k \frac{\mathrm{tr}(\bm \Pi^j)}{2N}\log\det\left(\bm I + \frac{d}{\mathrm{tr}(\bm \Pi^j)\epsilon^2}\bm Z\bm \Pi^j \bm Z^\top \right).
    \end{align*}
    When $\bm Z$ is given, $R_{\epsilon}^c(\Z \vert \bm \Pi)$ is a concave function of $\bm \Pi$. Then the MCR$^2$ principle for unsupervised learning problems is defined as follows:
    \begin{align*}
        \max_{\bm \Pi, \bm \theta}\Delta R_{\epsilon}\big(\Z \big\vert \bm \Pi) := R_{\epsilon}(\bm Z) - R_{\epsilon}^c(\Z \vert \bm \Pi)\qquad \mathrm{s.t.}\quad \bm Z = f(\bm X, \bm \theta),\ \|\bm Z_j\|_F^2 = N_j,\ j = 1,\dots,k,\ \bm\Pi \in \Omega. 
    \end{align*}
\end{remark}
...
\begin{theorem}[\bf Characterization of Global Optimal Solutions]
	...
	\begin{itemize}
		\item {\em Between-Class Discriminative}: As long as the ambient space is adequately large ($d \ge \sum_{j=1}^{k} d_j$), the subspaces are all orthogonal to each other, {\em i.e.} $(\Z_{i}^*)^{\top} \Z_{j}^* = \bm{0}$ for $i \not= j$.
		\item {\em Maximally Diverse Representation}:
		      As long as the coding precision is adequately high, i.e., $\epsilon ^4 < c\cdot \min_{j}\left\{ \frac{N_j}{N}\frac{d^2}{d_j^2}\right\}$, where $c>0$ is a constant. Each subspace achieves its maximal dimension, i.e. $\mathrm{rank}{(\Z_{j}^*)}= d_j$. In addition, the $d_j$ singular values of $\Z_{i}^*$ are equal.
...
\begin{example}[Classification of Images on CIFAR-10]
	...
	\begin{figure*}[h]
		\begin{center}
			...
			\includegraphics[width=0.42\textwidth]{figs_chap4/heatmap_mcr2.pdf}
			\hspace{0.25cm}
			\includegraphics[width=0.42\textwidth]{figs_chap4/heatmap_ce.png}
			\caption{\small Cosine similarity between learned features by using the MCR$^2$ objective  (\textbf{left}) and CE loss (\textbf{right}).}
			\label{fig:heatmap-plot}
		\end{center}
		\vskip -0.1in
