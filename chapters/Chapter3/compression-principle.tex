...
\subsection{Properties of Diffusion  and Denoising Processes}
...
\subsection{Learning and Sampling a  Distribution via Iterative Denoising}
...
\begin{algorithm}[htpb]
    \begin{algorithmic}[1]
        ...
        \Require{A distribution over timesteps \(t \in [0, T]\) to train on.\yima{any distribution? what do you gain from being general? Why not a fixed time step?}} 
        \Require{A weighting function \(w \colon [0, T] \to \R_{\geq 0}\).\yima{What is the role or benefit of a general weight? Never explained, or introduced in the main text. Why not simply a uniform one?}}
        \Require{Scale and noise level functions \(\alpha, \sigma \colon [0, T] \to \R_{\geq 0}\). \yima{Any functions? How are choices of such functions related to the weight and time distribution above? Theoretically necessary or practically beneficial to choose different functions? None of these are motivated or explained.}}
        \Require{A parametric function class \(\cF = \{\bar{\vx}_{\theta} \colon \theta \in \Theta\}\) with the parameter space \(\Theta\).}
