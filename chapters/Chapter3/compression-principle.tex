...
\end{equation}
the set of samples are mapped to $\Z = f(\X)$ and $\Z = \cup_{j=1}^k \Z_j$ where $\Z_j = f(\X_j)$. On one hand, for learned features to be discriminative, features of different classes/clusters are preferred to be {\em maximally incoherent} to each other. Hence, they together should span a space of the largest possible volume (or dimension) and the coding rate of the whole set $\Z$ should be as large as possible. On the other hand, learned features of the same class/cluster should be highly correlated and coherent. Hence, each class/cluster should only span a space (or subspace) of a very small volume and the coding rate should be as small as possible. 

\paragraph{Coding Rate of Features.} However, a practical challenge in evaluating the coding rate is that the underlying distribution of the feature representations $\Z$ is typically unknown. For the set of features $\Z = [\z_1, \ldots, \z_N]$  with $\z_{i} = f(\x_i, \bm\theta) \in \R^{d}, i = 1,\ldots, N$, we may simply view them all together as samples from a multivariate Gaussian distribution. As discussed in Chapter \ref{subsec:lossy DR}, the compactness of learned features $\Z$ {\em as a whole} can be measured in terms of the average coding length per sample (as the sample size $N$ is large),  a.k.a. the {\em coding rate} subject to a precision level $\epsilon > 0$ (see \eqref{eqn:rate-Gaussian}):
\begin{equation}
