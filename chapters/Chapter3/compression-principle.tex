...
\begin{equation}
	\cR_{\epsilon}(\x) = \min_{p(\x, \hat \x): \mathbb{E} [d(\x, \hat \x)] \le \epsilon} H(\x) - H(\x \mid \hat{\x}),
    \label{eqn:rate-distortion-general}
\end{equation}
where minimization is over all (joint) distributions $p(\x, \hat{\x})$ that satisfy  the distortion constraint $\mathbb{E}[d(\x, \hat \x)] \le \epsilon$. From the definition, we know that $\cR_{\epsilon}(\x)$ is a {\em non-increasing} function in $\epsilon$.
...
\begin{remark}
    ...
    \begin{equation}
	\cR_{\epsilon}(\x) = \min_{p(\x, \hat \x): \mathbb{E} [d(\x, \hat \x)] \le \epsilon} H(\x) - H(\x \mid \hat{\x}).  \label{eqn:rate-distortion-general-2}
\end{equation}
\end{remark}
...
\begin{figure}
	...
	\caption{Packing $\epsilon$-balls in the region spanned by the data vectors. The larger the volume of the space, the more balls are needed hence more bits are needed to encode and enumerate the balls. Each real-valued vector $\vx$ can be encoded as the number of the ball which it falls into.}
