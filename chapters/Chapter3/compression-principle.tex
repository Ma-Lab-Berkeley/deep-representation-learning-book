...
\begin{remark}
    ...
    \begin{align}\label{eq:MCRc}
        R_{\epsilon}^c(\Z \mid \bm \Pi) := \sum_{j=1}^k \frac{\mathrm{tr}(\bm \Pi_j)}{2N}\log\det\left(\bm I + \frac{d}{\mathrm{tr}(\bm \Pi_j)\epsilon^2}\bm Z\bm \Pi_j \bm Z^\top \right).
    \end{align}
    ...
    \begin{align*}
        \max_{\bm \Pi, \bm \theta} \Delta R_{\epsilon}\big(\Z  \mid \bm \Pi) := R_{\epsilon}(\bm Z) - R_{\epsilon}^c(\Z \mid \bm \Pi)\qquad \mathrm{s.t.}\quad \bm Z = f(\bm X, \bm \theta),\ \|\bm Z \bm \Pi_j\|_F^2 = N_j,\ j = 1,\dots,k,\ \bm\Pi \in \Omega. 
    \end{align*}
...
\begin{example}[Classification of Images on CIFAR-10]
	...
	Figure~\ref{fig:train-test-loss-pca-1} illustrates how the two rates and their difference (for both training and test data) evolves over epochs of training: After an initial phase, $R_\epsilon$ gradually increases while $R^c_\epsilon$ decreases, indicating that features $\bm Z$ are expanding as a whole while each class $\bm Z_j$ is being compressed.
