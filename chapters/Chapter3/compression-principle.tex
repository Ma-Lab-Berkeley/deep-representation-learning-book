...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\Require{Scale and noise level functions \(\alpha, \sigma \colon \{t_{\ell}\}_{\ell = 0}^{L} \to \R_{\geq 0}\).}
		\Require{A parameter space \(\bm\Theta\) and a denoiser architecture \(\bar{\vx}_{\bm \theta}\).}
		...
		\Ensure{A trained denoiser \(\bar{\vx}_{\bm\theta^{\ast}}\).}

		\Function{TrainDenoiser}{$\cD, \bm\Theta$}
		\State{Initialize \(\bm\theta^{(1)} \in \bm\Theta\)}
		\For{\(i \in [M]\)}
		\For{\(n \in [N]\)}
		...
		\State{\(\vx_{t, n}^{(i)} := \alpha_{t_{n}^{(i)}}\vx_{n}^{(i)} + \sigma_{t_{n}^{(i)}}\vg_{n}^{(i)}\)} \Comment{Compute the noised sample.}
		\State{\(w_{n}^{(i)} := w_{t_{n}^{(i)}}\)} \Comment{Compute the loss weight.}
		\EndFor
		\State{\(\hat{\cL}^{(i)} := \displaystyle \frac{1}{N}\sum_{n = 1}^{N}w_{n}^{(i)}\norm{\vx_{n}^{(i)} - \bar{\vx}_{\bm\theta^{(i)}}(t_{n}^{(i)}, \vx_{t, n}^{(i)})}_{2}^{2}\)} \Comment{Compute the loss estimate.}
		\State{\(\bm\theta^{(i + 1)} := \texttt{OptimizationUpdate}^{(i)}(\bm\theta^{(i)}, \nabla_{\bm\theta^{(i)}}\hat{\cL}^{(i)})\)} \Comment{Update parameters.}
		\EndFor
