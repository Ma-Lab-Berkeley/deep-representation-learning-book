...
\subsection{Introduction to Diffusion and Denoising Processes} \label{sub:intro_diffusion_denoising}
...
\begin{example}[Denoising Gaussian Noise from a Mixture of Gaussians]\label{example:denoising_gaussian_mixture}
	...
	\end{equation}
	Let us define \(\phi(\vx \mid \vmu, \vSigma)\) as the probability density of \(\dNorm(\vmu, \vSigma)\) evaluated at \(\vx\). In this notation, the density of \(\vx_{t}\) is 
	\begin{equation}
		p_{t}(\vx_{t}) = \sum_{k = 1}^{K}\pi_{k}\phi(\vx_{t} \mid \vmu_{k}, \vSigma_{k} + t^{2}\vI).
	\end{equation}
	...
	\end{equation}
	This shows that \(\vx\) and \(\vx_{t}\) are jointly Gaussian (conditioned on \(\vy\)) as claimed. Thus we can write
	\begin{equation}
		\mat{\vx \\ \vx_{t}} \sim \dNorm\rp{\mat{\vmu_{y} \\ \vmu_{y}}, \mat{\vSigma_{y} & \vSigma_{y} \\ \vSigma_{y} & \vSigma_{y} + t^{2}\vI}}.
	\end{equation}
	Thus the conditional expectation of \(\vx\) given \(\vx_{t}\) (i.e., the Bayes optimal denoiser conditioned on \(\vy\)) is famously
	\begin{equation}
	...
	\begin{align}
		...
		&= \frac{\pi_{k}\phi(\vx_{t} \mid \vmu_{k}, \vSigma_{k} + t^{2}\vI)}{\sum_{i = 1}^{K}\pi_{i}\phi(\vx_{t} \mid \vmu_{i}, \vSigma_{i} + t^{2}\vI)}.
	\end{align}
	...
	\begin{equation}\label{eq:gmm_bayes_optimal_denoiser}
		\bar{\vx}^{\ast}(t, \vx_{t}) = \sum_{k = 1}^{K}\frac{\pi_{k}\phi(\vx_{t} \mid \vmu_{k}, \vSigma_{k} + t^{2}\vI)}{\sum_{i = 1}^{K}\pi_{i}\phi(\vx_{t} \mid \vmu_{i}, \vSigma_{i} + t^{2}\vI)}\cdot\bp{\vmu_{k} + \vSigma_{k}(\vSigma_{k} + t^{2}\vI)^{-1}(\vx_{t} - \vmu_{k})}.
	\end{equation}
	...
	\yima{Can this special case be connected to how PCA is computed via power iteration in Chapter 2? }
...
\begin{figure}
	\centering 
	\includegraphics[width=\textwidth]{figs_chap3/ve_forward_diffusion_denoising.png}
	\caption{\small \textbf{Bayes optimal denoiser and score of a Gaussian mixture model.} In the same setting as \Cref{fig:fig:ve_forward_density}, we demonstrate the effect of the Bayes optimal denoiser \(\bar{\vx}^{\ast}\) by plotting \(\vx_{t}\) (red) and \(\bar{\vx}^{\ast}(t, \vx_{t})\) (green) for some choice \(t\) and \(\vx_{t}\). By Tweedie's formula \Cref{thm:tweedie}, the residual between them is proportional to the so-called (Hyv\"arinen) score \(\nabla_{\vx_{t}}\log p_{t}(\vx_{t})\). We can see that the score points towards the modes of the distribution of \(\vx_{t}\).}
...
\end{theorem}
\begin{proof}
	For the proof let us suppose that \(\vx\) has a density (even though the theorem is true without this assumption), and call this density \(p\). Let \(p_{0 \mid t}\) and \(p_{t \mid 0}\) be the conditional densities of \(\vx = \vx_{0}\) given \(\vx_{t}\) and \(\vx_{t}\) given \(\vx\) respectively. Let \(\phi(\vx \mid \vmu, \vSigma)\) be the density of \(\dNorm(\vmu, \vSigma)\) evaluated at \(\vx\), so that \(p_{t \mid 0}(\vx_{t} \mid \vx) = \phi(\vx_{t} \mid \vx, t^{2}\vI)\). Then a simple calculation gives 
	\begin{align}
		\nabla_{\vx_{t}} \log p_{t}(\vx_{t})
		&= \frac{\nabla_{\vx_{t}}p_{t}(\vx_{t})}{p_{t}(\vx_{t})} \\
		&= \frac{1}{p_{t}(\vx_{t})}\nabla_{\vx_{t}}\int_{\R^{D}}p(\vx)p_{t \mid 0}(\vx_{t} \mid \vx)\odif{\vx} \\
		&= \frac{1}{p_{t}(\vx_{t})}\nabla_{\vx_{t}}\int_{\R^{D}}p(\vx)\phi(\vx_{t} \mid \vx, t^{2}\vI)\odif{\vx} \\
		&= \frac{1}{p_{t}(\vx_{t})}\int_{\R^{D}}p(\vx)[\nabla_{\vx_{t}}\phi(\vx_{t} \mid \vx, t^{2}\vI)]\odif{\vx} \\
		&= \frac{1}{p_{t}(\vx_{t})}\int_{\R^{D}}p(\vx)\phi(\vx_{t} \mid \vx, t^{2}\vI)\bs{-\frac{\vx_{t} - \vx}{t^{2}}}\odif{\vx} \\
		&= \frac{1}{t^{2}p_{t}(\vx_{t})}\int_{\R^{D}}p(\vx)\phi(\vx_{t} \mid \vx, t^{2}\vI)[\vx - \vx_{t}]\odif{\vx} \\
		&= \frac{1}{t^{2}p_{t}(\vx_{t})}\int_{\R^{D}}p(\vx)\phi(\vx_{t} \mid \vx, t^{2}\vI)\vx \odif{\vx} - \frac{\vx_{t}}{t^{2}p_{t}(\vx_{t})}\int_{\R^{D}}p(\vx)\phi(\vx_{t} \mid \vx, t^{2}\vI)\odif{\vx} \\
	...
	\end{align}
	Simple rearranging proves the theorem.
\end{proof}
...
\begin{example}[Denoising a Two-Point Mixture]\label{example:denoising_twopoints}
	...
	\begin{equation}
		\bar{x}^{\ast}(t, x_{t}) = \frac{\phi(x_{t} \mid +1, t^{2}) - \phi(x_{t} \mid -1, t^{2})}{\phi(x_{t} \mid 1, t^{2}) + \phi(x_{t} \mid -1, t^{2})} = \tanh\rp{-\frac{x_{t}}{t^{2}}}.
	\end{equation}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{figs_chap3/ve_gmm_denoising.png}
	\caption{\small\textbf{Denoising a low-rank mixture of Gaussians.} \textit{Top left:} The original data, distributed on two lines and a plane, colored in red, orange, and gray respectively. \textit{Top middle:} The very noisy samples at timestep \(L = 500\), colored in blue. \textit{Top right, bottom left, bottom middle:} Visualizing the incremental denoising process, as the samples are pushed more and more towards the low-dimensional structures of the distribution. \textit{Bottom right:} The completely denoised copies of the noisy samples in the top middle panel. \DP{Make plots bigger, also check whether we should add the original data to the end instead of the beginning}}
...
\begin{theorem}[Simplified Version of \Cref{thm:conditioning_reduces_entropy}]
	...
	\end{equation}
\end{theorem}
The full statement of the theorem, and the proof itself, requires some technicality, so it is postponed to \Cref{sub:denoising_entropy_decreases}. \DP{Appendix in progress...}
...
\end{equation}
where \(\vU_{k} \in \O(D, P) \subseteq \R^{D \times P}\) is an orthogonal matrix. Then the optimal denoiser under \eqref{eq:additive_gaussian_noise_model} is (from \Cref{example:gmm_denoising})
\begin{equation}
	\bar{\vx}^{\ast}(t, \vx_{t}) = \sum_{k = 1}^{K}\frac{\phi(\vx_{t} \mid \vzero, \vU_{k}\vU_{k}^{\top} + t^{2}\vI)}{\sum_{i = 1}^{K}\phi(\vx_{t} \mid \vzero, \vU_{i}\vU_{i}^{\top} + t^{2}\vI)}\cdot\bp{\vU_{k}\vU_{k}^{\top}(\vU_{k}\vU_{k}^{\top} + t^{2}\vI)^{-1}\vx_{t}}.
\end{equation}
...
\begin{equation}\label{eq:sherman_morrison_woodbury}
	(\vA + \vU\vC\vV)^{-1} = \vA^{-1} - \vA^{-1}\vU(\vC^{-1} + \vV\vA^{-1}\vU)^{-1}\vV\vA^{-}.
\end{equation}
We apply this identity with \(\vA = t^{2}\vI\), \(\vU = \vU_{k}\), \(\vV = \vU_{k}^{\top}\), and \(\vC = \vI\), obtaining
\begin{align}
...
\begin{align}
	\frac{\phi(\vx_{t} \mid \vzero, \vU_{k}\vU_{k}^{\top} + t^{2}\vI)}{\sum_{i = 1}^{K}\phi(\vx_{t} \mid \vzero, \vU_{i}\vU_{i}^{\top} + t^{2}\vI)} 
...
\begin{figure}
	\centering 
	\includegraphics[width=0.75\textwidth]{figs_chap3/xT_vs_noise.png}
...
\end{equation}
The previous setup has \(\alpha_{t} = 1\) and \(\sigma_{t} = t\), and this is called the \textit{variance-exploding (VE) process}. A popular choice which decreases the contribution of \(\vx\), as we described originally, has \(T = 1\) (so that \(t \in [0, 1]\)), \(\alpha_{t} = \sqrt{1 - t^{2}}\) and \(\sigma_{t} = t\) (and \(T = 1\)); this is the \textit{variance-preserving (VP) process}. Note that under the VP process, \(\vx_{1} \sim \dNorm(\vzero, \vI)\) exactly, so we can just sample from this standard distribution and iteratively denoise. As a result, the VP process is much easier to analyze theoretically and more stable empirically.\footnote{Why use the whole \(\alpha, \sigma\) setup? As we will see in Exercise \DP{TODO}, it encapsulates and unifies many proposed processes, including the recently popular so-called \textit{flow matching} process. Despite this, the VE and VP processes are still the most popular empirically and theoretically (so far), and so we will consider them in this Section.} 
...
\begin{equation}\label{eq:gen_gmm_bayes_optimal_denoiser}
	\bar{\vx}^{\ast}(t, \vx_{t}) = \sum_{k = 1}^{K}\frac{\pi_{k}\phi(\vx_{t} \mid \alpha_{t}\vmu_{k}, \alpha_{t}^{2}\vSigma_{k} + \sigma_{t}^{2}\vI)}{\sum_{i = 1}^{K}\pi_{i}\phi(\vx_{t} \mid \alpha_{t}\vmu_{i}, \alpha_{t}^{2}\vSigma_{i} + \sigma_{t}^{2}\vI)}\cdot\bp{\vmu_{k} + \alpha_{t}\vSigma_{k}(\alpha_{t}^{2}\vSigma_{k} + \sigma_{t}^{2}\vI)^{-1}(\vx_{t} - \alpha_{t}\vmu_{k})}.
\end{equation}
...
\begin{figure}
	\centering 
	\includegraphics[width=\textwidth]{figs_chap3/vp_gmm_denoising.png}
	\caption{\small\textbf{Denoising a mixture of Gaussians using the VP diffusion process.} \textit{Top left:} We start with three Gaussians, colored gray, orange, and red, supported on a \(2\)-d subspace and two \(1\)-d subspaces respectively in \(\R^{3}\). \textit{Top middle:} We begin the (variance-preserving) denoising process \eqref{eq:gen_denoising_iteration} by sampling some noise from \(\dNorm(\vzero, \vI)\). \textit{Top right:} After \(\ell = 200\) iterations, the current iterate \(\tilde{\vx}_{t_{\ell}}\)  starts to develop some structure aligned with the true distribution. \textit{Bottom left, bottom middle:} The same figure with some more denoising iterations, showing that the sampled data structure increases further, aligning more with the true data distribution. \textit{Bottom right:} The output of the sampling procedure, which is distributed very similarly to the original dataset. Compared to \Cref{fig:ve_gmm_denoising}, the noise distribution is much more concentrated around the origin.}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Require{Scale and noise level functions \(\alpha, \sigma \colon [t_{0}, t_{L}] \to \R_{\geq 0}\).}
		...
		\State{Compute
			\begin{equation*}
				\hat{\vx}_{t_{\ell - 1}} := \frac{\sigma_{t_{\ell - 1}}}{\sigma_{t_{\ell}}}\hat{\vx}_{t_{\ell}} + \bp{\alpha_{t_{\ell - 1}} - \frac{\sigma_{t_{\ell - 1}}}{\sigma_{t_{\ell}}}\alpha_{t_{\ell1}}}\bar{\vx}(t_{\ell}, \hat{\vx}_{t_{\ell}})
			\end{equation*}
		}
		\EndFor
...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\State{\(t_{n}^{(k)} \simiid \dUnif(\{t_{\ell}\}_{\ell = 1}^{L})\)} \Comment{Sample a timestep.}
		...
		\EndFor
		\State{\(\hat{\cL}^{(k)} := \displaystyle \frac{1}{N}\sum_{n = 1}^{N}w_{n}^{(k)}\norm{\vx_{n}^{(k)} - \bar{\vx}_{\theta^{(k)}}(t_{n}^{(k)}, \vx_{t, n}^{(k)})}_{2}^{2}\)} \Comment{Compute the loss estimate.}
...
\begin{equation}
	\Ex_{t}w_{t}\Ex_{\vg, \vx_{t}}\norm{\vg - \bar{\vg}(t, \vx_{t})}_{2}^{2} = \Ex_{t}w_{t}\frac{\alpha_{t}}{\sigma_{t}}\Ex_{\vx, \vx_{t}}\norm{\vx - \bar{\vx}(t, \vx_{t})}_{2}^{2}.
\end{equation}
...
\begin{remark}
	The theory presented at the end of the last \Cref{sub:intro_diffusion_denoising} seems to suggest (loosely speaking) that in practice, using a transformer-like network is reasonable for denoising. This is reasonable, but what is the problem with using any old neural network (such as a multi-layer perceptron (MLP)) and just trying to scale it up to infinity? To observe the problem with this, let us look at another special case of the Gaussian mixture model studied in \Cref{example:denoising_gaussian_mixture}. Namely, the \textit{empirical distribution} is an instance of a degenerate Gaussian mixture model, with \(K = N\) components \(\dNorm(\vx_{i}, \vzero)\) sampled with equal probability \(\pi_{i} = \frac{1}{N}\). In this case the Bayes optimal denoiser is
...
% \begin{figure}[h]
% 	\centering
% 	\includegraphics[width=\textwidth]{figs_chap3/gaussian_mixture_denoising.png}
...
\end{equation}
where minimization is over all (joint) distributions $p(\z, \hat{\z})$ that satisfy  the distortion constraint $\mathbb{E}[d(\z, \hat \z)] \le \epsilon$. From the definition, we know that $R_{\epsilon}(\z)$ is a {\em non-increasing} function in $\epsilon$.
...
\begin{remark}
	Notice that given a set of data points $\X = [\x_1,\ldots, \x_N]$, one can always interpret them as samples from a uniform discrete distribution with equal probability $1/N$ on these $N$ vectors. The entropy for such a distribution is $H(\X) = \frac{1}{N}\log_2 N$.\footnote{Note again, even if we can encode these vectors with this coding rate, we cannot decode them with an arbitrary precision.} However, even if $\X$ is  a uniform distribution on its samples, the coding rate $R_{\epsilon}(\X)$ achievable with a lossy coding scheme could be significantly lower than $H(\X)$ if these samples are not so evenly distributed and many are clustered closely to each other.\footnote{Nevertheless, one can show that for this discrete uniform distribution, when $\epsilon$ is small enough, we always have $H(\X) = R_{\epsilon}(\X)$.}
\end{remark}
...
\begin{equation}
	\min_{\boldsymbol{\Pi}}  \bc{ R_{\epsilon}^c(\X \mid \boldsymbol{\Pi}) = \sum_{i=1}^k \frac{N_i}{N}R_{\epsilon}(\X_i)}.
\end{equation}
...
\begin{figure}[t]
    ...
    \label{fig:image-segmentation}
\end{figure}
>>>>>>> 569be837c3fd47c7529351380a7589e96cd29895
...
\begin{equation}
   \min_{\bm \theta \in \bm \Theta} \; \mbox{CE}(\bm \theta, \x, \y) \doteq - \mathbb{E}[\langle \y, \log(f(\x, \bm \theta)) \rangle] \, \approx - \frac{1}{N}\sum_{i=1}^N \langle \y_i, \log[f(\x_i, \bm \theta)] \rangle.
   \label{chap4-eqn:cross-entropy}
>>>>>>> 569be837c3fd47c7529351380a7589e96cd29895
\end{equation}
The optimal network parameters $\bm \theta $ is typically found by optimizing the above objective through an efficient gradient descent scheme such as the back propagation (BP) described in Section \ref{app:BP-section} of Appendix \ref{app:optimization}.
...
\end{figure}
=======
Despite its effectiveness and enormous popularity, this approach has two serious limitations:  1) It aims only to predict the labels $\y$, even if they might be mislabeled. Empirical studies show that deep networks, used as a ``black box,'' can even fit random labels~\cite{zhang2017understanding}. 
2) With such an end-to-end data fitting, 
despite plenty of empirical efforts in trying to interpret the so-learned features~\cite{Zeiler-ECCV2014}, 
it is not clear to what extent the intermediate features learned by the network capture the intrinsic structures of the data that make meaningful classification possible in the first place. The precise geometric and statistical properties of the learned features are also often obscured, which leads to a lack of interpretability and subsequent performance guarantees (e.g., generalizability, transferability, and robustness etc.) in deep learning. 
Therefore, {\em the goal of this chapter is to address such limitations by reformulating the objective towards learning explicitly meaningful and useful representations for the data $\x$, not limited to classification.}
...
In general, although the intrinsic structures of each class/cluster may be low-dimensional, they are by no means simply linear (or Gaussian) in their original representation $\x$ and they need to be made linear first, through some nonlinear transform.\footnote{We will discuss how this can be done explicitly in Chapter \ref{ch:autoencoding}.} Therefore, overall, we use the nonlinear transform $f(\x)$ to seek a representation of the data such that the subspaces that represent all the classes are maximally incoherent linear subspaces. To be more precise, we want to learn a mapping {$\z = f(\x, \bm \theta)$} that maps each of the submanifolds $\mathcal{M}^j \subset \Re^D$ (Figure~\ref{chap4-fig:mcr-diagram} left) to a {\em linear} subspace $\mathcal{S}^j \subset \Re^n$ (Figure~\ref{chap4-fig:mcr-diagram} right). 
>>>>>>> 569be837c3fd47c7529351380a7589e96cd29895
\begin{figure}[ht]
	\centering
...
=======
To some extent, the resulting multiple subspaces $\{\mathcal{S}^j\}$ can be viewed as discriminative {\em generalized principal components} \cite{GPCA} or, if orthogonal, {\em independent components} \cite{hyvarinen2000independent} of the resulting features $\z$ for the original data $\bm x$. 
As we will see in the next Chapter \ref{ch:representation}, deep networks precisely play the role of modeling and realizing this nonlinear transform from the data distribution to a linear discriminative representation. 
...
On one hand, for learned features to be discriminative, features of different classes/clusters are preferred to be {\em maximally incoherent} to each other. Hence they together should span a space of the largest possible volume (or dimension) and the coding rate of the whole set $\Z$ should be as large as possible. Hence we want the coding rate (or length) for the whole set of features $R_{\epsilon}(\Z)$ to be large (see \eqref{chap4-eqn:coding-length-eval}). \pw{We should relate here to Chpa 3.3.3!} On the other hand, learned features of the same class/cluster should be highly correlated and coherent. Hence, each class/cluster should only span a space (or subspace) of a very small volume and the coding rate should be as small as possible. Hence we want the coding rate for each subset $\Z_j$ to be small. We denote the sum of all these coding rates as
\begin{equation}
	R_{\epsilon}^c(\Z \mid  \bm{\Pi}) \doteq \sum_{j=1}^k R_{\epsilon}(\Z_j \mid \bm{\Pi}),
\end{equation}
...
\begin{equation}
	\Delta R_{\epsilon}(\Z, \bm{\Pi}) \doteq R_{\epsilon}(\Z,) - R_{\epsilon}^c(\Z \mid  \bm{\Pi}).
...
\begin{equation}
	\X
	\xrightarrow{\hspace{2mm} f(\x, \bm \theta)\hspace{2mm}} \Z(\theta) \xrightarrow{\hspace{2mm} \bm{\Pi},\epsilon \hspace{2mm}} \Delta R_{\epsilon}(\Z(\bm \theta), \bm{\Pi}).
...
\begin{equation}
	\begin{aligned}
		\max_{\bm \theta, \bm{\Pi}} \;  \Delta R_{\epsilon}\big(\Z(\bm \theta), \bm{\Pi}\big) & = R_{\epsilon}(\Z(\bm \theta)) - R_{\epsilon}^c(\Z(\bm \theta) \mid \bm{\Pi}), \\
...
\begin{equation}
	R_{\epsilon}(\Z) \doteq \frac{1}{2}\log\det\left(\I + \frac{d}{N\epsilon^{2}}\Z\Z^{\top}\right).
...
\begin{align}\label{eq:MCR-reg}
	\max_{\bm Z}\ F(\bm Z) := R_{\epsilon}(\Z) - R_{\epsilon}^c(\Z\mid \bm{\Pi}) - \frac{\lambda}{2}\|\bm Z\|_F^2,
\end{align}
