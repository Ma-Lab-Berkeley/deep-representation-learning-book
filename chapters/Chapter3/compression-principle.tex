...
To solve this problem, we use the following algorithm outlined \Cref{alg:denoiser}. \yima{something wrong with this reference.}
\begin{algorithm}
    \label{alg:denoiser}
...
and \eqref{chap4-eqn:compress-loss-eval}, respectively. At the maximal rate reduction, MCR$^2$ achieves its optimal representations, denoted as $\Z^{\star} = \Z_{1,\star}\cup \cdots \cup \Z_{k,\star} \subset \Re^d$ with $\rank{(\Z_{j,\star})}\le d_j$. One can show that $\Z^{\star}$ has the following desired properties (see \cite{yu2020learning} for a formal statement and detailed proofs).

\begin{theorem}[\bf Global Optima]
Suppose $\Z^{\star} = \Z_{1,\star}\cup \cdots \cup \Z_{k,\star}$ is the global optimal solution that maximizes the rate reduction~\eqref{eqn:maximal-rate-reduction}. We have:
\begin{itemize}
