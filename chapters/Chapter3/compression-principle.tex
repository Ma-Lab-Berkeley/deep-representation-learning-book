...
When the random variable $\x$ is continuous and has a probability density $p(\x)$, one may view that the limit of the above sum \eqref{eqn:entropy-discrete} becomes an integral: 
\begin{equation}
...
Given a random vector $\bm{x} \in \mathbb{R}^D$ that is drawn from a mixture of $k$ (component) distributions  $\mathcal{D} = \{\mathcal{D}^j\}_{j=1}^k$, one of the most fundamental problems is how to effectively and efficiently {\em learn the distribution} from a finite set of i.i.d. samples, say $\X = [\x^1, \x^2, \ldots, \x^m] \in \Re^{D\times m}$. 
To this end, we {\em seek a good representation}  through a continuous mapping, $f(\x): \mathbb{R}^D \rightarrow \mathbb{R}^d$, that captures intrinsic structures of $\x$ and best facilitates the subsequent classification task.
...
To ease the task of learning distribution $\mathcal{D}$, in the popular supervised classification setting, a true class label (or a code word for each class), usually represented by a one-hot vector $\y^i \in \mathbb{R}^k$, is given for each sample $\x^i$. Extensive studies have shown that for many practical datasets (e.g., images, audios, and natural languages), the (encoding) mapping from the data $\bm{x}$ to its class label $\bm{y}$ can be effectively modeled by training a deep network,\footnote{Here let us not worry about yet which network we should use here and why. It suffices the purpose here to consider any empirically tested deep network. We will leave the justification of the network architectures to the next Chapter. } here denoted as $$f(\x, \bm \theta):\x \mapsto \y$$ with network parameters $\bm \theta \in \bm \Theta$. 

For the output $f(\x, \bm \theta)$ to match well with the label $\y$, we like to  minimize the {\em cross-entropy loss} over a training set $\{(\x^i, \y^i)\}_{i=1}^m$: 
\begin{equation}
...
\begin{figure}[ht]
    ...
    \caption{The distribution $\mathcal D$ of high-dim data $\x\in \Re^D$ is supported on  a manifold $\mathcal{M}$ and its classes on low-dim submanifolds $\mathcal{M}^j$, we learn a map $f(\x, \bm  \theta)$ such that $\z^i = f(\x^i, \bm \theta)$ are on a union of maximally uncorrelated subspaces $\{\mathcal{S}^j\}$.}
...
Once the representations are comparable, the goal becomes to learn a set of features $\Z(\theta) = f(\X, \theta)$ and their partition $\bm \Pi$ (if not given in advance) such that they maximize the reduction between the coding rate of all features and that of the sum of features w.r.t. their classes:
\begin{equation}
\begin{aligned}
...
\paragraph{Rate Distortion of Gaussians.} When evaluating the lossy coding rate $R$, one practical difficulty is that we normally do not know the distribution of $\z$. For the set of features $\Z = [\z^1, \ldots, \z^m]$  with $\z^{i} = f(\x^i, \theta) \in \R^{d}, i = 1,\ldots, m$, we may simply view them all together as samples from a random Gaussian distribution. Hence, the compactness of learned features $\Z$ {\em as a whole} can be measured in terms of the average coding length per sample (as the sample size $m$ is large),  a.k.a. the {\em coding rate} subject to the distortion $\epsilon$:
\begin{equation}
...
\begin{figure}
    ...
    \caption{{\textbf Local optimization landscape:} According to Theorem \ref{thm:MCR2-properties}, the global maximum of the rate reduction objective corresponds to a solution with mutually incoherent subspaces.}
