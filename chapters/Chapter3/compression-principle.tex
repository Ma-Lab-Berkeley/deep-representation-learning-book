...
\begin{equation}
   \min_{\bm \theta \in \bm \Theta} \; \mbox{CE}(\bm \theta, \x, \y) \doteq - \mathbb{E}[\langle \y, \log(f(\x, \bm \theta)) \rangle] \, \approx - \frac{1}{N}\sum_{i=1}^N \langle \y_i, \log\left(f(\x_i, \bm \theta)\right) \rangle.
...
\begin{remark}
    Neural collapse refers to a phenomenon observed in deep neural networks trained for classification, where, at the late stages of training, the learned feature representations and classifier weights exhibit highly symmetric and structured behavior \cite{papyan2020prevalence,zhu2021geometric}. Specifically, within each class, features collapse to their class mean, and across classes, these means become maximally separated, forming a simplex equiangular configuration. Additionally, the last-layer classifier converges to a simple nearest-class-mean decision rule aligned with the feature geometry. Neural collapse reveals deep connections between optimization dynamics, generalization, and geometric structures arising in supervised learning.  
\end{remark}
...
\begin{figure}
	\centering
	\includegraphics[width=0.65\linewidth]{figs_chap3/LDA.png}\vspace{-0.1in}
	\caption{Comparison between principal component analysis and linear discriminative analysis. Figure from \url{https://sebastianraschka.com/Articles/2014_python_lda.html}.}
	\label{fig:LDA}
\end{figure}
...
Without loss of generality, assume that the distribution $\mathcal D$ of  the random vector $\x$ is supported on the mixture of distributions, $\mathcal D = \cup_{j=1}^k \mathcal{D}_j$,  where each $\mathcal{D}_j$ has a low intrinsic dimension in the high-dimensional ambient space $\Re^D$. Suppose that we have a set of samples $\X = \cup_{j=1}^k \X_j$ drawn from this distribution, where $\X_j$ is drawn from $\mathcal{D}_j$. Under an encoding mapping:
\begin{equation}
