...
\begin{algorithm}\label{alg:iterative_denoising}
    \textbf{Algorithm: Iterative Denoising.}
...
\begin{enumerate}
    \item Show that 
    \begin{align*}
    f(\bm X) = \log\det\left(\bm X\right)  
    \end{align*}
    is a concave function. ({\bf Hint:} The function $f(\bm x)$ is convex if and only if the function $f(\bm x+t\bm h)$ for all $\bm x$ and $\bm h$.)

    \item Show that: 
    \begin{align*}
        \log\det(\bm I + \bm X^\top\bm X) = \log\det(\bm I + \bm X\bm X^\top)
    \end{align*}

    \item Let $\bm A \in \R^{n\times n}$ be a positive definite matrix. Please show that:
\begin{align}
\log\det\left(\bm A \right) = \sum_{i=1}^n \log(\lambda_i),
\end{align}
where $\lambda_1,\lambda_2,\dots,\lambda_n$ are the eigenvalues of $\bm A$. 
\end{enumerate}
