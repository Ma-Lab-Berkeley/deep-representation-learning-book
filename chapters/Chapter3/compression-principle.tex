...
\begin{figure}
	...
	\caption{The distribution $\mathcal D$ of high-dimensional data $\x\in \Re^D$ is supported on  a manifold $\mathcal{M}$ and its classes on low-dimensional submanifolds $\mathcal{M}^j$. We aim to learn a mapping $f(\x, \bm  \theta)$ parameterized by $\bm \theta$ such that $\z_i = f(\x_i, \bm \theta)$ lie on a union of maximally uncorrelated subspaces $\{\mathcal{S}^j\}$.}
...
Without loss of generality, assume that the distribution $\mathcal D$ of the random vector $\x$ is supported on the mixture of distributions, i.e., $\mathcal D = \cup_{j=1}^k \mathcal{D}_j$,  where each $\mathcal{D}_j \subset \Re^D$ has a low intrinsic dimension in the high-dimensional ambient space $\Re^D$. Suppose that we have a set of samples $\X = \cup_{j=1}^k \X_j$ drawn from this distribution, where $\X_j$ is drawn from $\mathcal{D}_j$ for each $j=1,\dots,k$. Here, we use $\vx_i$ to denote the $i$-th sample of $\X$, i.e., $\X=[\vx_1,\dots,\vx_N] \in \Re^{D\times N}$. Under an encoding mapping:
\begin{equation}
	\x   \xrightarrow{\hspace{2mm} f(\x)\hspace{2mm}} \z,
\end{equation}
the set of samples are mapped to $\vz_i = f(\vx_i)$ for each $i=1,\dots,N$. With an abuse of notation, we write $\Z = f(\X)$ and $\Z_j = f(\X_j)$, where $\Z = [\vz_1,\dots\vz_N]$ and $\Z = \cup_{j=1}^k \Z_j$. On one hand, for learned features to be discriminative, features of different classes/clusters are preferred to be {\em maximally incoherent} to each other. Hence, they together should span a space of the largest possible volume (or dimension) and the coding rate of the whole set $\Z$ should be as large as possible. On the other hand, learned features of the same class/cluster should be highly correlated and coherent. Hence, each class/cluster should only span a space (or subspace) of a very small volume and the coding rate should be as small as possible. Now, we will introduce how to measure the coding rate of the learned features.  

\paragraph{Coding Rate of Features.} Notably, a practical challenge in evaluating the coding rate is that the underlying distribution of the feature representations $\Z$ is typically unknown. To address this, we may approximate the set of features $\Z = [\z_1, \ldots, \z_N]$ as samples from a multivariate Gaussian distribution. Under this assumption, as discussed in Chapter \ref{subsec:lossy DR}, the compactness of learned features $\Z$ {\em as a whole} can be measured in terms of the average coding length per sample (as the sample size $N$ is large),  referred to as the {\em coding rate}, subject to a precision level $\epsilon > 0$ (see \eqref{eqn:rate-Gaussian}) defined as follows:
\begin{equation}
	R_{\epsilon}(\Z) = \frac{1}{2}\log\det\left(\I + \frac{d}{N\epsilon^{2}}\Z\Z^{\top}\right).
	\label{chap4-eqn:coding-length-eval}
\end{equation}

On the other hand, we hope that a nonlinear transformation $f(\x)$ maps each class-specific submanifold $\mathcal{M}^j \subset \mathbb{R}^D$ to a maximally incoherent linear subspace $\mathcal{S}^j \subset \mathbb{R}^d$, so that the learned features $\Z$ lie in a union of low-dimensional subspaces. This structure enables a more accurate evaluation of the coding rate by analyzing each subspace separately. To this end, we partition the data $\Z$ into multiple subsets: $\Z = \Z_1 \cup \cdots \cup \Z_k$, {with} each in one low-dimensional subspace, and the above coding rate \eqref{chap4-eqn:coding-length-eval} is accurate for each subset.
