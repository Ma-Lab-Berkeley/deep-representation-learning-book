...
\end{itemize} As we will see, the fundamental idea of {\em compression}, or {\em dimension reduction}, that has been shown to be very effective for the linear/independent case still serves as a general principle for developing effective computational models and methods for learning general low-dimensional distributions.
...
\end{equation}
This is known as the {\em entropy} of the (dictrete) distribution $p(\cdot)$. Note that this entropy is always nonnegative and it is zero if and only if $p(\x_i) = 1$ for some $\x_i$ with $i \in [N]$.\footnote{Here notice that we use the fact $\lim_{p\rightarrow 0} p \log p = 0$.} 
...
Let $p(\x), q(\x)$ be two probability density functions (that have the same support). Then $D(p \| q) \ge 0$ with equality if and only $p(\x) = q(\x)$. 
\end{theorem}
\begin{proof}
    \begin{eqnarray}
    - D(p \| q) 
    ...
    &=& \log 1 = 0,
    \end{eqnarray}
where the step \eqref{eqn:jensen-KL} follows from {\em Jensen's inequality} and the fact that the function $\log(\cdot)$ is strictly concave. The equality holds if and only $p(\x) = q(\x)$.
\end{proof}
...
This observation gives us a general guideline on how we may be able to pursue a  distribution $p(\x)$ which has low-dimensional structure. It suggests two possible approaches:
\begin{enumerate}
...
\end{equation}
The following theorem shows that, by adding Gaussian noise, the entropy of the resulting distribution increases. 
\begin{theorem}[Diffusion and Entropy]\label{thm:diffusion-and-entropy}
    Suppose that \(\vz_{o} \in \R^{d}\) is a random variable with \(\Ex\norm{\vz_{o}}_{2}^{2} < \infty\), \(\vg \sim \dNorm(\vzero, \vI)\) is a standard Gaussian random variable which is independent of \(\vz_{o}\), \(\sigma > 0\) is a positive real number, and \(\vz_{\sigma} = \vz_{o} + \sigma \vg\). Then \(h(\vz_{\sigma}) > h(\vz_{o})\).
\end{theorem}
...
\begin{equation}\label{eq:denoiser_maximum_likelihood}
    \vmu_{\sigma} \in \argmax_{\substack{f \colon \R^{d} \to \R^{d} \\ f\ \text{measurable} \\ \Ex\norm{f(\vz_{\sigma})}_{2}^{2} < \infty}}\Ex p_{\sigma \mid o}(f(\vz_{\sigma})),
\end{equation}
In particular, the denoiser \(\vmu_{\sigma}\) finds the value of the true data which makes the noisy observations most likely. It is easy to show that finding the function \(\vmu_{\sigma}\), with the Gaussian noise model \eqref{eq:additive_gaussian_noise_model}, is equivalent to solving the problem\footnote{If you aren't sure what \textit{measurable} means, don't worry about it; it's merely a technical condition, which holds for any function you can write down. Its exact definition can be looked up in any graduate-level real analysis or probability textbook.}
\begin{equation}\label{eq:denoiser_conditional_expectation}
    \vmu_{\sigma} \in \argmin_{\substack{f \colon \R^{d} \to \R^{d} \\ \text{\(f\) measurable} \\ \Ex\norm{f(\vz_{\sigma})}_{2}^{2} < \infty}}\Ex\norm{\vz_{o} - f(\vz_{\sigma})}_{2}^{2}.
\end{equation}
...
\end{equation}
It turns out that solving the minimization problem in \eqref{eq:denoiser_conditional_expectation} is \textit{much} more computationally tractable than solving the maximum likelihood problem \eqref{eq:denoiser_maximum_likelihood}. To solve the former problem, we use the following outlined Algorithm: Learning a Denoiser.
\begin{algorithm}\label{alg:denoiser}
    \textbf{Algorithm: Learning a Denoiser.}\vspace{1mm}
    
    \textbf{Initialize:} An optimization algorithm, a parametric function class \(\cF = \{f_{\theta} \colon \theta \in \Theta\}\), an initial parameter \(\theta^{1}\), and data \(\vz_{o}^{1}, \dots, \vz_{o}^{N} \simiid \vz_{o}\).
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{equation}
                \ell^{j}(\theta^{j}) \doteq \frac{1}{N}\sum_{i = 1}^{N}\norm{\vz_{o}^{i} - f_{\theta^{j}}(\vz_{\sigma, j}^{i})}_{2}^{2}.
            \end{equation}
            \item Construct \(\theta^{j + 1}\) by applying the optimization algorithm:
            \begin{equation}
                \theta^{j + 1} \doteq \texttt{OptUpdate}(\theta^{j}; \ell^{j}).
            \end{equation}
            For instance, if the optimization update is simple gradient descent, we have \(\theta^{j + 1} = \theta^{j} - \alpha \nabla_{\theta}\ell^{j}(\theta^{j})\), for some learning rate \(\alpha\).
        \end{itemize}
    \end{itemize}
...
\begin{example}[Denoising Gaussian noise from a Gaussian]\label{example:gaussian_denoising}
    ...
    \end{equation}
    which shows that as \(\sigma\) grows, the denoiser collapses the residual \(\vxi - \vu\) to \(0\) fastest in the directions where the eigenvalues of \(\vSigma\) are smallest.

    Also, if we are trying to learn a Gaussian (for instance), we can parameterize the denoiser by the mean \(\vu\) and covariance \(\vSigma\), so that \(\theta = (\vu, \vSigma)\), and consider denoisers of the functional form \eqref{eq:gaussian_denoiser}.
\end{example}
...
\begin{theorem}[Denoising and Entropy]\label{thm:conditioning_reduces_entropy}
    Suppose that \(\vz_{o} \in \R^{d}\) is a random variable with \(\Ex\norm{\vz_{o}}_{2}^{2} < \infty\), \(\vg \sim \dNorm(\vzero, \vI)\) is a standard Gaussian random variable which is independent of \(\vz_{o}\), \(\sigma > 0\) is a positive real number, \(\vz_{\sigma} = \vz_{o} + \sigma \vg\), and \(\vmu_{\sigma}(\vz) = \Ex[\vz_{o} \mid \vz_{\sigma} = \vz]\). Then it holds that \(h(\vmu_{\sigma}(\vz_{\sigma})) < h(\vz_{\sigma})\). 
\end{theorem}
The proof is slightly technical and deferred to Appendix \ref{app:diffusion-denoising}. In the sequel, we will use \textit{iterative denoising} to lower the entropy of a high-entropy distribution until it reaches the data distribution, realizing the second approach to pursuing a distribution discussed at the end of Section \ref{sub:min_entropy}.
...
\begin{proof}
    ...
    \end{align}
    Let us consider the remaining integral. Remember that \(\phi(\vxi \mid \vx, \sigma^{2}\vI) = p_{\sigma \mid o}(\vxi \mid \vx)\). By Bayes' theorem, we can write 
    \begin{equation}
