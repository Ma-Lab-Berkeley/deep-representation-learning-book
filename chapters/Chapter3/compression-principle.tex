...
\begin{remark}
\textbf{Difference between the denoiser interpretation and power iterations.} At the first glance, the iterates process in \eqref{eq:contraction} may appear similar to power iterations (see \Cref{subsec:power iterations}).  However, the two are fundamentally different. Power iteration is designed to find the leading principal component of a data matrix. In contrast, the iterates in \eqref{eq:contraction} converge to the mean \(\vmu\) of the underlying distribution, rather than the principle components.  
\end{remark}

\begin{figure}
	\centering 
	\includegraphics[width=\textwidth]{figs_chap3/ve_forward_diffusion_denoising.png}\vspace{-0.15in}
...
\begin{align*}
    ...
    = & \left\|\vx - \frac{1}{1 + t^{2}} \vV\vV^\top \vx\right\|_{2}^{2} - \frac{t}{1+t^2}\Ex_{\vg} \langle \vx - \frac{1}{1 + t^{2}} \vV\vV^\top \vx,  \vV\vV^\top\vg \rangle + \frac{t^2}{(1+t^2)^2} \Ex_{\vg}\left\| \vV\vV^\top \vg \right\|_2^2 \\
