...
\begin{remark}
    Note that the above MCR$^2$  principle is designed for supervised learning problems, where the group memberships (or class labels) are known. However, this principle can be naturally extended to unsupervised learning problems by introducing a membership matrix, which encodes the (potentially soft) assignment of each data point to latent groups or clusters. Specifically, let $\bm \Pi = \{\bm \Pi_j\}_{j=1}^k \subset \R^{N\times N}$ be a set of diagonal matrices whose diagonal entries encode the membership of the $N$ samples into $k$ classes. That is, $\bm \Pi$ lies in a simplex $\Omega := \{\bm \Pi: \bm \Pi_j \ge \bm 0: \sum_{j=1}^k \bm \Pi_j = \bm I_N\}$. Then, we can define the average coding rate with respect to the partition $\bm \Pi$ as
    \begin{align*}
        R_{\epsilon}^c(\Z \vert \bm \Pi) := \sum_{j=1}^k \frac{\mathrm{tr}(\bm \Pi_j)}{2N}\log\det\left(\bm I + \frac{d}{\mathrm{tr}(\bm \Pi_j)\epsilon^2}\bm Z\bm \Pi_j \bm Z^\top \right).
    \end{align*}
    ...
    \begin{align*}
        \max_{\bm \Pi, \bm \theta}\Delta R_{\epsilon}\big(\Z \big\vert \bm \Pi) := R_{\epsilon}(\bm Z) - R_{\epsilon}^c(\Z \vert \bm \Pi)\qquad \mathrm{s.t.}\quad \bm Z = f(\bm X, \bm \theta),\ \|\bm Z \bm \Pi_j\|_F^2 = N_j,\ j = 1,\dots,k,\ \bm\Pi \in \Omega. 
    \end{align*}
