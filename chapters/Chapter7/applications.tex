...
In particular, DINO minimizes the difference between the probability vector \(\vp_{\theta, \vW, \vmu}(\vX_{g}) := h_{\vW, \vmu}(\vz_{\theta}(\vX_{g}))\) for each global view \(\vX_{g} := v_{g}(\vX)\) and the probability vector \(\vp_{\theta, \vW}(\vX_{c}) := h_{\vW, \vzero_{m}}(\vz_{\theta}(\vX_{c}))\) for each view \(\vX_{c} := v_{c}(\vX)\). Here, \(v_{c}\) can \textit{either} be a local view or a global view. We will discuss the implementation of \(f_{\theta}\) and \(f_{\theta}^{\ext}\) shortly in \Cref{sub:contrastive_learning_architecture}. Overall, DINO solves the problem
 \begin{equation}\label{eq:dino_loss}
     \min_{\theta, \vW, \vmu}\cL_{\dino}(\theta, \vW, \vmu) \qquad \text{where} \qquad \cL_{\dino}(\theta, \vW, \vmu) := \Ex[d_{\CE}(\vp_{\theta, \vW}(\vX_{c}), \vp_{\theta, \vW, \vmu}(\vX_{g}))],
\end{equation}
...
\begin{equation}\label{eq:simdino_loss}
    \cL_{\simdino}(\theta) := \Ex[d_{\ell^{2}}(\vz_{\theta}(\vX_{c}), \vz_{\theta}(\vX_{g})] - \frac{\gamma}{2}\logdet\rp{\vI + \frac{d}{\eps^{2}}\Cov(\vz_{\theta}(\vX_{g}))},
\end{equation}
...
\begin{equation}\label{eq:dino_loss_teacherstudent}
    \cL_{\dino{}-\student\teacher}(\theta_{\student}, \theta_{\teacher}, \vW_{\student}, \vW_{\teacher}, \vmu) := \Ex[d_{\CE}(\vp_{\theta_{\student}, \vW_{\student}}(\vX_{c}), \vp_{\theta_{\teacher}, \vW_{\teacher}, \vmu}(\vX_{g}))].
\end{equation}
...
\begin{equation}\label{eq:simdino_loss_teacherstudent}
    \cL_{\simdino-\student\teacher}(\theta_{\student}, \theta_{\teacher}) :=  \Ex\rs{d_{\ell^{2}}(\vz_{\theta_{\student}}(\vX_{c}), \vz_{\theta_{\teacher}}(\vX_{g}))} + \frac{\gamma}{2}\logdet\rp{\vI + \frac{d}{\eps^{2}}\Cov(\vz_{\theta_{\student}}(\vX_{g})))}.
\end{equation}
...
\begin{equation}\label{eq:classification_ce_loss}
    \min_{\theta}\bc{\cL_{\CE}(\theta) := \Ex[\CE(\vy, \hat{\vy}_{\theta}(\vX_{\cc}))]}.
\end{equation}
...
\begin{itemize}
    ...
    \begin{equation}
        \hat{\cL}_{\CE}^{(k)}(\theta) := \frac{1}{B}\sum_{b = 1}^{B}\CE(\vy_{b}^{(k)}, \hat{\vy}_{\theta}(\vX_{b, \cc}^{(k)})).
    \end{equation}
...
%     \cL_{\CE}(f, f^{\mathrm{head}}) \doteq \mathbb{E}_{\vX, \vy}\mathopen{}\left[H\big(\vy, \operatorname{softmax}\big\{(f^{\mathrm{head}} \circ f)(\vX)\big\}\big)\right].
% \end{equation}
...
% \begin{equation}
%     \cL_{\CE}(f, f^{\mathrm{head}}) \doteq \mathbb{E}_{\vX, \vy}\mathopen{}\left[H(\vy, \operatorname{softmax}\{(f^{\mathrm{head}} \circ f)(\vX)\})\right].
% \end{equation}
...
%   \cL_{\text{BERT}}(f, f^{\mathrm{head}}) = \mathbb{E}_{\vX, \Omega}\left[\frac{1}{\abs{\Omega}}\sum_{i\in \Omega}  H(\bm{1}(\vx_{i}), \operatorname{softmax}\{(f^{\mathrm{head}} \circ f)(\vX)_{i}\})\right],
% \end{equation}
...
%   \cL_{\text{GPT}}(f, f^{\text{head}}) = \mathbb{E}_{\vX}\bigg[\frac{1}{\abs{\vX} -1} \sum_{i = 1}^{\abs{\vX} - 1} H(\bm{1}(\vx_{i + 1}), \operatorname{softmax}\{(f^{\mathrm{head}} \circ f)(\vX)_{i}\}) \bigg],
% \end{equation}
