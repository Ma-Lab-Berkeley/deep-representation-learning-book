...
We now study \textit{next-token prediction} for text, which is a special case of classification in which the set of classes (i.e., the ``vocabulary'') is very large. Next-token prediction is the pre-training objective for large language models (LLMs) and certain kinds of vision models (with a so-called ``visual tokenizer''). In this section, we mainly focus on generative pre-training (GPT) \cite{radford2019language} tasks for learning language representations. To explore the CRATE architecture for language tasks, we use the following datasets. For pre-training, we use OpenWebText (OWT)~\cite{Gokaslan2019OpenWeb}, an open-source reproduction of the unreleased WebText dataset used by OpenAI to train GPT-2. For zero-shot evaluation, we evaluate cross-entropy validation loss on WikiText~\cite{merity2016pointer}\footnote{For WikiText2 and WikiText103~\cite{merity2016pointer}, the test splits are the same, so we merge them as a single dataset referred to as WikiText.}, LAMBADA~\cite{paperno2016lambadadatasetwordprediction}\footnote{To obtain the accuracy on LAMBADA dataset, we use greedy decoding.}, and PTB~\cite{marcus-etal-1993-building}. % In addition, we compute zero-shot accuracy on LAMBADA for predicting the final word of sentences, as well as on the Children’s Book Test (CBT)~\cite{hill2016goldilocksprinciplereadingchildrens}, where the task is to choose either common nouns or named entities from 10 possible options for an omitted word in a paragraph.
...
\begin{align*}
    L_{\rm GPT}\left(f ,f^{\rm head}\right) = \mathbb{E}_{\vX}\left[ \frac{1}{N - 1}\sum_{i=1}^{N-1} \CE\left(\mathbf{1}(\bm x_{i+1}), \mathrm{softmax}\left\{ (f^{\rm head} \odot f)(\bm X)_i \right\}\right) \right],
\end{align*}
...
\begin{figure}
    ...
    \caption{The loss curve of CRATE-GPT-Base trained on the OpenWebText dataset.}
...
\def\arraystretch{1.1}
    \small
    \caption{\small Zero-shot cross-entropy loss of the CRATE-GPT2-Base model and GPT2-Small, GPT2-Base model evaluated on the test split of the datasets (lower is better). \pt{Maybe an arrow point down instead}
    }
    \centering
