...
\end{itemize}
However, in our honest opinion, solutions and results given here are designed simply to verify that the methodology works. As such, there is great room for future improvement, in both engineering and theoretical understanding, to potentially improve the state-of-the-art. We will discuss some future directions in \Cref{ch:future}.  
...
\paragraph{Backbone.} Given a token sequence \(\vZ_{\theta}^{1}(\vX) := f_{\theta}^{\emb}(\vX) \in \R^{d \times *}\), we process it using the backbone map \(f_{\theta}^{\backbone}\) as follows and as depicted in \Cref{fig:transformer_backbone}. The function \(f_{\theta}^{\backbone}\) is composed of \(L\) \textit{layers} \(f_{\theta}^{\ell}\), i.e.,
\begin{equation}
...
\begin{itemize}
    ...
    \end{equation}
    are also known as the \textit{\(k\th\) attention map} and \textit{\(k\th\) attention head output} at layer \(\ell\), respectively.
...
\begin{equation}
    \hat{\vy}_{\theta}(\vX \mid \{(\vX_{b}, \vy_{b})\}_{b = 1}^{B}) = \ve_{i^{\star}} \quad \text{where} \quad i^{\star} := \argmax_{i \in [Q]}\sum_{b = 1}^{B}\vy_{b}\indvar[\vz_{\theta}(\vX_{\cc, b}) \in \operatorname{NN}_{k}(\vz_{\theta}(\vX_{\cc}))].
\end{equation}
Here, \(\ve_{i}\) is the \(i^{\mathrm{th}}\) standard basis vector, which is \(1\) in the \(i^{\mathrm{th}}\) coordinate and \(0\) elsewhere. That is, this procedure takes the most common label among the \(k\) nearest points in feature space. The \(k\)-nearest neighbor classification accuracy is just the accuracy of this predicted label, namely,
\begin{equation}
...
\paragraph{Datasets and optimization.} For pre-training, both our DINO reproduction and SimDINO use the ImageNet-1K dataset across all methods. We use AdamW \citep{loshchilov2017decoupled} as the optimizer, which is a very standard choice. We follow the following hyperparameter recommendations:
\begin{itemize}
...
\end{equation}
where \((\vW^{\head}, \vb^{\head}) \in \R^{N_{\cls} \times d} \times \R^{N_{\cls}}\) are trainable parameters in the parameter set \(\theta\), such that the map \(\vX_{\cc} \mapsto \hat{\vy}_{\theta}(\vX_{\cc}) := h_{\theta}(\vz_{\theta}(\vX_{\cc}))\) predicts a smoothed label for the view \(\vX_{\cc} = v_{\cc}(\vX)\) of the input \(\vX\). The learning problem attempts to minimize the distance between \(\hat{\vy}_{\theta}\) and \(\vy\) measured through cross-entropy:
\begin{equation}\label{eq:classification_ce_loss}
    \min_{\theta}\bc{L_{\CE}(\theta) := \Ex[\CE(\vy, \hat{\vy}_{\theta}(\vX_{\cc}))]}.
\end{equation}
...
\begin{itemize}
    ...
    \item Compute the smoothed pseudo-labels \(\hat{\vy}_{\theta}(\vX_{b, \cc}^{(k)}) := (h_{\theta} \circ f_{\theta}^{\ext} \circ f_{\theta})(\vX_{b, \cc}^{(k)})\).
    \item Form the surrogate stochastic loss 
    \begin{equation}
        \hat{L}_{\CE}^{(k)}(\theta) := \frac{1}{B}\sum_{b = 1}^{B}\CE(\vy_{b}^{(k)}, \hat{\vy}_{\theta}(\vX_{b, \cc}^{(k)})).
    \end{equation}
...
Since CRATE is directly based on the transformer, we compare the optimal settings for ViT as given by \citet{dosovitskiy2020image,touvron2020training} with the same settings applied to CRATE for fair comparison.

\paragraph{Model architecture.} The center crop resizes the whole image so that the shorter edge is of size \(256\) (i.e., \(S_{\rsz} = 256\)) before taking a center crop of size \(224 \times 224\) (i.e., \(S_{\cc} = 224\)), both in evaluation and training. We take patch size \(16\) (i.e., \(P_{H} = P_{W} = 16\)). We use the tiny, small, base, and large models of the ViT \citep{dosovitskiy2020image} architecture as the embedding and backbone, swapping out the MHSA and MLP components for MSSA and ISTA respectively, using the same number of heads and head dimension in the case of MSSA, and therefore reducing the number of training parameters drastically. For CRATE, we set \((\beta, \lambda) = (1, 0.1)\).
...
\DP{@YM: note that CRATE-\(\alpha\) paper has no linear probing experiments, only full fine-tuning. Since full fine-tuning doesn't make sense from representation learning perspective, we omit it. Feel free to delete this comment when you are done reading.}
...
\end{equation}
It turns out that this architecture is \textit{also} viable, though the depth of the network needs to be much deeper in order to achieve equivalent performance as the usual CRATE or transformer architecture (i.e., \Cref{tab:aot_lm}).
...
This is the first autoencoder network we discuss in this section. We use the same center crop view \(v_{\cc}\) as in \Cref{sub:contrastive_learning_evals,sub:image_classification_evals}, resizing the final image to a square with side length \(S_{\mask}\) pixels so as to match the shapes of the input images seen during training. 
...
\end{equation}
as in (for example) \Cref{sub:image_classification_objective,sub:image_classification_architecture}. With this, we have a way to obtain aggregate features \(\vz_{\theta}(\vX_{\cc}) := (f_{\theta}^{\ext} \circ f_{\theta})(\vX_{\cc})\), at which point we can perform linear probing.
