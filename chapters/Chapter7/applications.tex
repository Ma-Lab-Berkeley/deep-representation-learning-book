...
We use a CRATE encoder and decoder, depicted in \Cref{fig:}, though of course it is possible to use a regular transformer encoder and decoder. Details follow now.
...
This is the first autoencoder network we discuss in this section. We use the same center crop view \(v_{\cc}\) as in \Cref{sub:contrastive_learning_evals,sub:image_classification_evals}, resizing the final image to a square with side length \(S_{\cc} = S_{\mask}\) pixels so as to match the shapes of the input images seen during training. 
...
\begin{figure}
    \centering 
    \includegraphics[width=\textwidth]{figs_chap7/crate_mae_semantic_heads.pdf}
