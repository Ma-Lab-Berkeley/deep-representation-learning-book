...
\begin{itemize}
    ...
    \end{itemize}
    We also define \(f_{\theta} := f_{\theta}^{\backbone} \circ f_{\theta}^{\emb} \colon \cD \to \R^{d \times *}\). Given an input \(\vX\), we write \(\vZ_{\theta}(\vX) := f_{\theta}(\vX)\) and \(\vz_{\theta}(\vX) := f_{\theta}^{\ext}(\vZ_{\theta}(\vX))\). The overall pipeline is depicted in \Cref{fig:overall_encoder_pipeline}.
    ...
    \end{itemize}
    We also define \(f_{\theta} := f_{\theta}^{\backbone} \circ f_{\theta}^{\emb} \colon \cD \to \R^{d \times *}\) and \(g_{\eta} := g_{\eta}^{\unemb} \circ g_{\eta}^{\backbone} \colon \R^{d \times *} \to \cD\). Given an input \(\vX\), we write \(\vZ_{\theta}(\vX) := f_{\theta}(\vX)\) and \(\hat{\vX}_{\theta, \eta}(\vX) := g_{\eta}(\vZ_{\theta}(\vX))\). The overall pipeline is depicted in \Cref{fig:overall_autoencoder_pipeline}.
\end{itemize}
...
For pre-training, we use OpenWebText~\citep{Gokaslan2019OpenWeb}, an open-source reproduction of the unreleased WebText dataset used by OpenAI to train GPT2. For zero-shot evaluation, we use the following datasets for cross entropy validation loss:  WikiText~\citep{merity2016pointer}\footnote{For WikiText2 and WikiText103~\citep{merity2016pointer}, the test splits are the same, so we merge them as a single dataset referred to as WikiText.}, LAMBADA~\citep{paperno2016lambadadatasetwordprediction}\footnote{To obtain the accuracy on LAMBADA dataset, we use greedy decoding.}, and PTB~\citep{marcus-etal-1993-building}. In addition, we compute zero-shot accuracy on LAMBADA for predicting the final word of sentences, as well as on the Children’s Book Test (CBT)~\citep{hill2016goldilocksprinciplereadingchildrens}, where the task is to choose either common nouns (CN) or named entities (NE) from 10 possible options for an omitted word in a paragraph.
