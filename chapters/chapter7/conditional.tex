...
\begin{remark}[Energy versus Entropy]
Due to the conspicuous quadratic form of the two terms in the above equation, they can also be interpreted as certain ``energy'' functions. Such a formulation is often referred to as ``Energy Minimization'' in the machine learning literature, advocated by people like Yann LeCun \cite{LeCun-2006}. Here the energy of a point $\x$ is measured by its distance to the support of the distribution. Notice that in \Cref{ch:denoising} and \Cref{ch:lossy-compression}, we have argued that the notion of entropy is to measure the ``volume'' or uncertainty of the data distribution. The above continuation process can be viewed as a ``denoising'' process, studied in  \Cref{ch:denoising}, that gradually enforces the data to be closer to the distribution support. As we minimizes the energy, the entropy (or uncertainty) of the data distribution reduces. 
\end{remark}
