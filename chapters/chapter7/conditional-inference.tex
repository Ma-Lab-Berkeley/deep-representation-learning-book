...
\begin{example}[Image Completion and Text Prediction]
    ...
    we will study in greater details later.
    \begin{figure}
        \centering
...
is the most desired one -- it is the  sample on the distribution of
the highly probability to produce the observation $\y$, but is
typically the most expensive to compute.
...
\Cref{ch:denoising,ch:compression}, we may approximate the distance
...
\eqref{eqn:loglikelihoo-gradient-ascent},
...
\begin{example}[Gradient of $F(\x)$] For the distance function
    ...
    typical unique at a fixed point, the gradients of the distance
...
to the correct lydistribution support. Notice that the above
...
$\x$ to the support of the distribution. As we minimizes the above
...
\begin{itemize}
    \item {\em Case 1:} Both a model for the distribution of $\x$ and
        the observation model $\y = h(\x)$ $(+ \boldsymbol{w})$ are
    ...
    \item {\em Case 2:} We do not have a model for the distribution
        ...
        the observation model $\y = h(\x)$ $ (+ \boldsymbol{w})$ is
        ...
        class of problems.
    \item {\em Case 3:} We only have the paired samples: $(\X, \Y) =
...
practical problem. Throughout the chapter, you should keep
...
$\x$ of interest is already given {\em apriori}, even in a certain
...
partial observation $\y$ is because (support of) the distribution of
$\X$ is known or specified {\em apriori}, say as the set of all
...
$p_{\X \mid \X_v}$ is a generalized function---if $\vX$ is fully
determined by $\vX_v$ it is the delta function, and more generally
...
\begin{figure}[t]
    ...
    \caption{Statistical dependency diagrams for the conditional
        ...
        $\vy$ given $\vx$. It implies that $p_{\posteriorsample_t} =
        p_{\vx_t \mid
...
\begin{example}\label{example:denoising-conditional-gaussian}
    ...
    \end{equation}
    The righthand side of this approximation is equal to
    $\vSigma_{\vy \mid \vx}$.
    So we have in turn
    \begin{equation}
        \nabla_{\vxi} \log p_{\vy \mid \vx_t}(\vnu \mid \vxi)
...
\end{equation}
In other words,  the representation \eqref{eq:conditional-denoising-mmse-denoiser-paired-data}
...
In particular, \citet{Dhariwal2021-hg} found that it was necessary to \textit{explicitly
...
\textbf{classifier-free guidance (CFG)}. Instead of representing the conditional denoiser
...
\paragraph{Deriving Classifier-Free Guidance.} To see how this structure arises, we
...
They provide explanations for some parts of the overall CFG methodology---itself encompassing denoiser
...
\begin{example}\label{example:denoising-gaussian-mixture-cfg}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Poorly-correlated regime:} In contrast, if $\vx_t$
            ...
            process towards the conditioning class and away from the previous
...
\begin{example}\label{example:denoising-gaussian-mixture-cfg-parameterization}
    ...
    % More precisely, 
    For tractability, we add an additional assumption associated to the subspaces
    ...
    asymptotic regime where $P, D \to \infty$ with their ratio $P/D$ converging
    to a fixed constant. In such a scenario, the \textit{aspect ratio} of the
    subspaces is fixed while their sizes go to infinity, which can act as
    a model for some discretization phenomena (e.g., a fixed scene sampled as an
    ...
    \eqref{eq:mog-conditional-denoising-unified-operator} at the embedding for
    ...
    The resulting
    operator is a function of $(t, \vx_t, y)$, and computationally, the subspaces
    $(\vU_k)_{k=1}^K$ and embeddings $y \mapsto \vv_{t, y}$ become its learnable
    parameters.
\end{example}

\Cref{example:denoising-gaussian-mixture-cfg-parameterization} shows that in the special case of
...
In \Cref{sub:cfg} we have seen, conceptually, how to set up denoisers for use with
...
The space of possible designs for a denoiser for $(\vx_t, \vy)$ is vast, and
almost certainly it has not been exhaustively explored by prior research. Hence
it is invaluable to begin from first principles and build our design outwards.
In motivating our development, we will make crucial use of two thought
experiments. The first is based on the intuitions we have developed in our study
of consistent representation learning in
\Cref{ch:lossy-compression,ch:consistent-representations}, and it will lead us
to a general information-theoretic criterion for learning useful representations
of paired data with distinct (but not independent) modalities.
The second is based on the simple mixture-of-Gaussians model in
\Cref{example:denoising-gaussian-mixture-cfg-parameterization}, and it will lead
us to a natural simplification of the vast space of possible per-modality
embedding designs, and in turn a simple practical criterion for learning the
embeddings.

\paragraph{Learning representations for paired data.}
% The following perspective on representation learning for paired data that we
% discussed at the beginning of \Cref{sec:paired-data} provides our launching
% point.



\paragraph{Joint embedding models: motivation and consequences.}
...
Below, we will describe an early instantiation of the preceding techniques, which
...
\bR^{D_{\mathrm{text}} \times N}$ in this context, since it
...
\begin{figure}[tbp]
    ...
    \caption{A high-level schematic of training and applying a text-to-image
        ...
        for a conditional
        denoiser, trained as described in \Cref{sub:cfg}. The text
        encoder may be
        ...
        use with an encoded text prompt). For full details of the
        process of encoding
...
is the encoder and $g$ is the decoder. Subsequent model development
has shown that
...
\cite{zhao2023michelangelo} for conditional 3D shape generation based
on images or texts.
