...
\begin{itemize}
    \item {\em Case 1:} Both a model for the distribution of $\x$ and
        ...
        in \Cref{ch:classic} and the low-rank matrix recovery problem
...
demonstrate an insight into the \textit{parameterization} of the
denoiser in the presence of
...
\begin{example}\label{example:denoising-gaussian-mixture-cfg}
    ...
    \begin{equation}\label{eq:cfg-denoiser-mog-low-rank}
        ...
        \vx_t.
    \end{equation}
    This denoiser has a simple, interpretable form. The first term,
    corresponding
    to the unconditional denoiser, performs denoising of the signal $\vx_t$
    against an average of the denoisers associated with each
    subspace, weighted by
    how correlated $\vx_t$ is with each subspace. The second term,
    corresponding to
    ...
    \begin{enumerate}
        ...
        \item \textbf{Poorly-correlated regime:} In contrast, if $\vx_t$
            ...
            before denoising).
            In other words, CFG steers the iterative denoising
            process towards the
            conditioning class and away from the previous iterate, a different
            dynamics from purely conditional sampling (i.e., the case
            $\gamma = 1$).
    \end{enumerate}
    ...
    \eqref{eq:cfg-denoiser-mog-low-rank} has
    ...
    [K]$ with $k \neq k'$, we can find a set of $K$ directions
    $\vv_{k} \in \bR^D$ such
    that
    \begin{equation}
    ...
    \begin{eqnarray}
        ...
        \vU_{k}\vU_{k}^{\top}\vx_t.
    \end{eqnarray}
    Now, because $\vx_t = \alpha_t \vx + \sigma_t \vg$, if the
    subspace dimension
    ...
    optimal unconditional denoiser
    \eqref{eq:gmm_lowrank_denoiser-ch6-uncond} for $\vx_t$.
