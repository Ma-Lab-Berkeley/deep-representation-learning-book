...
\begin{equation}\label{eq:infomax-x}
    \max_{f}\, I(f(\vx); \vy).
\end{equation}
This is known as the \textit{Infomax principle} \cite{infomax}, and it dates to the
...
\eqref{eq:infomax-x}.
As we discussed in the context of rate distortion and lossy coding when we introduced
...
with the embeddings $\vU_{\vx}$ of $\vx \in \R^{D}$ (generally a matrix) via $\vu_y^\top
...
more
complex nonlinear high-dimensional data is a challenging question that we will dig into in
...
CLIP trains two encoders—an image encoder $f_{\theta}(\x)$ and a text encoder $g_{\plainphi}(\y)$—to map paired samples into a shared $d$-dimensional unit sphere.  
...
\begin{equation}
    \mathcal{L}_{\mathrm{CLIP}} = \mathbb{E}_{[(\x_i, \y_i)]_{i=1}^N \sim \mathcal{D}} \left[ - \log \frac{\exp\left( \frac{f_{\theta}(\x_i)^\top g_{\plainphi}(\y_i)}{\|f_{\theta}(\x_i)\|_2 \|g_{\plainphi}(\y_i)\|_2} \right)}{\sum_{j=1}^N \exp\left( \frac{f_{\theta}(\x_i)^\top g_{\plainphi}(\y_j)}{\|f_{\theta}(\x_i)\|_2 \|g_{\plainphi}(\y_j)\|_2} \right)} \right], \label{eq:clip-loss}
\end{equation}
