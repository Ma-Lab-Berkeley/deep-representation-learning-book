...
CLIP trains two encoders—an image encoder $f_{\theta}(\x)$ and a text encoder $g_{\phi}(\y)$—to map paired samples into a shared $d$-dimensional unit sphere.  
To achieve this, we can adopt a simple symmetric cross-entropy loss that pushes the cosine similarity of matched pairs $(f_{\theta}(\x_i),g_{\phi}(\y_i))$ closer together while repelling unmatched pairs:
\begin{equation}
    \mathcal{L}_{\mathrm{CLIP}} = \mathbb{E}_{[(\x_i, \y_i)]_{i=1}^N \sim \mathcal{D}} \left[ - \log \frac{\exp\left( \frac{f_{\theta}(\x_i)^\top g_{\phi}(\y_i)}{\|f_{\theta}(\x_i)\|_2 \|g_{\phi}(\y_i)\|_2} \right)}{\sum_{j=1}^N \exp\left( \frac{f_{\theta}(\x_i)^\top g_{\phi}(\y_j)}{\|f_{\theta}(\x_i)\|_2 \|g_{\phi}(\y_j)\|_2} \right)} \right], \label{eq:clip-loss}
\end{equation}
...
\begin{equation}
    I\!\big(f_{\theta}(\x),g_{\phi}(\y)\big) \geq - \mathcal{L}_{\mathrm{CLIP}}.
\end{equation}

Thus, CLIP provides a practical recipe for learning joint embeddings for paired data that increases the mutual information between the paired sample. Empricially, CLIP has been shown to be effective at learning joint embeddings for image-text pairs, and has been used in a variety of applications, including image captioning, image-text retrieval, image generation and beyond. We will see more specific examples in \Cref{ch:applications}.
...
Below, we will describe an early instantiation of the preceding
techniques, which
