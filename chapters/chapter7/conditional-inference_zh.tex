...
辅助“类别 token (class token)”的常见做法，可以被视为通过压缩给定（含噪）
样本 $(\x, \vw)$（的编码率）来学习这样
一个表征。如果数据 $\x$ 的分布
已经是一个（低维）高斯混合分布，文献
...
Stable Diffusion 遵循我们在 \Cref{sub:cfg} 中概述的条件生成方法，但有两个关键修改：（i）条件信号是 token化后的文本提示 $\vY \in
...
这里，$N_{\mathrm{text}}$ 是 token化后文本提示的最大长度，而 $N_{\mathrm{img}}$ 大致对应于表征中的图像通道数（依赖于层），如果输入图像分辨率固定，则该数值也是固定的。交叉注意力（具有 $K$ 个头，且无偏置）定义为
\begin{equation}
...
\end{equation}
或者，进一步地，我们可以假设我们有足够数量的立体对 $(\y^0, \y^1)$ 样本，并且也学习了这些对的联合分布。我们也用 $\y = h(\x)$ 来表示对 $\y = (\y^0, \y^1)$，并用 $p(\y)$ 表征学习到的该对的概率分布（例如通过如上的去噪器）。
