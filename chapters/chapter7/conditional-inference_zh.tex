...
\begin{equation}
    f: \X \mapsto \Z
\end{equation}
使得 $\Z$ 的分布可以很好地建模为子空间的混合，例如 $\{\vU_{[K]}\}$，从而在最小化稀疏性的同时最大化编码率递减（rate reduction）：
...
\Cref{ch:lossy-compression,ch:deep-networks,ch:consistent-representations} 中研究的高斯编码率递减函数相联系）。
...
从概念上讲，这种联合图文嵌入空间和交叉注意力层本身，与我们在上一节中推导的条件高斯混合去噪器（回想 \eqref{eq:mog-conditional-denoising-unified-operator}）在单 token 序列的特例下非常相似。在多 token 设置中，可以遵循 \Cref{ch:deep-networks} 中讨论的用于推导深度网络架构的编码率递减（rate reduction）框架得出更深层次的联系，这在 CRATE 类 Transformer 架构的推导中得到了体现。
...
\begin{remark}请注意，上述目标与现代几何学中克莱因的埃尔朗根纲领（Erlangen Program）非常吻合，该纲领旨在研究流形在变换群下的不变量。在这里，我们可以将感兴趣的流形视为 3D 场景的自我中心表征的分布。我们已经看到，其上存在一组三维刚体运动的群作用。值得注意的是，我们的大脑已经学会了有效地将此类变换与观测到的3D世界解耦。
\end{remark}
