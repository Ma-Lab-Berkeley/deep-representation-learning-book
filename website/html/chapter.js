  ...
  ready(function () {
    ...
    try {
      const headings = document.querySelectorAll(
        ".ltx_title_section, .ltx_title_subsection, .ltx_title_paragraph"
      );
      headings.forEach((h) => {
    ...
    try {
      document
        .querySelectorAll(".ltx_equation[id], .ltx_equationgroup[id]")
        .forEach((eq) => {
    ...
    try {
      document
        .querySelectorAll(
          ".ltx_figure[id], .ltx_table[id], .ltx_float.ltx_float_algorithm[id]"
        )
        .forEach((box) => {
          ...
          a.setAttribute(
            "aria-label",
            "Copy link to this figure/table/algorithm"
          );
          a.textContent = "🔗";
    ...
    try {
      ...
      document
        .querySelectorAll(".ltx_float.ltx_float_algorithm .ltx_caption")
        .forEach((cap) => {
    ...
    try {
      function adjustAlgCommentPadding(root) {
        ...
        scope
          .querySelectorAll(".ltx_float.ltx_float_algorithm .ltx_listingline")
          .forEach((line) => {
            // Reset previous padding
            line.style.paddingRight = "";
            const rightComment = line.querySelector(
              '.ltx_text[style*="float:right"]'
            );
    ...
    try {
      ...
      document.querySelectorAll(".ltx_note.ltx_role_footnote").forEach((fn) => {
        ...
        const content =
          fn.querySelector(".ltx_note_content") ||
          fn.querySelector(".ltx_note_outer .ltx_note_content");
